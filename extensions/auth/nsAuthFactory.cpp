...
static nsresult nsSysNTLMAuthConstructor(nsISupports *outer, REFNSIID iid,
                                         void **result) {
  if (outer) return NS_ERROR_NO_AGGREGATION;

  nsAuthSSPI *auth = new nsAuthSSPI(PACKAGE_TYPE_NTLM);
  if (!auth) return NS_ERROR_OUT_OF_MEMORY;
...
static nsresult nsKerbSSPIAuthConstructor(nsISupports *outer, REFNSIID iid,
                                          void **result) {
  if (outer) return NS_ERROR_NO_AGGREGATION;
...
#define NS_SAMBANTLMAUTH_CID                         \
  { /* bc54f001-6eb0-4e32-9f49-7e064d8e70ef */       \
    0xbc54f001, 0x6eb0, 0x4e32, {                    \
      0x9f, 0x49, 0x7e, 0x06, 0x4d, 0x8e, 0x70, 0xef \
    }                                                \
  }

#include "nsAuthSambaNTLM.h"
static nsresult nsSambaNTLMAuthConstructor(nsISupports *outer, REFNSIID iid,
                                           void **result) {
  if (outer) return NS_ERROR_NO_AGGREGATION;
...
static nsresult nsKerbGSSAPIAuthConstructor(nsISupports *outer, REFNSIID iid,
                                            void **result) {
  if (outer) return NS_ERROR_NO_AGGREGATION;
...
static nsresult nsGSSAPIAuthConstructor(nsISupports *outer, REFNSIID iid,
                                        void **result) {
  if (outer) return NS_ERROR_NO_AGGREGATION;
...
#endif
    {&kNS_HTTPNEGOTIATEAUTH_CID, false, nullptr,
...
// setup nspr logging ...
static nsresult InitNegotiateAuth() { return NS_OK; }

static void DestroyNegotiateAuth() { nsAuthGSSAPI::Shutdown(); }

static const mozilla::Module kAuthModule = {mozilla::Module::kVersion,
                                            kAuthCIDs,
                                            kAuthContracts,
                                            nullptr,
                                            nullptr,
