...
static already_AddRefed<nsIFile> GetFileViaSpecialDirectory(
    const char* aSpecialDirName, const char* aFileName) {
...
static NS_IMETHODIMP GetFilePathViaSpecialDirectory(const char* aSpecialDirName,
...
static bool SetupPrintHelper(const char* aFileName,
                             PDFViaEMFPrintHelper* aHelper) {
  nsCOMPtr<nsIFile> file =
...
static uint32_t GetFileContents(const nsAutoString& aFile, Vector<char>& aBuf) {
...
static void CompareTwoFiles(const nsAutoString& aTestFile,
                            const nsAutoString& aRefFile) {
...
#define PDFIUM_FILENAME "pdfium_ref_x86.dll"
      ...
      MOZ_RELEASE_ASSERT(NS_SUCCEEDED(GetFilePathViaSpecialDirectory(
          NS_OS_CURRENT_WORKING_DIR, PDFIUM_FILENAME, pdfiumPath)));
...
TEST(TestEMFConversion, CompareEMFWithReference) {
  ...
  ASSERT_TRUE(
      PDFHelper->SavePageToFile(emfPath.get(), 0, pageWidth, pageHeight));
  ...
  ASSERT_TRUE(
      ExtHelper->SavePageToFile(emfPathRef.get(), 0, pageWidth, pageHeight));
