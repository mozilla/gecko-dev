...
bool PDFiumChild::Init(base::ProcessId aParentPid, MessageLoop* aIOLoop,
...
mozilla::ipc::IPCResult PDFiumChild::RecvConvertToEMF(const FileDescriptor& aFD,
  ...
  MOZ_ASSERT(convertor.GetPageCount() == 1,
             "we assume each given PDF contains"
             "one page only");
...
void PDFiumChild::OnChannelConnected(int32_t pid) { SetOtherProcessId(pid); }

void PDFiumChild::ActorDestroy(ActorDestroyReason why) {
