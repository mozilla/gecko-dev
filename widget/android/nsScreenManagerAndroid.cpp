...
#define ALOG(args...) \
  __android_log_print(ANDROID_LOG_INFO, "nsScreenManagerAndroid", ##args)
...
float nsScreenAndroid::GetDensity() {
  if (mDensity != 0.0) {
    return mDensity;
  }
  if (mDisplayType == DisplayType::DISPLAY_PRIMARY) {
...
NS_IMETHODIMP
nsScreenAndroid::GetRect(int32_t *outLeft, int32_t *outTop, int32_t *outWidth,
                         int32_t *outHeight) {
  if (mDisplayType != DisplayType::DISPLAY_PRIMARY) {
    *outLeft = mRect.x;
    *outTop = mRect.y;
...
NS_IMETHODIMP
nsScreenAndroid::GetAvailRect(int32_t *outLeft, int32_t *outTop,
    ...
    SyncRunnable::DispatchToThread(
        ...
        NS_NewRunnableFunction(
            ...
            [&aDisplayType, &aWidth, &aHeight, &aDensity, &screenId] {
              ...
              RefPtr<nsScreenAndroid> screen = screenMgrAndroid->AddScreen(
...
already_AddRefed<nsScreenAndroid> nsScreenManagerAndroid::ScreenForId(
    uint32_t aId) {
...
NS_IMETHODIMP
nsScreenManagerAndroid::ScreenForRect(int32_t inLeft, int32_t inTop,
...
already_AddRefed<nsScreenAndroid> nsScreenManagerAndroid::AddScreen(
    DisplayType aDisplayType, nsIntRect aRect) {
  ALOG("nsScreenManagerAndroid: add %s screen",
       (aDisplayType == DisplayType::DISPLAY_PRIMARY
            ? "PRIMARY"
...
void nsScreenManagerAndroid::RemoveScreen(uint32_t aScreenId) {
  for (size_t i = 0; i < mScreens.Length(); i++) {
