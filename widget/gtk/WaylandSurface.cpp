...
WaylandSurface::WaylandSurface(RefPtr<WaylandSurface> aParent,
                               gfx::IntSize aSize)
    : mParent(aParent), mSizeScaled(aSize) {
...
void WaylandSurface::FrameCallbackHandler(struct wl_callback* aCallback,
  ...
  std::vector<FrameCallback> cbs;
  {
    ...
    // Clear emulated frame callback timer - will be set again in
    // RequestFrameCallbackLocked if we need it.
    if (emulatedCallback) {
      mFrameCheckTimerID = 0;
    }
...
bool WaylandSurface::MapLocked(const WaylandSurfaceLock& aProofOfLock,
  ...
  if (aParentWLSurface) {
    ...
    MOZ_DIAGNOSTIC_ASSERT(!mParentWaylandSurface, "Missing parent wl_surface!");
  } else {
    MOZ_DIAGNOSTIC_ASSERT(!mParentSurface, "Already mapped?");
    mParentWaylandSurface = aParentWaylandSurfaceLock->GetWaylandSurface();
    MOZ_DIAGNOSTIC_ASSERT(mParentWaylandSurface->IsMapped(),
                          "Parent surface is not mapped?");
    mParentSurface = mParentWaylandSurface->mSurface;
  }
