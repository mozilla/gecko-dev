...
static GtkWindow *GetGtkWindow(nsIDOMDocument *aDocument) {
...
nsDragService::InvokeDragSession(
  ...
  return nsBaseDragService::InvokeDragSession(aDOMNode, aPrincipalURISpec,
                                              aArrayTransferables, aRegion,
                                              aActionType, aContentPolicyType);
}

// nsBaseDragService
nsresult nsDragService::InvokeDragSessionImpl(nsIArray *aArrayTransferables,
    ...
    for (uint32_t itemIndex = 0; itemIndex < numDragItems; ++itemIndex) {
      ...
      if (currItem) {
        ...
        if (flavorList) {
          ...
          for (uint32_t flavorIndex = 0; flavorIndex < numFlavors;
               ++flavorIndex) {
            nsCOMPtr<nsISupportsCString> currentFlavor;
  ...
  for (i = 0; i < count; i++) {
    ...
    if (item) {
      ...
      if (NS_SUCCEEDED(rv)) {
        ...
        nsCOMPtr<nsISupports> data;
        rv =
            item->GetTransferData(kFileMime, getter_AddRefs(data), &tmpDataLen);
        if (NS_SUCCEEDED(rv)) {
  ...
  if (item) {
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      if (mimeFlavor.EqualsLiteral(gTextUriListType)) {
        ...
        gtk_selection_data_set(aSelectionData, target, 8, (guchar *)uriList,
                               length);
        g_free(uriList);
        return;
      }
...
void nsDragService::SetDragIcon(GdkDragContext *aContext) {
  ...
  DrawDrag(mSourceNode, mSourceRegion, mScreenPosition, &dragRect, &surface,
           &pc);
  if (!pc) return;
