...
class WaylandSurface final {
 ...
 private:
  ...
  // Right now we use parent to read scale only so we don't need
  // to care about correct mapping, subsurfaces orders etc.
  ...
  // Parent WaylandSurface, it's used by layer rendering.
  RefPtr<WaylandSurface> mParentWaylandSurface;
