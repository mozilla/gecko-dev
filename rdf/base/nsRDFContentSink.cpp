...
class RDFContentSinkImpl : public nsIRDFContentSink, public nsIExpatSink {
 public:
  RDFContentSinkImpl();
...
#undef RDF_ATOM
 ...
 protected:
  ...
  nsresult GetIdAboutAttribute(const char16_t** aAttributes,
                               nsIRDFResource** aResource,
                               bool* aIsAnonymous = nullptr);
  nsresult GetResourceAttribute(const char16_t** aAttributes,
                                nsIRDFResource** aResource);
  nsresult AddProperties(const char16_t** aAttributes, nsIRDFResource* aSubject,
                         int32_t* aCount = nullptr);
  ...
  nsresult InitContainer(nsIRDFResource* aContainerType,
                         nsIRDFResource* aContainer);
  nsresult ReinitContainer(nsIRDFResource* aContainerType,
                           nsIRDFResource* aContainer);
  ...
  int32_t PushContext(nsIRDFResource* aContext, RDFContentSinkState aState,
                      RDFContentSinkParseMode aParseMode);

  nsresult PopContext(nsIRDFResource*& aContext, RDFContentSinkState& aState,
                      RDFContentSinkParseMode& aParseMode);
...
#define RDF_ATOM(name_, value_) NS_STATIC_ATOM_SETUP(RDFContentSinkImpl, name_)
#include "nsRDFContentSinkAtomList.h"
#undef RDF_ATOM
};

// static
void nsRDFAtoms::RegisterAtoms() {
  ...
  if (gRefCnt++ == 0) {
    ...
    if (NS_SUCCEEDED(rv)) {
      rv = gRDFService->GetResource(
          NS_LITERAL_CSTRING(RDF_NAMESPACE_URI "type"), &kRDF_type);
      rv = gRDFService->GetResource(
...
RDFContentSinkImpl::~RDFContentSinkImpl() {
  ...
  if (mContextStack) {
    MOZ_LOG(gLog, LogLevel::Warning, ("rdfxml: warning! unclosed tag"));
    ...
    while (0 < i--) {
      ...
      if (resource && MOZ_LOG_TEST(gLog, LogLevel::Debug)) {
        ...
        MOZ_LOG(gLog, LogLevel::Debug, ("rdfxml:   uri=%s", uri.get()));
      }
...
RDFContentSinkImpl::QueryInterface(REFNSIID iid, void** result) {
  ...
  *result = nullptr;
  if (iid.Equals(kIRDFContentSinkIID) || iid.Equals(kIXMLContentSinkIID) ||
      iid.Equals(kIContentSinkIID) || iid.Equals(kISupportsIID)) {
  ...
  switch (mState) {
    ...
    case eRDFContentSinkState_InEpilog:
      MOZ_LOG(gLog, LogLevel::Warning,
              ("rdfxml: unexpected content in epilog at line %d", aLineNumber));
      break;
  }
...
RDFContentSinkImpl::HandleEndElement(const char16_t* aName) {
  ...
  if (NS_FAILED(PopContext(resource, mState, mParseMode))) {
    ...
    if (MOZ_LOG_TEST(gLog, LogLevel::Warning)) {
      ...
      MOZ_LOG(gLog, LogLevel::Warning,
              ("rdfxml: extra close tag '%s' at line %d", tagCStr,
               0 /*XXX fix me */));
  ...
  switch (mState) {
    ...
    case eRDFContentSinkState_InPropertyElement: {
      mDataSource->Assert(GetContextElement(1), GetContextElement(0), resource,
                          true);
    } break;
    default:
      break;
  ...
  if (mContextStack->IsEmpty()) mState = eRDFContentSinkState_InEpilog;
...
NS_IMETHODIMP
RDFContentSinkImpl::HandleComment(const char16_t* aName) { return NS_OK; }
...
RDFContentSinkImpl::WillResume(void) {
  if (mDataSource) {
    nsCOMPtr<nsIRDFXMLSink> sink = do_QueryInterface(mDataSource);
    if (sink) return sink->Resume();
  }
  return NS_OK;
}

NS_IMETHODIMP
RDFContentSinkImpl::SetParser(nsParserBase* aParser) { return NS_OK; }
...
RDFContentSinkImpl::Init(nsIURI* aURL) {
  ...
  if (!aURL) return NS_ERROR_NULL_POINTER;
...
static bool rdf_IsDataInBuffer(char16_t* buffer, int32_t length) {
...
void RDFContentSinkImpl::ParseText(nsIRDFNode** aResult) {
  ...
  switch (mParseMode) {
    ...
    case eRDFContentSinkParseMode_Date: {
      PRTime t = rdf_ParseDate(nsDependentCString(
          NS_LossyConvertUTF16toASCII(value).get(), value.Length()));
      nsIRDFDate* result;
...
nsresult RDFContentSinkImpl::FlushText() {
  nsresult rv = NS_OK;
  if (0 != mTextLength) {
    if (rdf_IsDataInBuffer(mText, mTextLength)) {
      ...
      switch (mState) {
        ...
        case eRDFContentSinkState_InPropertyElement: {
          ...
          mDataSource->Assert(GetContextElement(1), GetContextElement(0), node,
                              true);
        } break;
...
nsresult RDFContentSinkImpl::AddText(const char16_t* aText, int32_t aLength) {
  ...
  if (amount < aLength) {
    ...
    int32_t newSize = (2 * mTextSize > (mTextSize + aLength))
                          ? (2 * mTextSize)
                          : (mTextSize + aLength);
    char16_t* newText = (char16_t*)realloc(mText, sizeof(char16_t) * newSize);
    if (!newText) return NS_ERROR_OUT_OF_MEMORY;
    mTextSize = newSize;
    mText = newText;
  }
...
bool rdf_RequiresAbsoluteURI(const nsString& uri) {
...
nsresult RDFContentSinkImpl::GetIdAboutAttribute(const char16_t** aAttributes,
                                                 nsIRDFResource** aResource,
                                                 bool* aIsAnonymous) {
  ...
  for (; *aAttributes; aAttributes += 2) {
    ...
    if (localName == kAboutAtom) {
      if (aIsAnonymous) *aIsAnonymous = false;
  ...
  if (aIsAnonymous) *aIsAnonymous = true;
...
nsresult RDFContentSinkImpl::GetResourceAttribute(const char16_t** aAttributes,
                                                  nsIRDFResource** aResource) {
  RefPtr<nsAtom> localName;
...
nsresult RDFContentSinkImpl::AddProperties(const char16_t** aAttributes,
  ...
  for (; *aAttributes; aAttributes += 2) {
    ...
    gRDFService->GetLiteral(aAttributes[1], getter_AddRefs(target));
...
void RDFContentSinkImpl::SetParseMode(const char16_t** aAttributes) {
  RefPtr<nsAtom> localName;
  for (; *aAttributes; aAttributes += 2) {
    const nsDependentSubstring& nameSpaceURI =
        SplitExpatName(aAttributes[0], getter_AddRefs(localName));
...
nsresult RDFContentSinkImpl::OpenRDF(const char16_t* aName) {
...
nsresult RDFContentSinkImpl::OpenObject(const char16_t* aName,
...
nsresult RDFContentSinkImpl::OpenProperty(const char16_t* aName,
                                          const char16_t** aAttributes) {
  nsresult rv;
...
nsresult RDFContentSinkImpl::OpenMember(const char16_t* aName,
                                        const char16_t** aAttributes) {
  ...
  if (!nameSpaceURI.EqualsLiteral(RDF_NAMESPACE_URI) || localName != kLiAtom) {
...
nsresult RDFContentSinkImpl::OpenValue(const char16_t* aName,
                                       const char16_t** aAttributes) {
...
void RDFContentSinkImpl::RegisterNamespaces(const char16_t** aAttributes) {
  nsCOMPtr<nsIRDFXMLSink> sink = do_QueryInterface(mDataSource);
  if (!sink) {
    return;
  }
  ...
  for (; *aAttributes; aAttributes += 2) {
    ...
    if (*attr != 0xFFFF || xmlnsP != xmlns.EndReading()) {
      continue;
    }
...
const nsDependentSubstring RDFContentSinkImpl::SplitExpatName(
    const char16_t* aExpatName, nsAtom** aLocalName) {
  /**
...
nsresult RDFContentSinkImpl::InitContainer(nsIRDFResource* aContainerType,
  ...
  static const ContainerInfo gContainerInfo[] = {
      {&RDFContentSinkImpl::kRDF_Alt, &nsIRDFContainerUtils::IsAlt,
       &nsIRDFContainerUtils::MakeAlt},
      {&RDFContentSinkImpl::kRDF_Bag, &nsIRDFContainerUtils::IsBag,
       &nsIRDFContainerUtils::MakeBag},
      {&RDFContentSinkImpl::kRDF_Seq, &nsIRDFContainerUtils::IsSeq,
       &nsIRDFContainerUtils::MakeSeq},
      {0, 0, 0},
  };

  for (const ContainerInfo* info = gContainerInfo; info->mType != 0; ++info) {
    if (*info->mType != aContainerType) continue;

    bool isContainer;
    rv = (gRDFContainerUtils->*(info->mTestFn))(mDataSource, aContainer,
                                                &isContainer);
    if (isContainer) {
      rv = ReinitContainer(aContainerType, aContainer);
    } else {
      rv = (gRDFContainerUtils->*(info->mMakeFn))(mDataSource, aContainer,
                                                  nullptr);
    }
    return rv;
  }
...
nsresult RDFContentSinkImpl::ReinitContainer(nsIRDFResource* aContainerType,
  ...
  rv = mDataSource->GetTarget(aContainer, kRDF_nextVal, true,
...
nsIRDFResource* RDFContentSinkImpl::GetContextElement(
    int32_t ancestor /* = 0 */) {
  ...
  return mContextStack->ElementAt(mContextStack->Length() - ancestor - 1)
      .mResource;
}

int32_t RDFContentSinkImpl::PushContext(nsIRDFResource* aResource,
                                        RDFContentSinkState aState,
                                        RDFContentSinkParseMode aParseMode) {
  if (!mContextStack) {
    mContextStack = new AutoTArray<RDFContextStackElement, 8>();
    if (!mContextStack) return 0;
  }

  RDFContextStackElement* e = mContextStack->AppendElement();
  if (!e) return mContextStack->Length();
...
nsresult RDFContentSinkImpl::PopContext(nsIRDFResource*& aResource,
                                        RDFContentSinkState& aState,
                                        RDFContentSinkParseMode& aParseMode) {
  if ((nullptr == mContextStack) || (mContextStack->IsEmpty())) {
