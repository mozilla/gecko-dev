...
nsNameSpaceMap::nsNameSpaceMap() : mEntries(nullptr) {
...
nsresult nsNameSpaceMap::Put(const nsAString& aURI, nsAtom* aPrefix) {
  nsCString uriUTF8;
  AppendUTF16toUTF8(aURI, uriUTF8);
  return Put(uriUTF8, aPrefix);
}

nsresult nsNameSpaceMap::Put(const nsACString& aURI, nsAtom* aPrefix) {
  Entry* entry;
...
nsNameSpaceMap::const_iterator nsNameSpaceMap::GetNameSpaceOf(
    const nsACString& aURI) const {
  for (Entry* entry = mEntries; entry != nullptr; entry = entry->mNext) {
    if (StringBeginsWith(aURI, entry->mURI)) return const_iterator(entry);
  }
