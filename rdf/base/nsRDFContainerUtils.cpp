...
NS_IMETHODIMP
RDFContainerUtilsImpl::IsOrdinalProperty(nsIRDFResource* aProperty,
                                         bool* _retval) {
  ...
  if (PL_strncmp(propertyStr, kRDFNameSpaceURI, sizeof(kRDFNameSpaceURI) - 1) !=
      0) {
    *_retval = false;
    return NS_OK;
  }
...
NS_IMETHODIMP
RDFContainerUtilsImpl::IndexToOrdinalResource(int32_t aIndex,
                                              nsIRDFResource** aOrdinal) {
  NS_PRECONDITION(aIndex > 0, "illegal value");
  if (aIndex <= 0) return NS_ERROR_ILLEGAL_VALUE;
...
NS_IMETHODIMP
RDFContainerUtilsImpl::OrdinalResourceToIndex(nsIRDFResource* aOrdinal,
                                              int32_t* aIndex) {
  NS_PRECONDITION(aOrdinal != nullptr, "null ptr");
  if (!aOrdinal) return NS_ERROR_NULL_POINTER;

  const char* ordinalStr;
  if (NS_FAILED(aOrdinal->GetValueConst(&ordinalStr))) return NS_ERROR_FAILURE;
...
NS_IMETHODIMP
RDFContainerUtilsImpl::IsContainer(nsIRDFDataSource* aDataSource,
                                   nsIRDFResource* aResource, bool* _retval) {
...
RDFContainerUtilsImpl::IsEmpty(nsIRDFDataSource* aDataSource,
  ...
  *_retval = true;
  ...
  rv = aDataSource->GetTarget(aResource, kRDF_nextVal, true,
                              getter_AddRefs(nextValNode));
  if (NS_FAILED(rv)) return rv;

  if (rv == NS_RDF_NO_VALUE) return NS_OK;
  ...
  if (nextValLiteral.get() != kOne) *_retval = false;

  return NS_OK;
}

NS_IMETHODIMP
RDFContainerUtilsImpl::IsBag(nsIRDFDataSource* aDataSource,
                             nsIRDFResource* aResource, bool* _retval) {
  NS_PRECONDITION(aDataSource != nullptr, "null ptr");
  if (!aDataSource) return NS_ERROR_NULL_POINTER;
...
NS_IMETHODIMP
RDFContainerUtilsImpl::IsSeq(nsIRDFDataSource* aDataSource,
                             nsIRDFResource* aResource, bool* _retval) {
  NS_PRECONDITION(aDataSource != nullptr, "null ptr");
  if (!aDataSource) return NS_ERROR_NULL_POINTER;
...
NS_IMETHODIMP
RDFContainerUtilsImpl::IsAlt(nsIRDFDataSource* aDataSource,
                             nsIRDFResource* aResource, bool* _retval) {
  NS_PRECONDITION(aDataSource != nullptr, "null ptr");
  if (!aDataSource) return NS_ERROR_NULL_POINTER;
...
NS_IMETHODIMP
RDFContainerUtilsImpl::MakeBag(nsIRDFDataSource* aDataSource,
...
NS_IMETHODIMP
RDFContainerUtilsImpl::MakeSeq(nsIRDFDataSource* aDataSource,
                               nsIRDFResource* aResource,
                               nsIRDFContainer** _retval) {
...
NS_IMETHODIMP
RDFContainerUtilsImpl::MakeAlt(nsIRDFDataSource* aDataSource,
                               nsIRDFResource* aResource,
                               nsIRDFContainer** _retval) {
...
nsresult NS_NewRDFContainerUtils(nsIRDFContainerUtils** aResult) {
  ...
  RDFContainerUtilsImpl* result = new RDFContainerUtilsImpl();
...
bool RDFContainerUtilsImpl::IsA(nsIRDFDataSource* aDataSource,
                                nsIRDFResource* aResource,
                                nsIRDFResource* aType) {
...
NS_IMETHODIMP
RDFContainerUtilsImpl::IndexOf(nsIRDFDataSource* aDataSource,
                               nsIRDFResource* aContainer, nsIRDFNode* aElement,
                               int32_t* aIndex) {
  if (!aDataSource || !aContainer) return NS_ERROR_NULL_POINTER;
  ...
  *aIndex = -1;
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (!hasMoreSources) break;

      nsCOMPtr<nsISupports> isupports2;
      sources->GetNext(getter_AddRefs(isupports2));
      if (!isupports2) break;

      nsCOMPtr<nsIRDFResource> source = do_QueryInterface(isupports2);
