...
class Assertion {
 public:
  ...
  explicit Assertion(
      nsIRDFResource* aSource);  // PLDHashTable assertion variant
...
Assertion::Assertion(nsIRDFResource* aSource)
    : mSource(aSource), mNext(nullptr), mRefCnt(0), mHashEntry(true) {
...
Assertion::Assertion(nsIRDFResource* aSource, nsIRDFResource* aProperty,
                     nsIRDFNode* aTarget, bool aTruthValue)
    : mSource(aSource), mNext(nullptr), mRefCnt(0), mHashEntry(false) {
  MOZ_COUNT_CTOR(Assertion);
...
class InMemoryDataSource : public nsIRDFDataSource,
 ...
 protected:
  ...
  friend class InMemoryResourceEnumeratorImpl;  // b/c it needs to enumerate
                                                // mForwardArcs

  // Thread-safe writer implementation methods.
  nsresult LockedAssert(nsIRDFResource* source, nsIRDFResource* property,
                        nsIRDFNode* target, bool tv);

  nsresult LockedUnassert(nsIRDFResource* source, nsIRDFResource* property,
                          nsIRDFNode* target);

  explicit InMemoryDataSource(nsISupports* aOuter);
  virtual ~InMemoryDataSource();

  friend nsresult NS_NewRDFInMemoryDataSource(nsISupports* aOuter,
                                              const nsIID& aIID,
                                              void** aResult);
 ...
 protected:
  ...
  static void SweepForwardArcsEntries(PLDHashTable* aTable, SweepInfo* aArg);
 ...
 public:
  ...
  void SetForwardArcs(nsIRDFResource* u, Assertion* as) {
    if (as) {
      auto entry = static_cast<Entry*>(mForwardArcs.Add(u, mozilla::fallible));
      if (entry) {
        entry->mNode = u;
  ...
  void SetReverseArcs(nsIRDFNode* v, Assertion* as) {
    if (as) {
      auto entry = static_cast<Entry*>(mReverseArcs.Add(v, mozilla::fallible));
      if (entry) {
        entry->mNode = v;
        entry->mAssertions = as;
      }
  ...
  void LogOperation(const char* aOperation, nsIRDFResource* asource,
...
InMemoryAssertionEnumeratorImpl::InMemoryAssertionEnumeratorImpl(
    InMemoryDataSource* aDataSource, nsIRDFResource* aSource,
    nsIRDFResource* aProperty, nsIRDFNode* aTarget, bool aTruthValue)
  ...
  if (mSource) {
    ...
    if (mNextAssertion && mNextAssertion->mHashEntry) {
      ...
      mNextAssertion = hdr ? static_cast<Entry*>(hdr)->mAssertions : nullptr;
    }
  } else {
  ...
  if (mNextAssertion) mNextAssertion->AddRef();
}
...
InMemoryAssertionEnumeratorImpl::~InMemoryAssertionEnumeratorImpl() {
  ...
  if (mNextAssertion) mNextAssertion->Release();
...
InMemoryAssertionEnumeratorImpl::HasMoreElements(bool* aResult) {
  ...
  while (mNextAssertion) {
    ...
    // iterate
    mNextAssertion =
        (mSource) ? mNextAssertion->mNext : mNextAssertion->u.as.mInvNext;
...
class InMemoryArcsEnumeratorImpl : public nsISimpleEnumerator {
 ...
 public:
  InMemoryArcsEnumeratorImpl(InMemoryDataSource* aDataSource,
                             nsIRDFResource* aSource, nsIRDFNode* aTarget);
...
InMemoryArcsEnumeratorImpl::InMemoryArcsEnumeratorImpl(
  ...
  if (mSource) {
    ...
    if (mAssertion && mAssertion->mHashEntry) {
      ...
      for (auto i = mAssertion->u.hash.mPropertyHash->Iter(); !i.Done();
           i.Next()) {
        auto entry = static_cast<Entry*>(i.Get());
...
InMemoryArcsEnumeratorImpl::HasMoreElements(bool* aResult) {
  ...
  if (mHashArcs) {
    if (!mHashArcs->IsEmpty()) {
      ...
      nsCOMPtr<nsIRDFResource> tmp(
          do_QueryInterface(mHashArcs->ObjectAt(last)));
      tmp.forget(&mCurrent);
...
nsresult NS_NewRDFInMemoryDataSource(nsISupports* aOuter, const nsIID& aIID,
  ...
  *aResult = nullptr;
  ...
  nsresult rv =
      datasource->AggregatedQueryInterface(aIID, aResult);  // This'll AddRef()
...
void InMemoryDataSource::LogOperation(const char* aOperation,
                                      nsIRDFResource* aSource,
                                      nsIRDFResource* aProperty,
                                      nsIRDFNode* aTarget, bool aTruthValue) {
  if (!MOZ_LOG_TEST(gLog, LogLevel::Debug)) return;
  ...
  MOZ_LOG(gLog, LogLevel::Debug, ("  [(%p)%s]--", aSource, uri.get()));
  ...
  if ((resource = do_QueryInterface(aTarget)) != nullptr) {
    ...
    MOZ_LOG(gLog, LogLevel::Debug, ("  -->[(%p)%s]", aTarget, uri.get()));
    ...
    MOZ_LOG(gLog, LogLevel::Debug, ("  -->(unknown-type)\n"));
  }
}
...
InMemoryDataSource::GetSource(nsIRDFResource* property, nsIRDFNode* target,
  ...
  if (!property) return NS_ERROR_NULL_POINTER;
...
InMemoryDataSource::HasAssertion(nsIRDFResource* source,
  ...
  if (as && as->mHashEntry) {
    ...
    for (; as != nullptr; as = as->mNext) {
      ...
      if (target != as->u.as.mTarget) continue;

      if (property != as->u.as.mProperty) continue;

      if (tv != (as->u.as.mTruthValue)) continue;
...
NS_IMETHODIMP
InMemoryDataSource::GetSources(nsIRDFResource* aProperty, nsIRDFNode* aTarget,
                               bool aTruthValue,
  ...
  InMemoryAssertionEnumeratorImpl* result = new InMemoryAssertionEnumeratorImpl(
      this, nullptr, aProperty, aTarget, aTruthValue);
...
InMemoryDataSource::GetTargets(nsIRDFResource* aSource,
  ...
  InMemoryAssertionEnumeratorImpl* result = new InMemoryAssertionEnumeratorImpl(
      this, aSource, aProperty, nullptr, aTruthValue);
...
nsresult InMemoryDataSource::LockedAssert(nsIRDFResource* aSource,
                                          nsIRDFResource* aProperty,
                                          nsIRDFNode* aTarget,
                                          bool aTruthValue) {
  ...
  if (!as) return NS_ERROR_OUT_OF_MEMORY;
  ...
  if (haveHash) {
    PLDHashEntryHdr* hdr = next->u.hash.mPropertyHash->Search(aProperty);
    Assertion* asRef = hdr ? static_cast<Entry*>(hdr)->mAssertions : nullptr;
    if (asRef) {
...
NS_IMETHODIMP
InMemoryDataSource::Assert(nsIRDFResource* aSource, nsIRDFResource* aProperty,
                           nsIRDFNode* aTarget, bool aTruthValue) {
  NS_PRECONDITION(aSource != nullptr, "null ptr");
  if (!aSource) return NS_ERROR_NULL_POINTER;
  ...
  // notify observers
  for (int32_t i = (int32_t)mNumObservers - 1; mPropagateChanges && i >= 0;
       --i) {
...
nsresult InMemoryDataSource::LockedUnassert(nsIRDFResource* aSource,
                                            nsIRDFResource* aProperty,
                                            nsIRDFNode* aTarget) {
  ...
  if (haveHash) {
    ...
    if (first) {
      ...
      if (next && next->mNext) {
        PLDHashEntryHdr* hdr =
            root->u.hash.mPropertyHash->Add(aProperty, mozilla::fallible);
        if (hdr) {
          Entry* entry = static_cast<Entry*>(hdr);
          entry->mNode = aProperty;
...
NS_IMETHODIMP
InMemoryDataSource::Unassert(nsIRDFResource* aSource, nsIRDFResource* aProperty,
                             nsIRDFNode* aTarget) {
  NS_PRECONDITION(aSource != nullptr, "null ptr");
  if (!aSource) return NS_ERROR_NULL_POINTER;
...
NS_IMETHODIMP
InMemoryDataSource::Change(nsIRDFResource* aSource, nsIRDFResource* aProperty,
...
NS_IMETHODIMP
InMemoryDataSource::Move(nsIRDFResource* aOldSource, nsIRDFResource* aNewSource,
                         nsIRDFResource* aProperty, nsIRDFNode* aTarget) {
...
NS_IMETHODIMP
InMemoryDataSource::HasArcIn(nsIRDFNode* aNode, nsIRDFResource* aArc,
                             bool* result) {
...
NS_IMETHODIMP
InMemoryDataSource::HasArcOut(nsIRDFResource* aSource, nsIRDFResource* aArc,
                              bool* result) {
...
NS_IMETHODIMP
InMemoryDataSource::ArcLabelsIn(nsIRDFNode* aTarget,
                                nsISimpleEnumerator** aResult) {
  NS_PRECONDITION(aTarget != nullptr, "null ptr");
  if (!aTarget) return NS_ERROR_NULL_POINTER;
...
NS_IMETHODIMP
InMemoryDataSource::ArcLabelsOut(nsIRDFResource* aSource,
                                 nsISimpleEnumerator** aResult) {
  NS_PRECONDITION(aSource != nullptr, "null ptr");
  if (!aSource) return NS_ERROR_NULL_POINTER;
...
InMemoryDataSource::IsCommandEnabled(nsISupports* aSources,
                                     ...
                                     nsISupports* aArguments, bool* aResult) {
  return NS_ERROR_NOT_IMPLEMENTED;
}

NS_IMETHODIMP
InMemoryDataSource::DoCommand(nsISupports* aSources, nsIRDFResource* aCommand,
...
NS_IMETHODIMP
InMemoryDataSource::Mark(nsIRDFResource* aSource, nsIRDFResource* aProperty,
                         nsIRDFNode* aTarget, bool aTruthValue,
                         bool* aDidMark) {
  NS_PRECONDITION(aSource != nullptr, "null ptr");
  if (!aSource) return NS_ERROR_NULL_POINTER;
  ...
  if (as && as->mHashEntry) {
    ...
    for (; as != nullptr; as = as->mNext) {
      // check target first as its most unique
      if (aTarget != as->u.as.mTarget) continue;

      if (aProperty != as->u.as.mProperty) continue;

      if (aTruthValue != (as->u.as.mTruthValue)) continue;
...
InMemoryDataSource::Sweep() {
  ...
  while (as) {
    LogOperation("SWEEP", as->mSource, as->u.as.mProperty, as->u.as.mTarget,
                 as->u.as.mTruthValue);
    if (!(as->mHashEntry)) {
      for (int32_t i = int32_t(mNumObservers) - 1; mPropagateChanges && i >= 0;
           --i) {
        ...
        obs->OnUnassert(this, as->mSource, as->u.as.mProperty,
                        as->u.as.mTarget);
        // XXX ignore return value?
      }
    }
...
void InMemoryDataSource::SweepForwardArcsEntries(PLDHashTable* aTable,
                                                 SweepInfo* aInfo) {
  for (auto iter = aTable->Iter(); !iter.Done(); iter.Next()) {
    ...
    while (as) {
      if (as->IsMarked()) {
        ...
        PLDHashEntryHdr* hdr = aInfo->mReverseArcs->Search(as->u.as.mTarget);
