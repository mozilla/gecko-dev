...
class RDFXMLDataSourceImpl : public nsIRDFDataSource,
 ...
 protected:
  ...
  friend nsresult NS_NewRDFXMLDataSource(nsIRDFDataSource** aResult);
 ...
 public:
  ...
  NS_IMETHOD HasArcIn(nsIRDFNode* aNode, nsIRDFResource* aArc,
  ...
  NS_IMETHOD GetAllCmds(
      nsIRDFResource* source,
      nsISimpleEnumerator /*<nsIRDFResource>*/** commands) override {
    return mInner->GetAllCmds(source, commands);
  }

  NS_IMETHOD IsCommandEnabled(nsISupports* aSources, nsIRDFResource* aCommand,
                              nsISupports* aArguments, bool* aResult) override {
    return NS_ERROR_NOT_IMPLEMENTED;
  }

  NS_IMETHOD DoCommand(nsISupports* aSources, nsIRDFResource* aCommand,
                       nsISupports* aArguments) override {
    return NS_ERROR_NOT_IMPLEMENTED;
  }

  NS_IMETHOD BeginUpdateBatch() override { return mInner->BeginUpdateBatch(); }

  NS_IMETHOD EndUpdateBatch() override { return mInner->EndUpdateBatch(); }
  ...
  bool MakeQName(nsIRDFResource* aResource, nsString& property,
                 nsString& nameSpacePrefix, nsString& nameSpaceURI);

  nsresult SerializeAssertion(nsIOutputStream* aStream,
                              nsIRDFResource* aResource,
                              nsIRDFResource* aProperty, nsIRDFNode* aValue);
...
nsresult NS_NewRDFXMLDataSource(nsIRDFDataSource** aResult) {
  ...
  if (!datasource) return NS_ERROR_OUT_OF_MEMORY;
...
RDFXMLDataSourceImpl::RDFXMLDataSourceImpl(void)
    : mIsWritable(true), mIsDirty(false), mLoadState(eLoadState_Unloaded) {}

nsresult RDFXMLDataSourceImpl::Init() {
  nsresult rv;
...
RDFXMLDataSourceImpl::~RDFXMLDataSourceImpl(void) {
  ...
  if (--gRefCnt == 0) NS_IF_RELEASE(gRDFService);
}
...
nsresult RDFXMLDataSourceImpl::BlockingParse(nsIURI* aURL,
  ...
  while (NS_SUCCEEDED(rv)) {
    ...
    uint64_t avail;
    if (NS_FAILED(rv = bufStream->Available(&avail))) break;  // error

    if (avail == 0) break;  // eof

    if (avail > UINT32_MAX) avail = UINT32_MAX;

    rv = aConsumer->OnDataAvailable(channel, nullptr, bufStream, offset,
                                    (uint32_t)avail);
    if (NS_SUCCEEDED(rv)) offset += avail;
  }

  if (NS_FAILED(rv)) channel->Cancel(rv);
  ...
  for (i = mObservers.Count() - 1; i >= 0; --i) {
    ...
    if (obs) {
      if (NS_FAILED(rv)) obs->OnError(this, rv, nullptr);
...
RDFXMLDataSourceImpl::Init(const char* uri) {
  ...
  if (!mInner) return NS_ERROR_OUT_OF_MEMORY;
  ...
  if ((PL_strncmp(uri, kFileURIPrefix, sizeof(kFileURIPrefix) - 1) != 0) &&
      (PL_strncmp(uri, kResourceURIPrefix, sizeof(kResourceURIPrefix) - 1) !=
       0)) {
    mIsWritable = false;
  }
...
NS_IMETHODIMP
RDFXMLDataSourceImpl::Assert(nsIRDFResource* aSource, nsIRDFResource* aProperty,
  ...
  if (IsLoading()) {
    ...
    if (gcable) {
      rv =
          gcable->Mark(aSource, aProperty, aTarget, aTruthValue, &hasAssertion);
      if (NS_FAILED(rv)) return rv;
    }
...
NS_IMETHODIMP
RDFXMLDataSourceImpl::Unassert(nsIRDFResource* source, nsIRDFResource* property,
                               nsIRDFNode* target) {
  // We don't accept assertions unless we're writable (except in the
  // case that we're actually _reading_ the datasource in).
  nsresult rv;
  ...
  if (IsLoading() || mIsWritable) {
    ...
    if (!IsLoading() && rv == NS_RDF_ASSERTION_ACCEPTED) mIsDirty = true;
  } else {
...
NS_IMETHODIMP
RDFXMLDataSourceImpl::Change(nsIRDFResource* aSource, nsIRDFResource* aProperty,
...
nsresult RDFXMLDataSourceImpl::rdfXMLFlush(nsIURI* aURI) {
  nsresult rv;
  ...
  if (fileURL) {
    ...
    if (file) {
      // get a safe output stream, so we don't clobber the datasource file
      // unless all the writes succeeded.
      nsCOMPtr<nsIOutputStream> out;
      rv = NS_NewSafeLocalFileOutputStream(getter_AddRefs(out), file,
                                           PR_WRONLY | PR_CREATE_FILE,
                                           /*octal*/ 0666, 0);
      if (NS_FAILED(rv)) return rv;

      nsCOMPtr<nsIOutputStream> bufferedOut;
      rv = NS_NewBufferedOutputStream(getter_AddRefs(bufferedOut), out.forget(),
                                      4096);
      if (NS_FAILED(rv)) return rv;
      ...
      // All went ok. Maybe except for problems in Write(), but the stream
      // detects that for us
      nsCOMPtr<nsISafeOutputStream> safeStream =
          do_QueryInterface(bufferedOut, &rv);
      if (NS_FAILED(rv)) return rv;
...
RDFXMLDataSourceImpl::FlushTo(const char* aURI) {
  ...
  if ((PL_strncmp(aURI, kFileURIPrefix, sizeof(kFileURIPrefix) - 1) != 0) &&
      (PL_strncmp(aURI, kResourceURIPrefix, sizeof(kResourceURIPrefix) - 1) !=
       0)) {
...
RDFXMLDataSourceImpl::SetReadOnly(bool aIsReadOnly) {
  if (mIsWritable && aIsReadOnly) mIsWritable = false;
...
NS_IMETHODIMP
RDFXMLDataSourceImpl::AsyncOnChannelRedirect(
...
RDFXMLDataSourceImpl::Refresh(bool aBlocking) {
  ...
  MOZ_LOG(gLog, LogLevel::Debug,
          ("rdfxml[%p] refresh(%s) %sblocking", this, spec.get(),
           (aBlocking ? "" : "non")));
  ...
  if (!mURL) return NS_ERROR_FAILURE;
  ...
  if (aBlocking) {
    ...
    // Null LoadGroup ?
    nsCOMPtr<nsIChannel> channel;
    rv = NS_NewChannel(getter_AddRefs(channel), mURL,
                       nsContentUtils::GetSystemPrincipal(),
                       nsILoadInfo::SEC_ALLOW_CROSS_ORIGIN_DATA_IS_NULL,
                       nsIContentPolicy::TYPE_OTHER,
...
NS_IMETHODIMP
RDFXMLDataSourceImpl::OnStopRequest(nsIRequest* request, nsISupports* ctxt,
                                    nsresult status) {
  if (NS_FAILED(status)) {
    for (int32_t i = mObservers.Count() - 1; i >= 0; --i) {
      // Make sure to hold a strong reference to the observer so
...
NS_IMETHODIMP
RDFXMLDataSourceImpl::OnDataAvailable(nsIRequest* request, nsISupports* ctxt,
...
RDFXMLDataSourceImpl::Serialize(nsIOutputStream* aStream) {
  ...
  for (nsNameSpaceMap::const_iterator iter = mNameSpaces.first(); iter != last;
       ++iter) {
  ...
  if (!source) return NS_ERROR_FAILURE;
