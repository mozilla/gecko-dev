...
nsresult nsRDFXMLSerializer::Create(nsISupports* aOuter, REFNSIID aIID,
                                    void** aResult) {
...
nsRDFXMLSerializer::Init(nsIRDFDataSource* aDataSource) {
  ...
  AddNameSpace(
      prefix, NS_LITERAL_STRING("http://www.w3.org/1999/02/22-rdf-syntax-ns#"));
...
static nsresult rdf_BlockingWrite(nsIOutputStream* stream, const char* buf,
                                  uint32_t size) {
  uint32_t written = 0;
  ...
  while (remaining > 0) {
    ...
    if (NS_FAILED(rv = stream->Write(buf + written, remaining, &cb))) return rv;
...
already_AddRefed<nsAtom> nsRDFXMLSerializer::EnsureNewPrefix() {
  nsAutoString qname;
  RefPtr<nsAtom> prefix;
  bool isNewPrefix;
  do {
...
nsresult nsRDFXMLSerializer::RegisterQName(nsIRDFResource* aResource) {
...
bool nsRDFXMLSerializer::IsContainerProperty(nsIRDFResource* aProperty) {
  // Return `true' if the property is an internal property related
  // to being a container.
  if (aProperty == kRDF_instanceOf) return true;

  if (aProperty == kRDF_nextVal) return true;

  bool isOrdinal = false;
  gRDFC->IsOrdinalProperty(aProperty, &isOrdinal);
  if (isOrdinal) return true;
...
nsresult nsRDFXMLSerializer::SerializeChildAssertion(nsIOutputStream* aStream,
  ...
  if ((resource = do_QueryInterface(aValue)) != nullptr) {
    ...
    rv = rdf_BlockingWrite(aStream, kRDFResource1, sizeof(kRDFResource1) - 1);
    if (NS_FAILED(rv)) return rv;
    rv = rdf_BlockingWrite(aStream, uri);
    if (NS_FAILED(rv)) return rv;
    rv = rdf_BlockingWrite(aStream, kRDFResource2, sizeof(kRDFResource2) - 1);
    if (NS_FAILED(rv)) return rv;
...
nsresult nsRDFXMLSerializer::SerializeProperty(nsIOutputStream* aStream,
  ...
  mDataSource->GetTargets(aResource, aProperty, true,
                          getter_AddRefs(assertions));
  if (!assertions) return NS_ERROR_FAILURE;
...
nsresult nsRDFXMLSerializer::SerializeDescription(nsIOutputStream* aStream,
                                                  nsIRDFResource* aResource) {
  nsresult rv;
  ...
  if (arcs) {
    ...
    if (isTypedNode) visited.AppendElement(kRDF_type);
    ...
    while (1) {
      ...
      if (IsContainerProperty(property)) continue;

      // Only serialize values for the property once.
      if (visited.Contains(property.get())) continue;
...
nsresult nsRDFXMLSerializer::SerializeMember(nsIOutputStream* aStream,
  ...
  nsresult rv = rdf_BlockingWrite(aStream, kRDFLIOpen, sizeof(kRDFLIOpen) - 1);
  if (NS_FAILED(rv)) return rv;
  ...
  if ((resource = do_QueryInterface(aMember)) != nullptr) {
    ...
    rv = rdf_BlockingWrite(aStream, kRDFLIOpenGT, sizeof(kRDFLIOpenGT) - 1);
    if (NS_FAILED(rv)) return rv;
...
nsresult nsRDFXMLSerializer::SerializePrologue(nsIOutputStream* aStream) {
...
nsresult nsRDFXMLSerializer::SerializeEpilogue(nsIOutputStream* aStream) {
...
class QNameCollector final : public rdfITripleVisitor {
 public:
  ...
  explicit QNameCollector(nsRDFXMLSerializer* aParent) : mParent(aParent) {}
...
nsresult QNameCollector::Visit(nsIRDFNode* aSubject, nsIRDFResource* aPredicate,
...
nsresult nsRDFXMLSerializer::CollectNamespaces() {
  ...
  nsCOMPtr<rdfITripleVisitor> collector = new QNameCollector(this);
...
bool nsRDFXMLSerializer::IsA(nsIRDFDataSource* aDataSource,
                             nsIRDFResource* aResource, nsIRDFResource* aType) {
  nsresult rv;

  bool result;
  rv = aDataSource->HasAssertion(aResource, kRDF_instanceOf, aType, true,
                                 &result);
