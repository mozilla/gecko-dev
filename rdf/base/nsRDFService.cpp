...
static void *DataSourceAllocTable(void *pool, size_t size) {
  return malloc(size);
}

static void DataSourceFreeTable(void *pool, void *item) { free(item); }

static PLHashEntry *DataSourceAllocEntry(void *pool, const void *key) {
  return (PLHashEntry *)malloc(sizeof(PLHashEntry));
}

static void DataSourceFreeEntry(void *pool, PLHashEntry *he, unsigned flag) {
...
static PLHashAllocOps dataSourceHashAllocOps = {
    DataSourceAllocTable, DataSourceFreeTable, DataSourceAllocEntry,
    DataSourceFreeEntry};
...
static const PLDHashTableOps gResourceTableOps = {
    ResourceHashEntry::HashKey, ResourceHashEntry::MatchEntry,
...
static const PLDHashTableOps gLiteralTableOps = {
    LiteralHashEntry::HashKey, LiteralHashEntry::MatchEntry,
    PLDHashTable::MoveEntryStub, PLDHashTable::ClearEntryStub, nullptr};
...
static const PLDHashTableOps gIntTableOps = {
    IntHashEntry::HashKey, IntHashEntry::MatchEntry,
    PLDHashTable::MoveEntryStub, PLDHashTable::ClearEntryStub, nullptr};
...
static const PLDHashTableOps gDateTableOps = {
    DateHashEntry::HashKey, DateHashEntry::MatchEntry,
    PLDHashTable::MoveEntryStub, PLDHashTable::ClearEntryStub, nullptr};
...
static const PLDHashTableOps gBlobTableOps = {
    BlobHashEntry::HashKey, BlobHashEntry::MatchEntry,
    PLDHashTable::MoveEntryStub, PLDHashTable::ClearEntryStub, nullptr};
...
nsresult LiteralImpl::Create(const char16_t *aValue, nsIRDFLiteral **aResult) {
...
nsresult LiteralImpl::QueryInterface(REFNSIID iid, void **result) {
...
nsresult DateImpl::QueryInterface(REFNSIID iid, void **result) {
  if (!result) return NS_ERROR_NULL_POINTER;
...
nsresult DateImpl::EqualsDate(nsIRDFDate *date, bool *result) {
...
nsresult IntImpl::QueryInterface(REFNSIID iid, void **result) {
  if (!result) return NS_ERROR_NULL_POINTER;
...
nsresult IntImpl::EqualsInt(nsIRDFInt *intValue, bool *result) {
...
nsresult RDFServiceImpl::Init() {
  nsresult rv;

  mNamedDataSources =
      PL_NewHashTable(23, PL_HashString, PL_CompareStrings, PL_CompareValues,
...
// static
nsresult RDFServiceImpl::CreateSingleton(nsISupports *aOuter, const nsIID &aIID,
                                         void **aResult) {
...
static inline bool IsLegalSchemeCharacter(const char aChar) {
...
NS_IMETHODIMP
RDFServiceImpl::GetResource(const nsACString &aURI,
  ...
  rv = factory->CreateInstance(nullptr, NS_GET_IID(nsIRDFResource),
                               (void **)&result);
  if (NS_FAILED(rv)) return rv;
  ...
  if (hdr) {
    ...
    MOZ_LOG(gLog, LogLevel::Debug,
            ("rdfserv   replace-resource [%p] <-- [%p] %s",
             static_cast<ResourceHashEntry *>(hdr)->mResource, aResource,
             (const char *)uri));
  } else {
...
NS_IMETHODIMP
RDFServiceImpl::RegisterDataSource(nsIRDFDataSource *aDataSource,
                                   bool aReplace) {
  ...
  if (*hep) {
    ...
    // refcounts.
    MOZ_LOG(gLog, LogLevel::Debug,
            ("rdfserv    replace-datasource [%p] <-- [%p] %s", (*hep)->value,
             aDataSource, uri.get()));
...
nsresult RDFServiceImpl::GetDataSource(const char *aURI, bool aBlock,
                                       nsIRDFDataSource **aDataSource) {
...
nsresult RDFServiceImpl::RegisterLiteral(nsIRDFLiteral *aLiteral) {
  ...
  MOZ_LOG(gLog, LogLevel::Debug,
          ("rdfserv   register-literal [%p] %s", aLiteral,
           NS_ConvertUTF16toUTF8(value).get()));

  return NS_OK;
}

nsresult RDFServiceImpl::UnregisterLiteral(nsIRDFLiteral *aLiteral) {
  const char16_t *value;
  aLiteral->GetValueConst(&value);
  ...
  MOZ_LOG(gLog, LogLevel::Debug,
          ("rdfserv unregister-literal [%p] %s", aLiteral,
           NS_ConvertUTF16toUTF8(value).get()));
...
nsresult RDFServiceImpl::RegisterInt(nsIRDFInt *aInt) {
  int32_t value;
  aInt->GetValue(&value);
  ...
  MOZ_LOG(gLog, LogLevel::Debug,
          ("rdfserv   register-int [%p] %d", aInt, value));

  return NS_OK;
}

nsresult RDFServiceImpl::UnregisterInt(nsIRDFInt *aInt) {
  int32_t value;
  aInt->GetValue(&value);
  ...
  // reference to it in the hashtable.
  MOZ_LOG(gLog, LogLevel::Debug,
          ("rdfserv unregister-int [%p] %d", aInt, value));
...
nsresult RDFServiceImpl::RegisterDate(nsIRDFDate *aDate) {
  PRTime value;
...
nsresult RDFServiceImpl::UnregisterDate(nsIRDFDate *aDate) {
  PRTime value;
  aDate->GetValue(&value);
...
nsresult RDFServiceImpl::RegisterBlob(BlobImpl *aBlob) {
...
nsresult RDFServiceImpl::UnregisterBlob(BlobImpl *aBlob) {
