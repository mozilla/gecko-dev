...
class MOZ_STACK_CLASS AutoSelectionSetterAfterTableEdit final {
 ...
 public:
  AutoSelectionSetterAfterTableEdit(HTMLEditor& aHTMLEditor,
                                    nsIDOMElement* aTable, int32_t aRow,
...
nsresult HTMLEditor::InsertCell(nsIDOMElement* aDOMCell, int32_t aRowSpan,
  ...
  nsresult rv = CreateElementWithDefaults(
...
nsresult HTMLEditor::SetColSpan(nsIDOMElement* aCell, int32_t aColSpan) {
...
nsresult HTMLEditor::SetRowSpan(nsIDOMElement* aCell, int32_t aRowSpan) {
  NS_ENSURE_TRUE(aCell, NS_ERROR_NULL_POINTER);
  nsAutoString newSpan;
...
HTMLEditor::InsertTableCell(int32_t aNumber, bool aAfter) {
  ...
  nsresult rv = GetCellContext(
      nullptr, getter_AddRefs(table), getter_AddRefs(curCell),
      getter_AddRefs(cellParent), &cellOffset, &startRowIndex, &startColIndex);
  ...
  rv = GetCellDataAt(table, startRowIndex, startColIndex,
                     getter_AddRefs(curCell), &curStartRowIndex,
                     &curStartColIndex, &rowSpan, &colSpan, &actualRowSpan,
                     &actualColSpan, &isSelected);
  NS_ENSURE_SUCCESS(rv, rv);
...
HTMLEditor::GetFirstRow(nsIDOMElement* aTableElement, nsIDOMNode** aRowNode) {
  ...
  *aRowNode = nullptr;
  ...
  tableElement =
      GetElementOrParentByTagName(NS_LITERAL_STRING("table"), tableElement);
  NS_ENSURE_TRUE(tableElement, NS_ERROR_NULL_POINTER);
...
HTMLEditor::GetNextRow(nsIDOMNode* aCurrentRowNode, nsIDOMNode** aRowNode) {
  ...
  *aRowNode = nullptr;
  ...
  while (parentSibling) {
    ...
    //  or first child of section is not a row (bad HTML or more "_moz_text"
    //  nodes!)
...
nsresult HTMLEditor::GetLastCellInRow(nsIDOMNode* aRowNode,
...
HTMLEditor::InsertTableColumn(int32_t aNumber, bool aAfter) {
  ...
  nsresult rv = GetCellContext(getter_AddRefs(selection), getter_AddRefs(table),
                               getter_AddRefs(curCell), nullptr, nullptr,
  ...
  for (rowIndex = 0; rowIndex < rowCount; rowIndex++) {
    if (startColIndex < colCount) {
      ...
      rv =
          GetCellDataAt(table, rowIndex, startColIndex, getter_AddRefs(curCell),
                        &curStartRowIndex, &curStartColIndex, &rowSpan,
                        &colSpan, &actualRowSpan, &actualColSpan, &isSelected);
      NS_ENSURE_SUCCESS(rv, rv);
...
HTMLEditor::InsertTableRow(int32_t aNumber, bool aAfter) {
  ...
  int32_t startRowIndex, startColIndex;
  nsresult rv =
      GetCellContext(nullptr, getter_AddRefs(table), getter_AddRefs(curCell),
  ...
  if (startRowIndex < rowCount) {
    ...
    int32_t colIndex = 0;
    while (NS_SUCCEEDED(
        GetCellDataAt(table, startRowIndex, colIndex, getter_AddRefs(curCell),
    ...
    while (NS_SUCCEEDED(
        GetCellDataAt(table, lastRow, tempColIndex, getter_AddRefs(curCell),
                      &curStartRowIndex, &curStartColIndex, &rowSpan, &colSpan,
                      &actualRowSpan, &actualColSpan, &isSelected))) {
      if (!rowSpan) {
        cellsInRow -= actualColSpan;
...
nsresult HTMLEditor::DeleteTable2(nsIDOMElement* aTable,
...
HTMLEditor::DeleteTableCell(int32_t aNumber) {
  ...
  nsresult rv = GetCellContext(getter_AddRefs(selection), getter_AddRefs(table),
                               getter_AddRefs(cell), nullptr, nullptr,
                               &startRowIndex, &startColIndex);
  ...
  if (firstCell && selection->RangeCount() > 1) {
    ...
    while (cell) {
      ...
      if (!deleteRow) {
        if (checkToDeleteColumn) {
          ...
          // Clear this so we don't repeat AllCellsInColSelected within the same
          // Col
    ...
    for (int32_t i = 0; i < aNumber; i++) {
      rv = GetCellContext(getter_AddRefs(selection), getter_AddRefs(table),
                          getter_AddRefs(cell), nullptr, nullptr,
                          &startRowIndex, &startColIndex);
      NS_ENSURE_SUCCESS(rv, rv);
      // Don't fail if no cell found
      ...
      if (GetNumberOfCellsInRow(table, startRowIndex) == 1) {
        ...
        // The setCaret object will call AutoSelectionSetterAfterTableEdit in
        // its destructor
        AutoSelectionSetterAfterTableEdit setCaret(
            *this, table, startRowIndex, startColIndex, ePreviousColumn, false);
        AutoTransactionsConserveSelection dontChangeSelection(this);
...
nsresult HTMLEditor::DeleteCellContents(nsIDOMElement* aCell) {
...
nsresult HTMLEditor::DeleteColumn(nsIDOMElement* aTable, int32_t aColIndex) {
  NS_ENSURE_TRUE(aTable, NS_ERROR_NULL_POINTER);
...
nsresult HTMLEditor::DeleteRow(nsIDOMElement* aTable, int32_t aRowIndex) {
  NS_ENSURE_TRUE(aTable, NS_ERROR_NULL_POINTER);
...
HTMLEditor::SelectTableRow() {
  ...
  rv = GetCellContext(getter_AddRefs(selection), getter_AddRefs(table),
                      getter_AddRefs(cell), nullptr, nullptr, &startRowIndex,
                      &startColIndex);
  NS_ENSURE_SUCCESS(rv, rv);
  NS_ENSURE_TRUE(table, NS_ERROR_FAILURE);
...
HTMLEditor::SplitTableCell() {
  ...
  nsresult rv =
      GetCellContext(nullptr, getter_AddRefs(table), getter_AddRefs(cell),
                     nullptr, nullptr, &startRowIndex, &startColIndex);
  NS_ENSURE_SUCCESS(rv, rv);
  if (!table || !cell) {
    return NS_SUCCESS_EDITOR_ELEMENT_NOT_FOUND;
...
nsresult HTMLEditor::CopyCellBackgroundColor(nsIDOMElement* destCell,
...
nsresult HTMLEditor::SplitCellIntoColumns(nsIDOMElement* aTable,
...
nsresult HTMLEditor::SplitCellIntoRows(nsIDOMElement* aTable, int32_t aRowIndex,
  ...
  for (;;) {
    ...
    rv = GetCellDataAt(aTable, rowBelowIndex, colIndex, getter_AddRefs(cell2),
...
HTMLEditor::JoinTableCells(bool aMergeNonContiguousContents) {
  ...
  // Get cell, table, etc. at selection anchor node
  nsresult rv =
      GetCellContext(nullptr, getter_AddRefs(table), getter_AddRefs(targetCell),
                     nullptr, nullptr, &startRowIndex, &startColIndex);
  NS_ENSURE_SUCCESS(rv, rv);
  if (!table || !targetCell) {
    return NS_SUCCESS_EDITOR_ELEMENT_NOT_FOUND;
  ...
  if (joinSelectedCells) {
    ...
    rv = GetCellSpansAt(table, firstRowIndex, firstColIndex, firstRowSpan,
                        firstColSpan);
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    for (rowIndex = firstRowIndex; rowIndex <= lastRowIndex; rowIndex++) {
      ...
      for (colIndex = firstColIndex; colIndex < colCount;
        ...
        rv = GetCellDataAt(table, rowIndex, colIndex, getter_AddRefs(cell2),
                           &startRowIndex2, &startColIndex2, &rowSpan2,
                           &colSpan2, &actualRowSpan2, &actualColSpan2,
                           &isSelected2);
        NS_ENSURE_SUCCESS(rv, rv);
    ...
    // cell
    rv = GetCellDataAt(table, startRowIndex, startColIndex,
                       getter_AddRefs(targetCell), &startRowIndex,
                       &startColIndex, &rowSpan, &colSpan, &actualRowSpan,
                       &actualColSpan, &isSelected);
    NS_ENSURE_SUCCESS(rv, rv);
    ...
    rv = GetCellDataAt(table, startRowIndex, startColIndex + actualColSpan,
                       getter_AddRefs(cell2), &startRowIndex2, &startColIndex2,
                       &rowSpan2, &colSpan2, &actualRowSpan2, &actualColSpan2,
                       &isSelected2);
    NS_ENSURE_SUCCESS(rv, rv);
    if (!cell2) {
...
nsresult HTMLEditor::MergeCells(nsCOMPtr<nsIDOMElement> aTargetCell,
...
nsresult HTMLEditor::FixBadRowSpan(nsIDOMElement* aTable, int32_t aRowIndex,
...
nsresult HTMLEditor::FixBadColSpan(nsIDOMElement* aTable, int32_t aColIndex,
...
NS_IMETHODIMP
HTMLEditor::GetCellIndexes(nsIDOMElement* aCell, int32_t* aRowIndex,
                           int32_t* aColIndex) {
...
nsTableWrapperFrame* HTMLEditor::GetTableFrame(nsIDOMElement* aTable) {
...
int32_t HTMLEditor::GetNumberOfCellsInRow(nsIDOMElement* aTable,
                                          int32_t rowIndex) {
  int32_t cellCount = 0;
  nsCOMPtr<nsIDOMElement> cell;
  int32_t colIndex = 0;
...
NS_IMETHODIMP
HTMLEditor::GetTableSize(nsIDOMElement* aTable, int32_t* aRowCount,
                         int32_t* aColCount) {
  nsCOMPtr<Element> table = do_QueryInterface(aTable);
  return GetTableSize(table, aRowCount, aColCount);
}

nsresult HTMLEditor::GetTableSize(Element* aTable, int32_t* aRowCount,
                                  int32_t* aColCount) {
...
NS_IMETHODIMP
HTMLEditor::GetCellDataAt(nsIDOMElement* aTable, int32_t aRowIndex,
...
NS_IMETHODIMP
HTMLEditor::GetCellAt(nsIDOMElement* aTable, int32_t aRowIndex,
                      int32_t aColIndex, nsIDOMElement** aCell) {
...
nsresult HTMLEditor::GetCellSpansAt(nsIDOMElement* aTable, int32_t aRowIndex,
...
nsresult HTMLEditor::GetCellContext(Selection** aSelection,
...
nsresult HTMLEditor::GetCellFromRange(nsRange* aRange, nsIDOMElement** aCell) {
...
NS_IMETHODIMP
HTMLEditor::GetFirstSelectedCellInTable(int32_t* aRowIndex, int32_t* aColIndex,
                                        nsIDOMElement** aCell) {
  NS_ENSURE_TRUE(aCell, NS_ERROR_NULL_POINTER);
  *aCell = nullptr;
  if (aRowIndex) {
...
void HTMLEditor::SetSelectionAfterTableEdit(nsIDOMElement* aTable, int32_t aRow,
                                            int32_t aCol, int32_t aDirection,
                                            bool aSelected) {
    ...
    *aSelectedCount = selection->RangeCount();
    ...
    if (anchorNode->HasChildNodes()) {
      ...
      if (!selectedNode) {
        ...
        // If anchor doesn't have a child, we can't be selecting a table
        // element,
...
bool HTMLEditor::AllCellsInRowSelected(nsIDOMElement* aTable, int32_t aRowIndex,
  ...
  for (int32_t col = 0; col < aNumberOfColumns;
    ...
    nsCOMPtr<nsIDOMElement> cell;
    nsresult rv =
        GetCellDataAt(aTable, aRowIndex, col, getter_AddRefs(cell),
                      &curStartRowIndex, &curStartColIndex, &rowSpan, &colSpan,
                      &actualRowSpan, &actualColSpan, &isSelected);
...
bool HTMLEditor::AllCellsInColumnSelected(nsIDOMElement* aTable,
                                          int32_t aColIndex,
  ...
  for (int32_t row = 0; row < aNumberOfRows;
    ...
    nsCOMPtr<nsIDOMElement> cell;
    nsresult rv =
        GetCellDataAt(aTable, row, aColIndex, getter_AddRefs(cell),
                      &curStartRowIndex, &curStartColIndex, &rowSpan, &colSpan,
                      &actualRowSpan, &actualColSpan, &isSelected);
