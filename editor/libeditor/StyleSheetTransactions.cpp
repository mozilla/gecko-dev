...
static void AddStyleSheet(EditorBase& aEditor, StyleSheet* aSheet) {
...
static void RemoveStyleSheet(EditorBase& aEditor, StyleSheet* aSheet) {
  nsCOMPtr<nsIDocument> doc = aEditor.GetDocument();
  if (doc) {
    doc->BeginUpdate(UPDATE_STYLE);
...
AddStyleSheetTransaction::AddStyleSheetTransaction(EditorBase& aEditorBase,
    ...
    : mEditorBase(&aEditorBase), mSheet(&aStyleSheet) {}

NS_IMPL_CYCLE_COLLECTION_INHERITED(AddStyleSheetTransaction,
                                   EditTransactionBase, mEditorBase, mSheet)
