...
void nsTransactionStack::Push(nsTransactionItem* aTransactionItem) {
  if (!aTransactionItem) {
    return;
  }
...
void nsTransactionStack::Push(
    already_AddRefed<nsTransactionItem> aTransactionItem) {
  RefPtr<nsTransactionItem> item(aTransactionItem);
  if (!item) {
    return;
  }
...
already_AddRefed<nsTransactionItem> nsTransactionStack::Pop() {
  RefPtr<nsTransactionItem> item =
      dont_AddRef(static_cast<nsTransactionItem*>(nsDeque::Pop()));
  return item.forget();
}

already_AddRefed<nsTransactionItem> nsTransactionStack::PopBottom() {
  RefPtr<nsTransactionItem> item =
      dont_AddRef(static_cast<nsTransactionItem*>(nsDeque::PopFront()));
  return item.forget();
}

already_AddRefed<nsTransactionItem> nsTransactionStack::Peek() {
  RefPtr<nsTransactionItem> item =
      static_cast<nsTransactionItem*>(nsDeque::Peek());
  return item.forget();
}

already_AddRefed<nsTransactionItem> nsTransactionStack::GetItem(
    int32_t aIndex) {
...
void nsTransactionStack::Clear() {
  while (GetSize() != 0) {
    RefPtr<nsTransactionItem> item = mType == FOR_UNDO ? Pop() : PopBottom();
  }
}

void nsTransactionStack::DoTraverse(nsCycleCollectionTraversalCallback& cb) {
