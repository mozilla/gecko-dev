...
nsresult nsPrefetchNode::OpenChannel() {
  ...
  if (source->IsHTMLElement(nsGkAtoms::link)) {
    dom::HTMLLinkElement *link =
        static_cast<dom::HTMLLinkElement *>(source.get());
...
nsresult nsPrefetchService::Preload(nsIURI *aURI, nsIURI *aReferrerURI,
    ...
    if (domNode && domNode->IsInComposedDoc()) {
      RefPtr<AsyncEventDispatcher> asyncDispatcher =
          new AsyncEventDispatcher(  // domNode->OwnerDoc(),
              domNode, NS_LITERAL_STRING("error"),
              /* aCanBubble = */ false,
              /* aCancelable = */ false);
  ...
  if (NS_SUCCEEDED(rv)) {
    ...
    if (domNode && domNode->IsInComposedDoc()) {
      RefPtr<AsyncEventDispatcher> asyncDispatcher =
          new AsyncEventDispatcher(domNode, NS_LITERAL_STRING("error"),
                                   /* aCanBubble = */ false,
                                   /* aCancelable = */ false);
      asyncDispatcher->RunDOMEventWhenSafe();
    }
...
nsresult nsPrefetchService::Prefetch(nsIURI *aURI, nsIURI *aReferrerURI,
                                     nsIDOMNode *aSource, bool aExplicit) {
  NS_ENSURE_ARG_POINTER(aURI);
  NS_ENSURE_ARG_POINTER(aReferrerURI);
...
NS_IMETHODIMP
nsPrefetchService::CancelPrefetchPreloadURI(nsIURI *aURI, nsIDOMNode *aSource) {
  NS_ENSURE_ARG_POINTER(aURI);
