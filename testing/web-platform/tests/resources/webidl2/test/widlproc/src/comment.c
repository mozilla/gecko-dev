 ...
 * refer back.
 */
static struct comment *joininlinecomments(struct comment *comments) {
  ...
  for (;;) {
    ...
    if (comment->type != TOK_INLINECOMMENT) {
      ...
      pcomment = &comment->next;
               ...
               (!comment->next || comment->next->type != TOK_INLINECOMMENT ||
      ...
      *pcomment = comment->next;
      ...
      for (;;) {
        ...
        do {
          ...
          if (newcomment) memcpy(wp, comment2->text, len);
          wp += len;
          linenum--;
        ...
        newcomment = memalloc(sizeof(struct comment) + wp - newcomment->text);
 ...
 *                   descriptive element
 */
static void outputchildren(struct cnode *cnode, unsigned int indent,
                           int indesc) {
 ...
 */
static int default_askend(struct cnode *cnode, const struct cnodefuncs *type) {
  return 1;
}
 ...
 * Return:  non-zero if this node wants to end
 */
static int root_askend(struct cnode *cnode, const struct cnodefuncs *type) {
  return 0;
}
 ...
 */
static void root_output(struct cnode *cnode, unsigned int indent) {
 ...
 */
static struct cnode *endcnode(struct cnode *cnode) {
  if (cnode->funcs->end) (*cnode->funcs->end)(cnode);
 ...
 */
static struct cnode *endspecificcnode(struct cnode *cnode,
 ...
 */
static struct cnode *startcnode(struct cnode *cnode, struct cnode *newcnode) {
 ...
 *          indent = indent (nesting) level
 */
static void para_output(struct cnode *cnode, unsigned int indent) {
  printf("%*s<p>\n", indent, "");
 ...
 * Enter:   cnode struct
 */
static void para_end(struct cnode *cnode) {
  /* If the para cnode is empty, remove it. */
  if (!cnode->children) cnode->parent->children = cnode->next;
}
 ...
 *          indent = indent (nesting) level
 */
static void brief_output(struct cnode *cnode, unsigned int indent) {
  printf("%*s<brief>\n", indent, "");
  outputchildren(cnode, indent, 1);
  printf("%*s</brief>\n", indent, "");
}
 ...
 *          indent = indent (nesting) level
 */
static void return_output(struct cnode *cnode, unsigned int indent) {
 ...
 *          indent = indent (nesting) level
 */
static void name_output(struct cnode *cnode, unsigned int indent) {
  printf("%*s<name>\n", indent, "");
  outputchildren(cnode, indent, 1);
  printf("%*s</name>\n", indent, "");
}
 ...
 *          indent = indent (nesting) level
 */
static void author_output(struct cnode *cnode, unsigned int indent) {
  printf("%*s<author>\n", indent, "");
  outputchildren(cnode, indent, 1);
  printf("%*s</author>\n", indent, "");
}
 ...
 *          indent = indent (nesting) level
 */
static void version_output(struct cnode *cnode, unsigned int indent) {
  printf("%*s<version>\n", indent, "");
  outputchildren(cnode, indent, 1);
  printf("%*s</version>\n", indent, "");
}
 ...
 * Enter:   cnode struct
 */
static void code_end(struct cnode *cnode) {
  if (incode) {
 ...
 *          indent = indent (nesting) level
 */
static void code_output(struct cnode *cnode, unsigned int indent) {
 ...
 * Return:  new current cnode
 */
static struct cnode *startpara(struct cnode *cnode,
                               const struct cnodefuncs *type) {
  struct cnode *newcnode;
  while ((*cnode->funcs->askend)(cnode, type)) cnode = endcnode(cnode);
 ...
 * Enter:   cnode struct
 */
static void text_end(struct cnode *cnode) {
 ...
 *          indent = indent (nesting) level
 */
static void text_output(struct cnode *cnode, unsigned int indent) {
 ...
 * Enter:   cnode struct
 */
static void html_end(struct cnode *cnode) {
  if (((struct htmlcnode *)cnode)->desc->flags & HTMLEL_BLOCK) inhtmlblock--;
}
 ...
 *          indent = indent (nesting) level
 */
static void html_output(struct cnode *cnode, unsigned int indent) {
  struct htmlcnode *htmlcnode = (void *)cnode;
  if (!(htmlcnode->desc->flags & HTMLEL_INLINE)) printf("%*s", indent, "");
  if (htmlcnode->cn.children) {
    printf("<%s%s>", htmlcnode->desc->name, htmlcnode->attrs);
    if (!(htmlcnode->desc->flags & HTMLEL_INLINE)) putchar('\n');
 ...
 * Return:  new current cnode
 */
static struct cnode *starthtmlcnode(struct cnode *cnode,
    ...
    if (cnode->funcs != &html_funcs) {
      ...
      if (!(htmleldesc->flags & HTMLEL_INLINE)) {
        if (!(htmleldesc->flags & HTMLEL_BLOCK))
          locerrorexit(filename, linenum, "<%s> not valid here",
                       htmleldesc->name);
        while ((*cnode->funcs->askend)(cnode, 0)) cnode = endcnode(cnode);
      } else {
        while (cnode->funcs == &text_funcs) cnode = endcnode(cnode);
      }
      break;
    }
  ...
  if (htmleldesc->flags & HTMLEL_BLOCK) inhtmlblock++;
 ...
 */
static void param_output(struct cnode *cnode, unsigned int indent) {
 ...
 * Return:  new current cnode
 */
static struct cnode *startparamcnode(struct cnode *cnode, const char *word,
                                     unsigned int wordlen, int inout,
                                     const struct cnodefuncs *funcs) {
 ...
 *          indent = indent (nesting) level
 */
static void api_feature_output(struct cnode *cnode, unsigned int indent) {
  struct paramcnode *paramcnode = (void *)cnode;
 ...
 *          indent = indent (nesting) level
 */
static void device_cap_output(struct cnode *cnode, unsigned int indent) {
  struct paramcnode *paramcnode = (void *)cnode;
...
/***********************************************************************
 * def_api_feature_askend : ask if def-api-feature cnode wants to end at new
 * para
 *
 ...
 */
static int def_api_feature_askend(struct cnode *cnode,
                                  const struct cnodefuncs *type) {
 ...
 *          indent = indent (nesting) level
 */
static void def_api_feature_output(struct cnode *cnode, unsigned int indent) {
  struct paramcnode *paramcnode = (void *)cnode;
...
/***********************************************************************
 * def_api_feature_set_askend : ask if def-api-feature-set cnode wants to end at
 * new para
 *
 ...
 */
static int def_api_feature_set_askend(struct cnode *cnode,
                                      const struct cnodefuncs *type) {
  /* A def-api-feature-set does not end at a plain para, an html block element,
   * a brief para, or an api-feature. */
  if (!type || type == &para_funcs || type == &api_feature_funcs ||
      type == &brief_funcs)
    return 0;
  return 1;
}
 ...
 *          indent = indent (nesting) level
 */
static void def_api_feature_set_output(struct cnode *cnode,
                                       unsigned int indent) {
  struct paramcnode *paramcnode = (void *)cnode;
...
/***********************************************************************
 * def_instantiated_askend : ask if def-instantiated cnode wants to end at new
 * para
 *
 ...
 */
static int def_instantiated_askend(struct cnode *cnode,
                                   const struct cnodefuncs *type) {
 ...
 *          indent = indent (nesting) level
 */
static void def_instantiated_output(struct cnode *cnode, unsigned int indent) {
 ...
 */
static int def_device_cap_askend(struct cnode *cnode,
                                 const struct cnodefuncs *type) {
 ...
 *          indent = indent (nesting) level
 */
static void def_device_cap_output(struct cnode *cnode, unsigned int indent) {
  struct paramcnode *paramcnode = (void *)cnode;
 ...
 * Return:  new current cnode
 */
static struct cnode *addtext(struct cnode *cnode, const char *text,
                             unsigned int len) {
 ...
 */
static const char *parseword(const char **pp) {
  const char *p = *pp, *word = 0;
  int ch = *p;
  while (iswhitespace(ch)) ch = *++p;
  word = p;
  while ((unsigned)((ch & ~0x20) - 'A') <= 'Z' - 'A' ||
         (unsigned)(ch - '0') < 10 || ch == '_' || ch == '.' || ch == ':' ||
         ch == '/' || ch == '-') {
    ch = *++p;
  }
  if (p == word) return 0;
 ...
 *          filename, linenum = current filename and line number (for error
 * reporting)
 *
 ...
 */
static const char *dox_b(const char *p, struct cnode **pcnode,
                         const struct cnodefuncs *type, const char *filename,
                         unsigned int linenum, const char *cmdname) {
  ...
  if (word) {
    ...
    mycnode = cnode =
        starthtmlcnode(cnode, HTMLELDESC_B, 0, 0, filename, linenum);
    cnode = addtext(cnode, word, p - word);
    while (cnode != mycnode) cnode = endcnode(cnode);
    cnode = endcnode(cnode);
  }
  *pcnode = cnode;
  return p;
 ...
 */
static const char *dox_n(const char *p, struct cnode **pcnode,
                         const struct cnodefuncs *type, const char *filename,
                         unsigned int linenum, const char *cmdname) {
  struct cnode *cnode = *pcnode;
 ...
 */
static const char *dox_code(const char *p, struct cnode **pcnode,
                            const struct cnodefuncs *type, const char *filename,
                            unsigned int linenum, const char *cmdname) {
 ...
 */
static const char *dox_endcode(const char *p, struct cnode **pcnode,
                               const struct cnodefuncs *type,
 ...
 */
static const char *dox_param(const char *p, struct cnode **pcnode,
                             const struct cnodefuncs *type,
                             const char *filename, unsigned int linenum,
                             const char *cmdname) {
  struct cnode *cnode = *pcnode;
  ...
  if (*p == '[') {
    ...
    if (*p != ']') locerrorexit(filename, linenum, "bad attributes on \\param");
    p++;
  }
  /* Get the next word as the parameter name. */
  word = parseword(&p);
  if (!word) locerrorexit(filename, linenum, "expected word after \\param");
 ...
 */
static const char *dox_para(const char *p, struct cnode **pcnode,
                            const struct cnodefuncs *type, const char *filename,
                            unsigned int linenum, const char *cmdname) {
 ...
 */
static const char *dox_throw(const char *p, struct cnode **pcnode,
                             const struct cnodefuncs *type,
                             const char *filename, unsigned int linenum,
                             const char *cmdname) {
  struct cnode *cnode = *pcnode;
  ...
  word = parseword(&p);
  if (!word) locerrorexit(filename, linenum, "expected word after \\throw");
  /* Close any open nodes. */
  while ((*cnode->funcs->askend)(cnode, type)) cnode = endcnode(cnode);
 ...
 */
static const char *dox_attr(const char *p, struct cnode **pcnode,
                            const struct cnodefuncs *type, const char *filename,
                            unsigned int linenum, const char *cmdname) {
  struct cnode *cnode = *pcnode;
  const char *word;
 ...
 */
struct command {
  const char *(*func)(const char *p, struct cnode **pcnode,
                      const struct cnodefuncs *type, const char *filename,
                      unsigned int linenum, const char *cmdname);
  const struct cnodefuncs *type;
  unsigned int namelen;
 ...
 */
static const char *parsehtmltag(const char *start, struct cnode **pcnode,
  ...
  for (;;) {
    if (!ch) locerrorexit(filename, *plinenum, "unterminated HTML tag");
    if (ch == '\n')
      linenum++;
  ...
  if (!close && endname != name && end[-2] == '/') close = 2;
  ...
  if (close == 1) {
    ...
    for (;;) {
      ...
      if (cnode->funcs != &text_funcs) {
        if (cnode->funcs != &html_funcs) {
          locerrorexit(filename, *plinenum, "mismatched %.*s", end - start,
                       start);
        }
        htmlcnode = (struct htmlcnode *)cnode;
        if (htmlcnode->desc == htmleldesc) break;
        if (!(htmlcnode->desc->flags & HTMLEL_AUTOCLOSE)) {
          locerrorexit(filename, htmlcnode->cn.linenum, "mismatched <%.*s>",
    ...
    /* Opening tag. */
    if (close != 2)
      cnode = starthtmlcnode(cnode, htmleldesc, endname, end - 1 - endname,
                             filename, *plinenum);
    else  // don't include the closing "/" in the attributes list
      cnode = starthtmlcnode(cnode, htmleldesc, endname, end - 2 - endname,
                             filename, *plinenum);
 ...
 */
static void parsecomment(struct comment *comment) {
  ...
  for (;;) {
    ...
    if (cnode->funcs->needpara) cnode = startpara(cnode, &para_funcs);
    ...
    while (ch && ch != '\n') {
      ...
      if (p - starttext) cnode = addtext(cnode, starttext, p - starttext);
      ...
      if (ch == '&' && p[1] != '#') {
        ...
        for (;;) {
          len = strlen(entity);
          if (!len)
            locerrorexit(comment->filename, linenum,
                         "unrecognised HTML entity &%.*s;", semicolon - p, p);
          if (len == semicolon - p && !memcmp(p, entity, len)) break;
          entity += len + 1;
      ...
      }
      {
        ...
        if (!cmdlen)
          locerrorexit(comment->filename, linenum,
                       "\\ or @ without Doxygen command");
        /* Look it up in the table. */
        command = commands;
        for (;;) {
          if (!command->namelen) {
            locerrorexit(comment->filename, linenum,
                         "unrecognized Doxygen command '%.*s'", cmdlen + 1,
                         start - 1);
          }
        ...
        p = (*command->func)(p, &cnode, command->type, comment->filename,
    ...
    if (!ch) break;
    if (cnode->funcs == &text_funcs) addtext(cnode, "\n", 1);
    p++;
  }
 ...
 */
static void parsecomments(struct comment *comment) {
  while (comment) {
 ...
 */
static void attachcommenttonode(struct node *node, struct comment *comment) {
 ...
 */
static void attachcomments(struct comment *comment, struct node *root) {
  while (comment) {
    ...
    for (;;) {
      ...
      if (cnode->funcs == &param_funcs || cnode->funcs == &return_funcs ||
        ...
        if (cnode->funcs == &param_funcs) {
          ...
          if (!node)
            locerrorexit(comment->filename, cnode->linenum,
...
void outputdescriptive(struct node *node, unsigned int indent) {
  ...
  while (comment) {
    ...
    if (!indescriptive) printf("%*s<descriptive>\n", indent, "");
    indescriptive = 1;
