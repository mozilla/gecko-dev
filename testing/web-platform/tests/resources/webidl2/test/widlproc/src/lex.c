...
void readinput(const char *const *argv) {
  ...
  for (;;) {
    ...
    if (isstdin) {
      ...
      if (!handle) errorexit("%s: %s", filename, strerror(errno));
    }
    for (;;) {
    ...
    if (ferror(handle)) errorexit("%s: I/O error", filename);
    if (!isstdin) fclose(handle);
    buf[len] = 0;
 ...
 */
static void lexerrorexit(const char *format, ...) {
  va_list ap;
  va_start(ap, format);
 ...
 */
static struct tok *lexblockcomment(const char *start) {
  const char *p = start + 1;
  tok.filename = file->filename;
  tok.linenum = file->linenum;
  for (;;) {
    int ch = *++p;
    if (!ch) lexerrorexit("unterminated block comment");
    if (ch != '*') {
      if (ch == '\n') file->linenum++;
      continue;
    }
 ...
 * Return:  tok struct, lifetime until next call to lex
 */
static struct tok *lexinlinecomment(const char *start) {
 ...
 */
static struct tok *lexnumber(const char *start) {
  for (;;) {
    const char *p = start;
    ...
    for (;;) {
      if ((unsigned)(ch - '0') >= 8) {
        if ((ch & -2) == '8') {
          ...
          if (state != STATE_HEX) {
            if ((ch & ~0x20) != 'E') break;
            if (state == STATE_HEX || state >= STATE_EXPSTART ||
                state == STATE_START)
              break;
            state = STATE_EXPSTART;
          }
        } else if (ch == '.') {
          if (state == STATE_HEX || state >= STATE_DP) break;
          state = STATE_DP;
        } else if (ch == '-') {
          if (state != STATE_EXPSTART) break;
          state = STATE_EXPSIGN;
        } else
          break;
      }
    ...
    switch (state) {
      ...
      case STATE_OCTAL:
        ...
        break;
      case STATE_HEX:
        if (p - start == 2 || (p - start == 3 && *start == '-')) p = start + 1;
        tok.type = TOK_INTEGER;
        break;
      case STATE_EXP:
      case STATE_DP:
 ...
 * Return:  tok struct, lifetime until next call to lex
 */
static struct tok *lexstring(const char *start) {
  for (;;) {
    const char *p = start + 1;
    int ch = *p;
    for (;;) {
 ...
 * Return:  tok struct, lifetime until next call to lex
 */
static struct tok *lexidentifier(const char *start) {
  const char *p = start + 1;
  for (;;) {
    int ch = *p;
...
struct tok *lex(void) {
  ...
  for (;;) {
    ...
    p--;
    if (ch) break;
    if (p != file->end) lexerrorexit("\\0 byte not allowed");
    file = file->next;
  }
  ...
  if (ch == '/') {
    ...
    if (ch == '-' || (unsigned)(ch - '0') < 10) return lexnumber(p);
    /* Handle string. */
    if (ch == '"') return lexstring(p);
    /* Handle identifier. */
...
void outputwidl(struct node *node) {
  ...
  while (node && !node->start) node = nodewalk(node);
  ...
  for (;;) {
    ...
    while (start != end) {
      ...
      if (p == start || p[-1] == '\n') {
        ...
        p = endcomment;
        while (p != thisend && ((ch = *p) == ' ' || ch == '\t')) p++;
        if (p != thisend && *p == '\n') p++;
        endcomment = p;
      }
      printtext(start, comment - start, 1);
      start = endcomment;
      if (start > thisend) start = thisend;
    }
    if (final) break;
    file = file->next;
    assert(file);
