 ...
 */
static void tokerrorexit(struct tok *tok, const char *format, ...) {
  va_list ap;
  char *m;
  va_start(ap, format);
  ...
  if (tok->type == TOK_EOF)
    ...
    locerrorexit(tok->filename, tok->linenum, "at '%.*s': %s", tok->len,
                 tok->start, m);
  va_end(ap);
}
 ...
 */
static struct tok *lexnocomment(void) {
  struct tok *tok;
  for (;;) {
    tok = lex();
    if (tok->type != TOK_BLOCKCOMMENT && tok->type != TOK_INLINECOMMENT) break;
    addcomment(tok);
  }
  return tok;
}
 ...
 */
static void eat(struct tok *tok, int type) {
  if (tok->type != type) {
    const char *p;
    if (type < TOK_DOMString) tokerrorexit(tok, "expected '%c'", type);
    p = keywords;
 ...
 */
static void setid(struct node *node) { node->id = getattr(node, "name"); }
 ...
 */
static char *setidentifier(struct tok *tok) {
  char *s;
  if (tok->type != TOK_IDENTIFIER) tokerrorexit(tok, "expected identifier");
 ...
 * Return:  allocated 0-terminated string
 */
static char *setargumentname(struct tok *tok) {
  char *s;
...
static struct node *parsetypesuffixstartingwitharray(struct tok *tok,
                                                     struct node *node);
 ...
 *          tok updated
 */
static struct node *parsescopedname(struct tok *tok, const char *name,
                                    int ref) {
 ...
 *          tok updated
 */
static struct node *parsescopednamelist(struct tok *tok, const char *name,
                                        const char *name2, int comment) {
  struct node *node = newelement(name);
  for (;;) {
    struct node *attr = parsescopedname(tok, "name", 1);
    struct node *n = newelement(name2);
    if (comment) setcommentnode(n);
    addnode(n, attr);
    addnode(node, n);
    if (tok->type != ',') break;
    lexnocomment();
 ...
 *          tok updated
 */
static struct node *parsereturntype(struct tok *tok) {
 ...
 */
static const char *parseunsignedintegertype(struct tok *tok) {
  static const char *names[] = {
      "short",          "long",          "long long",         0,
      "unsigned short", "unsigned long", "unsigned long long"};
  enum { TYPE_SHORT, TYPE_LONG, TYPE_LONGLONG, TYPE_UNSIGNED = 4 };
  int type = 0;
 ...
 *          tok updated
 */
static struct node *parsetypesuffix(struct tok *tok, struct node *node) {
 ...
 *          tok updated
 */
static struct node *parseprimitiveorstringtype(struct tok *tok) {
  struct node *node;
  switch (tok->type) {
    case TOK_unsigned:
    case TOK_short:
 ...
 *          tok updated
 */
static struct node *parsenonanytype(struct tok *tok) {
  struct node *node;
  switch (tok->type) {
    case TOK_IDENTIFIER:
      node = newelement("Type");
 ...
 *          tok updated
 */
static struct node *parseunionmembertype(struct tok *tok) {
  struct node *node;
  if (tok->type == TOK_any) {
 ...
 * and an ArgumentList.
 */
static struct node *parseextendedattribute(struct tok *tok) {
  const char *start;
  ...
  if (!strcmp(attrname, "Constructor") ||
      !strcmp(attrname, "NamedConstructor")) {
    setcommentnode(node);
  }
  lexnocomment();
  if (tok->type == '=') {
...
static struct node *parseextendedattributelist(struct tok *tok) {
  ...
  if (tok->type != ']') tokerrorexit(tok, "expected ',' or ']'");
  lexnocomment();
  return node;
}
 ...
 *          tok updated
 */
static struct node *parseexceptionfield(struct tok *tok, struct node *eal) {
 ...
 */
static struct node *parseargument(struct tok *tok) {
 ...
 *          tok on terminating ';'
 */
static struct node *parseoperationrest(struct tok *tok, struct node *node) {
 ...
 *          tok on terminating ';'
 */
static struct node *parsereturntypeandoperationrest(struct tok *tok,
                                                    struct node *eal,
                                                    struct node *attrs) {
 ...
 *          tok on terminating ';'
 */
static struct node *parseoptionaliteratorinterface(struct tok *tok,
                                                   struct node *node) {
  if (tok->type == '=') {
    lexnocomment();
 ...
 *          tok on terminating ';'
 */
static struct node *parseoperationoriteratorrest(struct tok *tok,
                                                 struct node *eal,
                                                 struct node *attrs) {
  struct node *node;
  struct node *nodeType = parsereturntype(tok);
 ...
 *          tok on terminating ';'
 */
static struct node *parseattribute(struct tok *tok, struct node *eal,
                                   struct node *attrs) {
 ...
 *          tok updated
 */
static struct node *parseserializer(struct tok *tok, struct node *eal) {
  ...
  if (tok->type == '=') {
    ...
    if (tok->type == TOK_IDENTIFIER) {
      ...
      if (tok->type == TOK_getter) {
        ...
        tokerrorexit(tok,
                     "expected 'attribute', 'getter', 'inherit' or attribute "
                     "identifiers in serializer map");
      }
      if (done) {
        lexnocomment();
        eat(tok, '}');
        ...
        do {
          if (tok->type != TOK_IDENTIFIER)
            tokerrorexit(tok,
                         "expected attribute identifiers in serializer map %s",
                         tok->prestart);
      ...
      if (tok->type == TOK_getter) {
        ...
        do {
          if (tok->type != TOK_IDENTIFIER)
            tokerrorexit(tok,
                         "expected attribute identifiers in serializer list");
          nodeAttribute = newelement("PatternAttribute");
          addnode(nodeAttribute, newattr("name", setidentifier(tok)));
      ...
      tokerrorexit(tok,
                   "Expected '{', '[' or an attribute identifier in the "
...
/***********************************************************************
 * parseattributeoroperationoriterator : parse [15]
 ...
 *          tok on terminating ';'
 */
static struct node *parseattributeoroperationoriterator(struct tok *tok,
                                                        struct node *eal) {
  int alreadyseen;
  ...
  if (tok->type == TOK_inherit || tok->type == TOK_readonly ||
      tok->type == TOK_attribute)
    return parseattribute(tok, eal, attrs);
  if (!nodeisempty(attrs))
    return parsereturntypeandoperationrest(tok, eal, attrs);
  alreadyseen = 0;
  for (;;) {
    static const int t[] = {TOK_getter,  TOK_setter,       TOK_creator,
                            TOK_deleter, TOK_legacycaller, 0};
    const int *tt = t;
    char *s;
    while (*tt && *tt != tok->type) tt++;
    if (!*tt) break;
    s = memprintf("%.*s", tok->len, tok->start);
 ...
 *          tok updated
 */
static struct node *parseconstexpr(struct tok *tok, struct node *node) {
  char *s;
  switch (tok->type) {
    case TOK_true:
    case TOK_false:
 ...
 *          tok updated
 */
static struct node *parsedefaultvalue(struct tok *tok, struct node *node) {
  char *s;
  if (tok->type == TOK_STRING) {
    s = memalloc(tok->len + 1);
    memcpy(s, tok->start, tok->len);
 ...
 *          tok on terminating ';'
 */
static struct node *parsedictionarymember(struct tok *tok, struct node *eal) {
 ...
 *          tok on terminating ';'
 */
static struct node *parseconst(struct tok *tok, struct node *eal) {
 ...
 */
static struct node *parseimplementsstatement(struct tok *tok,
                                             struct node *eal) {
 ...
 *          tok updated to the terminating ';'
 */
static struct node *parsetypedef(struct tok *tok, struct node *eal) {
  struct node *ealtype;
 ...
 *          tok updated to the terminating ';'
 */
static struct node *parseexception(struct tok *tok, struct node *eal) {
 ...
 *          tok updated to the terminating ';'
 */
static struct node *parseinterface(struct tok *tok, struct node *eal) {
 ...
 *          tok updated to the terminating ';'
 */
static struct node *parsecallback(struct tok *tok, struct node *eal) {
  struct node *node;
 ...
 *          tok updated to the terminating ';'
 */
static struct node *parsedictionary(struct tok *tok, struct node *eal) {
 ...
 *          tok updated to the terminating ';'
 */
static struct node *parseenum(struct tok *tok, struct node *eal) {
  char *s;
  ...
  for (;;) {
    ...
    switch (tok->type) {
      ...
      default:
        if (eal)
          tokerrorexit(tok,
                       "expected definition after extended attribute list");
        node = 0;
        break;
    }
    if (!node) break;
...
struct node *parse(void) {
  ...
  if (tok->type != TOK_EOF) tokerrorexit(tok, "expected end of input");
