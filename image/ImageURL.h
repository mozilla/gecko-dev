...
class ImageURL {
 public:
  ...
  enum TruncatedSpecStatus { FitsInto1k, TruncatedTo1k };
 ...
 private:
  ...
  PLDHashNumber ComputeHash(const Maybe<uint64_t>& aBlobSerial) const {
    if (aBlobSerial) {
      // For blob URIs, we hash the serial number of the underlying blob, so
      // that different blob URIs which point to the same blob share a cache
      // entry. We also include the ref portion of the URI to support media
      // fragments which requires us to create different Image objects even if
