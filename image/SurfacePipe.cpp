...
nsresult SurfaceSink::Configure(const SurfaceConfig& aConfig) {
  ...
  // altogether.
  nsresult rv = aConfig.mDecoder->AllocateFrame(aConfig.mFrameNum, surfaceSize,
                                                frameRect, aConfig.mFormat);
  if (NS_FAILED(rv)) {
    return rv;
  }
...
nsresult PalettedSurfaceSink::Configure(const PalettedSurfaceConfig& aConfig) {
  ...
  nsresult rv = aConfig.mDecoder->AllocateFrame(
      aConfig.mFrameNum, aConfig.mOutputSize, aConfig.mFrameRect,
      aConfig.mFormat, aConfig.mPaletteDepth);
  if (NS_FAILED(rv)) {
    return rv;
  }
