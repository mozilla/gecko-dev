...
void maybe_split_segment(Elf *elf, ElfSegment *segment, bool fill) {
  ...
  for (ElfSection *last = *(it++); it != segment->end(); last = *(it++)) {
    ...
    if (((*it)->getType() != SHT_NOBITS) && (last->getType() != SHT_NOBITS) &&
      ...
      phdr.p_type = PT_LOAD;
      phdr.p_vaddr = (previous->getAddr() + previous->getSize() +
                      segment->getAlign() - 1) &
                     ~(segment->getAlign() - 1);
      phdr.p_paddr = phdr.p_vaddr + segment->getVPDiff();
      phdr.p_flags = 0;
      phdr.p_align = 0;
      phdr.p_filesz =
          (section->getAddr() & ~(newSegment->getAlign() - 1)) - phdr.p_vaddr;
...
int do_relocation_section(Elf *elf, unsigned int rel_type,
                          unsigned int rel_type2, bool force, bool fill) {
  ...
  Elf32_Shdr relhack32_section = {0,
                                  SHT_PROGBITS,
                                  SHF_ALLOC,
                                  0,
                                  (Elf32_Off)-1,
                                  ...
                                  Elf_RelHack::size(
                                      elf->getClass())};  // TODO: sh_addralign
                                                          // should be an
                                                          // alignment, not size
  ...
  for (typename std::vector<Rel_Type>::iterator i = section->rels.begin();
    ...
    if (init_array && i->r_offset == init_array->getAddr()) {
      if (init_array_reloc) {
        fprintf(stderr,
                "Found multiple relocations for the first init_array entry. "
                "Skipping\n");
        return -1;
      }
  ...
  if (init_array && !init_array_reloc) {
    ...
    if (empty) {
      ...
      fprintf(
          stderr,
          "Didn't find relocation for DT_INIT_ARRAY's first entry. Skipping\n");
      return -1;
    }
  } else if (init_array) {
    Rel_Type *rel = &new_rels[init_array_reloc - 1];
  ...
  if (init_array) {
    ...
    set_relative_reloc(&section->rels[init_array_reloc - 1], elf,
                       init->getValue());
...
void undo_file(const char *name, bool backup = false) {
  ...
  if (first != second) {
    fprintf(stderr, elfhack_data " and " elfhack_text
                                 " not in the same segment. Skipping\n");
    return;
  }
  ...
  if (filler) elf.removeSegment(filler);
