                                        ...
                                        nsAutoPtr<UnixSocketRawData>& aMessage)
{
  ...
  switch (opCode) {
    ...
    case ObexRequestCode::GetFinal: {
        /**
         * When |mVCardDataStream| needs multiple response packets to complete,
         * the client should continue to issue GET requests until the final
         * body information (in an End-of-Body header) arrives, along with the
  ...
  const AppParameterTag aTagId)
{
  ...
  switch (aTagId) {
    ...
    case AppParameterTag::MaxListCount: {
      ...
      AppendNamedValue(aValues, "maxListCount", (uint32_t) maxListCount);
      break;
    }
    case AppParameterTag::ListStartOffset: {
      ...
      AppendNamedValue(aValues, "listStartOffset", (uint32_t) listStartOffset);
      break;
    }
    ...
    case AppParameterTag::vCardSelector: {
      ...
      if (hasVCardSelectorOperator && buf[0]) {
        AppendNamedValue(aValues, "vCardSelector_AND",
                         BluetoothValue(props));
      } else {
        AppendNamedValue(aValues, "vCardSelector_OR",
                         BluetoothValue(props));
      }
      break;
    }
...
bool
BluetoothPbapManager::ReplyToPullPhonebook(nsIDOMBlob* aBlob,
                                           uint16_t aPhonebookSize)
{
  if (!mConnected) {
    return false;
...
bool
BluetoothPbapManager::ReplyToPullvCardListing(nsIDOMBlob* aBlob,
                                              uint16_t aPhonebookSize)
{
  if (!mConnected) {
    return false;
...
bool
BluetoothPbapManager::ReplyToPullvCardEntry(nsIDOMBlob* aBlob)
{
  if (!mConnected) {
    return false;
  }
...
BluetoothPbapManager::GetInputStreamFromBlob(nsIDOMBlob* aBlob)
{
  ...
  nsresult rv = aBlob->GetInternalStream(getter_AddRefs(mVCardDataStream));
  if (NS_FAILED(rv)) {
    BT_LOGR("Failed to get internal stream from blob. rv=0x%x", rv);
    return false;
  }
