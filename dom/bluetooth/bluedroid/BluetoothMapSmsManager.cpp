  ...
  switch (opCode) {
    ...
    case ObexRequestCode::GetFinal: {
      ...
      if (mDataStream) {
        ...
        if (!ReplyToGetWithHeaderBody(res, kObexRespHeaderSize)) {
  ...
  long aMasId, const nsAString& aHandleId, bool aStatus)
{
  ...
  uint8_t* handleId = new uint8_t[(len + 1) * 2];
...
BluetoothMapSmsManager::SendReply(uint8_t aResponseCode)
{
  ...
  SendMasObexData(req, aResponseCode, index);
}
