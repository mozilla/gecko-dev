...
BluetoothMapSmsManager::Listen()
{
   ...
   */
  if (mMasServerSocket) {
    mMasServerSocket->CloseSocket();
    mMasServerSocket = nullptr;
  }

  mMasServerSocket = new BluetoothSocket(this,  BluetoothSocketType::RFCOMM,
                                         false, true);
  ...
  if (NS_WARN_IF(!mMasServerSocket->ListenSocket(sdpString, kMapMas, -1))) {
    mMasServerSocket = nullptr;
    return false;
  }
...
void
BluetoothMapSmsManager::MnsDataHandler(UnixSocketRawData* aMessage)
{
...
void
BluetoothMapSmsManager::MasDataHandler(UnixSocketRawData* aMessage)
{
  /**
   * Ensure
...
BluetoothMapSmsManager::CreateMnsObexConnection()
{
  ...
  mMnsSocket = new BluetoothSocket(this, BluetoothSocketType::RFCOMM, false,
                                   false);
  // Already encrypted in previous session
  mMnsSocket->ConnectSocket(mDeviceAddress, kMapMns, -1);
}
...
BluetoothMapSmsManager::HandleSmsMmsPushMessage(const ObexHeaderSet& aHeader)
{
  ...
  bs->DistributeSignal(
    BluetoothSignal(NS_LITERAL_STRING(MAP_PUSH_MESSAGE_REQ_ID),
                    NS_LITERAL_STRING(KEY_ADAPTER), data));
}

bool
BluetoothMapSmsManager::GetInputStreamFromBlob(nsIDOMBlob* aBlob)
{
  if (mDataStream) {
    mDataStream->Close();
    mDataStream = nullptr;
  ...
  nsresult rv = aBlob->GetInternalStream(getter_AddRefs(mDataStream));
  if (NS_FAILED(rv)) {
