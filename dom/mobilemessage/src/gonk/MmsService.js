...
const PREF_SEND_RETRY_INTERVALS = (function () {
  let intervals =
    Services.prefs.getCharPref("dom.mms.sendRetryIntervals").split(",");
    ...
    value: function run(callback) {
      ...
      let retryCallback = (function (mmsStatus, msg) {
        if ((MMS.MMS_PDU_ERROR_TRANSIENT_FAILURE == mmsStatus ||
          ...
          if (DEBUG) {
            debug("Fail to send. Will retry after: " + PREF_SEND_RETRY_INTERVALS[this.retryCount]);
          }
          ...
          this.timer.initWithCallback(this.send.bind(this, retryCallback),
                                      PREF_SEND_RETRY_INTERVALS[this.retryCount],
                                      Ci.nsITimer.TYPE_ONE_SHOT);
          this.retryCount++;
          return;
        }
