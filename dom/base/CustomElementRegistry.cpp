...
nsAtom* CustomElementData::GetCustomElementType() { return mType; }
...
static void CheckLifeCycleCallbacks(JSContext* aCx,
...
void CustomElementRegistry::Define(const nsAString& aName,
  ...
  {  // Set mIsCustomDefinitionRunning.
    ...
    {  // Enter constructor's compartment.
      ...
      // object.
      if (!JS_GetProperty(cx, constructor, "prototype",
      ...
      /**
       * 10.2. If Type(prototype) is not Object, then throw a TypeError
       * exception.
       */
    ...
    {  // Enter constructorProtoUnwrapped's compartment
       ...
       *       "connectedCallback", "disconnectedCallback", "adoptedCallback",
       * and "attributeChangedCallback", each of which belongs to an entry whose
       *       value is null.
       ...
       *          entry in lifecycleCallbacks with key callbackName to the
       * result of converting callbackValue to the Web IDL Function callback
      ...
      if (callbacksHolder->mAttributeChangedCallback.WasPassed()) {
        ...
        if (!observedAttributesIterable.isUndefined()) {
          ...
          while (true) {
            ...
            if (!ConvertJSValueToString(cx, attribute, eStringify, eStringify,
                                        attrStr)) {
              aRv.StealExceptionFromJSContext(cx);
              return;
            }
  ...
  RefPtr<CustomElementDefinition> definition = new CustomElementDefinition(
      nameAtom, localNameAtom, &aFunctionConstructor, Move(observedAttributes),
      callbacks);
...
CustomElementDefinition::CustomElementDefinition(
    nsAtom* aType, nsAtom* aLocalName, Function* aConstructor,
