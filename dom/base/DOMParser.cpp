...
already_AddRefed<nsIDocument> DOMParser::ParseFromString(const nsAString& aStr,
  ...
  rv = ParseFromString(aStr, StringFromSupportedType(aType),
...
NS_IMETHODIMP
DOMParser::ParseFromString(const char16_t* str, const char* contentType,
...
nsresult DOMParser::ParseFromString(const nsAString& str,
  ...
  rv = NS_NewByteInputStream(getter_AddRefs(stream), utf8str.get(),
                             utf8str.Length(), NS_ASSIGNMENT_DEPEND);
  if (NS_FAILED(rv)) return rv;

  return ParseFromStream(stream, "UTF-8", utf8str.Length(), contentType,
                         aResult);
}

already_AddRefed<nsIDocument> DOMParser::ParseFromBuffer(
    const Sequence<uint8_t>& aBuf, uint32_t aBufLen, SupportedType aType,
    ErrorResult& rv) {
...
NS_IMETHODIMP
DOMParser::ParseFromBuffer(const uint8_t* buf, uint32_t bufLen,
                           const char* contentType, nsIDOMDocument** aResult) {
...
already_AddRefed<nsIDocument> DOMParser::ParseFromStream(
  ...
  nsCOMPtr<nsIDOMDocument> domDocument;
  rv = DOMParser::ParseFromStream(
      aStream, NS_ConvertUTF16toUTF8(aCharset).get(), aContentLength,
      StringFromSupportedType(aType), getter_AddRefs(domDocument));
  nsCOMPtr<nsIDocument> document(do_QueryInterface(domDocument));
  return document.forget();
}

NS_IMETHODIMP
DOMParser::ParseFromStream(nsIInputStream* aStream, const char* aCharset,
                           int32_t aContentLength, const char* aContentType,
                           nsIDOMDocument** aResult) {
...
NS_IMETHODIMP
DOMParser::Init(nsIPrincipal* principal, nsIURI* documentURI, nsIURI* baseURI,
  ...
  if (!mPrincipal) {
    ...
    nsContentUtils::ReportToConsole(
        ...
        "ChromeScriptedDOMParserWithoutPrincipal", nullptr, 0, documentURI);
...
/*static */ already_AddRefed<DOMParser> DOMParser::Constructor(
    const GlobalObject& aOwner, nsIPrincipal* aPrincipal, nsIURI* aDocumentURI,
...
nsresult DOMParser::InitInternal(nsISupports* aOwner, nsIPrincipal* prin,
...
void DOMParser::Init(nsIPrincipal* aPrincipal, nsIURI* aDocumentURI,
...
nsresult DOMParser::SetUpDocument(DocumentFlavor aFlavor,
                                  nsIDOMDocument** aResult) {
