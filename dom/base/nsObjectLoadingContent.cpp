...
nsSimplePluginEvent::Run() {
  if (mDocument && mDocument->IsActive()) {
    ...
    nsContentUtils::DispatchTrustedEvent(mDocument, mTarget, mEvent, true,
                                         true);
  }
  return NS_OK;
}
...
nsresult nsObjectLoadingContent::InstantiatePluginInstance(bool aIsLoading) {
  ...
  rv = pluginHost->InstantiatePluginInstance(mContentType, mURI.get(), this,
...
bool nsObjectLoadingContent::CheckLoadPolicy(int16_t* aContentPolicy) {
  ...
  *aContentPolicy = nsIContentPolicy::ACCEPT;
  nsresult rv = NS_CheckContentLoadPolicy(
      contentPolicyType, mURI,
      doc->NodePrincipal(),  // loading principal
      doc->NodePrincipal(),  // triggering principal
      thisContent, mContentType,
      nullptr,  // extra
      aContentPolicy, nsContentUtils::GetContentPolicy());
  NS_ENSURE_SUCCESS(rv, false);
...
bool nsObjectLoadingContent::CheckProcessPolicy(int16_t* aContentPolicy) {
  ...
  *aContentPolicy = nsIContentPolicy::ACCEPT;
  nsresult rv = NS_CheckContentProcessPolicy(
      objectType, mURI ? mURI : mBaseURI,
      doc->NodePrincipal(),  // loading principal
      doc->NodePrincipal(),  // triggering principal
      static_cast<nsIImageLoadingContent*>(this), mContentType,
      nullptr,  // extra
      aContentPolicy, nsContentUtils::GetContentPolicy());
  NS_ENSURE_SUCCESS(rv, false);
...
nsresult nsObjectLoadingContent::ScriptRequestPluginInstance(
...
nsresult nsObjectLoadingContent::GetPluginJSObject(
    ...
    nsNPAPIPluginInstance* plugin_inst, JS::MutableHandle<JSObject*> plugin_obj,
