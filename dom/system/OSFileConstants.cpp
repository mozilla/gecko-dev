...
bool OSFileConstantsService::DefineOSFileConstants(
  ...
  if (!SetStringProperty(aCx, objPath, "userApplicationDataDir",
                         mPaths->userApplicationDataDir)) {
    return false;
  }

#if defined(XP_MACOSX)
  if (!SetStringProperty(aCx, objPath, "macUserLibDir",
                         mPaths->macUserLibDir)) {
    return false;
  }
#endif  // defined(XP_MACOSX)
