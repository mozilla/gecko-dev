 ...
 *  ]
 *
 * The first element of each array has to be a valid directive-name, otherwise
 * we can ignore the remaining elements of the array. Also, if the directive
 * already exists in the current policy, we can ignore the remaining elements of
 * that array. (http://www.w3.org/TR/CSP/#parsing)
 */
...
class nsCSPTokenizer {
 public:
  static void tokenizeCSPPolicy(const nsAString& aPolicyString,
                                cspTokens& outTokens);
...
class nsCSPParser {
 ...
 private:
  nsCSPParser(cspTokens& aTokens, nsIURI* aSelfURI, nsCSPContext* aCSPContext,
              bool aDeliveredViaMetaTag);
