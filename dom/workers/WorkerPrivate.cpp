...
: EventTarget(aParent ? aCx : NULL), mMutex("WorkerPrivateParent Mutex"),
  ...
  mParentStatus(Pending), mJSContextOptions(0), mGCZeal(0),
  mJSObjectRooted(false), mParentSuspended(false),
  ...
  mMainThreadObjectsForgotten(false), mEvalAllowed(aEvalAllowed)
{
  ...
  if (aParent) {
    ...
    aParent->GetMemoryParameters(mMemoryParams);
    ...
    RuntimeService::GetDefaultMemoryParameters(mMemoryParams);
                                                            ...
                                                            uint32_t aValue)
{
...

  {
    ...
    for (uint32_t index = 0; index < mMemoryParams.Length(); index++) {
      MemoryParameter& param = mMemoryParams[index];
      if (param.key == aKey) {
        param.value = aValue;
        found = true;
        break;
...
WorkerPrivate::DoRunLoop(JSContext* aCx)
{
  ...
  for (;;) {
    ...
    if (scheduleIdleGC) {
      NS_ASSERTION(JS_GetGlobalObject(aCx), "Should have global here!");
