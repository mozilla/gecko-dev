...
void LoadContextOptions(const char* aPrefName, void* /* aClosure */) {
  ...
  contextOptions.setAsmJS(GetWorkerPref<bool>(NS_LITERAL_CSTRING("asmjs")))
      ...
      .setArrayProtoValues(
          GetWorkerPref<bool>(NS_LITERAL_CSTRING("array_prototype_values")));
...
bool ContentSecurityPolicyAllows(JSContext* aCx) {
  WorkerPrivate* worker = GetWorkerPrivateFromContext(aCx);
...
class WorkerThreadContextPrivate : private PerThreadAtomCache {
 ...
 private:
  explicit WorkerThreadContextPrivate(WorkerPrivate* aWorkerPrivate)
  ...
  ~WorkerThreadContextPrivate() { MOZ_ASSERT(!NS_IsMainThread()); }

  WorkerThreadContextPrivate(const WorkerThreadContextPrivate&) = delete;

  WorkerThreadContextPrivate& operator=(const WorkerThreadContextPrivate&) =
      delete;
};
...
static bool PreserveWrapper(JSContext* cx, JSObject* obj) {
  MOZ_ASSERT(cx);
  MOZ_ASSERT(obj);
...
class MOZ_STACK_CLASS WorkerJSContext final
...
bool RuntimeService::RegisterWorker(WorkerPrivate* aWorkerPrivate) {
  ...
  bool queued = false;
  {
    ...
    if (isSharedWorker) {
#ifdef DEBUG
      for (const UniquePtr<SharedWorkerInfo>& data :
           domainInfo->mSharedWorkerInfos) {
        if (data->mScriptSpec == sharedWorkerScriptSpec &&
            ...
            data->mWorkerPrivate->GetPrincipal()->Subsumes(
                aWorkerPrivate->GetPrincipal()) &&
            aWorkerPrivate->GetPrincipal()->Subsumes(
                data->mWorkerPrivate->GetPrincipal())) {
...
void RuntimeService::RemoveSharedWorker(WorkerDomainInfo* aDomainInfo,
...
nsresult RuntimeService::CreateSharedWorker(const GlobalObject& aGlobal,
  ...
  nsresult rv = WorkerPrivate::GetLoadInfo(
      cx, window, nullptr, aScriptURL, false, WorkerPrivate::OverrideLoadGroup,
...
nsresult RuntimeService::CreateSharedWorkerFromLoadInfo(
    JSContext* aCx, WorkerLoadInfo* aLoadInfo, const nsAString& aScriptURL,
    const nsAString& aName, SharedWorker** aSharedWorker) {
  AssertIsOnMainThread();
  ...
  RefPtr<WorkerPrivate> workerPrivate;
  {
    ...
    MOZ_DIAGNOSTIC_ASSERT(aLoadInfo->mPrincipal &&
                          aLoadInfo->mLoadingPrincipal);

    WorkerDomainInfo* domainInfo;
    if (mDomainMap.Get(aLoadInfo->mDomain, &domainInfo)) {
      for (const UniquePtr<SharedWorkerInfo>& data :
           domainInfo->mSharedWorkerInfos) {
        if (data->mScriptSpec == scriptSpec && data->mName == aName &&
            // We want to be sure that the window's principal subsumes the
            // SharedWorker's loading principal and vice versa.
            aLoadInfo->mLoadingPrincipal->Subsumes(
                data->mWorkerPrivate->GetLoadingPrincipal()) &&
            data->mWorkerPrivate->GetLoadingPrincipal()->Subsumes(
                aLoadInfo->mLoadingPrincipal)) {
  ...
  if (!workerPrivate) {
    workerPrivate =
        WorkerPrivate::Constructor(aCx, aScriptURL, false, WorkerTypeShared,
                                   aName, VoidCString(), aLoadInfo, rv);
  ...
  RefPtr<SharedWorker> sharedWorker =
      new SharedWorker(window, workerPrivate, channel->Port1());
  ...
  if (!shouldAttachToWorkerPrivate) {
    ...
    RefPtr<AsyncEventDispatcher> errorEvent = new AsyncEventDispatcher(
        sharedWorker, NS_LITERAL_STRING("error"), false);
...
void RuntimeService::ForgetSharedWorker(WorkerPrivate* aWorkerPrivate) {
  AssertIsOnMainThread();
  MOZ_ASSERT(aWorkerPrivate);
  MOZ_ASSERT(aWorkerPrivate->IsSharedWorker());
...
WorkerPrivate* GetWorkerPrivateFromContext(JSContext* aCx) {
  ...
  return static_cast<WorkerThreadContextPrivate*>(cxPrivate)
      ->GetWorkerPrivate();
}
