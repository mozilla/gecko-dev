...
class Event : public EventBase, public nsWrapperCache {
 public:
 ...
 public:
  static Event* FromSupports(nsISupports* aSupports) {
    nsIDOMEvent* event = static_cast<nsIDOMEvent*>(aSupports);
#ifdef DEBUG
    {
      nsCOMPtr<nsIDOMEvent> target_qi = do_QueryInterface(aSupports);
...
#undef GENERATED_EVENT
  ...
  static PopupControlState GetEventPopupControlState(
      WidgetEvent* aEvent, nsIDOMEvent* aDOMEvent = nullptr);
...
#define NS_FORWARD_NSIDOMEVENT_NO_SERIALIZATION_NO_DUPLICATION(_to)            \
  ...
  NS_IMETHOD GetCurrentTarget(nsIDOMEventTarget** aCurrentTarget) override {   \
    return _to GetCurrentTarget(aCurrentTarget);                               \
  }                                                                            \
  NS_IMETHOD GetEventPhase(uint16_t* aEventPhase) override {                   \
    return _to GetEventPhase(aEventPhase);                                     \
  }                                                                            \
  NS_IMETHOD GetBubbles(bool* aBubbles) override {                             \
    return _to GetBubbles(aBubbles);                                           \
  }                                                                            \
  NS_IMETHOD GetCancelable(bool* aCancelable) override {                       \
    return _to GetCancelable(aCancelable);                                     \
  }                                                                            \
  NS_IMETHOD GetTimeStamp(DOMTimeStamp* aTimeStamp) override {                 \
    return _to GetTimeStamp(aTimeStamp);                                       \
  }                                                                            \
  NS_IMETHOD StopPropagation(void) override { return _to StopPropagation(); }  \
  NS_IMETHOD StopCrossProcessForwarding(void) override {                       \
    return _to StopCrossProcessForwarding();                                   \
  }                                                                            \
  NS_IMETHOD PreventDefault(void) override { return _to PreventDefault(); }    \
  void InitEvent(const nsAString& eventTypeArg, bool canBubbleArg,             \
                 bool cancelableArg) override {                                \
    _to InitEvent(eventTypeArg, canBubbleArg, cancelableArg);                  \
  }                                                                            \
  NS_IMETHOD GetDefaultPrevented(bool* aDefaultPrevented) override {           \
    return _to GetDefaultPrevented(aDefaultPrevented);                         \
  }                                                                            \
  NS_IMETHOD StopImmediatePropagation(void) override {                         \
    return _to StopImmediatePropagation();                                     \
  }                                                                            \
  NS_IMETHOD GetOriginalTarget(nsIDOMEventTarget** aOriginalTarget) override { \
    return _to GetOriginalTarget(aOriginalTarget);                             \
  }                                                                            \
  NS_IMETHOD GetExplicitOriginalTarget(                                        \
      nsIDOMEventTarget** aExplicitOriginalTarget) override {                  \
    return _to GetExplicitOriginalTarget(aExplicitOriginalTarget);             \
  }                                                                            \
  NS_IMETHOD GetIsTrusted(bool* aIsTrusted) override {                         \
    return _to GetIsTrusted(aIsTrusted);                                       \
  }                                                                            \
  NS_IMETHOD SetTarget(nsIDOMEventTarget* aTarget) override {                  \
    return _to SetTarget(aTarget);                                             \
  }                                                                            \
  NS_IMETHOD_(bool) IsDispatchStopped(void) override {                         \
    return _to IsDispatchStopped();                                            \
  }                                                                            \
  NS_IMETHOD_(WidgetEvent*) WidgetEventPtr(void) override {                    \
    return _to WidgetEventPtr();                                               \
  }                                                                            \
  NS_IMETHOD_(void) SetTrusted(bool aTrusted) override {                       \
    _to SetTrusted(aTrusted);                                                  \
  }                                                                            \
  NS_IMETHOD_(void) SetOwner(EventTarget* aOwner) override {                   \
    _to SetOwner(aOwner);                                                      \
  }                                                                            \
  NS_IMETHOD_(Event*) InternalDOMEvent() override {                            \
    return _to InternalDOMEvent();                                             \
  }                                                                            \
  NS_IMETHOD GetCancelBubble(bool* aCancelBubble) override {                   \
    return _to GetCancelBubble(aCancelBubble);                                 \
  }                                                                            \
  NS_IMETHOD SetCancelBubble(bool aCancelBubble) override {                    \
    return _to SetCancelBubble(aCancelBubble);                                 \
  }
...
inline nsISupports* ToSupports(mozilla::dom::Event* e) {
  return static_cast<nsIDOMEvent*>(e);
}

inline nsISupports* ToCanonicalSupports(mozilla::dom::Event* e) {
  return static_cast<nsIDOMEvent*>(e);
}
