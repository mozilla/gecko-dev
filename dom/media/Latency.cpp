...
class LogEvent : public Runnable {
 public:
  LogEvent(AsyncLatencyLogger::LatencyLogIndex aIndex, uint64_t aID,
           int64_t aValue, TimeStamp aTimeStamp)
...
void LogLatency(AsyncLatencyLogger::LatencyLogIndex aIndex, uint64_t aID,
                int64_t aValue) {
  AsyncLatencyLogger::Get()->Log(aIndex, aID, aValue);
}

void LogTime(AsyncLatencyLogger::LatencyLogIndex aIndex, uint64_t aID,
             int64_t aValue) {
...
void LogLatency(uint32_t aIndex, uint64_t aID, int64_t aValue) {
  LogLatency(static_cast<AsyncLatencyLogger::LatencyLogIndex>(aIndex), aID,
             aValue);
}
...
/* static */
void AsyncLatencyLogger::ShutdownLogger() { gAsyncLogger = nullptr; }

/* static */
AsyncLatencyLogger* AsyncLatencyLogger::Get(bool aStartTimer) {
  // Users don't generally null-check the result since we should live longer
  // than they
...
nsresult AsyncLatencyLogger::Observe(nsISupports* aSubject, const char* aTopic,
...
void AsyncLatencyLogger::WriteLog(LatencyLogIndex aIndex, uint64_t aID,
                                  int64_t aValue, TimeStamp aTimeStamp) {
...
void AsyncLatencyLogger::Log(LatencyLogIndex aIndex, uint64_t aID,
                             int64_t aValue) {
  TimeStamp null;
