...
RefPtr<GenericPromise> AudioSink::Init(const PlaybackParams& aParams) {
...
void AudioSink::NotifyAudioNeeded() {
  ...
  while (mAudioQueue.GetSize() &&
    ...
    if (!mConverter ||
      ...
      mConverter =
          MakeUnique<AudioConverter>(AudioConfig(data->mChannels, data->mRate),
    ...
    if (missingFrames.value() > AUDIO_FUZZ_FRAMES) {
      ...
      if (mConverter->InputConfig() != mConverter->OutputConfig()) {
        AlignedAudioBuffer convertedData =
            mConverter->Process(AudioSampleBuffer(Move(silenceBuffer)))
                .Forget();
...
already_AddRefed<AudioData> AudioSink::CreateAudioFromBuffer(
  ...
  RefPtr<AudioData> data =
      new AudioData(aReference->mOffset, aReference->mTime, duration, frames,
                    Move(aBuffer), mOutputChannels, mOutputRate);
  return data.forget();
}
