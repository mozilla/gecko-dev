...
void AudioListener::RegisterPannerNode(PannerNode* aPannerNode) {
...
void AudioListener::SendDoubleParameterToStream(uint32_t aIndex,
                                                double aValue) {
...
void AudioListener::SendThreeDPointParameterToStream(
    uint32_t aIndex, const ThreeDPoint& aValue) {
  for (uint32_t i = 0; i < mPanners.Length(); ++i) {
    if (mPanners[i]) {
