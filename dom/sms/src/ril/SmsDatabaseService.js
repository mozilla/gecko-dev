...
function numberFromMessage(aMessageRecord) {
...
SmsDatabaseService.prototype = {
  ...
  createSchema: function createSchema(db) {
    // This messageStore holds the main SMS data.
  ...
  upgradeSchema3: function upgradeSchema3(db, transaction) {
    ...
    let messageStore = transaction.messageStore(MESSAGE_STORE_NAME);
     ...
     * the SMS database. Each entry looks like this:
     *
  ...
  createMessageFromRecord: function createMessageFromRecord(aMessageRecord) {
    if (DEBUG) {
  ...
  saveMessage: function saveMessage(aMessageRecord, callback) {
    ...
    function notifyResult(rv) {
      ...
      let sms = self.createMessageFromRecord(aMessageRecord);
    ...
    this.newTxn(READ_WRITE, function(error, txn, stores) {
      ...
      let number = numberFromMessage(aMessageRecord);
  ...
  saveReceivedMessage: function saveReceivedMessage(
    ...
    return this.saveMessage(messageRecord, aCallback);
  },
  ...
  setMessageDelivery: function setMessageDelivery(
    ...
    function notifyResult(rv) {
      ...
      let sms = null;
      if (messageRecord) {
        sms = self.createMessageFromRecord(messageRecord);
      }
      callback.notify(rv, sms);
    }
  ...
  getMessage: function getMessage(messageId, aRequest) {
    ...
    this.newTxn(READ_ONLY, function (error, txn, messageStore) {
      ...
      txn.oncomplete = function oncomplete() {
        ...
        let sms = self.createMessageFromRecord(messageRecord);
      ...
      messageStore.get(messageId).onsuccess = function(event) {
        ...
        if (messageRecord) {
          ...
          messageStore.delete(messageId).onsuccess = function(event) {
            ...
            let number = numberFromMessage(messageRecord);
  ...
  createMessageList: function createMessageList(filter, reverse, aRequest) {
    ...
    this.newTxn(READ_ONLY, function (error, txn, messageStore) {
      ...
      if (filter.delivery || filter.numbers || filter.read != undefined) {
        ...
        let request = messageStore.index("timestamp")
