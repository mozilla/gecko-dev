...
class XULDocument final : public XMLDocument,
 ...
 public:
  ...
  virtual XULDocument* AsXULDocument() override { return this; }
  ...
  virtual nsIDocument::DocumentTheme ThreadSafeGetDocumentLWTheme()
      const override;
  ...
  static bool MatchAttribute(Element* aContent, int32_t aNameSpaceID,
  ...
  already_AddRefed<nsINodeList> GetElementsByAttribute(
      const nsAString& aAttribute, const nsAString& aValue);
  already_AddRefed<nsINodeList> GetElementsByAttributeNS(
 ...
 protected:
  ...
  nsresult LoadOverlayInternal(nsIURI* aURI, bool aIsDynamic,
                               bool* aShouldReturn, bool* aFailureFromContent);
  ...
  nsresult AddElementToDocumentPre(Element* aElement);

  nsresult AddElementToDocumentPost(Element* aElement);

  nsresult ExecuteOnBroadcastHandlerFor(Element* aBroadcaster,
                                        Element* aListener, nsAtom* aAttr);

  nsresult BroadcastAttributeChangeFromOverlay(nsIContent* aNode,
  ...
  nsresult Persist(mozilla::dom::Element* aElement, int32_t aNameSpaceID,
                   nsAtom* aAttribute);
  // Just like Persist but ignores the return value so we can use it
  // as a runnable method.
  void DoPersist(mozilla::dom::Element* aElement, int32_t aNameSpaceID,
                 nsAtom* aAttribute) {
 ...
 protected:
  ...
  class BroadcasterHookup : public nsForwardReference {
   ...
   public:
    BroadcasterHookup(XULDocument* aDocument, Element* aObservesElement)
        : mDocument(aDocument),
  ...
  nsresult CheckBroadcasterHookup(Element* aElement, bool* aNeedsHookup,
                                  bool* aDidResolve);
  ...
  static nsresult InsertElement(nsINode* aParent, nsIContent* aChild,
                                bool aNotify);
   ...
   */
  nsresult InsertXULOverlayPI(const nsXULPrototypePI* aProtoPI,
                              nsINode* aParent, nsINode* aBeforeThis,
                              nsIContent* aPINode);
  ...
  class ParserObserver : public nsIRequestObserver {
   ...
   public:
    ParserObserver(XULDocument* aDocument, nsXULPrototypeDocument* aPrototype);
  ...
  nsAutoPtr<nsInterfaceHashtable<nsURIHashKey, nsIObserver>>
