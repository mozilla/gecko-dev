...
function visibilityChangeHandler(e) {
  ...
  if (beps.length == 0) {
    win.removeEventListener('mozvisibilitychange', visibilityChangeHandler);
    return;
  }
...
function BrowserElementParent(frameLoader, hasRemoteFrame) {
  ...
  // Listen to mozvisibilitychange on the iframe's owner window, and forward
  // changes down to the child.  We want to do this while registering as few
  // mozvisibilitychange listeners on _window as possible, because such a
  // listener may live longer than this BrowserElementParent object.
  //
