...
public:
  ...
  virtual nsresult TakePicture(CameraSize aSize, int32_t aRotation, const nsAString& aFileFormat, CameraPosition aPosition, uint64_t aDateTime, nsICameraTakePictureCallback* onSuccess, nsICameraErrorCallback* onError) = 0;
  ...
  virtual nsresult Set(uint32_t aKey, const CameraSize& aSize) = 0;
  virtual nsresult Get(uint32_t aKey, CameraSize& aSize) = 0;
