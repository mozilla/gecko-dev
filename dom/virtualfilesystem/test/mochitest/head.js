...
function installApp(aTestToken, aTemplate) {
  var hostedManifestURL = window.location.origin +
                          '/tests/dom/virtualfilesystem/test/mochitest/file_app.sjs?testToken='
  ...
  request.onsuccess = function() {
    ...
    SpecialPowers.addPermission("filesystemprovider", true, { url: gApp.origin,
                                                              appId: appId,
...
function setupPrefsAndPermissions() {
  SpecialPowers.pushPrefEnv({"set": [["device.storage.enabled", true]]}, function() {
