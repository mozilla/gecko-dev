...
bool DoesTargetMatchDimensions(WebGLContext* webgl, TexImageTarget target,
                               uint8_t dims, const char* funcName);
...
class WebGLTexture final : public nsWrapperCache,
 ...
 public:
  ...
  // Or we can just max this out to 31, which is the number of unsigned bits in
  // GLsizei.
 ...
 protected:
  ...
  void SetImageInfo(const char* funcName, ImageInfo* target,
 ...
 public:
  ...
  class ImageInfo {
    friend void WebGLTexture::SetImageInfo(const char* funcName,
                                           ImageInfo* target,
                                           const ImageInfo& newInfo);
   ...
   public:
    ...
    ImageInfo(const webgl::FormatUsageInfo* format, uint32_t width,
              uint32_t height, uint32_t depth, bool isDataInitialized)
        : mFormat(format),
          mWidth(width),
          mHeight(height),
          mDepth(depth),
    ...
    ~ImageInfo() { MOZ_ASSERT(!mAttachPoints.size()); }
 ...
 protected:
  void TexOrSubImageBlob(bool isSubImage, const char* funcName,
  ...
  bool ValidateTexImageSpecification(
      const char* funcName, TexImageTarget target, GLint level, uint32_t width,
  ...
  bool ValidateTexImageSelection(const char* funcName, TexImageTarget target,
                                 ...
                                 uint32_t depth,
                                 WebGLTexture::ImageInfo** const out_imageInfo);
  bool ValidateCopyTexImageForFeedback(const char* funcName, uint32_t level,
                                       GLint layer = 0) const;
 ...
 public:
  void CompressedTexImage(const char* funcName, TexImageTarget target,
                          GLint level, GLenum internalFormat, GLsizei width,
                          GLsizei height, GLsizei depth, GLint border,
                          const TexImageSource& src,
                          const Maybe<GLsizei>& expectedImageSize);
  void CompressedTexSubImage(const char* funcName, TexImageTarget target,
                             GLint level, GLint xOffset, GLint yOffset,
 ...
 protected:
  ...
  void PopulateMipChain(const char* funcName, uint32_t baseLevel,
                        uint32_t maxLevel);
 ...
 public:
  ...
  const ImageInfo& ImageInfoAt(TexImageTarget texImageTarget,
                               GLint level) const {
    return const_cast<WebGLTexture*>(this)->ImageInfoAt(texImageTarget, level);
  }

  void SetImageInfoAt(const char* funcName, TexImageTarget texImageTarget,
                      GLint level, const ImageInfo& val) {
    ImageInfo* target = &ImageInfoAt(texImageTarget, level);
    SetImageInfo(funcName, target, val);
  }

  const ImageInfo& BaseImageInfo() const {
    if (mBaseMipmapLevel >= kMaxLevelCount) return ImageInfo::kUndefined;
  ...
  bool InitializeImageData(const char* funcName, TexImageTarget target,
                           uint32_t level);
 ...
 public:
  ...
  bool IsComplete(const char* funcName, uint32_t texUnit,
