 ...
 * raster image.
 */
static bool HasRasterImage(HTMLImageElement& aImageEl) {
  nsresult rv;
...
void ImageBitmap::SetIsCroppingAreaOutSideOfSourceImage(
...
ImageBitmapFormat ImageBitmap::FindOptimalFormat(
...
int32_t ImageBitmap::MappedDataLength(ImageBitmapFormat aFormat,
                                      ErrorResult& aRv) {
  if (!mDataWrapper) {
    aRv.Throw(NS_ERROR_NOT_AVAILABLE);
    return 0;
...
class MapDataIntoBufferSource {
 protected:
  MapDataIntoBufferSource(JSContext* aCx, Promise* aPromise,
                          ImageBitmap* aImageBitmap, const T& aBuffer,
                          int32_t aOffset, ImageBitmapFormat aFormat)
      : mPromise(aPromise),
        mImageBitmap(aImageBitmap),
  ...
  void DoMapDataIntoBufferSource() {
    ...
    if (JS_IsArrayBufferObject(mBuffer)) {
      js::GetArrayBufferLengthAndData(mBuffer, &bufferLength, &isSharedMemory,
                                      &bufferData);
    } else if (JS_IsArrayBufferViewObject(mBuffer)) {
      js::GetArrayBufferViewLengthAndData(mBuffer, &bufferLength,
                                          &isSharedMemory, &bufferData);
    } else {
    ...
    UniquePtr<ImagePixelLayout> layout =
        mImageBitmap->mDataWrapper->MapDataInto(bufferData, mOffset,
                                                bufferLength, mFormat, error);
...
class MapDataIntoBufferSourceTask final : public Runnable,
 ...
 public:
  MapDataIntoBufferSourceTask(JSContext* aCx, Promise* aPromise,
                              ImageBitmap* aImageBitmap, const T& aBuffer,
                              int32_t aOffset, ImageBitmapFormat aFormat)
      : Runnable("dom::MapDataIntoBufferSourceTask"),
        MapDataIntoBufferSource<T>(aCx, aPromise, aImageBitmap, aBuffer,
                                   aOffset, aFormat) {}
...
class MapDataIntoBufferSourceWorkerTask final
    : public WorkerSameThreadRunnable,
      public MapDataIntoBufferSource<T> {
 public:
  MapDataIntoBufferSourceWorkerTask(JSContext* aCx, Promise* aPromise,
                                    ImageBitmap* aImageBitmap, const T& aBuffer,
                                    int32_t aOffset, ImageBitmapFormat aFormat)
      : WorkerSameThreadRunnable(GetCurrentThreadWorkerPrivate()),
        MapDataIntoBufferSource<T>(aCx, aPromise, aImageBitmap, aBuffer,
...
void AsyncMapDataIntoBufferSource(JSContext* aCx, Promise* aPromise,
                                  ImageBitmap* aImageBitmap,
...
already_AddRefed<Promise> ImageBitmap::MapDataInto(
...
static SurfaceFormat ImageFormatToSurfaceFromat(
...
static already_AddRefed<layers::Image> CreateImageFromBufferSourceRawData(
    const uint8_t* aBufferData, uint32_t aBufferLength,
    mozilla::dom::ImageBitmapFormat aFormat,
  ...
  switch (aFormat) {
    ...
    case ImageBitmapFormat::DEPTH: {
      ...
      RefPtr<DataSourceSurface> dstDataSurface =
          Factory::CreateDataSourceSurfaceWithStride(srcSize, srcFormat,
                                                     srcStride);
...
/*static*/ already_AddRefed<Promise> ImageBitmap::Create(
    nsIGlobalObject* aGlobal, const ImageBitmapSource& aBuffer, int32_t aOffset,
  ...
  RefPtr<layers::Image> data;
  if (NS_IsMainThread()) {
    data = CreateImageFromBufferSourceRawData(bufferData + aOffset,
                                              bufferLength, aFormat, aLayout);
  } else {
...
CreateImageBitmapFromBlob::Create(Promise* aPromise, nsIGlobalObject* aGlobal,
  ...
  RefPtr<CreateImageBitmapFromBlob> task = new CreateImageBitmapFromBlob(
      aPromise, aGlobal, stream.forget(), mimeType, aCropRect,
...
nsresult CreateImageBitmapFromBlob::StartDecodeAndCropBlob() {
  MOZ_ASSERT(IsCurrentThread());
  ...
  if (!NS_IsMainThread()) {
    ...
    nsCOMPtr<nsIRunnable> r = NS_NewRunnableFunction(
        "CreateImageBitmapFromBlob::DecodeAndCropBlob", [self]() {
...
CreateImageBitmapFromBlob::OnImageReady(imgIContainer* aImgContainer,
  ...
  // Store the sourceSize value for the DecodeAndCropBlobCompletedMainThread
  // call.
...
void CreateImageBitmapFromBlob::DecodeAndCropBlobCompletedMainThread(
...
void CreateImageBitmapFromBlob::DecodeAndCropBlobCompletedOwningThread(
    layers::Image* aImage, nsresult aStatus) {
  MOZ_ASSERT(IsCurrentThread());
