    ...
    switch (mElementState) {
      ...
      case STATE_ACTIVE: {
        ...
        if (mCurrentInterval->End()->Time() <= sampleTime) {
          ...
          if (didApplyEarlyEnd) {
            NotifyChangedInterval(mOldIntervals[mOldIntervals.Length() - 1],
                                  false, true);
          }
          if (mElementState == STATE_WAITING) {
            NotifyNewInterval();
...
nsresult nsSMILTimedElement::SetBeginSpec(const nsAString& aBeginSpec,
  ...
  return SetBeginOrEndSpec(aBeginSpec, aContextNode, true /*isBegin*/, aRemove);
}
...
nsresult nsSMILTimedElement::SetEndSpec(const nsAString& aEndSpec,
                                        Element* aContextNode,
                                        RemovalTestFunction aRemove) {
  return SetBeginOrEndSpec(aEndSpec, aContextNode, false /*!isBegin*/, aRemove);
}
...
void nsSMILTimedElement::BindToTree(nsIContent* aContextNode) {
...
nsresult nsSMILTimedElement::SetBeginOrEndSpec(const nsAString& aSpec,
  ...
  while (tokenizer.hasMoreTokens()) {
    nsAutoPtr<nsSMILTimeValueSpec> spec(
        new nsSMILTimeValueSpec(*this, aIsBegin));
...
void nsSMILTimedElement::UpdateCurrentInterval(bool aForceChangeNotice) {
  ...
  if (GetNextInterval(GetPreviousInterval(), mCurrentInterval, beginTime,
                      updatedInterval)) {
