...
var SessionStoreInternal = {
  ...
  onTabGroupRemoveRequested: function ssi_onTabGroupRemoveRequested(
    ...
    let tabGroupState = this._buildClosedTabGroupState(tabGroup, win);
   ...
   */
  _buildClosedTabGroupState(tabGroup, win) {
  ...
  _resolveClosedDataSource(aSource) {
    ...
    if (aSource instanceof Ci.nsIDOMWindow) {
      winData = this._getWindowStateData(aSource);
    } else if (aSource.sourceWindow instanceof Ci.nsIDOMWindow) {
      winData = this._getWindowStateData(aSource.sourceWindow);
      ...
      winData = this._getWindowStateData(win);
    } else {
      throw Components.Exception(
   ...
   */
  _getWindowStateData: function ssi_getWindowStateData(aWindow) {
  ...
  addSavedTabGroup(tabGroup) {
    ...
    let tabGroupState = this._buildClosedTabGroupState(
      tabGroup,
