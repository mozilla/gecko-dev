...
export class FaviconFeed {
  ...
  /**
   * Get favicon data of URL. This function tries to get the data from DB. If no
   * data, get the data from Network. If could not get the data, return null.
   * Also, might throw an error.
   *
   * @param {String} pageURI
   *        nsIURI for the page
   * @param {String} faviconURI
   *        nsIURI for the favicon
   * @return {nsIURI} data URL
   *
   */
  async #getFaviconDataURL(pageURI, faviconURI) {
    if (faviconURI.schemeIs("data")) {
      return faviconURI;
    }

    // Get the data from DB first.
    const faviconInfo = await this.#getFaviconInfo(pageURI);

    if (faviconInfo) {
      return faviconInfo.iconUri;
    }

    // Then, if no data in DB, get the data from network,
    return await this.getFaviconDataURLFromNetwork(faviconURI);
  }
  ...
  async #setFaviconForPage(pageURI, faviconURI) {
    try {
      let dataURL = await this.#getFaviconDataURL(pageURI, faviconURI);
      lazy.PlacesUtils.favicons.setFaviconForPage(pageURI, faviconURI, dataURL);
    } catch (ex) {
