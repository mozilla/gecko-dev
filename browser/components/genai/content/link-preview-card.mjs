...
class LinkPreviewCard extends MozLitElement {
  ...
  renderOptInPlaceholderCard() {
    return html`
      <div class="ai-content">
        <h3 class="keypoints-header" @click=${this._handleOptinDeny}>
  ...
  renderNormalGenerationCard(pageUrl) {
    ...
    return html`
      <div class="ai-content">
        <h3 class="keypoints-header" @click=${this.toggleKeyPoints}>
          <div class="chevron-icon"></div>
          Key points
          <img
            class="icon"
  ...
  /**
   * Handles the chevron click in the opt-in placeholder card to dismiss and decline
   *
   * @param {MouseEvent} event - The click event
   */
  handleOptInChevronClick(event) {
    event.preventDefault();
  }
