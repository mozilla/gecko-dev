...
NS_IMPL_ISUPPORTS(TransportSecurityInfo, nsITransportSecurityInfo,
                  nsIInterfaceRequestor, nsISSLStatusProvider,
                  nsIAssociatedContentSecurity, nsISerializable, nsIClassInfo)
...
NS_IMETHODIMP
TransportSecurityInfo::Flush() { return NS_OK; }
...
TransportSecurityInfo::GetErrorMessage(char16_t** aText) {
  ...
  *aText = nullptr;
  ...
  if (mErrorMessageCached.IsEmpty()) {
    nsresult rv = formatErrorMessage(lock, mErrorCode, mErrorMessageType, true,
                                     true, mErrorMessageCached);
...
void TransportSecurityInfo::GetErrorLogMessage(
  ...
  MutexAutoLock lock(mMutex);
  (void)formatErrorMessage(lock, errorCode, errorMessageType, false, false,
                           result);
}

static nsresult formatPlainErrorMessage(const nsCString& host, int32_t port,
                                        PRErrorCode err, bool suppressPort443,
                                        /*out*/ nsString& returnedMessage);

static nsresult formatOverridableCertErrorMessage(
    nsISSLStatus& sslStatus, PRErrorCode errorCodeToReport,
    const nsCString& host, int32_t port, bool suppressPort443, bool wantsHtml,
    /*out*/ nsString& returnedMessage);

// XXX: uses nsNSSComponent string bundles off the main thread when called by
//      nsNSSSocketInfo::Write().
nsresult TransportSecurityInfo::formatErrorMessage(
    const MutexAutoLock& /*proofOfLock*/, PRErrorCode errorCode,
    SSLErrorMessageType errorMessageType, bool wantsHtml, bool suppressPort443,
    /*out*/ nsString& result) {
  result.Truncate();
  if (errorCode == 0) {
    return NS_OK;
  ...
  if (errorMessageType == SSLErrorMessageType::OverridableCert && mSSLStatus &&
      mSSLStatus->HasServerCert()) {
    rv = formatOverridableCertErrorMessage(*mSSLStatus, errorCode, mHostName,
                                           mPort, suppressPort443, wantsHtml,
                                           result);
  } else {
    rv = formatPlainErrorMessage(mHostName, mPort, errorCode, suppressPort443,
                                 result);
  }
...
TransportSecurityInfo::Write(nsIObjectOutputStream* stream) {
  ...
  rv = NS_WriteOptionalCompoundObject(stream, serializable,
                                      NS_GET_IID(nsISSLStatus), true);
  if (NS_FAILED(rv)) {
    return rv;
  }

  rv = NS_WriteOptionalCompoundObject(stream, mFailedCertChain,
...
nsresult TransportSecurityInfo::GetSSLStatus(nsISSLStatus** _result) {
...
void TransportSecurityInfo::SetSSLStatus(nsSSLStatus* aSSLStatus) {
...
static nsresult formatPlainErrorMessage(const nsCString& host, int32_t port,
  ...
  if (NS_SUCCEEDED(rv)) returnedMessage.Append(explanation);

  return NS_OK;
}

static void AppendErrorTextUntrusted(PRErrorCode errTrust, const nsString& host,
                                     nsIX509Cert* ix509,
  ...
  for (size_t i = 0; i < ArrayLength(errorIDs); i++) {
    ...
    nsString formattedString;
    nsresult rv =
        component->GetPIPNSSBundleString(errorIDs[i], formattedString);
    if (NS_SUCCEEDED(rv)) {
      returnedMessage.Append(formattedString);
...
static uint32_t GetSubjectAltNames(CERTCertificate* nssCert,
                                   nsString& allNames) {
  allNames.Truncate();
...
static nsresult AppendErrorTextMismatch(const nsString& host,
                                        nsIX509Cert* ix509,
                                        nsINSSComponent* component,
                                        bool wantsHtml,
                                        nsString& returnedMessage) {
  ...
  if (nameCount == 0) {
    ...
    const char* stringID =
        wantsHtml ? "certErrorMismatchSingle2" : "certErrorMismatchSinglePlain";
...
static void GetDateBoundary(nsIX509Cert* ix509, nsString& formattedDate,
                            nsString& nowDate,
  ...
  DateTimeFormat::FormatPRTime(kDateFormatLong, kTimeFormatNoSeconds, timeToUse,
                               formattedDate);
  DateTimeFormat::FormatPRTime(kDateFormatLong, kTimeFormatNoSeconds, now,
                               nowDate);
}

static void AppendErrorTextTime(nsIX509Cert* ix509, nsINSSComponent* component,
                                nsString& returnedMessage) {
  ...
  params[0] =
      formattedDate.get();  // might be empty, if helper function had a problem
  params[1] = nowDate.get();

  const char* key = trueExpired_falseNotYetValid ? "certErrorExpiredNow"
                                                 : "certErrorNotYetValidNow";
  nsresult rv;
  nsString formattedString;
  rv = component->PIPBundleFormatStringFromName(
      key, params, ArrayLength(params), formattedString);
  if (NS_SUCCEEDED(rv)) {
    returnedMessage.Append(formattedString);
    returnedMessage.Append('\n');
  }
...
static void AppendErrorTextCode(PRErrorCode errorCodeToReport,
                                nsINSSComponent* component,
                                nsString& returnedMessage) {
  const char* codeName =
      nsNSSErrors::getDefaultErrorStringName(errorCodeToReport);
  if (codeName) {
...
void RememberCertErrorsTable::RememberCertHasError(
    TransportSecurityInfo* infoObject, nsSSLStatus* status,
...
void TransportSecurityInfo::SetStatusErrorBits(nsNSSCertificate* cert,
  ...
  RememberCertErrorsTable::GetInstance().RememberCertHasError(this, mSSLStatus,
                                                              SECFailure);
}
