...
class Fake_MediaStreamListener {
 ...
 public:
  virtual void NotifyQueuedTrackChanges(
      mozilla::MediaStreamGraph* aGraph, mozilla::TrackID aID,
      mozilla::StreamTime aTrackOffset, mozilla::TrackEventCommand aTrackEvents,
      const mozilla::MediaSegment& aQueuedMedia, Fake_MediaStream* aInputStream,
      mozilla::TrackID aInputTrackID) {}
  virtual void NotifyPull(mozilla::MediaStreamGraph* aGraph,
                          mozilla::StreamTime aDesiredTime) = 0;
  virtual void NotifyQueuedAudioData(mozilla::MediaStreamGraph* aGraph,
                                     mozilla::TrackID aID,
...
class Fake_MediaStreamTrackListener {
 ...
 public:
  virtual void NotifyQueuedChanges(
      mozilla::MediaStreamGraph* aGraph, mozilla::StreamTime aTrackOffset,
      const mozilla::MediaSegment& aQueuedMedia) = 0;
};

class Fake_DirectMediaStreamTrackListener
    : public Fake_MediaStreamTrackListener {
 protected:
 ...
 public:
  ...
  enum class InstallationResult { STREAM_NOT_SUPPORTED, SUCCESS };
...
class Fake_MediaStream {
 ...
 public:
  Fake_MediaStream()
      : mListeners(), mTrackListeners(), mMutex("Fake MediaStream") {}
  ...
  mozilla::StreamTime TicksToTimeRoundDown(mozilla::TrackRate aRate,
 ...
 protected:
  ...
  mozilla::Mutex
      mMutex;  // Lock to prevent the listener list from being modified while
               // executing Periodic().
};

class Fake_MediaPeriodic : public nsITimerCallback {
 public:
  explicit Fake_MediaPeriodic(Fake_MediaStream* aStream)
      : mStream(aStream), mCount(0) {}
  void Detach() { mStream = nullptr; }
...
class Fake_SourceMediaStream : public Fake_MediaStream {
 public:
  ...
  void AddVideoSink(const RefPtr<Fake_VideoSink>& aSink) { mSink = aSink; }

  void AddTrack(mozilla::TrackID aID, mozilla::StreamTime aStart,
                mozilla::MediaSegment* aSegment, uint32_t aFlags = 0) {
    delete aSegment;
  }
  void AddAudioTrack(mozilla::TrackID aID, mozilla::TrackRate aRate,
                     mozilla::StreamTime aStart,
  ...
  void SetPullEnabled(bool aEnabled) { mPullEnabled = aEnabled; }
  ...
  virtual void StopStream() { mStop = true; }
  ...
  virtual int GetSegmentsAdded() { return mSegmentsAdded; }
...
class Fake_MediaStreamTrack {
 ...
 public:
  ...
  uint32_t typeSize() const { return sizeof(Fake_MediaStreamTrack); }
  const char* typeName() const { return "Fake_MediaStreamTrack"; }
  ...
  void AddDirectListener(Fake_DirectMediaStreamTrackListener* aListener) {
    aListener->NotifyDirectListenerInstalled(
        Fake_DirectMediaStreamTrackListener::InstallationResult::
...
class Fake_DOMMediaStream : public nsISupports {
 ...
 public:
  ...
  static already_AddRefed<Fake_DOMMediaStream> CreateSourceStreamAsInput(
      nsPIDOMWindowInner* aWindow, mozilla::MediaStreamGraph* aGraph,
  ...
  virtual bool AddDirectListener(Fake_MediaStreamListener* aListener) {
    return false;
  }
  ...
  enum { HINT_CONTENTS_AUDIO = 0x01, HINT_CONTENTS_VIDEO = 0x02 };
  uint32_t GetHintContents() const { return mHintContents; }
  void SetHintContents(uint32_t aHintContents) {
    mHintContents = aHintContents;
  }

  void GetTracks(nsTArray<RefPtr<Fake_MediaStreamTrack>>& aTracks) {
  ...
  void GetVideoTracks(nsTArray<RefPtr<Fake_MediaStreamTrack>>& aTracks) {
    if (mHintContents & HINT_CONTENTS_VIDEO) {
      aTracks.AppendElement(mVideoTrack);
    }
  }

  bool HasTrack(const Fake_MediaStreamTrack& aTrack) const {
    return ((mHintContents & HINT_CONTENTS_AUDIO) &&
            aTrack.AsAudioStreamTrack()) ||
           ((mHintContents & HINT_CONTENTS_VIDEO) &&
            aTrack.AsVideoStreamTrack());
  }

  bool OwnsTrack(const Fake_MediaStreamTrack& aTrack) const {
  ...
  already_AddRefed<Fake_MediaStreamTrack> CreateDOMTrack(
      mozilla::TrackID aTrackID, mozilla::MediaSegment::Type aType,
    ...
    switch (aType) {
      ...
      default: { MOZ_CRASH("Unkown media type"); }
    }
  }
...
class Fake_MediaStreamBase : public Fake_MediaStream {
 public:
  ...
  virtual int GetSegmentsAdded() { return mPeriodic->GetTimesCalled(); }
...
class Fake_AudioStreamSource : public Fake_MediaStreamBase {
 public:
  Fake_AudioStreamSource() : Fake_MediaStreamBase(), mCount(0), mStop(false) {}
  // Signaling Agent indicates us to stop generating
  // further audio.
  void StopStream() { mStop = true; }
  virtual void Periodic();
  int mCount;
  bool mStop;
};
