...
static const CLSID CLSID_WinParentalControls = {
    0xE77CC89B,
    0x7401,
    0x4C04,
    {0x8C, 0xED, 0x14, 0x9D, 0xB3, 0x5A, 0xDD, 0x04}};
static const IID IID_IWinParentalControls = {
    0x28B4D88B,
    0xE072,
    0x49E6,
    {0x80, 0x4D, 0x26, 0xED, 0xBE, 0x21, 0xA7, 0xB9}};
...
nsParentalControlsService::nsParentalControlsService()
  ...
  if (enable) {
    ...
    if (gAdvAPIDLLInst) {
      gEventWrite =
          (decltype(EventWrite) *)GetProcAddress(gAdvAPIDLLInst, "EventWrite");
      gEventRegister = (decltype(EventRegister) *)GetProcAddress(
          gAdvAPIDLLInst, "EventRegister");
      gEventUnregister = (decltype(EventUnregister) *)GetProcAddress(
          gAdvAPIDLLInst, "EventUnregister");
    }
    mEnabled = true;
  }
}
...
nsParentalControlsService::~nsParentalControlsService() {
  ...
  if (gEventUnregister && mProvider) gEventUnregister(mProvider);

  if (gAdvAPIDLLInst) ::FreeLibrary(gAdvAPIDLLInst);
}
...
nsParentalControlsService::Log(int16_t aEntryType, bool blocked,
  ...
  // channel.
  if (!mProvider) {
    if (!gEventRegister) return NS_ERROR_NOT_AVAILABLE;
...
void nsParentalControlsService::LogFileDownload(bool blocked, nsIURI *aSource,
  ...
  gEventWrite(mProvider, &WPCEVENT_WEB_FILEDOWNLOAD, ARRAYSIZE(eventData),
              eventData);
}
