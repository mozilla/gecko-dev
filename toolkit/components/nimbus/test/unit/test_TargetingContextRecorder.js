...
add_task(async function testNimbusTargetingEnvironmentUserSetPrefs() {
  ...
  await recordAndTestPingSubmission(() => {
    ...
    Assert.ok(
      !prefs.includes("nimbus.qa.pref-1"),
      "nimbus.qa.pref-1 is not set and not in telemetry"
    );
    Assert.ok(
      !prefs.includes("nimbus.qa.pref-2"),
      "nimbus.qa.pref-2 is not set and not in telemetry"
    );
    Assert.ok(
  ...
  Services.prefs.setStringPref("nimbus.qa.pref-1", "pref-1");
  Services.prefs.setStringPref("nimbus.qa.pref-2", "pref-2");
  ...
  await recordAndTestPingSubmission(() => {
    ...
    Assert.ok(
      prefs.includes("nimbus.qa.pref-1"),
      "nimbus.qa.pref-1 is set and in telemetry"
    );
    Assert.ok(
      prefs.includes("nimbus.qa.pref-2"),
      "nimbus.qa.pref-2 is set and in telemetry"
    );
    Assert.ok(
      !prefs.includes("nimbus.testing.testInt"),
...
add_task(async function testNimbusTargetingEnvironmentPrefValues() {
  ...
  await recordAndTestPingSubmission(() => {
    ...
    Assert.ok(
      !Object.hasOwn(prefs, "nimbus__qa__pref_1"),
      "nimbus.qa.pref-1 not set and not present in telemetry"
    );
    Assert.ok(
      !Object.hasOwn(prefs, "nimbus__qa__pref_2"),
      "nimbus.qa.pref-2 not set and not present in telemetry"
    );
  });

  Services.prefs
    .getDefaultBranch(null)
    .setStringPref("nimbus.qa.pref-1", "pref-1-default");
  Services.prefs
    .getDefaultBranch(null)
    .setStringPref("nimbus.qa.pref-2", "pref-2-default");
  ...
  await recordAndTestPingSubmission(() => {
    ...
    const prefs = Glean.nimbusTargetingEnvironment.prefValues.testGetValue();
    Assert.equal(
      prefs.nimbus__qa__pref_1,
      "pref-1-default",
      "nimbus.qa.pref-1 set on the default branch and present in telemetry"
    );
    Assert.equal(
      prefs.nimbus__qa__pref_2,
      "pref-2-default",
      "nimbus.qa.pref-2 set on the default branch and present in telemetry"
    );
  });

  Services.prefs.setStringPref("nimbus.qa.pref-1", "pref-1-user");
  Services.prefs.setStringPref("nimbus.qa.pref-2", "pref-2-user");
  ...
  await recordAndTestPingSubmission(() => {
    ...
    Assert.equal(
      prefs.nimbus__qa__pref_1,
      "pref-1-user",
      "nimbus.qa.pref-1 set on the user branch and present in telemetry"
    );
    Assert.equal(
      prefs.nimbus__qa__pref_2,
      "pref-2-user",
      "nimbus.qa.pref-2 set on the user branch and present in telemetry"
    );
  });
...
add_task(async function testErrorMetrics() {
  ...
  Services.prefs.setIntPref("nimbus.qa.pref-1", 123);
  Services.prefs.setIntPref("nimbus.qa.pref-2", 123);

  await recordAndTestPingSubmission(() => {
    assertRecordingFailures({
      prefTypeErrors: ["nimbus.qa.pref-1", "nimbus.qa.pref-2"],
    });
