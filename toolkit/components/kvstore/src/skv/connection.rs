...
use std::{borrow::Cow, fmt::Debug, num::NonZero, path::Path, sync::Mutex};
...
pub trait ConnectionOpener {
    ...
    /// the given version.
    fn upgrade(tx: &mut Transaction<'_>, to_version: NonZero<u32>) -> Result<(), Self::Error>;
}
                ...
                match tx.query_row_and_then("PRAGMA user_version", [], |row| row.get(0)) {
                    Ok(mut version @ 1..) => {
                        while version < O::MAX_SCHEMA_VERSION {
                            O::upgrade(&mut tx, NonZero::new(version + 1).unwrap())?;
                            version += 1;
                        }
                    }
