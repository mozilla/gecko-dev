...
nsresult nsUrlClassifierPrefixSet::LoadPrefixes(nsIInputStream* in) {
  mCanary.Check();
...
uint32_t nsUrlClassifierPrefixSet::CalculatePreallocateSize() {
...
nsresult nsUrlClassifierPrefixSet::WritePrefixes(nsIOutputStream* out) {
  mCanary.Check();
  ...
  rv = out->Write(reinterpret_cast<char*>(mIndexPrefixes.Elements()), writelen,
                  &written);
  NS_ENSURE_SUCCESS(rv, rv);
  NS_ENSURE_TRUE(written == writelen, NS_ERROR_FAILURE);

  rv = out->Write(reinterpret_cast<char*>(indexStarts.Elements()), writelen,
                  &written);
  NS_ENSURE_SUCCESS(rv, rv);
  NS_ENSURE_TRUE(written == writelen, NS_ERROR_FAILURE);
  ...
  if (totalDeltas > 0) {
    for (uint32_t i = 0; i < indexDeltaSize; i++) {
      ...
      rv = out->Write(reinterpret_cast<char*>(mIndexDeltas[i].Elements()),
                      writelen, &written);
      NS_ENSURE_SUCCESS(rv, rv);
      NS_ENSURE_TRUE(written == writelen, NS_ERROR_FAILURE);
    }
...
template <typename T>
void nsUrlClassifierPrefixSet::CalculateTArrayChecksum(nsTArray<T>& aArray,
