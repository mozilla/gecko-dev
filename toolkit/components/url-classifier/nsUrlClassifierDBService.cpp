...
static nsresult ProcessLookupResults(LookupResultArray* results,
...
nsUrlClassifierDBServiceWorker::BeginStream(const nsACString& table) {
  ...
  mProtocolParser =
      (useProtobuf
           ...
           : static_cast<ProtocolParser*>(new (fallible) ProtocolParserV2()));
  if (!mProtocolParser) {
    return NS_ERROR_OUT_OF_MEMORY;
  }
...
nsUrlClassifierDBServiceWorker::FinishUpdate() {
  ...
  nsresult rv =
      mClassifier->AsyncApplyUpdates(&mTableUpdates, [=](nsresult aRv) -> void {
#ifdef MOZ_SAFEBROWSING_DUMP_FAILED_UPDATES
...
nsresult nsUrlClassifierDBServiceWorker::NotifyUpdateObserver(
  ...
  if (NS_SUCCEEDED(mUpdateStatus)) {
    ...
    LOG(
        ("Treating NS_ERROR_NOT_IMPLEMENTED a successful update "
...
nsresult nsUrlClassifierDBServiceWorker::CacheResultToTableUpdate(
    CacheResult* aCacheResult, TableUpdate* aUpdate) {
...
nsresult nsUrlClassifierDBService::CacheCompletions(CacheResultArray* results) {
  NS_ENSURE_TRUE(gDbBackgroundThread, NS_ERROR_NOT_INITIALIZED);
