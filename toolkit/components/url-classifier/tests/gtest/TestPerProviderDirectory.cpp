...
template <typename T>
void VerifyPrivateStorePath(const char* aTableName, const char* aProvider,
                            nsIFile* aRootDir, bool aUsePerProviderStore) {
  ...
  printf("table: %s\nprovider: %s\nroot path: %s\nprivate path: %s\n\n",
         aTableName, aProvider, NS_ConvertUTF16toUTF8(rootStorePath).get(),
...
TEST(PerProviderDirectory, LookupCache) {
  RunTestInNewThread([]() -> void {
    ...
    VerifyPrivateStorePath<LookupCacheV2>("goog-phish-shavar", "google",
                                          rootDir, false);

    // For V4 tables, if provider is found, use per-provider subdirectory;
    // If not found, use root directory.
    VerifyPrivateStorePath<LookupCacheV4>("goog-noprovider-proto", "", rootDir,
                                          false);
    VerifyPrivateStorePath<LookupCacheV4>("goog-phish-proto", "google4",
                                          rootDir, true);
  });
}
...
TEST(PerProviderDirectory, HashStore) {
  RunTestInNewThread([]() -> void {
    ...
    // For V2 tables (NOT ending with '-proto'), root directory should be
    // used as the private store.
    VerifyPrivateStorePath<HashStore>("goog-phish-shavar", "google", rootDir,
                                      false);

    // For V4 tables, if provider is found, use per-provider subdirectory;
    // If not found, use root directory.
    VerifyPrivateStorePath<HashStore>("goog-noprovider-proto", "", rootDir,
                                      false);
    VerifyPrivateStorePath<HashStore>("goog-phish-proto", "google4", rootDir,
                                      true);
  });
}
