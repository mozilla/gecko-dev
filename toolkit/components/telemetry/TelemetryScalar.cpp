...
struct DynamicScalarInfo : BaseScalarInfo {
  ...
  DynamicScalarInfo(uint32_t aKind, bool aRecordOnRelease, bool aExpired,
      ...
      : BaseScalarInfo(aKind,
                       ...
                       RecordedProcessType::All, aKeyed, aBuiltin),
        mDynamicName(aName),
...
void ScalarUnsigned::SetValue(uint32_t aValue) { mStorage = aValue; }
...
void ScalarUnsigned::AddValue(uint32_t aValue) { mStorage += aValue; }
...
nsresult ScalarUnsigned::GetValue(nsCOMPtr<nsIVariant>& aResult) const {
...
nsresult ScalarString::GetValue(nsCOMPtr<nsIVariant>& aResult) const {
  nsCOMPtr<nsIWritableVariant> outVar(new nsVariant());
...
void ScalarBoolean::SetValue(bool aValue) { mStorage = aValue; }

nsresult ScalarBoolean::GetValue(nsCOMPtr<nsIVariant>& aResult) const {
  nsCOMPtr<nsIWritableVariant> outVar(new nsVariant());
 ...
 * the scalar type.
 */
ScalarBase* internal_ScalarAllocate(uint32_t aScalarKind) {
...
ScalarResult KeyedScalar::SetValue(const nsAString& aKey, nsIVariant* aValue) {
  ScalarBase* scalar = nullptr;
...
ScalarResult KeyedScalar::AddValue(const nsAString& aKey, nsIVariant* aValue) {
  ScalarBase* scalar = nullptr;
  ScalarResult sr = GetScalarForKey(aKey, &scalar);
  if (sr != ScalarResult::Ok) {
    return sr;
...
ScalarResult KeyedScalar::SetMaximum(const nsAString& aKey,
                                     nsIVariant* aValue) {
  ScalarBase* scalar = nullptr;
  ScalarResult sr = GetScalarForKey(aKey, &scalar);
  if (sr != ScalarResult::Ok) {
...
void KeyedScalar::SetValue(const nsAString& aKey, uint32_t aValue) {
  ScalarBase* scalar = nullptr;
  ScalarResult sr = GetScalarForKey(aKey, &scalar);
  if (sr != ScalarResult::Ok) {
    MOZ_ASSERT(false,
               "Key too long or too many keys are recorded in the scalar.");
    return;
  }
...
void KeyedScalar::SetValue(const nsAString& aKey, bool aValue) {
  ScalarBase* scalar = nullptr;
  ScalarResult sr = GetScalarForKey(aKey, &scalar);
  if (sr != ScalarResult::Ok) {
    MOZ_ASSERT(false,
...
void KeyedScalar::AddValue(const nsAString& aKey, uint32_t aValue) {
  ScalarBase* scalar = nullptr;
  ScalarResult sr = GetScalarForKey(aKey, &scalar);
  if (sr != ScalarResult::Ok) {
    MOZ_ASSERT(false,
...
void KeyedScalar::SetMaximum(const nsAString& aKey, uint32_t aValue) {
  ScalarBase* scalar = nullptr;
  ScalarResult sr = GetScalarForKey(aKey, &scalar);
  if (sr != ScalarResult::Ok) {
    MOZ_ASSERT(false,
 ...
 *         ScalarUnsigned).
 */
nsresult KeyedScalar::GetValue(nsTArray<KeyValuePair>& aValues) const {
 ...
 */
ScalarResult KeyedScalar::GetScalarForKey(const nsAString& aKey,
                                          ScalarBase** aRet) {
  if (aKey.IsEmpty()) {
...
void internal_LogScalarError(const nsACString& aScalarName, ScalarResult aSr) {
  ...
  switch (aSr) {
    ...
    case ScalarResult::StringTooLong:
      errorMessage.AppendLiteral(
          u" - Truncating scalar value to 50 characters.");
      break;
    ...
    case ScalarResult::KeyTooLong:
      errorMessage.AppendLiteral(
          u" - The key length must be limited to 70 characters.");
      break;
    case ScalarResult::TooManyKeys:
      errorMessage.AppendLiteral(
          u" - Keyed scalars cannot have more than 100 keys.");
      break;
 ...
 * when this function is called in parent process. If that's the case, if this
 * is not |GeckoProcessType_Default|, the process id is used to allocate
 * and store the scalars.
 ...
 * string scalar. NS_ERROR_NOT_AVAILABLE if the scalar is expired. NS_OK if the
 * scalar was found. If that's the case, aResult contains a valid pointer to a
 * scalar type.
 */
...
/**
 * Serializes the scalars from the given dataset to a json-style object and
 * resets them. The returned structure looks like:
 *    {"process": {"category1.probe":1,"category1.other_probe":false,...}, ...
 * }.
 *
 * @param aDataset DATASET_RELEASE_CHANNEL_OPTOUT or
 * DATASET_RELEASE_CHANNEL_OPTIN.
 * @param aClear Whether to clear out the scalars after snapshotting.
 */
nsresult TelemetryScalar::CreateSnapshots(
  ...
  nsDataHashtable<ProcessIDHashKey, ScalarArray> scalarsToReflect;
  {
    ...
    // scalars. We can use the same function and store the scalars in the same
    // output storage.
    auto snapshotter = [aDataset, &locker, &scalarsToReflect](
                           ProcessesScalarsMapType& aProcessStorage,
                           bool aIsBuiltinDynamic) -> nsresult {
      // Iterate the scalars in aProcessStorage. The storage may contain empty
      // or yet to be initialized scalars from all the supported processes.
      for (auto iter = aProcessStorage.Iter(); !iter.Done(); iter.Next()) {
        ...
        for (auto childIter = scalarStorage->Iter(); !childIter.Done();
          ...
          const BaseScalarInfo& info = internal_GetScalarInfo(
              locker, ScalarKey{childIter.Key(),
          ...
          if (IsInDataset(info.dataset, aDataset)) {
            ...
            processScalars.AppendElement(
                mozilla::MakePair(info.name(), scalarValue));
          }
        }
      }
      return NS_OK;
  ...
  for (auto iter = scalarsToReflect.Iter(); !iter.Done(); iter.Next()) {
    ...
    for (nsTArray<DataPair>::size_type i = 0; i < processScalars.Length();
         i++) {
      ...
      nsresult rv = nsContentUtils::XPConnect()->VariantToJS(
          aCx, processObj, scalar.second(), &scalarJsValue);
      if (NS_FAILED(rv)) {
        return rv;
      }

      // Add it to the scalar object.
      if (!JS_DefineProperty(aCx, processObj, scalar.first(), scalarJsValue,
                             JSPROP_ENUMERATE)) {
        return NS_ERROR_FAILURE;
      }
    }
...
/**
 * Serializes the scalars from the given dataset to a json-style object and
 * resets them. The returned structure looks like: { "process": {
 * "category1.probe": { "key_1": 2, "key_2": 1, ... }, ... }, ... }
 *
 * @param aDataset DATASET_RELEASE_CHANNEL_OPTOUT or
 * DATASET_RELEASE_CHANNEL_OPTIN.
...
nsresult TelemetryScalar::CreateKeyedSnapshots(
  ...
  // Only lock the mutex while accessing our data, without locking any JS
  // related code.
  typedef mozilla::Pair<const char*, nsTArray<KeyedScalar::KeyValuePair>>
      DataPair;
  typedef nsTArray<DataPair> ScalarArray;
  nsDataHashtable<ProcessIDHashKey, ScalarArray> scalarsToReflect;
  {
      ...
      for (auto iter = aProcessStorage.Iter(); !iter.Done(); iter.Next()) {
        ...
        for (auto childIter = scalarStorage->Iter(); !childIter.Done();
          ...
          if (IsInDataset(info.dataset, aDataset)) {
            ...
            // Append it to our list.
            processScalars.AppendElement(
                mozilla::MakePair(info.name(), scalarKeyedData));
          }
        }
      }
      return NS_OK;
  ...
  for (auto iter = scalarsToReflect.Iter(); !iter.Done(); iter.Next()) {
    ...
    for (nsTArray<DataPair>::size_type i = 0; i < processScalars.Length();
      ...
      if (!JS_DefineProperty(aCx, processObj, keyedScalarData.first(),
                             keyedScalarObj, JSPROP_ENUMERATE)) {
        return NS_ERROR_FAILURE;
      }
    }
  }
...
void TelemetryScalar::UpdateChildData(
  ...
  for (auto& upd : aScalarActions) {
    ...
    ScalarBase* scalar = nullptr;
    nsresult rv =
        internal_GetScalarByEnum(locker, uniqueId, aProcessType, &scalar);
    if (NS_FAILED(rv)) {
...
void TelemetryScalar::UpdateChildKeyedData(
  ...
  for (auto& upd : aScalarActions) {
    ...
    KeyedScalar* scalar = nullptr;
    nsresult rv =
        internal_GetKeyedScalarByEnum(locker, uniqueId, aProcessType, &scalar);
    if (NS_FAILED(rv)) {
      NS_WARNING("NS_FAILED internal_GetScalarByEnum for CHILD");
      continue;
    }
    ...
    switch (upd.mActionType) {
      case ScalarActionType::eSet: {
        switch (scalarType) {
          case nsITelemetry::SCALAR_TYPE_COUNT:
            scalar->SetValue(NS_ConvertUTF8toUTF16(upd.mKey),
      ...
      case ScalarActionType::eAdd: {
        ...
        scalar->AddValue(NS_ConvertUTF8toUTF16(upd.mKey),
                         upd.mData->as<uint32_t>());
        break;
      }
      case ScalarActionType::eSetMaximum: {
        ...
        // We only support SetMaximum on uint32_t.
        scalar->SetMaximum(NS_ConvertUTF8toUTF16(upd.mKey),
                           upd.mData->as<uint32_t>());
        break;
      }
      default:
...
void TelemetryScalar::AddDynamicScalarDefinitions(
  ...
  for (auto def : aDefs) {
    ...
    dynamicStubs.AppendElement(
        DynamicScalarInfo{def.type, recordOnRelease, def.expired, def.name,
                          def.keyed, false /* builtin */});
  }
