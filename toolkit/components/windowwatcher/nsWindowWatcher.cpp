...
nsresult nsWindowWatcher::OpenWindowInternal(
    ...
    nsIDocShellLoadInfo* aLoadInfo, mozIDOMWindowProxy** aResult) {
  nsresult rv = NS_OK;
    ...
    if (!aDialog && !chromeWin &&
      ...
      if (provider) {
        ...
        rv = provider->ProvideWindow(
            ...
            aLoadInfo, &windowIsNew, getter_AddRefs(newWindow));
  ...
  if (windowIsNew) {
    ...
    if (newWindow) {
      ...
      if (aIsPopupSpam) {
        ...
        if (!globalWin
                 ->IsPopupSpamWindow()) {  // Make sure we don't mess up our
...
uint32_t nsWindowWatcher::CalculateChromeFlagsHelper(
  ...
  if (WinHasOption(aFeatures, "scrollbars", 1, &presenceFlag) ||
      !presenceFlag) {
