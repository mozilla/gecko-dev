...
void nsTypeAheadFind::SaveFind() {
  if (mWebBrowserFind) mWebBrowserFind->SetSearchString(mTypeAheadBuffer.get());
    ...
    while (true) {  // === Inner while loop: go through a single doc ===
      ...
      if ((!canSeeRange && !usesIndependentSelection) ||
        ...
        if (aFindPrev) {
          ...
          nsresult rv = mStartPointRange->CompareBoundaryPoints(
              nsIDOMRange::START_TO_END, returnRange, &compareResult);
          ...
          int16_t compareResult;
          nsresult rv = mStartPointRange->CompareBoundaryPoints(
              nsIDOMRange::END_TO_START, returnRange, &compareResult);
          if (NS_SUCCEEDED(rv) && compareResult >= 0) {
      ...
      if (usesIndependentSelection) {
        ...
        while (node) {
          ...
          if (editable) {
            ...
            if (!shouldFocusEditableElement) break;

            // Otherwise move focus/caret to editable element
            if (fm) fm->SetFocus(mFoundEditable, 0);
            break;
          }
...
nsresult nsTypeAheadFind::GetSearchContainers(
  ...
  *aPresContext = nullptr;
  ...
  if (!rootContent) rootContent = doc->GetRootElement();
...
nsTypeAheadFind::Find(const nsAString &aSearchString, bool aLinksOnly,
  ...
  *aResult = FIND_NOTFOUND;
  ...
  if (bufferLength == 0) {
    ...
    if (selection) selection->GetIsCollapsed(&isSelectionCollapsed);
  ...
  if (NS_SUCCEEDED(rv)) {
    ...
    // Error sound
    if (mTypeAheadBuffer.Length() > mLastFindLength) PlayNotFoundSound();
  }
...
NS_IMETHODIMP
nsTypeAheadFind::IsRangeVisible(nsIDOMRange *aRange, bool aMustBeInViewPort,
                                bool *aResult) {
  nsCOMPtr<nsIDOMNode> domNode;
...
bool nsTypeAheadFind::IsRangeVisible(
    nsIPresShell *aPresShell, nsPresContext *aPresContext, nsIDOMRange *aRange,
    bool aMustBeInViewPort, bool aGetTopVisibleLeaf,
    nsIDOMRange **aFirstVisibleRange, bool *aUsesIndependentSelection) {
...
NS_IMETHODIMP
nsTypeAheadFind::IsRangeRendered(nsIDOMRange *aRange, bool *aResult) {
  nsCOMPtr<nsIDOMNode> domNode;
  aRange->GetStartContainer(getter_AddRefs(domNode));
  nsCOMPtr<nsINode> node = do_QueryInterface(domNode);
