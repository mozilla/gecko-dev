  ...
  MOZ_TRY_VAR(
      ...
      submitPromise->ThenWithCycleCollectedArgs(
          [aSubmitTimeoutMs](JSContext*, JS::HandleValue, ErrorResult& aRv,
            ...
            // The submit callback has finished successfully now (whether or not
            // it was async).
            if (aSubmitTimeoutMs) {
              // We have a submit timeout, which means that `aSubmitCallback`
              // has triggered the submission through some async means that does
              // not complete before it returns (e.g., idle dispatch from c++).
              // Reject `submittedPromise` after the given timeout to make sure
              // we don't hang forever waiting for a submission that might not
              // happen.
              ...
              // If there is no timeout then the ping should have been submitted
              // already. We attempt to reject the promise, which will have no
              // affect if the promise is already settled.
              aSubmittedPromise->MaybeRejectWithOperationError(
                  "Ping did not submit immediately");
            }

            // Chain into `submittedPromise` so that we will resolve/reject
            // appropriately.
