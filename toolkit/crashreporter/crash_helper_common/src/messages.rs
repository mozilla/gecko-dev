...
pub enum Kind {
    ...
    SetCrashReportPath = 3,
    SetCrashReportPathReply = 4,
    ...
    #[cfg(target_os = "windows")]
    WindowsErrorReporting = 9,
    #[cfg(target_os = "windows")]
    WindowsErrorReportingReply = 10,
}
...
/* SetCrashReportPath reply, received from the server after having changed the
 * path where crash reports are generated. */

pub struct SetCrashReportPathReply {}

impl Default for SetCrashReportPathReply {
    fn default() -> Self {
        Self::new()
    }
}

impl SetCrashReportPathReply {
    pub fn new() -> SetCrashReportPathReply {
...
impl Message for SetCrashReportPathReply {
    fn kind() -> Kind {
        Kind::SetCrashReportPathReply
    }
    ...
    fn decode(
        data: &[u8],
        ancillary_data: Option<AncillaryData>,
    ) -> Result<SetCrashReportPathReply, MessageError> {
        if ancillary_data.is_some() || !data.is_empty() {
            return Err(MessageError::InvalidData);
        }

        Ok(SetCrashReportPathReply::new())
    }
}
