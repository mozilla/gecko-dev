...
impl CrashGenerator {
        ...
        match kind {
            ...
            messages::Kind::SetCrashReportPath => {
                if (self.state != CrashGeneratorState::Initialized) || (pid != self.client_pid) {
                    panic!("Not initialized or attempting to set the path from the wrong process");
                }

                let message = messages::SetCrashReportPath::decode(data, ancillary_data)?;
                self.set_path(message.path)?;
                Ok(Some(Box::new(messages::SetCrashReportPathReply::new())))
            }
    ...
    fn set_path(&mut self, path: OsString) -> Result<()> {
        if let Some(breakpad_server) = &self.breakpad_server {
            breakpad_server.set_path(path);
            Ok(())
        } else {
            bail!("The breakpad crash generation server was not set")
        }
    }
