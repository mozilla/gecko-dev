...
MOZ_DEFINE_STYLO_METHODS(nsStyleContext, mozilla::GeckoStyleContext,
                         mozilla::ServoStyleContext);

#ifdef MOZ_OLD_STYLE
nsRuleNode* nsStyleContext::RuleNode() {
  MOZ_RELEASE_ASSERT(IsGecko());
  return AsGecko()->RuleNode();
}
#endif

const ServoComputedData* nsStyleContext::ComputedData() {
  MOZ_RELEASE_ASSERT(IsServo());
  return AsServo()->ComputedData();
}

void nsStyleContext::AddRef() { MOZ_STYLO_FORWARD(AddRef, ()) }

void nsStyleContext::Release() { MOZ_STYLO_FORWARD(Release, ()) }
...
nsPresContext* nsStyleContext::PresContext() const {
...
void nsStyleContext::StartBackgroundImageLoads() {
