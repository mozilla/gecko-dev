  ...
  class Ptr {
   public:
    ...
    StyleBackendType BackendType() const {
      return IsGecko() ? StyleBackendType::Gecko : StyleBackendType::Servo;
    }
...
#ifdef MOZ_OLD_STYLE
    const nsStyleSet* GetAsGecko() const {
      return IsGecko() ? AsGecko() : nullptr;
    }
#endif
    const ServoStyleSet* GetAsServo() const {
    ...
    inline already_AddRefed<nsStyleContext> ResolveStyleFor(
    ...
    inline already_AddRefed<nsStyleContext> ResolveStyleForText(
    ...
    inline already_AddRefed<nsStyleContext> ResolveStyleForPlaceholder();
    inline already_AddRefed<nsStyleContext> ResolvePseudoElementStyle(
        dom::Element* aParentElement, mozilla::CSSPseudoElementType aType,
        nsStyleContext* aParentContext, dom::Element* aPseudoElement);
    inline already_AddRefed<nsStyleContext> ResolveInheritingAnonymousBoxStyle(
        nsAtom* aPseudoTag, nsStyleContext* aParentContext);
    inline already_AddRefed<nsStyleContext>
    ResolveNonInheritingAnonymousBoxStyle(nsAtom* aPseudoTag);
#ifdef MOZ_XUL
    inline already_AddRefed<nsStyleContext> ResolveXULTreePseudoStyle(
    ...
    inline nsresult AddDocStyleSheet(StyleSheet* aSheet,
    ...
    inline already_AddRefed<nsStyleContext> ProbePseudoElementStyle(
        dom::Element* aParentElement, mozilla::CSSPseudoElementType aType,
...
#else
    MOZ_CRASH(
        "should not have a ServoStyleSet object when MOZ_STYLO is "
        "disabled");
#endif
  }
