...
NS_INTERFACE_MAP_BEGIN(ServoPageRuleDeclaration)
  ...
  css::Rule* ServoPageRuleDeclaration::GetParentRule() { return Rule(); }

  nsINode* ServoPageRuleDeclaration::GetParentObject() {
    return Rule()->GetDocument();
  }

  DeclarationBlock* ServoPageRuleDeclaration::GetCSSDeclaration(
      Operation aOperation) {
    return mDecls;
  }

  nsresult ServoPageRuleDeclaration::SetCSSDeclaration(DeclarationBlock *
                                                       aDecl) {
  ...
  nsIDocument* ServoPageRuleDeclaration::DocToUpdate() { return nullptr; }

  void ServoPageRuleDeclaration::GetCSSParsingEnvironment(
  ...
  bool ServoPageRule::IsCCLeaf() const {
    if (!Rule::IsCCLeaf()) {
      return false;
    }
  ...
  already_AddRefed<css::Rule> ServoPageRule::Clone() const {
  ...
  size_t ServoPageRule::SizeOfIncludingThis(MallocSizeOf aMallocSizeOf) const {
    // TODO Implement this!
    return aMallocSizeOf(this);
  }

#ifdef DEBUG
  void ServoPageRule::List(FILE * out, int32_t aIndent) const {
    nsAutoCString str;
  ...
  void ServoPageRule::GetCssText(nsAString & aCssText) const {
    Servo_PageRule_GetCssText(mRawRule, &aCssText);
  }

  /* CSSPageRule implementation */

  nsICSSDeclaration* ServoPageRule::Style() { return &mDecls; }
