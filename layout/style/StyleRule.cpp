...
nsAtomList::nsAtomList(nsAtom* aAtom) : mAtom(aAtom), mNext(nullptr) {
  MOZ_COUNT_CTOR(nsAtomList);
}

nsAtomList::nsAtomList(const nsString& aAtomValue)
    : mAtom(nullptr), mNext(nullptr) {
  MOZ_COUNT_CTOR(nsAtomList);
  mAtom = NS_Atomize(aAtomValue);
}

nsAtomList* nsAtomList::Clone(bool aDeep) const {
  nsAtomList* result = new nsAtomList(mAtom);
  if (!result) return nullptr;

  if (aDeep) NS_CSS_CLONE_LIST_MEMBER(nsAtomList, this, mNext, result, (false));
  return result;
}

size_t nsAtomList::SizeOfIncludingThis(
...
nsPseudoClassList* nsPseudoClassList::Clone(bool aDeep) const {
  ...
  if (aDeep)
    NS_CSS_CLONE_LIST_MEMBER(nsPseudoClassList, this, mNext, result, (false));

  return result;
}

size_t nsPseudoClassList::SizeOfIncludingThis(
    mozilla::MallocSizeOf aMallocSizeOf) const {
  size_t n = 0;
...
nsAttrSelector::nsAttrSelector(int32_t aNameSpace, const nsString& aAttr,
                               uint8_t aFunction, const nsString& aValue,
...
nsAttrSelector* nsAttrSelector::Clone(bool aDeep) const {
  nsAttrSelector* result =
      new nsAttrSelector(mNameSpace, mLowercaseAttr, mCasedAttr, mFunction,
                         mValue, mValueCaseSensitivity);
...
size_t nsAttrSelector::SizeOfIncludingThis(
    mozilla::MallocSizeOf aMallocSizeOf) const {
  size_t n = 0;
...
nsCSSSelector* nsCSSSelector::Clone(bool aDeepNext, bool aDeepNegations) const {
  ...
  if (aDeepNext) {
    NS_CSS_CLONE_LIST_MEMBER(nsCSSSelector, this, mNext, result, (false, true));
  }
...
void nsCSSSelector::AddAttribute(
    int32_t aNameSpace, const nsString& aAttr, uint8_t aFunc,
    const nsString& aValue,
  ...
  if (!aAttr.IsEmpty()) {
    ...
    *list =
        new nsAttrSelector(aNameSpace, aAttr, aFunc, aValue, aCaseSensitivity);
  }
}

void nsCSSSelector::SetOperator(char16_t aOperator) { mOperator = aOperator; }
...
int32_t nsCSSSelector::CalcWeightWithoutNegations() const {
  ...
  MOZ_ASSERT(!(IsPseudoElement() &&
               PseudoType() != CSSPseudoElementType::XULTree && mClassList),
...
//
void nsCSSSelector::ToString(nsAString& aString, CSSStyleSheet* aSheet,
                             bool aAppend) const {
  if (!aAppend) aString.Truncate();
...
void nsCSSSelector::AppendToStringWithoutCombinators(
...
#ifdef DEBUG
nsCString nsCSSSelector::RestrictedSelectorToString() const {
...
static bool AppendStandardNamespacePrefixToString(nsAString& aString,
                                                  int32_t aNameSpace) {
...
void nsCSSSelector::AppendToStringWithoutCombinatorsOrNegations(
    nsAString& aString, CSSStyleSheet* aSheet, bool aIsNegated,
    bool aUseStandardNamespacePrefixes) const {
  nsAutoString temp;
  ...
  if (!isPseudoElement || !mNext) {
    ...
    if (aUseStandardNamespacePrefixes) {
      ...
      NS_ASSERTION(
          mNameSpace == kNameSpaceID_Unknown || CanBeNamespaced(aIsNegated),
  ...
  if (!mLowercaseTag) {
    ...
    // inside of :not()
    if (wroteNamespace || (!mIDList && !mClassList && !mPseudoClassList &&
                           !mAttrList && (aIsNegated || !mNegations))) {
      aString.Append(char16_t('*'));
    }
  } else {
    // Append the tag name
  ...
  if (mClassList) {
    if (isPseudoElement) {
      ...
      for (nsAtomList* list = mClassList; list; list = list->mNext) {
        nsStyleUtil::AppendEscapedCSSIdent(nsDependentAtomString(list->mAtom),
                                           aString);
        aString.Append(char16_t(','));
      }
  ...
  for (nsPseudoClassList* list = mPseudoClassList; list; list = list->mNext) {
    ...
    if (list->u.mMemory) {
      ...
      if (nsCSSPseudoClasses::HasStringArg(list->mType)) {
        nsStyleUtil::AppendEscapedCSSIdent(nsDependentString(list->u.mString),
                                           aString);
      } else if (nsCSSPseudoClasses::HasNthPairArg(list->mType)) {
        int32_t a = list->u.mNumbers[0], b = list->u.mNumbers[1];
        temp.Truncate();
        if (a != 0) {
          if (a == -1) {
...
bool nsCSSSelector::CanBeNamespaced(bool aIsNegated) const {
  return !aIsNegated ||
         (!mIDList && !mClassList && !mPseudoClassList && !mAttrList);
}

size_t nsCSSSelector::SizeOfIncludingThis(
    mozilla::MallocSizeOf aMallocSizeOf) const {
  size_t n = 0;
...
nsCSSSelectorList::nsCSSSelectorList(void)
    : mSelectors(nullptr), mWeight(0), mNext(nullptr) {
...
nsCSSSelector* nsCSSSelectorList::AddSelector(char16_t aOperator) {
...
void nsCSSSelectorList::RemoveRightmostSelector() {
...
void nsCSSSelectorList::ToString(nsAString& aResult, CSSStyleSheet* aSheet) {
  aResult.Truncate();
...
nsCSSSelectorList* nsCSSSelectorList::Clone(bool aDeep) const {
  nsCSSSelectorList* result = new nsCSSSelectorList();
  result->mWeight = mWeight;
  NS_IF_CLONE(mSelectors);

  if (aDeep) {
    NS_CSS_CLONE_LIST_MEMBER(nsCSSSelectorList, this, mNext, result, (false));
  }
  return result;
}

size_t nsCSSSelectorList::SizeOfIncludingThis(
    mozilla::MallocSizeOf aMallocSizeOf) const {
  size_t n = 0;
...
NS_INTERFACE_MAP_BEGIN(DOMCSSDeclarationImpl)
  ...
  DeclarationBlock* DOMCSSDeclarationImpl::GetCSSDeclaration(
  ...
  void DOMCSSDeclarationImpl::GetCSSParsingEnvironment(
      CSSParsingEnvironment & aCSSParseEnv, nsIPrincipal * aSubjectPrincipal) {
  ...
  nsresult DOMCSSDeclarationImpl::SetCSSDeclaration(DeclarationBlock * aDecl) {
    NS_PRECONDITION(
        mRule,
  ...
  nsIDocument* DOMCSSDeclarationImpl::DocToUpdate() { return nullptr; }
  ...
  uint16_t StyleRule::Type() const { return CSSRuleBinding::STYLE_RULE; }

  nsICSSDeclaration* StyleRule::Style() {
    if (!mDOMDeclaration) {
  ...
  StyleRule::StyleRule(nsCSSSelectorList* aSelector, Declaration* aDeclaration,
  ...
  /* virtual */ int32_t StyleRule::GetType() const { return Rule::STYLE_RULE; }

  /* virtual */ already_AddRefed<Rule> StyleRule::Clone() const {
    RefPtr<Rule> clone = new StyleRule(*this);
    return clone.forget();
  }

  void StyleRule::SetDeclaration(Declaration* aDecl) {
...
#ifdef DEBUG
  /* virtual */ void StyleRule::List(FILE* out, int32_t aIndent) const {
    nsAutoCString str;
    // Indent
  ...
  void StyleRule::GetCssText(nsAString& aCssText) const {
    if (mSelector) {
  ...
  void StyleRule::GetSelectorText(nsAString& aSelectorText) {
    if (mSelector)
  ...
  void StyleRule::SetSelectorText(const nsAString& aSelectorText) {
    // XXX TBI - get a parser and re-parse the selectors,
    // XXX then need to re-compute the cascade
    // XXX and dirty sheet
  }

  /* virtual */ size_t StyleRule::SizeOfIncludingThis(
      mozilla::MallocSizeOf aMallocSizeOf) const {
  ...
  nsCSSSelectorList* StyleRule::GetSelectorAtIndex(uint32_t aIndex,
                                                   ErrorResult& rv) {
    for (nsCSSSelectorList *sel = mSelector; sel; sel = sel->mNext, --aIndex) {
      if (aIndex == 0) {
        return sel;
      }
    }
  ...
  uint32_t StyleRule::GetSelectorCount() {
    uint32_t count = 0;
  ...
  nsresult StyleRule::GetSelectorText(uint32_t aSelectorIndex,
                                      nsAString& aText) {
    ErrorResult rv;
  ...
  nsresult StyleRule::GetSpecificity(uint32_t aSelectorIndex,
  ...
  nsresult StyleRule::SelectorMatchesElement(Element* aElement,
    ...
    // XXXbz what exactly should we do with visited state here?  If we ever
    // start caring about it, remember to do FlushPendingLinkUpdates().
    TreeMatchContext matchingContext(
        false, nsRuleWalker::eRelevantLinkUnvisited, aElement->OwnerDoc(),
        TreeMatchContext::eNeverMatchVisited);
    *aMatches =
        nsCSSRuleProcessor::SelectorListMatches(aElement, matchingContext, sel);
    return NS_OK;
  }

  NotNull<DeclarationBlock*> StyleRule::GetDeclarationBlock() const {
