...
float ProcessTranslatePart(
    const nsCSSValue& aValue, GeckoStyleContext* aContext,
...
#ifdef MOZ_OLD_STYLE
      // Gecko backend
      nsRuleNode::ComputedCalc result = nsRuleNode::SpecifiedCalcToComputedCalc(
          aValue, aContext, aPresContext, aConditions);
  ...
  //       this branch.
    ...
    offset =
        nsRuleNode::CalcLength(aValue, aContext, aPresContext, aConditions);
#else
...
static void ProcessMatrix(Matrix4x4& aMatrix, const nsCSSValue::Array* aData,
   ...
   */
  result._31 =
      ProcessTranslatePart(aData->Item(5), aContext, aPresContext, aConditions,
                           &aRefBox, &TransformReferenceBox::Width);
  result._32 =
      ProcessTranslatePart(aData->Item(6), aContext, aPresContext, aConditions,
...
static void ProcessMatrix3D(Matrix4x4& aMatrix, const nsCSSValue::Array* aData,
  ...
  temp._41 =
      ProcessTranslatePart(aData->Item(13), aContext, aPresContext, aConditions,
                           &aRefBox, &TransformReferenceBox::Width);
  temp._42 =
      ProcessTranslatePart(aData->Item(14), aContext, aPresContext, aConditions,
                           &aRefBox, &TransformReferenceBox::Height);
  temp._43 = ProcessTranslatePart(aData->Item(15), aContext, aPresContext,
                                  aConditions, nullptr);
...
void ProcessAccumulateMatrix(Matrix4x4& aMatrix, const nsCSSValue::Array* aData,
  ...
  ProcessMatrixOperator<Accumulate>(aMatrix, aData, aContext, aPresContext,
                                    aConditions, aRefBox, aContains3dTransform);
}
...
static void ProcessTranslateX(Matrix4x4& aMatrix,
  ...
  temp.x =
      ProcessTranslatePart(aData->Item(1), aContext, aPresContext, aConditions,
                           &aRefBox, &TransformReferenceBox::Width);
...
static void ProcessTranslateZ(Matrix4x4& aMatrix,
  ...
  temp.z = ProcessTranslatePart(aData->Item(1), aContext, aPresContext,
                                aConditions, nullptr);
  aMatrix.PreTranslate(temp);
}
...
static void ProcessTranslate(Matrix4x4& aMatrix, const nsCSSValue::Array* aData,
  ...
  if (aData->Count() == 3) {
    temp.y = ProcessTranslatePart(aData->Item(2), aContext, aPresContext,
                                  aConditions, &aRefBox,
...
static void ProcessTranslate3D(Matrix4x4& aMatrix,
  ...
  temp.y =
      ProcessTranslatePart(aData->Item(2), aContext, aPresContext, aConditions,
                           &aRefBox, &TransformReferenceBox::Height);

  temp.z = ProcessTranslatePart(aData->Item(3), aContext, aPresContext,
                                aConditions, nullptr);
...
static void ProcessPerspective(Matrix4x4& aMatrix,
  ...
  float depth = ProcessTranslatePart(aData->Item(1), aContext, aPresContext,
                                     aConditions, nullptr);
...
static void MatrixForTransformFunction(Matrix4x4& aMatrix,
  ...
  switch (TransformFunctionOf(aData)) {
    case eCSSKeyword_translatex:
      ProcessTranslateX(aMatrix, aData, aContext, aPresContext, aConditions,
                        aRefBox);
      break;
    case eCSSKeyword_translatey:
      ProcessTranslateY(aMatrix, aData, aContext, aPresContext, aConditions,
                        aRefBox);
      break;
    case eCSSKeyword_translatez:
      *aContains3dTransform = true;
      ProcessTranslateZ(aMatrix, aData, aContext, aPresContext, aConditions);
      break;
    case eCSSKeyword_translate:
      ProcessTranslate(aMatrix, aData, aContext, aPresContext, aConditions,
                       aRefBox);
      break;
    case eCSSKeyword_translate3d:
      *aContains3dTransform = true;
      ProcessTranslate3D(aMatrix, aData, aContext, aPresContext, aConditions,
                         aRefBox);
      break;
    ...
    case eCSSKeyword_matrix3d:
      *aContains3dTransform = true;
      ProcessMatrix3D(aMatrix, aData, aContext, aPresContext, aConditions,
                      aRefBox);
      break;
    ...
    case eCSSKeyword_perspective:
      *aContains3dTransform = true;
      ProcessPerspective(aMatrix, aData, aContext, aPresContext, aConditions);
      break;
    default:
...
Matrix4x4 ReadTransforms(const nsCSSValueList* aList, nsStyleContext* aContext,
                         nsPresContext* aPresContext,
                         RuleNodeCacheConditions& aConditions,
                         TransformReferenceBox& aRefBox,
 ...
 * -cos(φ)sin(φ) and D = 1 - sin²(φ) = cos²(φ). Thus, in step 5, the Y scale
 * is sqrt(cos²(φ)(sin²(φ) + cos²(φ)) = cos(φ). Thus, after step 5, C =
 * -sin(φ), D = cos(φ), and the XY shear is tan(φ). Thus, in step 6, A * D -
 * B * C = cos²(φ) + sin²(φ) = 1. In step 7, the rotation is thus φ.
 *
 ...
 *     In step 3, the X scale factor is sqrt(1+tan²(φ)) = sqrt(sec²(φ)) =
 * sec(φ). Thus, after step 3, A = 1/sec(φ) = cos(φ) and B = tan(φ) / sec(φ) =
 * sin(φ). In step 4, the XY shear is cos(φ)tan(θ) + sin(φ). Thus, after
 * step 4, C = tan(θ) - cos(φ)(cos(φ)tan(θ) + sin(φ)) = tan(θ)sin²(φ) -
 * cos(φ)sin(φ) D = 1 - sin(φ)(cos(φ)tan(θ) + sin(φ)) = cos²(φ) -
 * sin(φ)cos(φ)tan(θ) Thus, in step 5, the Y scale is sqrt(C² + D²) =
...
Size GetScaleValue(const nsCSSValueSharedList* aList,
  ...
  Matrix4x4 transform = ReadTransforms(
      aList->mHead, aForFrame->StyleContext(), aForFrame->PresContext(),
      dontCare, refBox, aForFrame->PresContext()->AppUnitsPerDevPixel(),
      &dontCareBool);
  Matrix transform2d;
