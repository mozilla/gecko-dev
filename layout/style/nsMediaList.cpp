...
bool nsMediaExpression::Matches(nsPresContext* aPresContext,
  ...
  if (required.GetUnit() == eCSSUnit_Null) {
    if (actual.GetUnit() == eCSSUnit_Integer) return actual.GetIntValue() != 0;
    if (actual.IsLengthUnit()) return actual.GetFloatValue() != 0;
    return true;
  }

  NS_ASSERTION(mFeature->mRangeType == nsMediaFeature::eMinMaxAllowed ||
                   mRange == nsMediaExpression::eEqual,
               "yikes");
  ...
  switch (mFeature->mValueType) {
    case nsMediaFeature::eLength: {
      ...
      nscoord actualCoord =
          nsRuleNode::CalcLengthWithInitialFont(aPresContext, actual);
      nscoord requiredCoord =
          nsRuleNode::CalcLengthWithInitialFont(aPresContext, required);
      cmp = DoCompare(actualCoord, requiredCoord);
    } break;
    case nsMediaFeature::eInteger:
    case nsMediaFeature::eBoolInteger: {
      NS_ASSERTION(actual.GetUnit() == eCSSUnit_Integer, "bad actual value");
      ...
      NS_ASSERTION(mFeature->mValueType != nsMediaFeature::eBoolInteger ||
                       required.GetIntValue() == 0 ||
                       required.GetIntValue() == 1,
                   "bad required bool integer value");
      cmp = DoCompare(actual.GetIntValue(), required.GetIntValue());
    } break;
    case nsMediaFeature::eFloat: {
      NS_ASSERTION(actual.GetUnit() == eCSSUnit_Number, "bad actual value");
      NS_ASSERTION(required.GetUnit() == eCSSUnit_Number, "bad required value");
      cmp = DoCompare(actual.GetFloatValue(), required.GetFloatValue());
    } break;
    case nsMediaFeature::eIntRatio: {
      NS_ASSERTION(
          actual.GetUnit() == eCSSUnit_Array &&
              actual.GetArrayValue()->Count() == 2 &&
              actual.GetArrayValue()->Item(0).GetUnit() == eCSSUnit_Integer &&
              actual.GetArrayValue()->Item(1).GetUnit() == eCSSUnit_Integer,
          "bad actual value");
      NS_ASSERTION(
          required.GetUnit() == eCSSUnit_Array &&
              required.GetArrayValue()->Count() == 2 &&
              required.GetArrayValue()->Item(0).GetUnit() == eCSSUnit_Integer &&
              required.GetArrayValue()->Item(1).GetUnit() == eCSSUnit_Integer,
          "bad required value");
    ...
    case nsMediaFeature::eEnumerated: {
      NS_ASSERTION(actual.GetUnit() == eCSSUnit_Enumerated, "bad actual value");
    ...
    case nsMediaFeature::eIdent: {
      NS_ASSERTION(actual.GetUnit() == eCSSUnit_AtomIdent, "bad actual value");
...
void nsMediaQueryResultCacheKey::AddExpression(
    const nsMediaExpression* aExpression, bool aExpressionMatches) {
...
bool nsMediaQueryResultCacheKey::Matches(nsPresContext* aPresContext) const {
...
bool nsDocumentRuleResultCacheKey::AddMatchingRule(css::DocumentRule* aRule) {
  MOZ_ASSERT(!mFinalized);
  return mMatchingRules.AppendElement(aRule);
}

void nsDocumentRuleResultCacheKey::Finalize() {
  mMatchingRules.Sort();
#ifdef DEBUG
  mFinalized = true;
#endif
...
#ifdef DEBUG
static bool ArrayIsSorted(const nsTArray<css::DocumentRule*>& aRules) {
...
bool nsDocumentRuleResultCacheKey::Matches(
...
#ifdef DEBUG
void nsDocumentRuleResultCacheKey::List(FILE* aOut, int32_t aIndent) const {
  for (css::DocumentRule* rule : mMatchingRules) {
    nsCString str;
...
size_t nsDocumentRuleResultCacheKey::SizeOfExcludingThis(
...
void nsMediaQuery::AppendToString(nsAString& aString) const {
  ...
  for (uint32_t i = 0, i_end = mExpressions.Length(); i < i_end; ++i) {
    if (i > 0 || !mTypeOmitted) aString.AppendLiteral(" and ");
    aString.Append('(');
    ...
    if (expr.mValue.GetUnit() != eCSSUnit_Null) {
      ...
      switch (feature->mValueType) {
        ...
        case nsMediaFeature::eInteger:
        case nsMediaFeature::eBoolInteger:
          NS_ASSERTION(expr.mValue.GetUnit() == eCSSUnit_Integer, "bad unit");
          ...
          break;
        case nsMediaFeature::eFloat: {
          NS_ASSERTION(expr.mValue.GetUnit() == eCSSUnit_Number, "bad unit");
          // Use 'line-height' as a property that takes float values
          // written in the normal way.
          expr.mValue.AppendToString(eCSSProperty_line_height, aString);
        } break;
        case nsMediaFeature::eIntRatio: {
          NS_ASSERTION(expr.mValue.GetUnit() == eCSSUnit_Array, "bad unit");
...
nsMediaQuery* nsMediaQuery::Clone() const { return new nsMediaQuery(*this); }

bool nsMediaQuery::Matches(nsPresContext* aPresContext,
                           nsMediaQueryResultCacheKey* aKey) const {
  if (mHadUnknownExpression) return false;
...
void nsMediaList::GetText(nsAString& aMediaText) {
  aMediaText.Truncate();
...
// it should be doing....
void nsMediaList::SetText(const nsAString& aMediaText) {
  nsCSSParser parser;
  parser.ParseMediaList(aMediaText, nullptr, 0, this);
}

bool nsMediaList::Matches(nsPresContext* aPresContext,
                          nsMediaQueryResultCacheKey* aKey) const {
  for (int32_t i = 0, i_end = mArray.Length(); i < i_end; ++i) {
...
already_AddRefed<dom::MediaList> nsMediaList::Clone() {
...
void nsMediaList::IndexedGetter(uint32_t aIndex, bool& aFound,
                                nsAString& aReturn) {
  if (aIndex < Length()) {
    aFound = true;
    aReturn.Truncate();
...
nsresult nsMediaList::Delete(const nsAString& aOldMedium) {
  if (aOldMedium.IsEmpty()) return NS_ERROR_DOM_NOT_FOUND_ERR;
...
nsresult nsMediaList::Append(const nsAString& aNewMedium) {
  if (aNewMedium.IsEmpty()) return NS_ERROR_DOM_NOT_FOUND_ERR;
