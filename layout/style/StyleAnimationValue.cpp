 ...
 */
static StyleAnimationValue::Unit GetCommonUnit(
    nsCSSPropertyID aProperty, StyleAnimationValue::Unit aFirstUnit,
...
static nsCSSUnit GetCommonUnit(nsCSSPropertyID aProperty, nsCSSUnit aFirstUnit,
                               nsCSSUnit aSecondUnit) {
  if (aFirstUnit != aSecondUnit) {
    if (nsCSSProps::PropHasFlags(aProperty, CSS_PROPERTY_STORES_CALC) &&
        (aFirstUnit == eCSSUnit_Pixel || aFirstUnit == eCSSUnit_Percent ||
         aFirstUnit == eCSSUnit_Calc) &&
        (aSecondUnit == eCSSUnit_Pixel || aSecondUnit == eCSSUnit_Percent ||
...
static nsCSSKeyword ToPrimitive(nsCSSKeyword aKeyword) {
  switch (aKeyword) {
...
static bool HasAccumulateMatrix(const nsCSSValueList* aList) {
  const nsCSSValueList* item = aList;
  do {
    nsCSSKeyword func = nsStyleTransformMatrix::TransformFunctionOf(
        item->mValue.GetArrayValue());
...
static bool TransformFunctionsMatch(nsCSSKeyword func1, nsCSSKeyword func2) {
...
static bool TransformFunctionListsMatch(const nsCSSValueList* list1,
...
#ifdef MOZ_OLD_STYLE
static already_AddRefed<nsCSSValue::Array> ToPrimitive(
    nsCSSValue::Array* aArray) {
...
static void AppendCSSShadowValue(const nsCSSShadowItem* aShadow,
...
static PixelCalcValue ExtractCalcValueInternal(const nsCSSValue& aValue) {
  ...
  if (topval.GetUnit() == eCSSUnit_Pixel) {
    ...
    MOZ_ASSERT(topval.GetUnit() == eCSSUnit_Calc_Plus, "unexpected unit");
...
// Requires a canonical calc() value that we generated.
static PixelCalcValue ExtractCalcValue(const StyleAnimationValue& aValue) {
  PixelCalcValue result;
...
static PixelCalcValue ExtractCalcValue(const nsCSSValue& aValue) {
  PixelCalcValue result;
...
static void CalcValueToCSSValue(const nsStyleCoord::CalcValue* aCalc,
                                nsCSSValue& aValue) {
  aValue.SetCalcValue(aCalc);
}

static void CalcValueToCSSValue(const PixelCalcValue& aCalc,
                                nsCSSValue& aValue) {
...
static double CalcPositionSquareDistance(const nsCSSValue& aPos1,
                                         const nsCSSValue& aPos2) {
  NS_ASSERTION(
      aPos1.GetUnit() == eCSSUnit_Array && aPos2.GetUnit() == eCSSUnit_Array,
      "Expected two arrays");
...
static PixelCalcValue CalcBackgroundCoord(const nsCSSValue& aCoord) {
  NS_ASSERTION(aCoord.GetUnit() == eCSSUnit_Array, "Expected array");

  nsCSSValue::Array* array = aCoord.GetArrayValue();
  MOZ_ASSERT(array->Count() == 2 && array->Item(0).GetUnit() == eCSSUnit_Null &&
                 array->Item(1).GetUnit() != eCSSUnit_Null,
             "Invalid position value");
  return ExtractCalcValue(array->Item(1));
}

static double CalcPositionCoordSquareDistance(const nsCSSValue& aPos1,
                                              const nsCSSValue& aPos2) {
...
template <typename T>
MOZ_ALWAYS_INLINE T EnsureNotNan(T aValue) {
  return aValue;
}
template <>
MOZ_ALWAYS_INLINE float EnsureNotNan(float aValue) {
  // This would benefit from a MOZ_FLOAT_IS_NaN if we had one.
  return MOZ_LIKELY(!mozilla::IsNaN(aValue)) ? aValue : 0;
}
template <>
MOZ_ALWAYS_INLINE double EnsureNotNan(double aValue) {
  return MOZ_LIKELY(!mozilla::IsNaN(aValue)) ? aValue : 0;
}

template <typename T>
T RestrictValue(uint32_t aRestrictions, T aValue) {
...
template <typename T>
T RestrictValue(nsCSSPropertyID aProperty, T aValue) {
  return RestrictValue(nsCSSProps::ValueRestrictions(aProperty), aValue);
}

static void AddCSSValueAngle(double aCoeff1, const nsCSSValue& aValue1,
                             double aCoeff2, const nsCSSValue& aValue2,
                             nsCSSValue& aResult) {
  if (aValue1.GetUnit() == aValue2.GetUnit()) {
    // To avoid floating point error, if the units match, maintain the unit.
    aResult.SetFloatValue(EnsureNotNan(aCoeff1 * aValue1.GetFloatValue() +
...
static inline void AddCSSValuePercent(double aCoeff1, const nsCSSValue& aValue1,
                                      double aCoeff2, const nsCSSValue& aValue2,
                                      nsCSSValue& aResult,
                                      uint32_t aValueRestrictions = 0) {
  MOZ_ASSERT(aValue1.GetUnit() == eCSSUnit_Percent, "unexpected unit");
  MOZ_ASSERT(aValue2.GetUnit() == eCSSUnit_Percent, "unexpected unit");
  aResult.SetPercentValue(RestrictValue(
      aValueRestrictions, aCoeff1 * aValue1.GetPercentValue() +
                              aCoeff2 * aValue2.GetPercentValue()));
}

// Add two canonical-form calc values (eUnit_Calc) to make another
// canonical-form calc value.
static void AddCSSValueCanonicalCalc(double aCoeff1, const nsCSSValue& aValue1,
                                     double aCoeff2, const nsCSSValue& aValue2,
                                     nsCSSValue& aResult) {
...
static inline void AddCSSValuePixel(double aCoeff1, const nsCSSValue& aValue1,
                                    double aCoeff2, const nsCSSValue& aValue2,
                                    nsCSSValue& aResult,
                                    uint32_t aValueRestrictions = 0) {
  ...
  aResult.SetFloatValue(
      RestrictValue(aValueRestrictions, aCoeff1 * aValue1.GetFloatValue() +
                                            aCoeff2 * aValue2.GetFloatValue()),
      eCSSUnit_Pixel);
}

static bool AddCSSValuePixelPercentCalc(
    const uint32_t aValueRestrictions, const nsCSSUnit aCommonUnit,
    double aCoeff1, const nsCSSValue& aValue1, double aCoeff2,
    const nsCSSValue& aValue2, nsCSSValue& aResult) {
  switch (aCommonUnit) {
    case eCSSUnit_Pixel:
...
static void AddTransformTranslate(double aCoeff1, const nsCSSValue& aValue1,
                                  double aCoeff2, const nsCSSValue& aValue2,
                                  nsCSSValue& aResult) {
  // Only three possible units: eCSSUnit_Pixel, eCSSUnit_Percent, or
  // eCSSUnit_Calc.
  MOZ_ASSERT(aValue1.GetUnit() == eCSSUnit_Percent ||
                 aValue1.GetUnit() == eCSSUnit_Pixel || aValue1.IsCalcUnit(),
             "unexpected unit");
  MOZ_ASSERT(aValue2.GetUnit() == eCSSUnit_Percent ||
                 aValue2.GetUnit() == eCSSUnit_Pixel || aValue2.IsCalcUnit(),
             "unexpected unit");
  AddCSSValuePixelPercentCalc(
      0,
      (aValue1.GetUnit() != aValue2.GetUnit() || aValue1.IsCalcUnit())
          ? eCSSUnit_Calc
          : aValue1.GetUnit(),
      aCoeff1, aValue1, aCoeff2, aValue2, aResult);
}

// Unclamped AddWeightedColors.
static RGBAColorData AddWeightedColors(double aCoeff1,
                                       const RGBAColorData& aValue1,
                                       double aCoeff2,
                                       const RGBAColorData& aValue2) {
  ...
  return RGBAColorData(
      ...
      (aValue1.mB * factor1 + aValue2.mB * factor2) * resultFactor, resultA);
}

// CLASS METHODS
// -------------

static RGBAColorData ExtractColor(const nsCSSValue& aValue) {
  ...
  if (aValue.GetUnit() == eCSSUnit_PercentageRGBColor ||
    ...
    return {floatColor->Comp1(), floatColor->Comp2(), floatColor->Comp3(),
            floatColor->Alpha()};
  }
  return RGBAColorData(aValue.GetColorValue());
}

static RGBAColorData ExtractColor(const StyleAnimationValue& aValue) {
...
static ComplexColorData ExtractComplexColor(const StyleAnimationValue& aValue) {
...
StyleAnimationValue StyleAnimationValue::Add(nsCSSPropertyID aProperty,
                                             const StyleAnimationValue& aA,
                                             StyleAnimationValue&& aB) {
  StyleAnimationValue result(Move(aB));

  Unit commonUnit = GetCommonUnit(aProperty, result.GetUnit(), aA.GetUnit());
  switch (commonUnit) {
    case eUnit_Color: {
    ...
    case eUnit_Transform: {
      ...
      UniquePtr<nsCSSValueList> resultList(
          aA.GetCSSValueSharedListValue()->mHead->Clone());
    ...
    default:
      Unused << AddWeighted(aProperty, 1.0, result, 1, aA, result);
      break;
  }
...
double StyleAnimationValue::ComputeColorDistance(
    const RGBAColorData& aStartColor, const RGBAColorData& aEndColor) {
...
static UniquePtr<nsCSSValueList> AddWeightedFilterFunction(
    double aCoeff1, const nsCSSValueList* aList1, double aCoeff2,
    const nsCSSValueList* aList2, ColorAdditionType aColorAdditionType);

static inline float GetNumberOrPercent(const nsCSSValue& aValue);

static bool ComputeSingleShadowSquareDistance(const nsCSSValueList* aShadow1,
  ...
  for (size_t i = 0; i < 4; ++i) {
    ...
    MOZ_ASSERT(array1->Item(i).GetUnit() == eCSSUnit_Pixel, "unexpected unit");
    MOZ_ASSERT(array2->Item(i).GetUnit() == eCSSUnit_Pixel, "unexpected unit");
    double diff =
        array1->Item(i).GetFloatValue() - array2->Item(i).GetFloatValue();
  ...
  if (color1.GetUnit() != eCSSUnit_Null) {
    double colorDistance = StyleAnimationValue::ComputeColorDistance(
        ExtractColor(color1), ExtractColor(color2));
...
// functions.
static bool ComputeFilterSquareDistance(const nsCSSValueList* aList1,
  ...
  switch (filterFunction) {
    ...
    case eCSSKeyword_drop_shadow: {
      ...
      if (!ComputeSingleShadowSquareDistance(
              func1.GetListValue(), func2.GetListValue(), aSquareDistance,
              eCSSProperty_filter)) {
        return false;
      }
      break;
...
static bool ComputeFilterListDistance(const nsCSSValueList* aList1,
                                      const nsCSSValueList* aList2,
                                      double& aDistance) {
  double squareDistance = 0.0;
  while (aList1 || aList2) {
    ...
    if (!aList1) {
      ...
      UniquePtr<nsCSSValueList> none = AddWeightedFilterFunction(
          0, aList2, 0, aList2, ColorAdditionType::Clamped);
      ...
      UniquePtr<nsCSSValueList> none = AddWeightedFilterFunction(
          0, aList1, 0, aList1, ColorAdditionType::Clamped);
      ...
      if (!ComputeFilterSquareDistance(aList1, aList2, currentSquareDistance)) {
        return false;
      }
      aList1 = aList1->mNext;
      aList2 = aList2->mNext;
...
enum class Restrictions { Enable, Disable };

static already_AddRefed<nsCSSValue::Array> AddShapeFunction(
    nsCSSPropertyID aProperty, double aCoeff1, const nsCSSValue::Array* aArray1,
    double aCoeff2, const nsCSSValue::Array* aArray2,
    Restrictions aRestriction = Restrictions::Enable);

static bool ComputeShapeDistance(nsCSSPropertyID aProperty,
  ...
  RefPtr<nsCSSValue::Array> diffShape = AddShapeFunction(
      aProperty, 1.0, aArray2, -1.0, aArray1, Restrictions::Disable);
  if (!diffShape) {
    return false;
  }
  ...
  auto pixelCalcDistance = [](const PixelCalcValue& aValue) {
    MOZ_ASSERT(
        aValue.mHasPercent || aValue.mPercent == 0.0f,
        "can't have a nonzero percentage part without having percentages");
...
static nsCSSValueList* AddTransformLists(
    double aCoeff1, const nsCSSValueList* aList1, double aCoeff2,
    const nsCSSValueList* aList2,
    nsCSSKeyword aOperatorType = eCSSKeyword_interpolatematrix);

static double ComputeTransform2DMatrixDistance(const Matrix& aMatrix1,
  ...
  const double diffShear =
      atan(shear2[ShearType::XYSHEAR]) - atan(shear1[ShearType::XYSHEAR]);
  const double diffRotate =
      2.0 * (atan2f(rotate2.z, rotate2.w) - atan2f(rotate1.z, rotate1.w));
  ...
  return diffTranslate.DotProduct(diffTranslate) +
         diffScale.DotProduct(diffScale) + diffRotate * diffRotate +
         diffShear * diffShear;
}

static double ComputeTransform3DMatrixDistance(const Matrix4x4& aMatrix1,
  ...
  const Point3D diffTranslate = translate2 - translate1;
  const Point3D diffScale = scale2 - scale1;
  const Point3D diffShear(
      atan(shear2[ShearType::XYSHEAR]) - atan(shear1[ShearType::XYSHEAR]),
      atan(shear2[ShearType::XZSHEAR]) - atan(shear1[ShearType::XZSHEAR]),
      atan(shear2[ShearType::YZSHEAR]) - atan(shear1[ShearType::YZSHEAR]));
  ...
  return diffTranslate.DotProduct(diffTranslate) +
         ...
         diffShear.DotProduct(diffShear) + diffRotate * diffRotate;
}

static double ComputeTransformDistance(nsCSSValue::Array* aArray1,
  ...
  switch (tfunc) {
    ...
    case eCSSKeyword_skew: {
      ...
      AddCSSValueAngle(1.0, a2->Count() == 3 ? a2->Item(2) : zero, -1.0,
                       a1->Count() == 3 ? a1->Item(2) : zero, y);
    ...
    case eCSSKeyword_rotatez: {
      ...
      distance =
          angle.GetAngleValueInRadians() * angle.GetAngleValueInRadians();
      break;
    }
    case eCSSKeyword_rotate3d: {
      ...
      Point3D vector1(a1->Item(1).GetFloatValue(), a1->Item(2).GetFloatValue(),
                      a1->Item(3).GetFloatValue());
      double angle1 = a1->Item(4).GetAngleValueInRadians();

      Point3D vector2(a2->Item(1).GetFloatValue(), a2->Item(2).GetFloatValue(),
...
static double ComputeTransformListDistance(const nsCSSValueList* aList1,
                                           const nsCSSValueList* aList2) {
...
static double ComputeMismatchedTransfromListDistance(
    const nsCSSValueList* aList1, const nsCSSValueList* aList2,
  ...
  Matrix4x4 m1 = nsStyleTransformMatrix::ReadTransforms(
      aList1, aStyleContext, aStyleContext->PresContext(), dontCare,
      emptyRefBox, nsPresContext::AppUnitsPerCSSPixel(), &dontCareBool);
  Matrix4x4 m2 = nsStyleTransformMatrix::ReadTransforms(
      aList2, aStyleContext, aStyleContext->PresContext(), dontCare,
      emptyRefBox, nsPresContext::AppUnitsPerCSSPixel(), &dontCareBool);
  return sqrt(ComputeTransform3DMatrixDistance(m1, m2));
}

bool StyleAnimationValue::ComputeDistance(
  ...
  switch (commonUnit) {
    ...
    case eUnit_ComplexColor: {
      ...
      MOZ_ASSERT_UNREACHABLE(
          "We shouldn't get here as we only call "
    ...
    case eUnit_ObjectPosition: {
      ...
      double squareDistance =
          CalcPositionSquareDistance(*position1, *position2);
      aDistance = sqrt(squareDistance);
      return true;
    }
    ...
    case eUnit_CSSValueTriplet: {
      ...
      static nsCSSValue nsCSSValueTriplet::*const pairValues[3] = {
          &nsCSSValueTriplet::mXValue, &nsCSSValueTriplet::mYValue,
          &nsCSSValueTriplet::mZValue};
    ...
    case eUnit_Shadow: {
      ...
      while (shadow1) {
        double squareDistance = 0.0;
        if (!ComputeSingleShadowSquareDistance(shadow1, shadow2, squareDistance,
                                               aProperty)) {
          NS_ERROR(
              "Unexpected ComputeSingleShadowSquareDistance failure; "
    ...
    case eUnit_Shape:
      return ComputeShapeDistance(aProperty,
                                  ...
                                  aEndValue.GetCSSValueArrayValue(), aDistance);
...
static inline void AddCSSValueNumber(double aCoeff1, const nsCSSValue& aValue1,
                                     double aCoeff2, const nsCSSValue& aValue2,
                                     nsCSSValue& aResult,
                                     uint32_t aValueRestrictions = 0) {
...
static inline float GetNumberOrPercent(const nsCSSValue& aValue) {
  ...
  MOZ_ASSERT(unit == eCSSUnit_Number || unit == eCSSUnit_Percent,
             "unexpected unit");
  return (unit == eCSSUnit_Number) ? aValue.GetFloatValue()
                                   : aValue.GetPercentValue();
}

static inline void AddCSSValuePercentNumber(
    const uint32_t aValueRestrictions, double aCoeff1,
    const nsCSSValue& aValue1, double aCoeff2, const nsCSSValue& aValue2,
...
static nscolor DiluteColor(const RGBAColorData& aValue, double aDilutionRatio) {
...
static nscolor AddWeightedColorsAndClamp(double aCoeff1,
                                         const RGBAColorData& aValue1,
                                         double aCoeff2,
                                         const RGBAColorData& aValue2) {
...
static void AppendToCSSValueList(UniquePtr<nsCSSValueList>& aHead,
                                 UniquePtr<nsCSSValueList>&& aValueToAppend,
                                 nsCSSValueList** aTail) {
  MOZ_ASSERT(!aHead == !*aTail, "Can't have head w/o tail, & vice versa");
...
static void AppendToCSSValuePairList(
    UniquePtr<nsCSSValuePairList>& aHead,
...
static UniquePtr<nsCSSValueList> AddWeightedShadowItems(
    double aCoeff1, const nsCSSValue& aValue1, double aCoeff2,
    const nsCSSValue& aValue2, ColorAdditionType aColorAdditionType,
    nsCSSPropertyID aProperty) {
  // X, Y, Radius, Spread, Color, Inset
  MOZ_ASSERT(aValue1.GetUnit() == eCSSUnit_Array, "wrong unit");
...
static void AddTransformScale(double aCoeff1, const nsCSSValue& aValue1,
                              double aCoeff2, const nsCSSValue& aValue2,
                              nsCSSValue& aResult) {
...
static nsCSSValueList* AddDifferentTransformLists(double aCoeff1,
                                                  const nsCSSValueList* aList1,
                                                  double aCoeff2,
                                                  const nsCSSValueList* aList2,
  ...
  if (aCoeff1 == 0) {
    ...
    MOZ_ASSERT(
        (aOperatorType == eCSSKeyword_accumulatematrix && aCoeff1 == 1.0) ||
...
static UniquePtr<nsCSSValueList> AddWeightedFilterFunctionImpl(
  ...
  switch (filterFunction) {
    case eCSSKeyword_blur: {
      ...
      if (!AddCSSValuePixelPercentCalc(restrictions, unit, aCoeff1, funcArg1,
                                       aCoeff2, funcArg2, resultArg)) {
        return nullptr;
      }
      break;
    }
    ...
    case eCSSKeyword_opacity:
    case eCSSKeyword_saturate:
      AddCSSValuePercentNumber(restrictions, aCoeff1, funcArg1, aCoeff2,
                               funcArg2, resultArg, initialVal);
      break;
    case eCSSKeyword_hue_rotate:
      AddCSSValueAngle(aCoeff1, funcArg1, aCoeff2, funcArg2, resultArg);
      break;
    case eCSSKeyword_drop_shadow: {
      MOZ_ASSERT(
          !funcArg1.GetListValue()->mNext && !funcArg2.GetListValue()->mNext,
          "drop-shadow filter func doesn't support lists");
      UniquePtr<nsCSSValueList> shadowValue =
          AddWeightedShadowItems(aCoeff1, funcArg1.GetListValue()->mValue,
                                 ...
                                 aColorAdditionType, eCSSProperty_filter);
      if (!shadowValue) {
        return nullptr;
      }
...
static inline uint32_t ShapeArgumentCount(nsCSSKeyword aShapeFunction) {
...
static void AddPositions(double aCoeff1, const nsCSSValue& aPos1,
...
static Maybe<nsCSSValuePair> AddCSSValuePair(nsCSSPropertyID aProperty,
                                             uint32_t aRestrictions,
                                             double aCoeff1,
                                             const nsCSSValuePair* aPair1,
                                             double aCoeff2,
                                             const nsCSSValuePair* aPair2) {
  ...
  for (uint32_t i = 0; i < 2; ++i) {
    nsCSSValue nsCSSValuePair::*member = pairValues[i];
    if (!AddCSSValuePixelPercentCalc(aRestrictions, unit[i], aCoeff1,
                                     aPair1->*member, aCoeff2, aPair2->*member,
                                     result.ref().*member)) {
      MOZ_ASSERT(false, "unexpected unit");
      result.reset();
      return result;  // Nothing() (returning |result| for RVO)
...
static UniquePtr<nsCSSValuePairList> AddCSSValuePairList(
    nsCSSPropertyID aProperty, double aCoeff1, const nsCSSValuePairList* aList1,
  ...
  do {
    ...
    for (uint32_t i = 0; i < ArrayLength(pairListValues); ++i) {
      ...
      if (unit == eCSSUnit_Number) {
        AddCSSValueNumber(aCoeff1, v1, aCoeff2, v2, vr, restrictions);
      } else if (!AddCSSValuePixelPercentCalc(restrictions, unit, aCoeff1, v1,
                                              aCoeff2, v2, vr)) {
        if (v1 != v2) {
          return nullptr;
        }
...
static already_AddRefed<nsCSSValue::Array> AddShapeFunction(
  ...
  nsCSSValue::Array* resultFuncArgs = result->Item(0).InitFunction(
      shapeFuncName, ShapeArgumentCount(shapeFuncName));
  switch (shapeFuncName) {
    case eCSSKeyword_ellipse:
      // Add ellipses' |ry| values (but fail if we encounter an enum):
      if (!AddCSSValuePixelPercentCalc(
              aRestriction == Restrictions::Enable
                  ? CSS_PROPERTY_VALUE_NONNEGATIVE
                  : 0,
              GetCommonUnit(aProperty, func1->Item(2).GetUnit(),
                            func2->Item(2).GetUnit()),
              aCoeff1, func1->Item(2), aCoeff2, func2->Item(2),
    ...
    case eCSSKeyword_circle: {
      ...
      if (!AddCSSValuePixelPercentCalc(
              aRestriction == Restrictions::Enable
                  ? CSS_PROPERTY_VALUE_NONNEGATIVE
                  : 0,
              GetCommonUnit(aProperty, func1->Item(1).GetUnit(),
                            func2->Item(1).GetUnit()),
              aCoeff1, func1->Item(1), aCoeff2, func2->Item(1),
              resultFuncArgs->Item(1))) {
        return nullptr;
      }
      // Add center points (defined as a <position>).
      size_t posIndex = shapeFuncName == eCSSKeyword_circle ? 2 : 3;
      AddPositions(aCoeff1, func1->Item(posIndex), aCoeff2,
                   func2->Item(posIndex), resultFuncArgs->Item(posIndex));
      break;
    }
    case eCSSKeyword_polygon: {
    ...
    case eCSSKeyword_inset: {
      ...
      for (size_t i = 1; i <= 4; ++i) {
        if (!AddCSSValuePixelPercentCalc(
                aRestriction == Restrictions::Enable
                    ? CSS_PROPERTY_VALUE_NONNEGATIVE
                    : 0,
                GetCommonUnit(aProperty, func1->Item(i).GetUnit(),
                              func2->Item(i).GetUnit()),
                aCoeff1, func1->Item(i), aCoeff2, func2->Item(i),
      ...
      uint32_t restrictions = aRestriction == Restrictions::Enable
                                  ? nsCSSProps::ValueRestrictions(
                                        eCSSProperty_border_top_left_radius)
                                  : 0;
      for (size_t i = 0; i < 4; ++i) {
        const nsCSSValuePair& pair1 = radii1->Item(i).GetPairValue();
        const nsCSSValuePair& pair2 = radii2->Item(i).GetPairValue();
        const Maybe<nsCSSValuePair> pairResult = AddCSSValuePair(
            aProperty, restrictions, aCoeff1, &pair1, aCoeff2, &pair2);
        if (!pairResult) {
          return nullptr;
        }
...
static nsCSSValueList* AddTransformLists(double aCoeff1,
                                         const nsCSSValueList* aList1,
                                         double aCoeff2,
                                         const nsCSSValueList* aList2,
                                         nsCSSKeyword aOperatorType) {
  ...
  do {
    ...
    MOZ_ASSERT(TransformFunctionsMatch(
    ...
    nsCSSKeyword tfunc = nsStyleTransformMatrix::TransformFunctionOf(a1);
    RefPtr<nsCSSValue::Array> arr;
    if (tfunc != eCSSKeyword_matrix && tfunc != eCSSKeyword_matrix3d &&
        tfunc != eCSSKeyword_interpolatematrix &&
        tfunc != eCSSKeyword_rotate3d && tfunc != eCSSKeyword_perspective) {
    ...
    switch (tfunc) {
      ...
      case eCSSKeyword_skew: {
        ...
        AddCSSValueAngle(aCoeff1, a1->Count() == 3 ? a1->Item(2) : zero,
                         aCoeff2, a2->Count() == 3 ? a2->Item(2) : zero,
                         arr->Item(2));
      ...
      case eCSSKeyword_interpolatematrix: {
        ...
        if (aList1 == aList2) {
          *resultTail = AddDifferentTransformLists(aCoeff1, &tempList1, aCoeff2,
                                                   &tempList1, aOperatorType);
        } else {
          *resultTail = AddDifferentTransformLists(aCoeff1, &tempList1, aCoeff2,
                                                   &tempList2, aOperatorType);
        }
  ...
  MOZ_ASSERT(!*resultTail, "resultTail isn't pointing to the tail");

  return result.forget();
}

static void AddPositionCoords(double aCoeff1, const nsCSSValue& aPos1,
                              double aCoeff2, const nsCSSValue& aPos2,
                              nsCSSValue& aResultPos) {
  const nsCSSValue::Array* posArray1 = aPos1.GetArrayValue();
  const nsCSSValue::Array* posArray2 = aPos2.GetArrayValue();
...
static UniquePtr<nsCSSValueList> AddWeightedShadowList(
    double aCoeff1, const nsCSSValueList* aShadow1, double aCoeff2,
    const nsCSSValueList* aShadow2, ColorAdditionType aColorAdditionType,
    nsCSSPropertyID aProperty) {
  ...
  while (aShadow1 && aShadow2) {
    UniquePtr<nsCSSValueList> shadowValue =
        AddWeightedShadowItems(aCoeff1, aShadow1->mValue, aCoeff2,
                               aShadow2->mValue, aColorAdditionType, aProperty);
    if (!shadowValue) {
      return nullptr;
    }
  ...
  if (aShadow1 || aShadow2) {
    ...
    while (longShadow) {
      ...
      // the current shadow.
      UniquePtr<nsCSSValueList> shadowValue = AddWeightedShadowItems(
          longCoeff, longShadow->mValue, 0.0, longShadow->mValue,
...
static UniquePtr<nsCSSValueList> AddWeightedFilterList(
    double aCoeff1, const nsCSSValueList* aList1, double aCoeff2,
    const nsCSSValueList* aList2, ColorAdditionType aColorAdditionType) {
  UniquePtr<nsCSSValueList> result;
  nsCSSValueList* tail = nullptr;
  while (aList1 || aList2) {
    ...
    UniquePtr<nsCSSValueList> resultFunction = AddWeightedFilterFunction(
        aCoeff1, aList1, aCoeff2, aList2, aColorAdditionType);
    if (!resultFunction) {
...
bool StyleAnimationValue::AddWeighted(nsCSSPropertyID aProperty, double aCoeff1,
  ...
  switch (commonUnit) {
    ...
    case eUnit_Visibility: {
      ...
      int32_t result =
          interp > 0.0 ? NS_STYLE_VISIBILITY_VISIBLE : (val1 ? enum2 : enum1);
    ...
    case eUnit_Coord: {
      aResultValue.SetCoordValue(RestrictValue(
          aProperty, NSToCoordRound(aCoeff1 * aValue1.GetCoordValue() +
                                    aCoeff2 * aValue2.GetCoordValue())));
      return true;
    }
    case eUnit_Percent: {
      aResultValue.SetPercentValue(
          RestrictValue(aProperty, aCoeff1 * aValue1.GetPercentValue() +
                                       aCoeff2 * aValue2.GetPercentValue()));
      return true;
    }
    case eUnit_Float: {
      aResultValue.SetFloatValue(
          RestrictValue(aProperty, aCoeff1 * aValue1.GetFloatValue() +
                                       aCoeff2 * aValue2.GetFloatValue()));
      return true;
    }
    case eUnit_Color: {
    ...
    case eUnit_ComplexColor: {
      ...
      if (color1.IsNumericColor() && color2.IsCurrentColor()) {
        ...
        RGBAColorData resultColor =
            AddWeightedColors(aCoeff1, color1.mColor.WithAlpha(alpha1), aCoeff2,
    ...
    case eUnit_ObjectPosition: {
      ...
      AddPositions(aCoeff1, *position1, aCoeff2, *position2, *result);
    ...
    case eUnit_CSSValuePair: {
      uint32_t restrictions = nsCSSProps::ValueRestrictions(aProperty);
      Maybe<nsCSSValuePair> result = AddCSSValuePair(
          aProperty, restrictions, aCoeff1, aValue1.GetCSSValuePairValue(),
    ...
    case eUnit_CSSValueTriplet: {
      ...
      for (uint32_t i = 0; i < 3; ++i) {
        ...
        if (!AddCSSValuePixelPercentCalc(restrictions, unit[i], aCoeff1,
                                         &triplet1->*member, aCoeff2,
                                         &triplet2->*member, result->*member)) {
          MOZ_ASSERT(false, "unexpected unit");
          return false;
        }
      }
    ...
    case eUnit_Dasharray: {
      ...
      nsAutoPtr<nsCSSValueList> result;
      nsCSSValueList** resultTail = getter_Transfers(result);
      for (uint32_t i = 0, i_end = EuclidLCM<uint32_t>(len1, len2); i != i_end;
           ++i) {
        const nsCSSValue& v1 = list1->mValue;
        const nsCSSValue& v2 = list2->mValue;
        MOZ_ASSERT(
            v1.GetUnit() == eCSSUnit_Number || v1.GetUnit() == eCSSUnit_Percent,
            "unexpected");
        MOZ_ASSERT(
            v2.GetUnit() == eCSSUnit_Number || v2.GetUnit() == eCSSUnit_Percent,
            "unexpected");
    ...
    case eUnit_Shadow: {
      UniquePtr<nsCSSValueList> result =
          AddWeightedShadowList(aCoeff1, aValue1.GetCSSValueListValue(),
                                ...
                                ColorAdditionType::Clamped, aProperty);
      if (!result) {
        return false;
      }
    ...
    case eUnit_Shape: {
      RefPtr<nsCSSValue::Array> result =
          AddShapeFunction(aProperty, aCoeff1, aValue1.GetCSSValueArrayValue(),
    ...
    case eUnit_Filter: {
      UniquePtr<nsCSSValueList> result = AddWeightedFilterList(
          aCoeff1, aValue1.GetCSSValueListValue(), aCoeff2,
          aValue2.GetCSSValueListValue(), ColorAdditionType::Clamped);
      if (!result) {
        return false;
      }

      aResultValue.SetAndAdoptCSSValueListValue(result.release(), eUnit_Filter);
      return true;
    }
      ...
      if (list1->mValue.GetUnit() == eCSSUnit_None) {
        ...
        if (list2->mValue.GetUnit() == eCSSUnit_None) {
          if (HasAccumulateMatrix(list1)) {
            result = AddDifferentTransformLists(0, list1, aCoeff1, list1,
                                                eCSSKeyword_interpolatematrix);
          } else {
    ...
    case eUnit_BackgroundPositionCoord: {
      ...
      while (position1 && position2) {
        ...
        *resultTail = item;
        ...
        AddPositionCoords(aCoeff1, position1->mValue, aCoeff2,
                          position2->mValue, item->mValue);
...
StyleAnimationValue StyleAnimationValue::Accumulate(
    nsCSSPropertyID aProperty, const StyleAnimationValue& aA,
    StyleAnimationValue&& aB, uint64_t aCount) {
  StyleAnimationValue result(Move(aB));
  ...
  switch (commonUnit) {
    case eUnit_Filter: {
      UniquePtr<nsCSSValueList> resultList = AddWeightedFilterList(
          1.0, result.GetCSSValueListValue(), aCount, aA.GetCSSValueListValue(),
    ...
    case eUnit_Shadow: {
      UniquePtr<nsCSSValueList> resultList = AddWeightedShadowList(
          1.0, result.GetCSSValueListValue(), aCount, aA.GetCSSValueListValue(),
          ColorAdditionType::Unclamped, aProperty);
      if (resultList) {
    ...
    case eUnit_Transform: {
      const nsCSSValueList* listA = aA.GetCSSValueSharedListValue()->mHead;
      const nsCSSValueList* listB = result.GetCSSValueSharedListValue()->mHead;
      ...
      if (listA->mValue.GetUnit() == eCSSUnit_None) {
        ...
        resultList = AddDifferentTransformLists(1.0, listB, aCount, listA,
                                                eCSSKeyword_accumulatematrix);
      }
    ...
    default:
      Unused << AddWeighted(aProperty, 1.0, result, aCount, aA, result);
      break;
  }
  return result;
}

static already_AddRefed<css::StyleRule> BuildStyleRule(
    nsCSSPropertyID aProperty, dom::Element* aTargetElement,
  ...
  nsCSSPropertyID propertyToCheck =
...
static bool ComputeValuesFromStyleContext(
    nsCSSPropertyID aProperty, CSSEnabledState aEnabledState,
    GeckoStyleContext* aStyleContext,
...
static bool ComputeValuesFromStyleRule(
    nsCSSPropertyID aProperty, CSSEnabledState aEnabledState,
    GeckoStyleContext* aStyleContext, css::StyleRule* aStyleRule,
    nsTArray<PropertyStyleAnimationValuePair>& aValues,
...
/* static */ bool StyleAnimationValue::ComputeValue(
    nsCSSPropertyID aProperty, dom::Element* aTargetElement,
    GeckoStyleContext* aStyleContext, const nsAString& aSpecifiedValue,
    bool aUseSVGMode, StyleAnimationValue& aComputedValue,
    bool* aIsContextSensitive) {
  ...
  bool ok = ComputeValuesFromStyleRule(
      aProperty, CSSEnabledState::eIgnoreEnabledState, aStyleContext, styleRule,
...
template <class T>
bool ComputeValuesFromSpecifiedValue(
    nsCSSPropertyID aProperty, CSSEnabledState aEnabledState,
    dom::Element* aTargetElement, GeckoStyleContext* aStyleContext,
    T& aSpecifiedValue, bool aUseSVGMode,
  ...
  aResult.Clear();
  return ComputeValuesFromStyleRule(aProperty, aEnabledState, aStyleContext,
                                    styleRule, aResult,
                                    /* aIsContextSensitive */ nullptr);
}

/* static */ bool StyleAnimationValue::ComputeValues(
    nsCSSPropertyID aProperty, CSSEnabledState aEnabledState,
    dom::Element* aTargetElement, GeckoStyleContext* aStyleContext,
    const nsAString& aSpecifiedValue, bool aUseSVGMode,
    nsTArray<PropertyStyleAnimationValuePair>& aResult) {
  return ComputeValuesFromSpecifiedValue(aProperty, aEnabledState,
                                         ...
                                         aSpecifiedValue, aUseSVGMode, aResult);
}
...
bool StyleAnimationValue::UncomputeValue(
  ...
  switch (unit) {
    ...
    case eUnit_Enumerated:
    case eUnit_Visibility:
      aSpecifiedValue.SetIntValue(aComputedValue.GetIntValue(),
                                  eCSSUnit_Enumerated);
      break;
    case eUnit_Integer:
      aSpecifiedValue.SetIntValue(aComputedValue.GetIntValue(),
    ...
    case eUnit_Percent:
      ...
      break;
    case eUnit_Float:
      aSpecifiedValue.SetFloatValue(aComputedValue.GetFloatValue(),
                                    eCSSUnit_Number);
      break;
    case eUnit_CurrentColor:
    ...
    case eUnit_CSSValueTriplet: {
      ...
      // all properties.  At present, all animatable properties that
      // use pairs do expect collapsing.
      const nsCSSValueTriplet* triplet =
          aComputedValue.GetCSSValueTripletValue();
      if (triplet->mXValue == triplet->mYValue &&
          triplet->mYValue == triplet->mZValue) {
    ...
    case eUnit_Transform:
      aSpecifiedValue.SetSharedListValue(
          aComputedValue.GetCSSValueSharedListValue());
      break;
    case eUnit_CSSValuePairList:
      aSpecifiedValue.SetDependentPairListValue(
...
bool StyleAnimationValue::UncomputeValue(nsCSSPropertyID aProperty,
...
template <typename T>
inline const T& StyleDataAtOffset(const void* aStyleStruct, ptrdiff_t aOffset) {
  return *reinterpret_cast<const T*>(
      reinterpret_cast<const uint8_t*>(aStyleStruct) + aOffset);
}

static bool StyleCoordToValue(const nsStyleCoord& aCoord,
...
static bool StyleCoordToCSSValue(const nsStyleCoord& aCoord,
...
static void SetPositionValue(const Position& aPos, nsCSSValue& aCSSValue) {
...
static void SetPositionCoordValue(const Position::Coord& aPosCoord,
                                  nsCSSValue& aCSSValue) {
 ...
 */
static void SubstitutePixelValues(GeckoStyleContext* aStyleContext,
                                  const nsCSSValue& aInput,
                                  nsCSSValue& aOutput) {
  if (aInput.IsCalcUnit()) {
    RuleNodeCacheConditions conditions;
    nsRuleNode::ComputedCalc c = nsRuleNode::SpecifiedCalcToComputedCalc(
        aInput, aStyleContext, aStyleContext->PresContext(), conditions);
    ...
    for (size_t i = 0, i_end = inputArray->Count(); i < i_end; ++i) {
      SubstitutePixelValues(aStyleContext, inputArray->Item(i),
                            outputArray->Item(i));
    }
    aOutput.SetArrayValue(outputArray, aInput.GetUnit());
  } else if (aInput.IsLengthUnit() && aInput.GetUnit() != eCSSUnit_Pixel) {
    RuleNodeCacheConditions conditions;
    nscoord len = nsRuleNode::CalcLength(
        aInput, aStyleContext, aStyleContext->PresContext(), conditions);
...
static void ExtractImageLayerPositionXList(
    const nsStyleImageLayers& aLayer, StyleAnimationValue& aComputedValue) {
  ...
  for (uint32_t i = 0, i_end = aLayer.mPositionXCount; i != i_end; ++i) {
    ...
    *resultTail = item;
    resultTail = &item->mNext;
    SetPositionCoordValue(aLayer.mLayers[i].mPosition.mXPosition, item->mValue);
  }

  aComputedValue.SetAndAdoptCSSValueListValue(
      result.forget(), StyleAnimationValue::eUnit_BackgroundPositionCoord);
}

static void ExtractImageLayerPositionYList(
    const nsStyleImageLayers& aLayer, StyleAnimationValue& aComputedValue) {
  ...
  for (uint32_t i = 0, i_end = aLayer.mPositionYCount; i != i_end; ++i) {
    ...
    *resultTail = item;
    resultTail = &item->mNext;
    SetPositionCoordValue(aLayer.mLayers[i].mPosition.mYPosition, item->mValue);
  }
...
static void ExtractImageLayerSizePairList(const nsStyleImageLayers& aLayer,
                                          StyleAnimationValue& aComputedValue) {
    ...
    *resultTail = item;
    ...
    switch (size.mWidthType) {
      ...
      case nsStyleImageLayers::Size::eCover:
        item->mXValue.SetIntValue(size.mWidthType, eCSSUnit_Enumerated);
        break;
    ...
    switch (size.mHeightType) {
      ...
      case nsStyleImageLayers::Size::eLengthPercentage:
        ...
        if (!size.mHeight.mHasPercent &&
          ...
          MOZ_ASSERT(size.mHeight.mPercent == 0.0f, "Shouldn't have mPercent");
...
static bool StyleShapeSourceToCSSArray(const StyleShapeSource& aShapeSource,
  ...
  switch (shape->GetShapeType()) {
    ...
    case StyleBasicShapeType::Polygon: {
      functionArray = aResult->Item(0).InitFunction(
          functionName, ShapeArgumentCount(functionName));
...
static bool ExtractComputedValueFromShapeSource(
    const StyleShapeSource& aShapeSource, StyleAnimationValue& aComputedValue) {
...
static void SetFallbackValue(nsCSSValuePair* aPair,
...
bool StyleAnimationValue::ExtractComputedValue(
    nsCSSPropertyID aProperty, GeckoStyleContext* aStyleContext,
    StyleAnimationValue& aComputedValue) {
...
#undef BORDER_WIDTH_CASE
        ...
        case eCSSProperty_order: {
          ...
          aComputedValue.SetIntValue(stylePosition->mOrder, eUnit_Integer);
          break;
        }
        ...
        case eCSSProperty_border_spacing: {
          ...
          pair->mXValue.SetIntegerCoordValue(
              styleTableBorder->mBorderSpacingCol);
          pair->mYValue.SetIntegerCoordValue(
              styleTableBorder->mBorderSpacingRow);
              ...
              *resultTail = item;
              ...
              switch (coord.GetUnit()) {
                case eStyleUnit_Coord:
                  ...
                  value.SetFloatValue(nsPresContext::AppUnitsToFloatCSSPixels(
                                          coord.GetCoordValue()),
                                      eCSSUnit_Number);
                  break;
                case eStyleUnit_Factor:
                  value.SetFloatValue(coord.GetFactorValue(), eCSSUnit_Number);
                  break;
                case eStyleUnit_Percent:
            ...
            *resultTail = item;
            ...
            if (type == NS_STYLE_FILTER_URL) {
              ...
              nsCSSKeyword functionName = nsCSSProps::ValueToKeywordEnum(
                  type, nsCSSProps::kFilterFunctionKTable);
              nsCSSValue::Array* filterArray =
                  item->mValue.InitFunction(functionName, 1);
              if (type >= NS_STYLE_FILTER_BLUR &&
                  type <= NS_STYLE_FILTER_HUE_ROTATE) {
                if (!StyleCoordToCSSValue(filter.GetFilterParameter(),
                                          filterArray->Item(1))) {
                  return false;
                }
              } else if (type == NS_STYLE_FILTER_DROP_SHADOW) {
                nsCSSValueList* shadowResult =
                    filterArray->Item(1).SetListValue();
                nsAutoPtr<nsCSSValueList> tmpShadowValue;
                nsCSSValueList** tmpShadowResultTail =
                    getter_Transfers(tmpShadowValue);
        ...
        case eCSSProperty_transform: {
          const nsStyleDisplay* display =
              static_cast<const nsStyleDisplay*>(styleStruct);
          nsAutoPtr<nsCSSValueList> result;
          RefPtr<nsCSSValueSharedList> transformList =
              display->GetCombinedTransform();
          if (transformList) {
            // Clone, and convert all lengths (not percents) to pixels.
            nsCSSValueList** resultTail = getter_Transfers(result);
            for (const nsCSSValueList* l = transformList->mHead; l;
                 l = l->mNext) {
        ...
        case eCSSProperty__moz_window_transform: {
          ...
          if (uiReset->mSpecifiedWindowTransform) {
            // Clone, and convert all lengths (not percents) to pixels.
            nsCSSValueList** resultTail = getter_Transfers(result);
            for (const nsCSSValueList* l =
                     uiReset->mSpecifiedWindowTransform->mHead;
                 l; l = l->mNext) {
              nsCSSValueList* clone = new nsCSSValueList;
              *resultTail = clone;
              resultTail = &clone->mNext;
    ...
    case eStyleAnimType_Coord: {
      ...
      if (nsCSSProps::PropHasFlags(aProperty,
        ...
        aComputedValue.SetFloatValue(
            nsPresContext::AppUnitsToFloatCSSPixels(coord.GetCoordValue()));
        return true;
      }
    ...
    case eStyleAnimType_Sides_Left: {
      static_assert(
          eSideTop == eStyleAnimType_Sides_Top - eStyleAnimType_Sides_Top &&
              eSideRight ==
                  eStyleAnimType_Sides_Right - eStyleAnimType_Sides_Top &&
              eSideBottom ==
                  eStyleAnimType_Sides_Bottom - eStyleAnimType_Sides_Top &&
...
Size StyleAnimationValue::GetScaleValue(const nsIFrame* aForFrame) const {
...
StyleAnimationValue::StyleAnimationValue(nscoord aLength,
...
StyleAnimationValue& StyleAnimationValue::operator=(
    const StyleAnimationValue& aOther) {
  if (this == &aOther) {
    return *this;
  }
  ...
  switch (mUnit) {
    ...
    case eUnit_DiscreteCSSValue:
      ...
      break;
    case eUnit_CSSValuePair:
      MOZ_ASSERT(aOther.mValue.mCSSValuePair, "value pairs may not be null");
    ...
    case eUnit_CSSValueTriplet:
      ...
      mValue.mCSSValueTriplet =
          new nsCSSValueTriplet(*aOther.mValue.mCSSValueTriplet);
      break;
    case eUnit_CSSRect:
    ...
    case eUnit_BackgroundPositionCoord:
      ...
      break;
    case eUnit_Shape:
      MOZ_ASSERT(aOther.mValue.mCSSValueArray, "value arrays may not be null");
...
void StyleAnimationValue::SetNormalValue() {
  FreeValue();
  mUnit = eUnit_Normal;
}

void StyleAnimationValue::SetAutoValue() {
  FreeValue();
  mUnit = eUnit_Auto;
}

void StyleAnimationValue::SetNoneValue() {
  FreeValue();
  mUnit = eUnit_None;
}

void StyleAnimationValue::SetIntValue(int32_t aInt, Unit aUnit) {
  NS_ASSERTION(IsIntUnit(aUnit), "unit must be of integer type");
  FreeValue();
  mUnit = aUnit;
  mValue.mInt = aInt;
...
void StyleAnimationValue::SetCoordValue(nscoord aLength) {
  FreeValue();
  mUnit = eUnit_Coord;
  mValue.mCoord = aLength;
}

void StyleAnimationValue::SetPercentValue(float aPercent) {
  FreeValue();
  mUnit = eUnit_Percent;
  mValue.mFloat = aPercent;
  MOZ_ASSERT(!mozilla::IsNaN(mValue.mFloat));
...
void StyleAnimationValue::SetFloatValue(float aFloat) {
  FreeValue();
  mUnit = eUnit_Float;
  mValue.mFloat = aFloat;
  MOZ_ASSERT(!mozilla::IsNaN(mValue.mFloat));
...
void StyleAnimationValue::SetColorValue(nscolor aColor) {
  FreeValue();
  mUnit = eUnit_Color;
  mValue.mCSSValue = new nsCSSValue();
  mValue.mCSSValue->SetColorValue(aColor);
...
void StyleAnimationValue::SetCurrentColorValue() {
  FreeValue();
  mUnit = eUnit_CurrentColor;
}

void StyleAnimationValue::SetComplexColorValue(
...
void StyleAnimationValue::SetUnparsedStringValue(const nsString& aString) {
  FreeValue();
  mUnit = eUnit_UnparsedString;
  mValue.mString = nsCSSValue::BufferFromString(aString).take();
}

void StyleAnimationValue::SetAndAdoptCSSValueValue(nsCSSValue* aValue,
                                                   Unit aUnit) {
  FreeValue();
...
void StyleAnimationValue::SetAndAdoptCSSValuePairValue(
...
void StyleAnimationValue::SetAndAdoptCSSValueTripletValue(
...
void StyleAnimationValue::SetAndAdoptCSSRectValue(nsCSSRect* aRect,
                                                  Unit aUnit) {
  FreeValue();
...
void StyleAnimationValue::SetCSSValueArrayValue(nsCSSValue::Array* aValue,
                                                Unit aUnit) {
  FreeValue();
...
void StyleAnimationValue::SetAndAdoptCSSValueListValue(
    nsCSSValueList* aValueList, Unit aUnit) {
  FreeValue();
  MOZ_ASSERT(IsCSSValueListUnit(aUnit), "bad unit");
  MOZ_ASSERT(
      aUnit == eUnit_Shadow || aUnit == eUnit_Filter || aValueList != nullptr,
      "value lists other than shadows and filters may not be null");
  mUnit = aUnit;
  mValue.mCSSValueList = aValueList;  // take ownership
}

void StyleAnimationValue::SetTransformValue(nsCSSValueSharedList* aList) {
  FreeValue();
...
void StyleAnimationValue::FreeValue() {
...
bool StyleAnimationValue::operator==(const StyleAnimationValue& aOther) const {
...
bool AnimationValue::IsInterpolableWith(nsCSSPropertyID aProperty,
  ...
  return StyleAnimationValue::Interpolate(aProperty, mGecko, aToValue.mGecko,
                                          0.5, dummy);
#else
  MOZ_CRASH("old style system disabled");
#endif
...
double AnimationValue::ComputeDistance(nsCSSPropertyID aProperty,
  ...
  MOZ_ASSERT(!mServo != mGecko.IsNull());
  MOZ_ASSERT(
      mGecko.IsNull() == aOther.mGecko.IsNull() && !mServo == !aOther.mServo,
      "Animation values should have the same style engine");
#else
  MOZ_ASSERT(mServo);
...
#ifdef MOZ_OLD_STYLE
  return StyleAnimationValue::ComputeDistance(aProperty, mGecko, aOther.mGecko,
...
/* static */ AnimationValue AnimationValue::FromString(
  ...
  if (auto* servoContext = styleContext->GetAsServo()) {
    ...
    result.mServo = shell->StyleSet()->AsServo()->ComputeAnimationValue(
        aElement, declarations, servoContext);
    return result;
  }

#ifdef MOZ_OLD_STYLE
  if (!StyleAnimationValue::ComputeValue(
          aProperty, aElement, styleContext->AsGecko(), aValue,
          false /* |aUseSVGMode| */, result.mGecko)) {
...
/* static */ AnimationValue AnimationValue::Opacity(
    StyleBackendType aBackendType, float aOpacity) {
  AnimationValue result;
...
/* static */ AnimationValue AnimationValue::Transform(
    StyleBackendType aBackendType, nsCSSValueSharedList& aList) {
  AnimationValue result;
