...
#define CSS_PROP(name_, id_, method_, flags_, pref_, ...)         \
  static_assert(!((flags_)&CSS_PROPERTY_ENABLED_IN_CHROME) ||     \
                ...
                "Property '" #name_                               \
                "' is enabled in chrome, so it should "           \
...
static const char* const kCSSRawPredefinedCounterStyles[] = {
    ...
    "trad-chinese-formal", "cjk-ideographic",
...
static int SortPropertyAndCount(const void* s1, const void* s2, void* closure) {
...
// are no aliases.
enum { eCSSAliasCount = eCSSProperty_COUNT_with_aliases - eCSSProperty_COUNT };
...
void nsCSSProps::AddRefTable(void) {
  if (0 == gPropertyTableRefCount++) {
    ...
    gPropertyTable =
        CreateStaticTable(kCSSRawProperties, eCSSProperty_COUNT_with_aliases);
    ...
    gPredefinedCounterStyleTable =
        CreateStaticTable(kCSSRawPredefinedCounterStyles,
...
#define OBSERVE_PROP(pref_, id_)                                 \
  if (pref_[0]) {                                                \
    Preferences::AddBoolVarCache(&gPropertyEnabled[id_], pref_); \
  }
...
#undef OBSERVE_PROP
...
#ifdef DEBUG
    {
      ...
      static uint32_t flagsToCheck[] = {CSS_PROPERTY_ENABLED_IN_UA_SHEETS,
...
bool nsCSSProps::BuildShorthandsContainingTable() {
  uint32_t occurrenceCounts[eCSSProperty_COUNT_no_shorthands];
  memset(occurrenceCounts, 0, sizeof(occurrenceCounts));
  PropertyAndCount
      subpropCounts[eCSSProperty_COUNT - eCSSProperty_COUNT_no_shorthands];
  for (nsCSSPropertyID shorthand = eCSSProperty_COUNT_no_shorthands;
       shorthand < eCSSProperty_COUNT;
       shorthand = nsCSSPropertyID(shorthand + 1)) {
  ...
  if (!gShorthandsContainingPool) return false;
  ...
  for (const PropertyAndCount *shorthandAndCount = subpropCounts,
       ...
       shorthandAndCount < shorthandAndCountEnd; ++shorthandAndCount) {
#ifdef DEBUG_SHORTHANDS_CONTAINING
  ...
  for (nsCSSPropertyID shorthand = eCSSProperty_COUNT_no_shorthands;
    ...
    for (const nsCSSPropertyID* subprops = SubpropertyEntryFor(shorthand);
      ...
      for (const nsCSSPropertyID* shcont = ShorthandsContaining(*subprops);
           *shcont != eCSSProperty_UNKNOWN; ++shcont) {
        if (*shcont == shorthand) ++count;
      }
      MOZ_ASSERT(count == 1,
...
/* static */ bool nsCSSProps::IsCustomPropertyName(
...
nsCSSPropertyID nsCSSProps::LookupProperty(const nsACString& aProperty,
...
nsCSSPropertyID nsCSSProps::LookupProperty(const nsAString& aProperty,
                                           EnabledState aEnabled) {
  if (IsCustomPropertyName(aProperty)) {
    return eCSSPropertyExtra_variable;
  }
...
nsCSSCounterDesc nsCSSProps::LookupCounterDesc(const nsAString& aProperty) {
  MOZ_ASSERT(gCounterDescTable, "no lookup table, needs addref");
  return nsCSSCounterDesc(gCounterDescTable->Lookup(aProperty));
}

nsCSSCounterDesc nsCSSProps::LookupCounterDesc(const nsACString& aProperty) {
  MOZ_ASSERT(gCounterDescTable, "no lookup table, needs addref");
  return nsCSSCounterDesc(gCounterDescTable->Lookup(aProperty));
}

bool nsCSSProps::IsPredefinedCounterStyle(const nsAString& aStyle) {
  MOZ_ASSERT(gPredefinedCounterStyleTable, "no lookup table, needs addref");
  return gPredefinedCounterStyleTable->Lookup(aStyle) !=
         nsStaticCaseInsensitiveNameTable::NOT_FOUND;
}

bool nsCSSProps::IsPredefinedCounterStyle(const nsACString& aStyle) {
  MOZ_ASSERT(gPredefinedCounterStyleTable, "no lookup table, needs addref");
  return gPredefinedCounterStyleTable->Lookup(aStyle) !=
         nsStaticCaseInsensitiveNameTable::NOT_FOUND;
}

const nsCString& nsCSSProps::GetStringValue(nsCSSPropertyID aProperty) {
  MOZ_ASSERT(gPropertyTable, "no lookup table, needs addref");
  if (gPropertyTable) {
...
const char* const* nsCSSProps::GetListStyleTypes(int32_t* aLength) {
...
const KTableEntry nsCSSProps::kAnimationDirectionKTable[] = {
    ...
    {eCSSKeyword_reverse,
     static_cast<uint32_t>(dom::PlaybackDirection::Reverse)},
    {eCSSKeyword_alternate,
     static_cast<uint32_t>(dom::PlaybackDirection::Alternate)},
    {eCSSKeyword_alternate_reverse,
     static_cast<uint32_t>(dom::PlaybackDirection::Alternate_reverse)},
...
const KTableEntry nsCSSProps::kColorKTable[] = {
    ...
    {eCSSKeyword_inactivecaptiontext,
     LookAndFeel::eColorID_inactivecaptiontext},
    ...
    {eCSSKeyword__moz_dragtargetzone,
     LookAndFeel::eColorID__moz_dragtargetzone},
    {eCSSKeyword__moz_gtk_info_bar_text,
     LookAndFeel::eColorID__moz_gtk_info_bar_text},
    {eCSSKeyword__moz_hyperlinktext, NS_COLOR_MOZ_HYPERLINKTEXT},
    {eCSSKeyword__moz_html_cellhighlight,
     LookAndFeel::eColorID__moz_html_cellhighlight},
    {eCSSKeyword__moz_html_cellhighlighttext,
     LookAndFeel::eColorID__moz_html_cellhighlighttext},
    {eCSSKeyword__moz_mac_buttonactivetext,
     LookAndFeel::eColorID__moz_mac_buttonactivetext},
    {eCSSKeyword__moz_mac_chrome_active,
     LookAndFeel::eColorID__moz_mac_chrome_active},
    {eCSSKeyword__moz_mac_chrome_inactive,
     LookAndFeel::eColorID__moz_mac_chrome_inactive},
    {eCSSKeyword__moz_mac_defaultbuttontext,
     LookAndFeel::eColorID__moz_mac_defaultbuttontext},
    {eCSSKeyword__moz_mac_focusring, LookAndFeel::eColorID__moz_mac_focusring},
    {eCSSKeyword__moz_mac_menuselect,
     LookAndFeel::eColorID__moz_mac_menuselect},
    {eCSSKeyword__moz_mac_menushadow,
     LookAndFeel::eColorID__moz_mac_menushadow},
    {eCSSKeyword__moz_mac_menutextdisable,
     LookAndFeel::eColorID__moz_mac_menutextdisable},
    {eCSSKeyword__moz_mac_menutextselect,
     LookAndFeel::eColorID__moz_mac_menutextselect},
    {eCSSKeyword__moz_mac_disabledtoolbartext,
     LookAndFeel::eColorID__moz_mac_disabledtoolbartext},
    {eCSSKeyword__moz_mac_secondaryhighlight,
     LookAndFeel::eColorID__moz_mac_secondaryhighlight},
    {eCSSKeyword__moz_mac_vibrancy_light,
     LookAndFeel::eColorID__moz_mac_vibrancy_light},
    {eCSSKeyword__moz_mac_vibrancy_dark,
     LookAndFeel::eColorID__moz_mac_vibrancy_dark},
    {eCSSKeyword__moz_mac_vibrant_titlebar_light,
     LookAndFeel::eColorID__moz_mac_vibrant_titlebar_light},
    {eCSSKeyword__moz_mac_vibrant_titlebar_dark,
     LookAndFeel::eColorID__moz_mac_vibrant_titlebar_dark},
    {eCSSKeyword__moz_mac_menuitem, LookAndFeel::eColorID__moz_mac_menuitem},
    {eCSSKeyword__moz_mac_active_menuitem,
     LookAndFeel::eColorID__moz_mac_active_menuitem},
    {eCSSKeyword__moz_mac_menupopup, LookAndFeel::eColorID__moz_mac_menupopup},
    {eCSSKeyword__moz_mac_source_list,
     LookAndFeel::eColorID__moz_mac_source_list},
    {eCSSKeyword__moz_mac_source_list_selection,
     LookAndFeel::eColorID__moz_mac_source_list_selection},
    {eCSSKeyword__moz_mac_active_source_list_selection,
     LookAndFeel::eColorID__moz_mac_active_source_list_selection},
    ...
    {eCSSKeyword__moz_menubarhovertext,
     LookAndFeel::eColorID__moz_menubarhovertext},
...
const KTableEntry nsCSSProps::kAlignLegacy[] = {
    {eCSSKeyword_legacy, NS_STYLE_ALIGN_LEGACY}, {eCSSKeyword_UNKNOWN, -1}};
...
const KTableEntry nsCSSProps::kAlignAutoNormalStretchBaseline[] = {
    ...
    {eCSSKeyword_stretch, NS_STYLE_ALIGN_STRETCH},
    {eCSSKeyword_baseline, NS_STYLE_ALIGN_BASELINE},
    // Also "first baseline" & "last baseline"; see
    // CSSParserImpl::ParseAlignEnum
    {eCSSKeyword_UNKNOWN, -1}};
...
const KTableEntry nsCSSProps::kTextCombineUprightKTable[] = {
    ...
    {eCSSKeyword_digits,
     NS_STYLE_TEXT_COMBINE_UPRIGHT_DIGITS_2},  // w/o number ==> 2
    {eCSSKeyword_UNKNOWN, -1}};
...
const nsCString& nsCSSProps::LookupPropertyValue(nsCSSPropertyID aProp,
                                                 int32_t aValue) {
  MOZ_ASSERT(aProp >= 0 && aProp < eCSSProperty_COUNT, "property out of range");
#ifdef DEBUG
  ...
  if (kwtable) return ValueToKeyword(aValue, kwtable);
...
bool nsCSSProps::GetColorName(int32_t aPropValue, nsCString& aStr) {
  ...
  // first get the keyword corresponding to the property Value from the color
  // table
...
static const nsCSSPropertyID gAnimationSubpropTable[] = {
    ...
    eCSSProperty_animation_name, eCSSProperty_UNKNOWN};
...
static const nsCSSPropertyID gBorderRadiusSubpropTable[] = {
    ...
    eCSSProperty_border_bottom_left_radius, eCSSProperty_UNKNOWN};
...
static const nsCSSPropertyID gBackgroundSubpropTable[] = {
    eCSSProperty_background_color,      eCSSProperty_background_image,
    eCSSProperty_background_repeat,     eCSSProperty_background_attachment,
    eCSSProperty_background_clip,       eCSSProperty_background_origin,
    eCSSProperty_background_position_x, eCSSProperty_background_position_y,
    eCSSProperty_background_size,       eCSSProperty_UNKNOWN};
...
static const nsCSSPropertyID gBorderSubpropTable[] = {
    ...
    eCSSProperty_border_image_source, eCSSProperty_border_image_slice,
    eCSSProperty_border_image_width,  eCSSProperty_border_image_outset,
    eCSSProperty_border_image_repeat, eCSSProperty_UNKNOWN};
...
static const nsCSSPropertyID gBorderBlockEndSubpropTable[] = {
    ...
    eCSSProperty_border_block_end_color, eCSSProperty_UNKNOWN};
...
static const nsCSSPropertyID gBorderInlineStartSubpropTable[] = {
    ...
    eCSSProperty_border_inline_start_color, eCSSProperty_UNKNOWN};
...
static const nsCSSPropertyID gBorderTopSubpropTable[] = {
    // Declaration.cpp outputs the subproperties in this order.
    // It also depends on the color being third.
    eCSSProperty_border_top_width, eCSSProperty_border_top_style,
    eCSSProperty_border_top_color, eCSSProperty_UNKNOWN};
...
static const nsCSSPropertyID gListStyleSubpropTable[] = {
    ...
    eCSSProperty_list_style_position, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gMarginSubpropTable[] = {
    // Code relies on these being in top-right-bottom-left order.
    eCSSProperty_margin_top, eCSSProperty_margin_right,
    eCSSProperty_margin_bottom, eCSSProperty_margin_left, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gOutlineSubpropTable[] = {
    // nsCSSDeclaration.cpp outputs the subproperties in this order.
    // It also depends on the color being third.
    eCSSProperty_outline_width, eCSSProperty_outline_style,
    eCSSProperty_outline_color, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gColumnsSubpropTable[] = {
    eCSSProperty_column_count, eCSSProperty_column_width, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gColumnRuleSubpropTable[] = {
    // nsCSSDeclaration.cpp outputs the subproperties in this order.
    // It also depends on the color being third.
    eCSSProperty_column_rule_width, eCSSProperty_column_rule_style,
    eCSSProperty_column_rule_color, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gFlexSubpropTable[] = {
    eCSSProperty_flex_grow, eCSSProperty_flex_shrink, eCSSProperty_flex_basis,
    eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gFlexFlowSubpropTable[] = {
    eCSSProperty_flex_direction, eCSSProperty_flex_wrap, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gGridTemplateSubpropTable[] = {
    eCSSProperty_grid_template_areas, eCSSProperty_grid_template_rows,
    eCSSProperty_grid_template_columns, eCSSProperty_UNKNOWN};
...
static const nsCSSPropertyID gGridColumnSubpropTable[] = {
    eCSSProperty_grid_column_start, eCSSProperty_grid_column_end,
    eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gGridRowSubpropTable[] = {
    eCSSProperty_grid_row_start, eCSSProperty_grid_row_end,
    eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gGridAreaSubpropTable[] = {
    eCSSProperty_grid_row_start, eCSSProperty_grid_column_start,
    eCSSProperty_grid_row_end, eCSSProperty_grid_column_end,
    eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gGridGapSubpropTable[] = {
    eCSSProperty_grid_row_gap, eCSSProperty_grid_column_gap,
    eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gOverflowSubpropTable[] = {
    eCSSProperty_overflow_x, eCSSProperty_overflow_y, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gOverflowClipBoxSubpropTable[] = {
    eCSSProperty_overflow_clip_box_block, eCSSProperty_overflow_clip_box_inline,
    eCSSProperty_UNKNOWN};
...
static const nsCSSPropertyID gTextDecorationSubpropTable[] = {
    eCSSProperty_text_decoration_color, eCSSProperty_text_decoration_line,
    eCSSProperty_text_decoration_style, eCSSProperty_UNKNOWN};
...
static const nsCSSPropertyID gWebkitTextStrokeSubpropTable[] = {
    ...
    eCSSProperty__webkit_text_stroke_color, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gTransitionSubpropTable[] = {
    eCSSProperty_transition_property, eCSSProperty_transition_duration,
    eCSSProperty_transition_timing_function, eCSSProperty_transition_delay,
    eCSSProperty_UNKNOWN};
...
static const nsCSSPropertyID gMarkerSubpropTable[] = {
    eCSSProperty_marker_start, eCSSProperty_marker_mid, eCSSProperty_marker_end,
    eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gPlaceContentSubpropTable[] = {
    eCSSProperty_align_content, eCSSProperty_justify_content,
    eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gPlaceItemsSubpropTable[] = {
    eCSSProperty_align_items, eCSSProperty_justify_items, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gPlaceSelfSubpropTable[] = {
    eCSSProperty_align_self, eCSSProperty_justify_self, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gOverscrollBehaviorSubpropTable[] = {
    eCSSProperty_overscroll_behavior_x, eCSSProperty_overscroll_behavior_y,
    eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gScrollSnapTypeSubpropTable[] = {
    eCSSProperty_scroll_snap_type_x, eCSSProperty_scroll_snap_type_y,
    eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gMaskSubpropTable[] = {
    eCSSProperty_mask_image,      eCSSProperty_mask_repeat,
    eCSSProperty_mask_position_x, eCSSProperty_mask_position_y,
    eCSSProperty_mask_clip,       eCSSProperty_mask_origin,
    eCSSProperty_mask_size,       eCSSProperty_mask_composite,
    eCSSProperty_mask_mode,       eCSSProperty_UNKNOWN};
...
const nsCSSPropertyID* const
    nsCSSProps::kSubpropertyTable[eCSSProperty_COUNT -
...
static const nsCSSPropertyID gOffsetLogicalGroupTable[] = {
    eCSSProperty_top, eCSSProperty_right, eCSSProperty_bottom,
    eCSSProperty_left, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gMaxSizeLogicalGroupTable[] = {
    eCSSProperty_max_height, eCSSProperty_max_width, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gMinSizeLogicalGroupTable[] = {
    eCSSProperty_min_height, eCSSProperty_min_width, eCSSProperty_UNKNOWN};

static const nsCSSPropertyID gSizeLogicalGroupTable[] = {
    eCSSProperty_height, eCSSProperty_width, eCSSProperty_UNKNOWN};
...
#define CSS_PROP_LOGICAL(name_, id_, method_, flags_, pref_, parsevariant_,  \
                         kwtable_, group_, stylestruct_, stylestructoffset_, \
                         animtype_)                                          \
...
/* static */ const nsCSSPropertyID* nsCSSProps::LogicalGroup(
...
#define STYLE_STRUCT(name, checkdata_cb) ePropertyCount_for_##name,
...
#define IS_ENABLED_BY_DEFAULT(flags_) (!((flags_)&CSS_PROPERTY_ENABLED_MASK))
...
#define CSS_PROP_ALIAS(aliasname_, aliasid_, propid_, aliasmethod_, pref_) true,
#include "nsCSSPropAliasList.h"
#undef CSS_PROP_ALIAS
