...
public:
  ...
  virtual void ScrollBy(nsIntPoint aDelta, ScrollUnit aUnit, ScrollMode aMode,
                        ...
                        ScrollMomentum aIsMomentum = NOT_MOMENTUM,
                        ScrollSnapMode aSnap = DISABLE_SNAP) = 0;
   ...
   * snap points.
   * The value of FrameMetrics::mFlingSnapGeneration for the scroll frame is
   * passed into aFlingSnapGeneration to identify
   * @param aDestination The desired landing position of the fling, which
   * is used to select the best matching snap point.
   * @param aFlingSnapGeneration A unique integer identifying the fling snap
   * request; used to detect if the fling snap request in FrameMetrics has
   * been processed and to prevent multiple FlingSnaps from occuring within a
   * single scroll generation.
   */
  ...
  */
  ...
  /**
   * Returns the current generation counter for the fling snap.  This counter
   * increments every time the APZC triggers a fling snap.
   */
  virtual uint32_t CurrentFlingSnapGeneration() = 0;
  /**
