...
NS_IMETHODIMP TreeBoxObject::GetHorizontalPosition(
    int32_t* aHorizontalPosition) {
...
NS_IMETHODIMP TreeBoxObject::GetSelectionRegion(nsIScriptableRegion** aRegion) {
  *aRegion = nullptr;
  nsTreeBodyFrame* body = GetTreeBodyFrame();
  if (body) return body->GetSelectionRegion(aRegion);
  return NS_OK;
}

already_AddRefed<nsIScriptableRegion> TreeBoxObject::SelectionRegion() {
...
TreeBoxObject::EnsureCellIsVisible(int32_t aRow, nsITreeColumn* aCol) {
  nsTreeBodyFrame* body = GetTreeBodyFrame();
  if (body) return body->EnsureCellIsVisible(aRow, aCol);
  return NS_OK;
}
...
TreeBoxObject::ScrollToRow(int32_t aRow) {
  ...
  if (body) return body->ScrollToRow(aRow);
  return NS_OK;
}
...
TreeBoxObject::ScrollByLines(int32_t aNumLines) {
  nsTreeBodyFrame* body = GetTreeBodyFrame();
  if (body) return body->ScrollByLines(aNumLines);
  return NS_OK;
}
...
TreeBoxObject::ScrollByPages(int32_t aNumPages) {
  nsTreeBodyFrame* body = GetTreeBodyFrame();
  if (body) return body->ScrollByPages(aNumPages);
  return NS_OK;
}
...
TreeBoxObject::ScrollToCell(int32_t aRow, nsITreeColumn* aCol) {
  nsTreeBodyFrame* body = GetTreeBodyFrame();
  if (body) return body->ScrollToCell(aRow, aCol);
  return NS_OK;
}
...
TreeBoxObject::ScrollToColumn(nsITreeColumn* aCol) {
  nsTreeBodyFrame* body = GetTreeBodyFrame();
  if (body) return body->ScrollToColumn(aCol);
  return NS_OK;
}
...
TreeBoxObject::ScrollToHorizontalPosition(int32_t aHorizontalPosition) {
  nsTreeBodyFrame* body = GetTreeBodyFrame();
  if (body) return body->ScrollToHorizontalPosition(aHorizontalPosition);
  return NS_OK;
}
...
NS_IMETHODIMP
TreeBoxObject::InvalidateColumnRange(int32_t aStart, int32_t aEnd,
                                     nsITreeColumn* aCol) {
  nsTreeBodyFrame* body = GetTreeBodyFrame();
  if (body) return body->InvalidateColumnRange(aStart, aEnd, aCol);
  return NS_OK;
}
...
TreeBoxObject::GetRowAt(int32_t x, int32_t y, int32_t* aRow) {
  *aRow = 0;
  nsTreeBodyFrame* body = GetTreeBodyFrame();
  if (body) return body->GetRowAt(x, y, aRow);
  return NS_OK;
}
...
NS_IMETHODIMP
TreeBoxObject::GetCoordsForCellItem(int32_t aRow, nsITreeColumn* aCol,
...
NS_IMETHODIMP
TreeBoxObject::IsCellCropped(int32_t aRow, nsITreeColumn* aCol,
                             bool* aIsCropped) {
  *aIsCropped = false;
  nsTreeBodyFrame* body = GetTreeBodyFrame();
...
bool TreeBoxObject::IsCellCropped(int32_t row, nsITreeColumn* col,
                                  ErrorResult& aRv) {
  bool ret;
...
void TreeBoxObject::RemoveImageCacheEntry(int32_t row, nsITreeColumn& col,
                                          ErrorResult& aRv) {
