...
// Note: This file has been backported from trunk to this branch.  In
// particular, we were interested in getting the fix to bug 810192 for DMD.
...
#   define METER(x)
  ...
  private:
    ...
    // Resize the table down to the next-smallest power of 2 larger than
    // entryCount.  Since we call checkUnderloaded() on every remove, you only
    // need to call this after a bulk removal of items done without calling
    // remove().
    void compactIfUnderloaded()
    {
        int32_t resizeLog2 = 0;
        uint32_t newCapacity = capacity();
        while (newCapacity > sMinSize &&
               entryCount <= ((sMinAlphaFrac * newCapacity) >> 8)) {
