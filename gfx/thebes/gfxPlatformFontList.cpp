...
gfxFontListPrefObserver::Observe(nsISupports* aSubject, const char* aTopic,
  ...
  NS_ASSERTION(!strcmp(aTopic, NS_PREFBRANCH_PREFCHANGE_TOPIC_ID) ||
                   !strcmp(aTopic, LOCALES_CHANGED_TOPIC),
               "invalid topic");
...
bool gfxPlatformFontList::AddWithLegacyFamilyName(const nsAString& aLegacyName,
...
void gfxPlatformFontList::GenerateFontListKey(const nsAString& aKeyName,
                                              nsAString& aResult) {
  aResult = aKeyName;
  ToLowerCase(aResult);
}
...
gfxFontEntry* gfxPlatformFontList::FindFaceName(const nsAString& aFaceName) {
  gfxFontEntry* lookup;
...
gfxFontEntry* gfxPlatformFontList::SystemFindFontForChar(
  ...
  if (aCh == 0xFFFD && mReplacementCharFallbackFamily) {
    ...
    fontEntry =
        mReplacementCharFallbackFamily->FindFontForStyle(*aStyle, needsBold);
...
bool gfxPlatformFontList::FindAndAddFamilies(const nsAString& aFamily,
...
gfxFontEntry* gfxPlatformFontList::FindFontForFamily(const nsAString& aFamily,
                                                     const gfxFontStyle* aStyle,
                                                     bool& aNeedsBold) {
  ...
  if (familyEntry) return familyEntry->FindFontForStyle(*aStyle, aNeedsBold);
...
bool gfxPlatformFontList::GetStandardFamilyName(const nsAString& aFontName,
...
void gfxPlatformFontList::GetFontFamiliesFromGenericFamilies(
    nsTArray<nsString>& aGenericNameFamilies, nsAtom* aLangGroup,
    nsTArray<RefPtr<gfxFontFamily>>* aGenericFamilies) {
...
void gfxPlatformFontList::GetFontFamilyNames(
    nsTArray<nsString>& aFontFamilyNames) {
  for (auto iter = mFontFamilies.Iter(); !iter.Done(); iter.Next()) {
    RefPtr<gfxFontFamily>& family = iter.Data();
