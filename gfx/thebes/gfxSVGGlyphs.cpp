...
nsresult gfxSVGGlyphsDocument::SetupPresentation() {
  ...
  nsresult rv = catMan->GetCategoryEntry(
      "Gecko-Content-Viewers", "image/svg+xml", getter_Copies(contractId));
...
nsresult gfxSVGGlyphsDocument::ParseDocument(const uint8_t *aBuffer,
  ...
  nsCOMPtr<nsIURI> uri;
  nsHostObjectProtocolHandler::GenerateURIString(
      NS_LITERAL_CSTRING(FONTTABLEURI_SCHEME), nullptr, mSVGGlyphsDocumentURI);
