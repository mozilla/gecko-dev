...
CrossProcessCompositorBridgeParent::AllocPLayerTransactionParent(
    const nsTArray<LayersBackend>&, const uint64_t& aId) {
  MOZ_ASSERT(aId != 0);
  ...
  if (state && state->mLayerManager) {
    ...
    LayerTransactionParent* p =
        new LayerTransactionParent(lm, this, animStorage, aId);
    p->AddIPDLReference();
...
CrossProcessCompositorBridgeParent::AllocPWebRenderBridgeParent(
  ...
  parent = new WebRenderBridgeParent(this, aPipelineId, nullptr,
                                     root->CompositorScheduler(), Move(api),
                                     Move(holder), Move(animStorage));
  parent->AddRef();  // IPDL reference
...
void CrossProcessCompositorBridgeParent::ShadowLayersUpdated(
  ...
  if (aLayerTree->ShouldParentObserveEpoch()) {
    ...
    Unused << state->mParent->SendObserveLayerUpdate(
        id, aLayerTree->GetChildEpoch(), true);
  }

  aLayerTree->SetPendingTransactionId(aInfo.id(), aInfo.transactionStart(),
                                      aInfo.fwdTime());
}

void CrossProcessCompositorBridgeParent::DidComposite(
    uint64_t aId, TimeStamp& aCompositeStart, TimeStamp& aCompositeEnd) {
  MonitorAutoLock lock(*sIndirectLayerTreesLock);
...
void CrossProcessCompositorBridgeParent::DidCompositeLocked(
  ...
  } else if (WebRenderBridgeParent* wrbridge =
                 sIndirectLayerTrees[aId].mWrBridge) {
...
bool CrossProcessCompositorBridgeParent::SetTestSampleTime(
    const uint64_t& aId, const TimeStamp& aTime) {
  MOZ_ASSERT(aId != 0);
  const CompositorBridgeParent::LayerTreeState* state =
...
void CrossProcessCompositorBridgeParent::LeaveTestMode(const uint64_t& aId) {
  MOZ_ASSERT(aId != 0);
  const CompositorBridgeParent::LayerTreeState* state =
      CompositorBridgeParent::GetIndirectShadowTree(aId);
  if (!state) {
...
void CrossProcessCompositorBridgeParent::GetAPZTestData(
    const uint64_t& aLayersId, APZTestData* aOutData) {
  MOZ_ASSERT(aLayersId != 0);
  const CompositorBridgeParent::LayerTreeState* state =
      CompositorBridgeParent::GetIndirectShadowTree(aLayersId);
  if (!state || !state->mParent) {
...
mozilla::ipc::IPCResult
CrossProcessCompositorBridgeParent::RecvClearApproximatelyVisibleRegions(
...
mozilla::ipc::IPCResult
CrossProcessCompositorBridgeParent::RecvNotifyApproximatelyVisibleRegion(
...
void CrossProcessCompositorBridgeParent::ObserveLayerUpdate(uint64_t aLayersId,
                                                            uint64_t aEpoch,
                                                            bool aActive) {
  MOZ_ASSERT(aLayersId != 0);
