  ...
  while (nsDisplayItem* i = iter.GetNext()) {
    ...
    if (apzEnabled) {
      ...
      if (forceNewLayerData && !eventRegions.IsEmpty()) {
        ...
        // into |eventRegions|. This is because any ASR change causes us to
        // force a new layer data which flushes the eventRegions.
        MOZ_ASSERT(!mLayerScrollData.empty());
      ...
      if (itemType == DisplayItemType::TYPE_LAYER_EVENT_REGIONS) {
        ...
        EventRegions regions(
            regionsItem->HitRegion().ScaleToOutsidePixels(1.0f, 1.0f,
                                                          auPerDevPixel),
            regionsItem->MaybeHitRegion().ScaleToOutsidePixels(1.0f, 1.0f,
                                                               auPerDevPixel),
            regionsItem->DispatchToContentHitRegion().ScaleToOutsidePixels(
                1.0f, 1.0f, auPerDevPixel),
            regionsItem->NoActionRegion().ScaleToOutsidePixels(1.0f, 1.0f,
                                                               auPerDevPixel),
...
Maybe<wr::ImageKey> WebRenderCommandBuilder::CreateImageKey(
  ...
  if (aContainer->IsAsync()) {
    ...
    imageData->CreateAsyncImageWebRenderCommands(
        aBuilder, aContainer, aSc, rect, scBounds, gfx::Matrix4x4(),
        scaleToSize, wr::ImageRendering::Auto, wr::MixBlendMode::Normal,
...
bool WebRenderCommandBuilder::PushImage(
  ...
  gfx::IntSize size;
  Maybe<wr::ImageKey> key = CreateImageKey(aItem, aContainer, aBuilder,
                                           aResources, aSc, size, Some(aRect));
  if (aContainer->IsAsync()) {
  ...
  aBuilder.PushImage(r, r, !aItem->BackfaceIsHidden(),
...
WebRenderCommandBuilder::GenerateFallbackData(
  ...
  // e.g.: nsDisplayBoxShadowInner uses mVisibleRegion in Paint() and
  // mVisibleRegion is computed in nsDisplayBoxShadowInner::ComputeVisibility().
  ...
  // nsDisplayFilter is rendered via BasicLayerManager which means the
  ...
  if (needPaint || !fallbackData->GetKey()) {
    ...
    if (useBlobImage) {
          ...
          MakeAndAddRef<gfx::DrawEventRecorderMemory>(
              [&](MemStream& aStream,
                ...
                for (auto unscaled : aUnscaledFonts) {
                  wr::FontKey key =
                      mManager->WrBridge()->GetFontKeyForUnscaledFont(unscaled);
      ...
      bool isInvalidated = PaintItemByDrawTarget(
          aItem, dt, paintRect, offset, aDisplayListBuilder,
