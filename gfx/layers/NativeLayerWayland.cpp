...
void NativeLayerRootWayland::Init() {
  ...
  // Don't take reference to NativeLayerRootWayland for
  // callbacks.
  // We clear all callbacks on WaylandSurface::Unmap() and
  // keep WaylandSurface live till ~NativeLayerRootWayland.

  // Unmap all layers if nsWindow is unmapped
  WaylandSurfaceLock lock(mSurface);
  mSurface->SetUnmapCallbackLocked(lock, [this]() -> void {
  ...
  mSurface->SetGdkCommitCallbackLocked(lock, [this]() -> void {
