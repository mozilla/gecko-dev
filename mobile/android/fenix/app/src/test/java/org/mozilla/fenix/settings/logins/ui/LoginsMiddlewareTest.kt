...
class LoginsMiddlewareTest {
    ...
    @Test
    fun `GIVEN current screen is details WHEN copy username is clicked THEN show corresponding toast`() =
        runTestOnMain {
            `when`(loginsStorage.list()).thenReturn(loginList)
            var toastShowed = false
            showUsernameCopiedSnackbar = { toastShowed = true }
            val middleware = buildMiddleware()
            val store = middleware.makeStore()
            ...
            val loginItem = LoginItem(
                ...
                timeLastUsed = loginList[2].timeLastUsed,
            )
            store.dispatch(LoginClicked(loginItem))
            store.dispatch(DetailLoginAction.CopyUsernameClicked(loginList[2].username))
            store.waitUntilIdle()

            assertTrue(toastShowed)
        }

    @Test
    fun `GIVEN current screen is details WHEN copy password is clicked THEN show corresponding toast`() =
        runTestOnMain {
            `when`(loginsStorage.list()).thenReturn(loginList)
            var toastShowed = false
            showPasswordCopiedSnackbar = { toastShowed = true }
            val middleware = buildMiddleware()
            val store = middleware.makeStore()
            ...
            store.dispatch(LoginClicked(loginItem))
            store.dispatch(DetailLoginAction.CopyPasswordClicked(loginList[2].username))
            store.waitUntilIdle()
