...
class LoginsMiddlewareTest {
    ...
    @Test
    fun `GIVEN logins in storage WHEN store is initialized THEN logins will be loaded as display format`() = runTestOnMain {
    ...
    @Test
    fun `GIVEN a logins store WHEN SortMenuItem is clicked THEN Save the new sort order`() = runTestOnMain {
        `when`(loginsStorage.list()).thenReturn(loginList)
