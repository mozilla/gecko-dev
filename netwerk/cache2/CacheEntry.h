...
class CacheEntryHandle final : public nsICacheEntry {
 public:
  ...
  NS_IMETHOD OpenOutputStream(int64_t offset,
                              nsIOutputStream **_retval) override {
    return mEntry->OpenOutputStream(offset, _retval);
  }
  NS_IMETHOD GetPredictedDataSize(int64_t *aPredictedDataSize) override {
    return mEntry->GetPredictedDataSize(aPredictedDataSize);
  }
  NS_IMETHOD SetPredictedDataSize(int64_t aPredictedDataSize) override {
    return mEntry->SetPredictedDataSize(aPredictedDataSize);
  }
  ...
  NS_IMETHOD OpenAlternativeOutputStream(const nsACString &type,
                                         nsIOutputStream **_retval) override {
    return mEntry->OpenAlternativeOutputStream(type, _retval);
  }
