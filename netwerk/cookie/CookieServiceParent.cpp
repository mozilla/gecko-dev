...
void CookieServiceParent::TrackCookieLoad(nsIChannel* aChannel) {
  ...
  mCookieService->GetCookiesForURI(uri, isForeign, isSafeTopLevelNav,
...
mozilla::ipc::IPCResult CookieServiceParent::RecvPrepareCookieList(
  ...
  // Send matching cookies to Child.
  nsTArray<nsCookie*> foundCookieList;
  mCookieService->GetCookiesForURI(hostURI, aIsForeign, aIsSafeTopLevelNav,
...
mozilla::ipc::IPCResult CookieServiceParent::RecvGetCookieString(
    const URIParams& aHost, const bool& aIsForeign,
    const bool& aIsSafeTopLevelNav, const bool& aIsSameSiteForeign,
    const OriginAttributes& aAttrs, nsCString* aResult) {
  ...
  mCookieService->GetCookieStringInternal(
      hostURI, aIsForeign, aIsSafeTopLevelNav, aIsSameSiteForeign, false,
      aAttrs, *aResult);
  return IPC_OK();
}
...
mozilla::ipc::IPCResult CookieServiceParent::RecvSetCookieString(
  ...
  if (!channelURI) return IPC_FAIL_NO_REASON(this);
  ...
  nsCOMPtr<nsIChannel> dummyChannel;
  CreateDummyChannel(hostURI, channelURI, const_cast<OriginAttributes&>(aAttrs),
