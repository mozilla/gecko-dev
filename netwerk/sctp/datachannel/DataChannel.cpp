...
#ifdef MOZ_PEERCONNECTION
void DataChannelConnection::SetEvenOdd() {
...
bool DataChannelConnection::ConnectViaTransportFlow(TransportFlow *aFlow,
                                                    uint16_t localport,
                                                    uint16_t remoteport) {
  LOG(("Connect DTLS local %u, remote %u", localport, remoteport));

  NS_PRECONDITION(mMasterSocket,
                  "SCTP wasn't initialized before ConnectViaTransportFlow!");
  if (NS_WARN_IF(!aFlow)) {
    return false;
  }
...
void DataChannelConnection::CompleteConnect(TransportFlow *flow,
  ...
  if (state != TransportLayer::TS_OPEN || !mMasterSocket) return;
  ...
  if (r < 0) {
    ...
    if (r < 0) {
      ...
      // We set Even/Odd and fire ON_CONNECTION via SCTP_COMM_UP when we get
      // that This also avoids issues with calling TransportFlow stuff on
      // Mainthread
      return;
    }
  }
...
void DataChannelConnection::SctpDtlsInput(TransportFlow *flow,
                                          const unsigned char *data,
                                          size_t len) {
  if (MOZ_LOG_TEST(gSCTPLog, LogLevel::Debug)) {
    char *buf;

    if ((buf = usrsctp_dumppacket((void *)data, len, SCTP_DUMP_INBOUND)) !=
        nullptr) {
      SCTP_LOG(("%s", buf));
...
int DataChannelConnection::SendPacket(unsigned char data[], size_t len,
                                      bool release) {
...
int DataChannelConnection::SctpDtlsOutput(void *addr, void *buffer,
  ...
  if ((false /*peer->IsSTSThread()*/)) {
    ...
    peer->mSTS->Dispatch(
        WrapRunnable(RefPtr<DataChannelConnection>(peer),
