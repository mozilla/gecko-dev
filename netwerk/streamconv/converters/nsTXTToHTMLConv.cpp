...
// nsISupports methods
NS_IMPL_ISUPPORTS(nsTXTToHTMLConv, nsIStreamConverter, nsITXTToHTMLConv,
                  nsIRequestObserver, nsIStreamListener)

// nsIStreamConverter methods
NS_IMETHODIMP
nsTXTToHTMLConv::Convert(nsIInputStream *aFromStream, const char *aFromType,
...
NS_IMETHODIMP
nsTXTToHTMLConv::AsyncConvertData(const char *aFromType, const char *aToType,
...
nsTXTToHTMLConv::OnStartRequest(nsIRequest *request, nsISupports *aContext) {
  ...
  if (channel) channel->SetContentType(NS_LITERAL_CSTRING("text/html"));
  ...
  rv = mListener->OnDataAvailable(request, aContext, inputData, 0,
                                  mBuffer.Length());
  if (NS_FAILED(rv)) return rv;
  mBuffer.Truncate();
  return rv;
...
nsTXTToHTMLConv::OnDataAvailable(nsIRequest *request, nsISupports *aContext,
                                 nsIInputStream *aInStream, uint64_t aOffset,
                                 uint32_t aCount) {
  nsresult rv = NS_OK;
  nsString pushBuffer;
  uint32_t amtRead = 0;
...
nsTXTToHTMLConv::~nsTXTToHTMLConv() { mTokens.Clear(); }

nsresult nsTXTToHTMLConv::Init() {
  nsresult rv = NS_OK;
  ...
  if (!token) return NS_ERROR_OUT_OF_MEMORY;
  token->prepend = true;
  token->token.AssignLiteral(
      "http://");  // XXX need to iterate through all protos
  mTokens.AppendElement(token);
...
int32_t nsTXTToHTMLConv::FindToken(int32_t cursor, convToken **_retval) {
...
int32_t nsTXTToHTMLConv::CatHTML(int32_t front, int32_t back) {
  int32_t cursor = 0;
  ...
  if (!mToken->prepend) {
    ...
    nsCString escaped;
    if (NS_EscapeURL(linkTextUTF8.Data(), linkTextUTF8.Length(), esc_Minimal,
                     escaped)) {
