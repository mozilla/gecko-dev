...
void nsHttpChannelAuthProvider::InitializePrefs() {
  MOZ_ASSERT(NS_IsMainThread());
  mozilla::Preferences::AddUintVarCache(
      &sAuthAllowPref, "network.auth.subresource-http-auth-allow",
      SUBRESOURCE_AUTH_DIALOG_ALLOW_ALL);
...
bool nsHttpChannelAuthProvider::BlockPrompt(bool proxyAuth) {
  ...
  switch (sAuthAllowPref) {
    ...
    case SUBRESOURCE_AUTH_DIALOG_ALLOW_ALL:
      ...
      // blocked.
      if (mCrossOrigin && !sImgCrossOriginAuthAllowPref && loadInfo &&
          ((loadInfo->GetExternalContentPolicyType() ==
