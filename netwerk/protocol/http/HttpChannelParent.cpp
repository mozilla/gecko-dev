...
struct UploadStreamClosure {
  ...
  explicit UploadStreamClosure(
      const nsMainThreadPtrHandle<nsIInterfaceRequestor>& aChannel)
      : mChannel(aChannel) {}
};

void UploadCopyComplete(void* aClosure, nsresult aStatus) {
  // Called on the Stream Transport Service thread by NS_AsyncCopy
  MOZ_ASSERT(!NS_IsMainThread());
  UniquePtr<UploadStreamClosure> closure(
      static_cast<UploadStreamClosure*>(aClosure));
...
bool HttpChannelParent::DoAsyncOpen(
  ...
  if (stream) {
    // FIXME: The fast path of using the existing stream currently only applies
    // to streams
    //   that have had their entire contents serialized from the child at this
    //   point. Once bug 1294446 and bug 1294450 are fixed it is worth
    //   revisiting this heuristic.
    nsCOMPtr<nsIIPCSerializableInputStream> completeStream =
        do_QueryInterface(stream);
    if (!completeStream) {
      // Wait for completion of async copying IPC upload stream to a local input
      // stream.
      ++mAsyncOpenBarrier;
      ...
      // Accumulate the stream contents as the child sends it. We will continue
      // with the AsyncOpen process once the full stream has been received.
...
mozilla::ipc::IPCResult HttpChannelParent::RecvUpdateAssociatedContentSecurity(
    const int32_t& broken, const int32_t& no) {
...
HttpChannelParent::OnStartRequest(nsIRequest* aRequest, nsISupports* aContext) {
  ...
  if (mIPCClosed ||
      !SendOnStartRequest(
          channelStatus, responseHead ? *responseHead : nsHttpResponseHead(),
          !!responseHead, requestHead->Headers(), loadInfoForwarderArg,
          isFromCache, mCacheEntry ? true : false, cacheEntryId, fetchCount,
          expirationTime, cachedCharset, secInfoSerialization,
          chan->GetSelfAddr(), chan->GetPeerAddr(), redirectCount,
          cacheKeyValue, altDataType, altDataLen, ipcController,
          applyConversion)) {
...
HttpChannelParent::StartRedirect(uint32_t registrarId, nsIChannel* newChannel,
  ...
  if (!mIPCClosed) {
    result = SendRedirect1Begin(
        registrarId, uriParams, redirectFlags, loadInfoForwarderArg,
        responseHead ? *responseHead : nsHttpResponseHead(),
        secInfoSerialization, channelId, mChannel->GetPeerAddr());
  }
  if (!result) {
...
nsresult HttpChannelParent::OpenAlternativeOutputStream(
    const nsACString& type, nsIOutputStream** _retval) {
