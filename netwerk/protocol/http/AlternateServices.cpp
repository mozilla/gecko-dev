...
AltSvcMapping::GetConnectionInfo(nsHttpConnectionInfo **outCI,
                                 nsProxyInfo *pi)
{
  nsRefPtr<nsHttpConnectionInfo> ci = new nsHttpConnectionInfo(mAlternateHost,
                                                               mAlternatePort,
                                                               mNPNToken,
                                                               mUsername,
                                                               pi, mOriginHost,
...
#if 0
// bug https://bugzilla.mozilla.org/show_bug.cgi?id=1003448

// tests

// nice to haves
// clear based on origin when a fail happens
// upon establishment of channel, cancel and retry trans that have not yet written anything
// persistent storage including private browsing screen
// memory reporter for cache
// change parser to something centralized like sitesecurityservice

// testcase
http://www.penbaynetworks.com/other/nph-alt-service.cgi
reload :)
That last one should go over spdy/3.1 and TLS to www.penbaynetworks.com which has a self signed cert. true OE.
