...
this.FxAccountsClient.prototype = {
   ...
   *         }
   */
  deviceRegister(sessionTokenHex, name, type) {
   ...
   *         }
   */
  deviceUpdate(sessionTokenHex, id, name) {
    let path = "/account/device";
  ...
  _request: function hawkRequest(path, method, credentials, jsonPayload) {
    log.debug(`!!!!!!!!!! PHIL !!!!!!!!!! FxAccountsClient._request: ${method} ${path}`);
