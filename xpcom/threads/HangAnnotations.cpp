...
void HangAnnotations::AddAnnotation(const nsAString& aName,
                                    const int32_t aData) {
...
void HangAnnotations::AddAnnotation(const nsAString& aName, const bool aData) {
  if (aData) {
...
bool Annotators::Register(Annotator& aAnnotator) {
  MutexAutoLock lock(mMutex);
  auto result = mAnnotators.insert(&aAnnotator);
  return result.second;
}

bool Annotators::Unregister(Annotator& aAnnotator) {
  MutexAutoLock lock(mMutex);
...
HangAnnotations Annotators::GatherAnnotations() {
...
void ParamTraits<Annotation>::Write(Message* aMsg, const Annotation& aParam) {
  WriteParam(aMsg, aParam.mName);
  WriteParam(aMsg, aParam.mValue);
}

bool ParamTraits<Annotation>::Read(const Message* aMsg, PickleIterator* aIter,
                                   Annotation* aResult) {
