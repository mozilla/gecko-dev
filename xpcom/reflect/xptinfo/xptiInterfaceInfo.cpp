...
/* static */ xptiInterfaceEntry* xptiInterfaceEntry::Create(
    const char* aName, const nsID& aIID,
    const XPTInterfaceDescriptor* aDescriptor, xptiTypelibGuts* aTypelib) {
...
xptiInterfaceEntry::xptiInterfaceEntry(
    const char* aName, const nsID& aIID,
    const XPTInterfaceDescriptor* aDescriptor, xptiTypelibGuts* aTypelib)
    : mIID(aIID),
...
bool xptiInterfaceEntry::Resolve() {
  MutexAutoLock lock(XPTInterfaceInfoManager::GetResolveLock());
  return ResolveLocked();
}

bool xptiInterfaceEntry::ResolveLocked() {
  ...
  if (parent_index) {
    ...
    mMethodBaseIndex =
        parent->mMethodBaseIndex + parent->mDescriptor->mNumMethods;

    mConstantBaseIndex =
        parent->mConstantBaseIndex + parent->mDescriptor->mNumConstants;
  }
...
nsresult xptiInterfaceEntry::GetName(char** name) {
  // It is not necessary to Resolve because this info is read from manifest.
  *name = moz_xstrdup(mName);
  return NS_OK;
}

nsresult xptiInterfaceEntry::GetIID(nsIID** iid) {
  // It is not necessary to Resolve because this info is read from manifest.
  *iid = mIID.Clone();
  return NS_OK;
}

nsresult xptiInterfaceEntry::IsScriptable(bool* result) {
  // It is not necessary to Resolve because this info is read from manifest.
  *result = GetScriptableFlag();
  return NS_OK;
}

nsresult xptiInterfaceEntry::IsFunction(bool* result) {
...
nsresult xptiInterfaceEntry::GetMethodCount(uint16_t* count) {
  if (!EnsureResolved()) return NS_ERROR_UNEXPECTED;

  *count = mMethodBaseIndex + mDescriptor->mNumMethods;
  return NS_OK;
}

nsresult xptiInterfaceEntry::GetConstantCount(uint16_t* count) {
  if (!EnsureResolved()) return NS_ERROR_UNEXPECTED;

  if (!count) return NS_ERROR_UNEXPECTED;

  *count = mConstantBaseIndex + mDescriptor->mNumConstants;
  return NS_OK;
}

nsresult xptiInterfaceEntry::GetMethodInfo(uint16_t index,
                                           const nsXPTMethodInfo** info) {
  if (!EnsureResolved()) return NS_ERROR_UNEXPECTED;

  if (index < mMethodBaseIndex) return mParent->GetMethodInfo(index, info);

  if (index >= mMethodBaseIndex + mDescriptor->mNumMethods) {
    NS_ERROR("bad param");
    *info = nullptr;
...
nsresult xptiInterfaceEntry::GetMethodInfoForName(
    const char* methodName, uint16_t* index, const nsXPTMethodInfo** result) {
  if (!EnsureResolved()) return NS_ERROR_UNEXPECTED;
  ...
  for (uint16_t i = 0; i < mDescriptor->mNumMethods; ++i) {
    const nsXPTMethodInfo* info;
    info = static_cast<const nsXPTMethodInfo*>(
        &mDescriptor->mMethodDescriptors[i]);
...
nsresult xptiInterfaceEntry::GetConstant(uint16_t index,
  ...
  if (index >= mConstantBaseIndex + mDescriptor->mNumConstants) {
  ...
  switch (c.mType.mPrefix.mFlags) {
    ...
    default: { MOZ_ASSERT(false, "Invalid constant type found in interface"); }
  }
...
nsresult xptiInterfaceEntry::GetInterfaceIndexForParam(
  ...
  if (methodIndex >= mMethodBaseIndex + mDescriptor->mNumMethods) {
    NS_ERROR("bad param");
    return NS_ERROR_INVALID_ARG;
  }
...
nsresult xptiInterfaceEntry::GetEntryForParam(uint16_t methodIndex,
...
already_AddRefed<ShimInterfaceInfo> xptiInterfaceEntry::GetShimForParam(
    uint16_t methodIndex, const nsXPTParamInfo* param) {
  if (methodIndex < mMethodBaseIndex) {
...
nsresult xptiInterfaceEntry::GetInfoForParam(uint16_t methodIndex,
                                             const nsXPTParamInfo* param,
...
nsresult xptiInterfaceEntry::GetIIDForParam(uint16_t methodIndex,
                                            const nsXPTParamInfo* param,
                                            nsIID** iid) {
  xptiInterfaceEntry* entry;
  nsresult rv = GetEntryForParam(methodIndex, param, &entry);
...
nsresult xptiInterfaceEntry::GetIIDForParamNoAlloc(uint16_t methodIndex,
                                                   const nsXPTParamInfo* param,
                                                   nsIID* iid) {
  xptiInterfaceEntry* entry;
  nsresult rv = GetEntryForParam(methodIndex, param, &entry);
...
// this is a private helper
nsresult xptiInterfaceEntry::GetTypeInArray(const nsXPTParamInfo* param,
                                            uint16_t dimension,
                                            const XPTTypeDescriptor** type) {
  NS_ASSERTION(IsFullyResolved(), "bad state");

  const XPTTypeDescriptor* td = &param->mType;
  const XPTTypeDescriptor* additional_types = mDescriptor->mAdditionalTypes;
...
nsresult xptiInterfaceEntry::GetTypeForParam(uint16_t methodIndex,
                                             const nsXPTParamInfo* param,
                                             uint16_t dimension,
                                             nsXPTType* type) {
  if (!EnsureResolved()) return NS_ERROR_UNEXPECTED;
...
nsresult xptiInterfaceEntry::GetSizeIsArgNumberForParam(
    uint16_t methodIndex, const nsXPTParamInfo* param, uint16_t dimension,
    uint8_t* argnum) {
  if (!EnsureResolved()) return NS_ERROR_UNEXPECTED;

  if (methodIndex < mMethodBaseIndex)
    return mParent->GetSizeIsArgNumberForParam(methodIndex, param, dimension,
                                               argnum);
...
nsresult xptiInterfaceEntry::GetInterfaceIsArgNumberForParam(
    uint16_t methodIndex, const nsXPTParamInfo* param, uint8_t* argnum) {
  if (!EnsureResolved()) return NS_ERROR_UNEXPECTED;
...
nsresult xptiInterfaceEntry::IsIID(const nsIID* iid, bool* _retval) {
  // It is not necessary to Resolve because this info is read from manifest.
  *_retval = mIID.Equals(*iid);
  return NS_OK;
}

nsresult xptiInterfaceEntry::GetNameShared(const char** name) {
  // It is not necessary to Resolve because this info is read from manifest.
  *name = mName;
  return NS_OK;
}

nsresult xptiInterfaceEntry::GetIIDShared(const nsIID** iid) {
  // It is not necessary to Resolve because this info is read from manifest.
  *iid = &mIID;
  return NS_OK;
}

nsresult xptiInterfaceEntry::HasAncestor(const nsIID* iid, bool* _retval) {
  *_retval = false;
...
already_AddRefed<xptiInterfaceInfo> xptiInterfaceEntry::InterfaceInfo() {
#ifdef DEBUG
...
void xptiInterfaceEntry::LockedInvalidateInterfaceInfo() {
  if (mInfo) {
    mInfo->Invalidate();
    mInfo = nullptr;
  }
...
bool xptiInterfaceInfo::BuildParent() {
...
void xptiInterfaceInfo::Invalidate() {
  mParent = nullptr;
  mEntry = nullptr;
}

MozExternalRefCountType xptiInterfaceInfo::AddRef(void) {
  nsrefcnt cnt = ++mRefCnt;
  NS_LOG_ADDREF(this, cnt, "xptiInterfaceInfo", sizeof(*this));
  return cnt;
}

MozExternalRefCountType xptiInterfaceInfo::Release(void) {
