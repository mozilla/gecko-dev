...
static bool DoInterfaceDirectoryEntry(XPTArena *arena,
                                      NotNull<XPTCursor *> cursor,
                                      XPTInterfaceDirectoryEntry *ide);

static bool DoConstDescriptor(XPTArena *arena, NotNull<XPTCursor *> cursor,
                              XPTConstDescriptor *cd,
                              XPTInterfaceDescriptor *id);

static bool DoMethodDescriptor(XPTArena *arena, NotNull<XPTCursor *> cursor,
                               XPTMethodDescriptor *md,
                               XPTInterfaceDescriptor *id);

static bool SkipAnnotation(NotNull<XPTCursor *> cursor, bool *isLast);

static bool DoInterfaceDescriptor(XPTArena *arena, NotNull<XPTCursor *> outer,
                                  const XPTInterfaceDescriptor **idp);

static bool DoTypeDescriptorPrefix(XPTArena *arena, NotNull<XPTCursor *> cursor,
                                   XPTTypeDescriptorPrefix *tdp);

static bool DoTypeDescriptor(XPTArena *arena, NotNull<XPTCursor *> cursor,
                             XPTTypeDescriptor *td, XPTInterfaceDescriptor *id);

static bool DoParamDescriptor(XPTArena *arena, NotNull<XPTCursor *> cursor,
...
bool XPT_DoHeader(XPTArena *arena, NotNull<XPTCursor *> cursor,
  ...
  *headerp = header;
  ...
  for (i = 0; i < sizeof(magic); i++) {
    if (!XPT_Do8(cursor, &magic[i])) return false;
  }
  ...
  if (!XPT_Do16(cursor, &header->mNumInterfaces) ||
      !XPT_Do32(cursor, &file_length) || !XPT_Do32(cursor, &ide_offset)) {
    return false;
  }
   ...
   */
  if (file_length != 0 && cursor->state->pool_allocated < file_length) {
    fputs(
  ...
  uint32_t data_pool;
  if (!XPT_Do32(cursor, &data_pool)) return false;
  ...
  if (header->mNumInterfaces) {
    ...
    if (!interface_directory) return false;
  }
  ...
  bool isLast;
  do {
    if (!SkipAnnotation(cursor, &isLast)) return false;
  } while (!isLast);
  ...
  for (i = 0; i < header->mNumInterfaces; i++) {
    if (!DoInterfaceDirectoryEntry(arena, cursor, &interface_directory[i]))
      return false;
  }
...
bool DoInterfaceDirectoryEntry(XPTArena *arena, NotNull<XPTCursor *> cursor,
                               XPTInterfaceDirectoryEntry *ide) {
...
static bool InterfaceDescriptorAddType(XPTArena *arena,
                                       XPTInterfaceDescriptor *id,
  ...
  if (id->mNumAdditionalTypes == UINT8_MAX) return false;

  id->mNumAdditionalTypes += 1;
  return true;
}

bool DoInterfaceDescriptor(XPTArena *arena, NotNull<XPTCursor *> outer,
                           const XPTInterfaceDescriptor **idp) {
  XPTInterfaceDescriptor *id;
  XPTCursor curs;
  ...
  *idp = id;
  ...
  if (!XPT_MakeCursor(outer->state, XPT_DATA, id_sz, cursor)) return false;

  if (!XPT_Do32(outer, &cursor->offset)) return false;
  if (!cursor->offset) {
    *idp = NULL;
    return true;
  }
  ...
  if (id->mNumMethods) {
    ...
    if (!method_descriptors) return false;
  }
  ...
  if (id->mNumConstants) {
    ...
    if (!const_descriptors) return false;
  }
...
bool DoConstDescriptor(XPTArena *arena, NotNull<XPTCursor *> cursor,
...
bool DoMethodDescriptor(XPTArena *arena, NotNull<XPTCursor *> cursor,
  ...
  for (i = 0; i < md->mNumArgs; i++) {
    if (!DoParamDescriptor(arena, cursor, &params[i], id)) return false;
  }
  ...
  if (!DoParamDescriptor(arena, cursor, &result, id)) return false;

  return true;
}

bool DoParamDescriptor(XPTArena *arena, NotNull<XPTCursor *> cursor,
...
bool DoTypeDescriptorPrefix(XPTArena *arena, NotNull<XPTCursor *> cursor,
                            XPTTypeDescriptorPrefix *tdp) {
  return XPT_Do8(cursor, &tdp->mFlags);
}

bool DoTypeDescriptor(XPTArena *arena, NotNull<XPTCursor *> cursor,
                      XPTTypeDescriptor *td, XPTInterfaceDescriptor *id) {
  ...
  switch (td->Tag()) {
    case TD_INTERFACE_TYPE:
      uint16_t iface;
      if (!XPT_Do16(cursor, &iface)) return false;
      td->u.mIface.mIfaceHi8 = (iface >> 8) & 0xff;
      td->u.mIface.mIfaceLo8 = iface & 0xff;
      break;
    case TD_INTERFACE_IS_TYPE:
      if (!XPT_Do8(cursor, &td->u.mInterfaceIs.mArgNum)) return false;
      break;
    case TD_ARRAY: {
      // argnum2 appears in the on-disk format but it isn't used.
      uint8_t argnum2 = 0;
      if (!XPT_Do8(cursor, &td->u.mArray.mArgNum) || !XPT_Do8(cursor, &argnum2))
        return false;
...
bool SkipAnnotation(NotNull<XPTCursor *> cursor, bool *isLast) {
  uint8_t flags;
  if (!XPT_Do8(cursor, &flags)) return false;

  *isLast = XPT_ANN_IS_LAST(flags);

  if (XPT_ANN_IS_PRIVATE(flags)) {
    if (!XPT_SkipStringInline(cursor) || !XPT_SkipStringInline(cursor))
      return false;
  }
