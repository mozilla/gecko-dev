...
struct GCPointerPolicy {
  ...
  static void trace(JSTracer* trc, T* vp, const char* name) {
    if (*vp) js::UnsafeTraceManuallyBarrieredEdge(trc, vp, name);
  }
  static bool needsSweep(T* vp) {
    if (*vp) return js::gc::IsAboutToBeFinalizedUnbarriered(vp);
    return false;
  }
  static bool isValid(T v) { return js::gc::IsCellPointerValidOrNull(v); }
};
template <>
struct GCPolicy<JS::Symbol*> : public GCPointerPolicy<JS::Symbol*> {};
template <>
struct GCPolicy<JSAtom*> : public GCPointerPolicy<JSAtom*> {};
template <>
struct GCPolicy<JSFunction*> : public GCPointerPolicy<JSFunction*> {};
template <>
...
struct GCPolicy<mozilla::UniquePtr<T, D>> {
  ...
  static void trace(JSTracer* trc, mozilla::UniquePtr<T, D>* tp,
                    const char* name) {
    if (tp->get()) GCPolicy<T>::trace(trc, tp->get(), name);
  }
  static bool needsSweep(mozilla::UniquePtr<T, D>* tp) {
    if (tp->get()) return GCPolicy<T>::needsSweep(tp->get());
    return false;
  }
  static bool isValid(const mozilla::UniquePtr<T, D>& t) {
    if (t.get()) return GCPolicy<T>::isValid(*t.get());
    return true;
  }
};
...
struct GCPolicy<mozilla::Maybe<T>> {
  ...
  static void trace(JSTracer* trc, mozilla::Maybe<T>* tp, const char* name) {
    if (tp->isSome()) GCPolicy<T>::trace(trc, tp->ptr(), name);
  }
  static bool needsSweep(mozilla::Maybe<T>* tp) {
    if (tp->isSome()) return GCPolicy<T>::needsSweep(tp->ptr());
    return false;
  }
  static bool isValid(const mozilla::Maybe<T>& t) {
    if (t.isSome()) return GCPolicy<T>::isValid(t.ref());
    return true;
  }
};
