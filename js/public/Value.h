...
JS_ENUM_HEADER(JSValueType, uint8_t){
    JSVAL_TYPE_DOUBLE = 0x00, JSVAL_TYPE_INT32 = 0x01,
    JSVAL_TYPE_BOOLEAN = 0x02, JSVAL_TYPE_UNDEFINED = 0x03,
    JSVAL_TYPE_NULL = 0x04, JSVAL_TYPE_MAGIC = 0x05, JSVAL_TYPE_STRING = 0x06,
    JSVAL_TYPE_SYMBOL = 0x07, JSVAL_TYPE_PRIVATE_GCTHING = 0x08,
...
class MOZ_NON_PARAM alignas(8) Value {
 ...
 public:
...
#elif defined(JS_PUNBOX64)
    return reinterpret_cast<JS::Symbol*>(data.asBits ^
  ...
  JSObject* toObjectOrNull() const {
    ...
    uint64_t ptrBits =
        (data.asBits ^ JSVAL_SHIFTED_TAG_OBJECT) & ~JSVAL_OBJECT_OR_NULL_BIT;
    MOZ_ASSERT((ptrBits & 0x7) == 0);
    return reinterpret_cast<JSObject*>(ptrBits);
#endif
  }
...
static inline constexpr Value UndefinedValue() {
