...
class MacroAssemblerRiscv64 : public Assembler {
 public:
  ...
  void moveDouble(FloatRegister src, FloatRegister dest) { MOZ_CRASH(); }
  ...
  void store32(Register src, AbsoluteAddress address) { MOZ_CRASH(); }
  void store32(Register src, const Address& address) { MOZ_CRASH(); }
  void store32(Register src, const BaseIndex& address) { MOZ_CRASH(); }
  void store32(Imm32 src, const Address& address) { MOZ_CRASH(); }
  void store32(Imm32 src, const BaseIndex& address) { MOZ_CRASH(); }
...
class MacroAssemblerRiscv64Compat : public MacroAssemblerRiscv64 {
 public:
  ...
  void incrementInt32Value(const Address& addr) { MOZ_CRASH(); }
