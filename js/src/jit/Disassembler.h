...
class ComplexAddress {
 ...
 public:
  ...
  ComplexAddress(int32_t disp, Register::Encoding base,
                 Register::Encoding index, int scale)
      : disp_(disp),
        base_(base),
        index_(index),
        scale_(scale),
  ...
  bool isPCRelative() const { return isPCRelative_; }

  int32_t disp() const { return disp_; }

  bool hasBase() const { return base_ != Registers::Invalid; }
  ...
  bool hasIndex() const { return index_ != Registers::Invalid; }
...
class OtherOperand {
 ...
 public:
  OtherOperand() : kind_(Imm) {
    u_.imm = 0;
    MOZ_ASSERT(*this == *this);
  }

  explicit OtherOperand(int32_t imm) : kind_(Imm) {
    u_.imm = imm;
    MOZ_ASSERT(*this == *this);
  }

  explicit OtherOperand(Register::Encoding gpr) : kind_(GPR) {
    u_.gpr = gpr;
    MOZ_ASSERT(*this == *this);
  }

  explicit OtherOperand(FloatRegister::Encoding fpr) : kind_(FPR) {
    u_.fpr = fpr;
    MOZ_ASSERT(*this == *this);
  }
...
class HeapAccess {
 ...
 public:
  HeapAccess() : kind_(Unknown), size_(0) { MOZ_ASSERT(*this == *this); }

  HeapAccess(Kind kind, size_t size, const ComplexAddress& address,
             const OtherOperand& otherOperand)
      : kind_(kind),
        size_(size),
        address_(address),
    ...
    MOZ_ASSERT_IF(kind == Load || kind == LoadSext32,
                  otherOperand.kind() != OtherOperand::Imm);
    MOZ_ASSERT(*this == *this);
  }
  ...
  const ComplexAddress& address() const { return address_; }

  const OtherOperand& otherOperand() const { return otherOperand_; }
...
inline void VerifyHeapAccess(uint8_t* begin, uint8_t* end,
                             const HeapAccess& expected) {
