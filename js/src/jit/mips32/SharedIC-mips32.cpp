...
bool ICBinaryArith_Int32::Compiler::generateStubCode(MacroAssembler& masm) {
  ...
  savedRegs.set() = GeneralRegisterSet::Intersect(
      GeneralRegisterSet::NonVolatile(), savedRegs.set());
  ...
  switch (op_) {
    case JSOP_ADD:
      ...
      // required.
      masm.ma_addTestOverflow(scratchReg, R0.payloadReg(), R1.payloadReg(),
                              &failure);
      masm.move32(scratchReg, R0.payloadReg());
      break;
    case JSOP_SUB:
      masm.ma_subTestOverflow(scratchReg, R0.payloadReg(), R1.payloadReg(),
                              &failure);
      masm.move32(scratchReg, R0.payloadReg());
      break;
    case JSOP_MUL: {
      masm.ma_mul_branch_overflow(scratchReg, R0.payloadReg(), R1.payloadReg(),
                                  &failure);
    ...
    case JSOP_MOD: {
      ...
      masm.ma_b(R0.payloadReg(), Imm32(INT_MIN), &divTest1, Assembler::NotEqual,
                ShortJump);
      masm.ma_b(R1.payloadReg(), Imm32(-1), &failure, Assembler::Equal,
                ShortJump);
      masm.bind(&divTest1);

      // Check for division by zero
      masm.ma_b(R1.payloadReg(), Imm32(0), &failure, Assembler::Equal,
                ShortJump);

      // Check for 0 / X with X < 0 (results in -0).
      masm.ma_b(R0.payloadReg(), Imm32(0), &divTest2, Assembler::NotEqual,
                ShortJump);
      masm.ma_b(R1.payloadReg(), Imm32(0), &failure, Assembler::LessThan,
                ShortJump);
      masm.bind(&divTest2);
      ...
      if (op_ == JSOP_DIV) {
        ...
        masm.ma_b(scratchReg, Imm32(0), &failure, Assembler::NotEqual,
                  ShortJump);
        ...
        masm.ma_b(R0.payloadReg(), Imm32(0), &failure, Assembler::LessThan,
                  ShortJump);
        masm.bind(&done);
        masm.tagValue(JSVAL_TYPE_INT32, scratchReg, R0);
      }
    ...
    case JSOP_URSH:
      ...
      if (allowDouble_) {
        Label toUint;
        masm.ma_b(scratchReg, Imm32(0), &toUint, Assembler::LessThan,
                  ShortJump);
        ...
        masm.ma_b(scratchReg, Imm32(0), &failure, Assembler::LessThan,
                  ShortJump);
...
bool ICUnaryArith_Int32::Compiler::generateStubCode(MacroAssembler& masm) {
  Label failure;
  masm.branchTestInt32(Assembler::NotEqual, R0, &failure);
  ...
  switch (op) {
    ...
    case JSOP_NEG:
      ...
      masm.branchTest32(Assembler::Zero, R0.payloadReg(), Imm32(INT32_MAX),
                        &failure);
