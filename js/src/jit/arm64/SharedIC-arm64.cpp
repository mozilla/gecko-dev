...
bool ICBinaryArith_Int32::Compiler::generateStubCode(MacroAssembler& masm) {
  ...
  savedRegs.set() = GeneralRegisterSet::Intersect(
      GeneralRegisterSet::NonVolatile(), savedRegs);
#endif
  ...
  switch (op_) {
    ...
    case JSOP_MUL:
      masm.mul32(R0.valueReg(), R1.valueReg(), Rscratch, &failure,
                 &maybeNegZero);
    ...
    case JSOP_MOD: {
      ...
      if (op_ == JSOP_DIV) {
        ...
        // when (x/y)*y != x.
        masm.branch32(Assembler::NotEqual, R0.valueReg(), ExtractTemp0,
                      &revertRegister);
        masm.movePayload(Rscratch, R0_);
      } else {
        // Calculate the actual mod. Set the condition code, so we can see if it
        // is non-zero.
...
bool ICUnaryArith_Int32::Compiler::generateStubCode(MacroAssembler& masm) {
  ...
  switch (op) {
    ...
    case JSOP_NEG:
      ...
      // Compile -x as 0 - x.
      masm.Sub(ARMRegister(R1.valueReg(), 32), wzr,
               ARMRegister(R0.valueReg(), 32));
      masm.movePayload(R1.valueReg(), R0.valueReg());
      break;
    default:
