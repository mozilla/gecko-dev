...
bool ICBinaryArith_Int32::Compiler::generateStubCode(MacroAssembler& masm) {
  ...
  // Add R0 and R1.  Don't need to explicitly unbox, just use the TailCallReg
  ...
  switch (op_) {
    ...
    case JSOP_DIV: {
      ...
      masm.branchTest32(Assembler::Zero, R1.payloadReg(), R1.payloadReg(),
                        &failure);
    ...
    case JSOP_MOD: {
      ...
      masm.branchTest32(Assembler::Signed, scratchReg, scratchReg,
                        &revertRegister);
      masm.branchTest32(Assembler::Signed, R1.payloadReg(), R1.payloadReg(),
                        &revertRegister);
