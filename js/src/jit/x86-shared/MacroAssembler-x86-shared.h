...
class MacroAssemblerX86Shared : public Assembler {
 ...
 protected:
  ...
  struct Constant {
    ...
    Constant(Constant<T>&& other)
        : value(other.value), uses(mozilla::Move(other.uses)) {}
 ...
 public:
  ...
  void jump(wasm::OldTrapDesc target) { jmp(target); }
  ...
  class AutoEnsureByteRegister {
   ...
   public:
    ...
    ~AutoEnsureByteRegister() {
      if (original_ != substitute_) masm->pop(substitute_);
    }
  ...
  void packedUnsignedRightShiftByScalarInt16x8(FloatRegister src,
                                               FloatRegister dest) {
  ...
  void packedUnsignedRightShiftByScalarInt32x4(FloatRegister src,
                                               FloatRegister dest) {
  ...
  void convertDoubleToInt32(FloatRegister src, Register dest, Label* fail,
    ...
    // Check for -0.0
    if (negativeZeroCheck) branchNegativeZero(src, dest, fail);
  ...
  void convertFloat32ToInt32(FloatRegister src, Register dest, Label* fail,
                             bool negativeZeroCheck = true) {
    // Check for -0.0
    if (negativeZeroCheck) branchNegativeZeroFloat32(src, dest, fail);
  ...
  void emitSet(Assembler::Condition cond, Register dest,
    ...
    if (AllocatableGeneralRegisterSet(Registers::SingleByteRegs).has(dest)) {
      ...
      if (ifNaN == Assembler::NaN_IsFalse) j(Assembler::Parity, &ifFalse);
      ...
      movl(Imm32(1), dest);
      j(cond, &end);
      if (ifNaN == Assembler::NaN_IsTrue) j(Assembler::Parity, &end);
      bind(&ifFalse);
      mov(ImmWord(0), dest);
