...
void C1Spewer::spewRanges(GenericPrinter& out, BacktrackingAllocator* regalloc,
                          LNode* ins) {
  for (size_t k = 0; k < ins->numDefs(); k++) {
    const LDefinition* def = ins->isPhi() ? ins->toPhi()->getDef(k)
...
void C1Spewer::spewPass(GenericPrinter& out, MBasicBlock* block) {
  ...
  if (block->lir()) {
    ...
    for (LInstructionIterator i(block->lir()->begin());
         i != block->lir()->end(); i++)
      DumpLIR(out, *i);
    out.printf("    end_LIR\n");
  }
