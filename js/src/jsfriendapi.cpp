...
JS::RootingContext::RootingContext()
  ...
  for (auto& stackRootPtr : stackRoots_) stackRootPtr = nullptr;

  PodArrayZero(nativeStackLimit);
#if JS_STACK_GROWTH_DIRECTION > 0
  for (int i = 0; i < StackKindCount; i++) nativeStackLimit[i] = UINTPTR_MAX;
#endif
}
...
JS_FRIEND_API JSObject* JS_FindCompilationScope(JSContext* cx,
   ...
   * asked of us.
   */
  if (obj->is<WrapperObject>()) obj = UncheckedUnwrap(obj);
...
JS_FRIEND_API JSFunction* JS_GetObjectFunction(JSObject* obj) {
  if (obj->is<JSFunction>()) return &obj->as<JSFunction>();
  return nullptr;
}
...
JS_FRIEND_API JSObject* JS_NewObjectWithUniqueType(JSContext* cx,
   ...
   */
  RootedObject obj(cx, NewObjectWithGivenProto(cx, (const js::Class*)clasp,
                                               nullptr, SingletonObject));
  if (!obj) return nullptr;
  if (!JS_SplicePrototype(cx, obj, proto)) return nullptr;
  return obj;
}
...
JS_FRIEND_API bool JS_GetIsSecureContext(JSCompartment* compartment) {
...
JS_FRIEND_API void JS_SetCompartmentPrincipals(JSCompartment* compartment,
                                               JSPrincipals* principals) {
  // Short circuit if there's no change.
  if (principals == compartment->principals()) return;

  // Any compartment with the trusted principals -- and there can be
  // multiple -- is a system compartment.
  const JSPrincipals* trusted =
      compartment->runtimeFromActiveCooperatingThread()->trustedPrincipals();
...
JS_FRIEND_API JSCompartment* js::GetScriptCompartment(JSScript* script) {
...
JS_FRIEND_API bool JS_DefineFunctionsWithHelp(
  ...
  for (; fs->name; fs++) {
    ...
    if (fs->jitInfo) fun->setJitInfo(fs->jitInfo);

    if (fs->usage) {
      if (!DefineHelpProperty(cx, fun, "usage", fs->usage)) return false;
    }

    if (fs->help) {
      if (!DefineHelpProperty(cx, fun, "help", fs->help)) return false;
    }
  }
...
JS_FRIEND_API JS::Zone* js::GetCompartmentZone(JSCompartment* comp) {
  return comp->zone();
}

JS_FRIEND_API bool js::IsSystemCompartment(JSCompartment* comp) {
  return comp->isSystem();
}

JS_FRIEND_API bool js::IsSystemZone(Zone* zone) { return zone->isSystem; }

JS_FRIEND_API bool js::IsAtomsCompartment(JSCompartment* comp) {
...
JS_FRIEND_API JSObject* js::GetGlobalForObjectCrossCompartment(JSObject* obj) {
  return &obj->global();
}
...
JS_FRIEND_API bool js::GetObjectProto(JSContext* cx, JS::Handle<JSObject*> obj,
  ...
  if (IsProxy(obj)) return JS_GetPrototype(cx, obj, proto);
...
JS_FRIEND_API bool js::GetOriginalEval(JSContext* cx, HandleObject scope,
...
JS_FRIEND_API bool js::ZoneGlobalsAreAllGray(JS::Zone* zone) {
  for (CompartmentsInZoneIter comp(zone); !comp.done(); comp.next()) {
    ...
    if (!obj || !JS::ObjectIsMarkedGray(obj)) return false;
  }
  return true;
}
...
struct VisitGrayCallbackFunctor {
  ...
  void operator()(T tp) const {
    if ((*tp)->isMarkedGray()) callback_(closure_, JS::GCCellPtr(*tp));
  }
};
}  // namespace
...
static const char* FormatValue(JSContext* cx, const Value& vArg,
                               JSAutoByteString& bytes) {
  RootedValue v(cx, vArg);

  if (v.isMagic(JS_OPTIMIZED_OUT)) return "[unavailable]";
  ...
  if (found && (found - buf <= 2)) return "[function]";
  return buf;
}

// Wrapper for JS_sprintf_append() that reports allocation failure to the
// context.
static JS::UniqueChars MOZ_FORMAT_PRINTF(3, 4)
...
static JS::UniqueChars FormatFrame(JSContext* cx, const FrameIter& iter,
                                   JS::UniqueChars&& inBuf, int num,
                                   bool showArgs, bool showLocals,
                                   bool showThisProps) {
  ...
  if (fun) funname = fun->displayAtom();
  ...
  if (iter.hasUsableAbstractFramePtr() && iter.isFunctionFrame() && fun &&
    ...
    if (!GetFunctionThis(cx, iter.abstractFramePtr(), &thisVal)) return nullptr;
  }
  ...
  if (showArgs && iter.hasArgs()) {
    ...
    for (unsigned i = 0; i < iter.numActualArgs(); i++) {
      ...
      if (!value) {
        if (cx->isThrowingOutOfMemory()) return nullptr;
      ...
      if (value) {
        buf = sprintf_append(cx, Move(buf), "%s%s%s%s%s%s", !first ? ", " : "",
                             name ? name : "", name ? " = " : "",
                             arg.isString() ? "\"" : "", value,
                             arg.isString() ? "\"" : "");
        if (!buf) return nullptr;
  ...
  if (showLocals) {
    if (!thisVal.isUndefined()) {
      ...
      if (thisValStr) {
        ...
        buf =
            sprintf_append(cx, Move(buf), "    <failed to get 'this' value>\n");
      }
      if (!buf) return nullptr;
    }
  }
...
static JS::UniqueChars FormatWasmFrame(JSContext* cx, const FrameIter& iter,
                                       JS::UniqueChars&& inBuf, int num) {
  ...
  JS::UniqueChars buf =
      sprintf_append(cx, Move(inBuf), "%d %s()", num,
...
JS_FRIEND_API JS::UniqueChars JS::FormatStackDump(JSContext* cx,
                                                  JS::UniqueChars&& inBuf,
                                                  bool showArgs,
                                                  bool showLocals,
                                                  bool showThisProps) {
  int num = 0;
  ...
  for (AllFramesIter i(cx); !i.done(); ++i) {
    if (i.hasScript())
      buf = FormatFrame(cx, i, Move(buf), num, showArgs, showLocals,
                        showThisProps);
    else
  ...
  if (!num) buf = JS_sprintf_append(Move(buf), "JavaScript stack is empty\n");
...
static char MarkDescriptor(void* thing) {
  ...
  if (cell->isMarkedBlack()) return 'B';
  if (cell->isMarkedGray()) return 'G';
  if (cell->isMarkedAny()) return 'X';
  return 'W';
}
...
static void DumpHeapVisitCompartment(JSContext* cx, void* data,
                                     JSCompartment* comp) {
  char name[1024];
...
void DumpHeapTracer::onChild(const JS::GCCellPtr& thing) {
  if (gc::IsInsideNursery(thing.asCell())) return;
...
void js::DumpHeap(JSContext* cx, FILE* fp,
  ...
  dtrc.prefix = "> ";
  IterateHeapUnbarriered(cx, &dtrc, DumpHeapVisitZone, DumpHeapVisitCompartment,
                         DumpHeapVisitArena, DumpHeapVisitCell);

  fflush(dtrc.output);
}

JS_FRIEND_API void js::SetActivityCallback(JSContext* cx, ActivityCallback cb,
                                           void* arg) {
...
JS_FRIEND_API JSCompartment* js::GetAnyCompartmentInZone(JS::Zone* zone) {
...
JS_FRIEND_API JSObject* js::GetTestingFunctions(JSContext* cx) {
  ...
  if (!DefineTestingFunctions(cx, obj, false, false)) return nullptr;

  return obj;
}

#ifdef DEBUG
JS_FRIEND_API unsigned js::GetEnterCompartmentDepth(JSContext* cx) {
...
JS_FRIEND_API void js::PrepareScriptEnvironmentAndInvoke(
    JSContext* cx, HandleObject scope,
...
js::AutoCTypesActivityCallback::AutoCTypesActivityCallback(
  ...
  if (callback) callback(cx, beginType);
}
...
JS_FRIEND_API JSAtom* js::GetPropertyNameFromPC(JSScript* script,
                                                jsbytecode* pc) {
  if (!IsGetPropPC(pc) && !IsSetPropPC(pc)) return nullptr;
...
JS_FRIEND_API JSObject* js::ToWindowIfWindowProxy(JSObject* obj) {
  if (IsWindowProxy(obj)) return &obj->global();
  return obj;
}
...
JS_FRIEND_API void js::SetCompartmentValidAccessPtr(JSContext* cx,
                                                    JS::HandleObject global,
                                                    bool* accessp) {
  global->compartment()->setValidAccessPtr(accessp);
}

JS_FRIEND_API void js::SetCooperativeYieldCallback(JSContext* cx,
                                                   YieldCallback callback) {
  cx->setYieldCallback(callback);
}

JS_FRIEND_API bool js::SystemZoneAvailable(JSContext* cx) {
