...
test_guess:
  ...
  chars = rw->chars;
  do {
    if (*s++ != (unsigned char)(*chars++)) goto no_match;
  } while (--length != 0);
  return rw;
...
static const ReservedWordInfo* FindReservedWord(JSLinearString* str) {
...
template <typename CharT>
static bool IsIdentifier(const CharT* chars, size_t length) {
  using namespace js;

  if (length == 0) return false;

  if (!unicode::IsIdentifierStart(char16_t(*chars))) return false;

  const CharT* end = chars + length;
  while (++chars != end) {
    if (!unicode::IsIdentifierPart(char16_t(*chars))) return false;
  }
...
static bool IsIdentifierMaybeNonBMP(const char16_t* chars, size_t length) {
  using namespace js;

  if (IsIdentifier(chars, length)) return true;
  ...
  if (!unicode::IsIdentifierStart(codePoint)) return false;

  while (p < end) {
    codePoint = GetSingleCodePoint(&p, end);
    if (!unicode::IsIdentifierPart(codePoint)) return false;
  }
...
TokenKind ReservedWordTokenKind(PropertyName* str) {
  if (const ReservedWordInfo* rw = FindReservedWord(str)) return rw->tokentype;
...
MOZ_ALWAYS_INLINE bool TokenStreamAnyChars::SourceCoords::fill(
  ...
  for (size_t i = sentinelIndex + 1; i < other.lineStartOffsets_.length();
       i++) {
    if (!lineStartOffsets_.append(other.lineStartOffsets_[i])) return false;
  }
  return true;
}
...
TokenStreamCharsBase<CharT>::TokenStreamCharsBase(JSContext* cx,
                                                  ...
                                                  size_t length,
                                                  size_t startOffset)
    : userbuf(chars, length, startOffset), tokenbuf(cx) {}

template <typename CharT, class AnyCharsAccess>
TokenStreamSpecific<CharT, AnyCharsAccess>::TokenStreamSpecific(
    JSContext* cx, const ReadOnlyCompileOptions& options, const CharT* base,
    size_t length)
    : TokenStreamChars<CharT, AnyCharsAccess>(cx, base, length,
...
MOZ_ALWAYS_INLINE void TokenStreamAnyChars::updateFlagsForEOL() {
  flags.isDirtyLine = false;
}

// This gets the next char, normalizing all EOL sequences to '\n' as it goes.
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::getChar(int32_t* cp) {
...
template <typename CharT, class AnyCharsAccess>
int32_t GeneralTokenStreamChars<CharT, AnyCharsAccess>::getCharIgnoreEOL() {
  if (MOZ_LIKELY(userbuf.hasRawChars())) return userbuf.getRawChar();

  anyCharsAccess().flags.isEOF = true;
  return EOF;
}

void TokenStreamAnyChars::undoGetChar() {
...
template <typename CharT, class AnyCharsAccess>
void GeneralTokenStreamChars<CharT, AnyCharsAccess>::ungetChar(int32_t c) {
  if (c == EOF) return;
...
template <typename CharT>
void TokenStreamCharsBase<CharT>::ungetCharIgnoreEOL(int32_t c) {
  if (c == EOF) return;
...
void TokenStreamChars<char16_t, AnyCharsAccess>::ungetCodePointIgnoreEOL(
    uint32_t codePoint) {
  MOZ_ASSERT(!userbuf.atStart());
  ...
  while (numUnits-- > 0) ungetCharIgnoreEOL(units[numUnits]);
}
...
bool TokenStreamSpecific<CharT, AnyCharsAccess>::peekChars(int n, CharT* cp) {
  ...
  for (int j = i - 1; j >= 0; j--) ungetCharIgnoreEOL(cp[j]);

  return i == n;
}

template <typename CharT>
size_t TokenStreamCharsBase<CharT>::TokenBuf::findEOLMax(size_t start,
                                                         size_t max) {
  ...
  while (true) {
    ...
    if (n >= max) break;
    n++;
    if (TokenBuf::isRawEOLChar(*p++)) break;
  }
  return start + n;
}
...
template <typename CharT, class AnyCharsAccess>
void TokenStreamSpecific<CharT, AnyCharsAccess>::tell(Position* pos) {
  TokenStreamAnyChars& anyChars = anyCharsAccess();
  ...
  for (unsigned i = 0; i < anyChars.lookahead; i++)
    pos->lookaheadTokens[i] =
        anyChars.tokens[(anyChars.cursor + 1 + i) & ntokensMask];
}

template <typename CharT, class AnyCharsAccess>
void TokenStreamSpecific<CharT, AnyCharsAccess>::seek(const Position& pos) {
  TokenStreamAnyChars& anyChars = anyCharsAccess();
...
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::seek(
    const Position& pos, const TokenStreamAnyChars& other) {
  if (!anyCharsAccess().srcCoords.fill(other.srcCoords)) return false;
...
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::reportStrictModeErrorNumberVA(
    UniquePtr<JSErrorNotes> notes, uint32_t offset, bool strictMode,
    unsigned errorNumber, va_list* args) {
  TokenStreamAnyChars& anyChars = anyCharsAccess();
  if (!strictMode && !anyChars.options().extraWarningsOption) return true;

  ErrorMetadata metadata;
  if (!computeErrorMetadata(&metadata, offset)) return false;

  if (strictMode) {
    ReportCompileError(anyChars.cx, Move(metadata), Move(notes), JSREPORT_ERROR,
                       errorNumber, *args);
    return false;
  }

  return anyChars.compileWarning(Move(metadata), Move(notes),
...
bool TokenStreamAnyChars::compileWarning(ErrorMetadata&& metadata,
  ...
  if (options().werrorOption) {
    ...
    ReportCompileError(cx, Move(metadata), Move(notes), flags, errorNumber,
                       args);
    return false;
  }

  return ReportCompileWarning(cx, Move(metadata), Move(notes), flags,
                              errorNumber, args);
}
...
bool TokenStreamAnyChars::hasTokenizationStarted() const {
...
void TokenStreamAnyChars::currentLineAndColumn(uint32_t* line,
                                               uint32_t* column) const {
...
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::computeErrorMetadata(
  ...
  if (!anyCharsAccess().fillExcludingContext(err, offset)) return true;
...
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::computeLineOfContext(
    ErrorMetadata* err, uint32_t offset) {
  ...
  if (windowStart < userbuf.startOffset()) windowStart = userbuf.startOffset();
  ...
  if (!err->lineOfContext) return false;
...
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::reportStrictModeError(
...
template <typename CharT, class AnyCharsAccess>
void TokenStreamSpecific<CharT, AnyCharsAccess>::reportError(
    unsigned errorNumber, ...) {
  va_list args;
  va_start(args, errorNumber);
  ...
  if (computeErrorMetadata(&metadata, anyChars.currentToken().pos.begin)) {
    ReportCompileError(anyChars.cx, Move(metadata), nullptr, JSREPORT_ERROR,
                       errorNumber, args);
  }
...
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::warning(unsigned errorNumber,
                                                         ...) {
  va_list args;
  va_start(args, errorNumber);
  ...
  bool result =
      ...
      anyCharsAccess().compileWarning(Move(metadata), nullptr, JSREPORT_WARNING,
                                      errorNumber, args);
...
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::
    reportExtraWarningErrorNumberVA(UniquePtr<JSErrorNotes> notes,
                                    uint32_t offset, unsigned errorNumber,
                                    va_list* args) {
  TokenStreamAnyChars& anyChars = anyCharsAccess();
  if (!anyChars.options().extraWarningsOption) return true;
...
template <typename CharT, class AnyCharsAccess>
void TokenStreamSpecific<CharT, AnyCharsAccess>::error(unsigned errorNumber,
                                                       ...) {
  va_list args;
  va_start(args, errorNumber);
...
template <typename CharT, class AnyCharsAccess>
void TokenStreamSpecific<CharT, AnyCharsAccess>::errorAt(uint32_t offset,
                                                         unsigned errorNumber,
                                                         ...) {
  va_list args;
  va_start(args, errorNumber);
...
template <typename CharT, class AnyCharsAccess>
uint32_t TokenStreamSpecific<CharT, AnyCharsAccess>::peekUnicodeEscape(
    uint32_t* codePoint) {
  int32_t c = getCharIgnoreEOL();
  if (c != 'u') {
    ungetCharIgnoreEOL(c);
  ...
  uint32_t length;
  c = getCharIgnoreEOL();
  if (JS7_ISHEX(c) && peekChars(3, cp) && JS7_ISHEX(cp[0]) &&
      JS7_ISHEX(cp[1]) && JS7_ISHEX(cp[2])) {
    *codePoint = (JS7_UNHEX(c) << 12) | (JS7_UNHEX(cp[0]) << 8) |
...
template <typename CharT, class AnyCharsAccess>
uint32_t TokenStreamSpecific<CharT, AnyCharsAccess>::peekExtendedUnicodeEscape(
    uint32_t* codePoint) {
  ...
  ungetCharIgnoreEOL(c);
  while (i--) ungetCharIgnoreEOL(cp[i]);
  while (leadingZeros--) ungetCharIgnoreEOL('0');

  return length;
}

template <typename CharT, class AnyCharsAccess>
uint32_t TokenStreamSpecific<CharT, AnyCharsAccess>::matchUnicodeEscapeIdStart(
    uint32_t* codePoint) {
...
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::matchUnicodeEscapeIdent(
    uint32_t* codePoint) {
  uint32_t length = peekUnicodeEscape(codePoint);
...
template <>
MOZ_MUST_USE bool TokenStreamCharsBase<char16_t>::copyTokenbufTo(
...
MOZ_MUST_USE bool TokenStreamSpecific<CharT, AnyCharsAccess>::getDirective(
  ...
  // directive.
  if (!peekChars(directiveLength, peeked)) return true;
  ...
  do {
    ...
    if (c == EOF || unicode::IsSpaceOrBOM2(c)) break;
    ...
    if (!tokenbuf.append(c)) return false;
  } while (true);
...
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::getDisplayURL(
    bool isMultiline, bool shouldWarnDeprecated) {
...
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::getSourceMappingURL(
    bool isMultiline, bool shouldWarnDeprecated) {
...
template <typename CharT, class AnyCharsAccess>
MOZ_ALWAYS_INLINE Token* TokenStreamSpecific<CharT, AnyCharsAccess>::newToken(
    ptrdiff_t adjust) {
  TokenStreamAnyChars& anyChars = anyCharsAccess();
...
MOZ_MUST_USE bool TokenStreamCharsBase<char16_t>::appendCodePointToTokenbuf(
  ...
  if (!tokenbuf.append(units[0])) return false;

  if (numUnits == 1) return true;

  return tokenbuf.append(units[1]);
}

template <class AnyCharsAccess>
void TokenStreamChars<char16_t, AnyCharsAccess>::matchMultiUnitCodePointSlow(
    char16_t lead, uint32_t* codePoint) {
...
template <typename CharT, class AnyCharsAccess>
bool TokenStreamSpecific<CharT, AnyCharsAccess>::putIdentInTokenbuf(
  ...
  for (;;) {
    ...
    uint32_t codePoint;
    if (!matchMultiUnitCodePoint(c, &codePoint)) return false;
    if (codePoint) {
      if (!unicode::IsIdentifierPart(codePoint)) break;
    } else {
      if (unicode::IsIdentifierPart(char16_t(c))) {
        codePoint = c;
      } else {
        if (c != '\\' || !matchUnicodeEscapeIdent(&codePoint)) break;
      }
    }
...
MOZ_MUST_USE bool TokenStreamSpecific<CharT, AnyCharsAccess>::getTokenInternal(
    TokenKind* ttp, Modifier modifier) {
  int c;
  uint32_t qc;
  Token* tp;
  ...
  if (MOZ_UNLIKELY(modifier == TemplateTail)) {
    if (!getStringOrTemplateToken('`', &tp)) goto error;
    goto out;
  }
...
retry:
  ...
  if (MOZ_UNLIKELY(c >= 128)) {
    if (unicode::IsSpaceOrBOM2(c)) {
      if (c == unicode::LINE_SEPARATOR || c == unicode::PARA_SEPARATOR) {
        if (!updateLineInfoForEOL()) goto error;
    ...
    if (!matchMultiUnitCodePoint(c, &codePoint)) goto error;
  ...
  identifier:
    for (;;) {
      ...
      if (!unicode::IsIdentifierPart(char16_t(c))) {
        if (c != '\\' || !matchUnicodeEscapeIdent(&qc)) break;
    ...
    size_t length;
    if (hadUnicodeEscape) {
      if (!putIdentInTokenbuf(identStart)) goto error;
  ...
  decimal:
    ...
    hasExp = false;
    while (JS7_ISDEC(c)) c = getCharIgnoreEOL();
    ...
    if (!((decimalPoint == HasDecimal) || hasExp)) {
      ...
      const CharT* dummy;
      if (!js_strtod(anyCharsAccess().cx, numStart,
                     userbuf.addressOfNextRawChar(), &dummy, &dval)) {
        goto error;
      }
    }
  ...
  //
  if (c1kind == String) {
    if (!getStringOrTemplateToken(static_cast<char>(c), &tp)) goto error;
    goto out;
  }
  ...
  if (c1kind == BasePrefix) {
    ...
    if (c == 'x' || c == 'X') {
      ...
      while (JS7_ISHEX(c)) c = getCharIgnoreEOL();
    } else if (c == 'b' || c == 'B') {
      radix = 2;
      c = getCharIgnoreEOL();
      if (c != '0' && c != '1') {
      ...
      while (c == '0' || c == '1') c = getCharIgnoreEOL();
    } else if (c == 'o' || c == 'O') {
      radix = 8;
      c = getCharIgnoreEOL();
      if (c < '0' || c > '7') {
      ...
      while ('0' <= c && c <= '7') c = getCharIgnoreEOL();
    } else if (JS7_ISDEC(c)) {
      radix = 8;
      numStart = userbuf.addressOfNextRawChar() - 1;  // one past the '0'
      while (JS7_ISDEC(c)) {
        // Octal integer literals are not permitted in strict mode code.
        if (!reportStrictModeError(JSMSG_DEPRECATED_OCTAL)) goto error;
        ...
        // about it.
        if (c >= '8') {
          if (!warning(JSMSG_BAD_OCTAL, c == '8' ? "08" : "09")) goto error;
    ...
    if (!GetPrefixInteger(anyCharsAccess().cx, numStart,
                          userbuf.addressOfNextRawChar(), radix, &dummy,
                          &dval)) {
      goto error;
    }
  ...
  switch (c) {
    ...
    case '|':
      if (matchChar('|')) tp->type = TokenKind::Or;
    ...
    case '/':
      ...
      if (matchChar('/')) {
        if (!peekChar(&c)) goto error;
        if (c == '@' || c == '#') {
          consumeKnownChar(c);

          bool shouldWarn = c == '@';
          if (!getDirectives(false, shouldWarn)) goto error;
        }

      skipline:
        do {
          if (!getChar(&c)) goto error;
        } while (c != EOF && c != '\n');
      ...
      if (matchChar('*')) {
        ...
        do {
          ...
          if (c == '@' || c == '#') {
            bool shouldWarn = c == '@';
            if (!getDirectives(true, shouldWarn)) goto error;
          }
        } while (true);

        if (linenoBefore != anyChars.lineno) anyChars.updateFlagsForEOL();
        anyChars.cursor = (anyChars.cursor - 1) & ntokensMask;
        goto retry;
      }
      ...
      if (modifier == Operand) {
        ...
        do {
          ...
          if (c == '\\') {
            if (!tokenbuf.append(c)) goto error;
            if (!getChar(&c)) goto error;
          } else if (c == '[') {
            inCharClass = true;
          } else if (c == ']') {
        ...
        case 'u': {
          ...
          if (c2 == '{') {
            ...
            if (code < unicode::NonBMPMin) {
              ...
              if (!tokenbuf.append(unicode::LeadSurrogate(code))) return false;
          ...
          CharT cp[4];
          if (peekChars(4, cp) && JS7_ISHEX(cp[0]) && JS7_ISHEX(cp[1]) &&
        ...
        default:
          ...
          if (JS7_ISOCT(c)) {
            ...
            if (val != 0 || JS7_ISDEC(c)) {
              ...
              if (!reportStrictModeError(JSMSG_DEPRECATED_OCTAL)) return false;
...
template class frontend::TokenStreamChars<char16_t,
                                          frontend::TokenStreamAnyCharsAccess>;
template class frontend::TokenStreamSpecific<
    char16_t, frontend::TokenStreamAnyCharsAccess>;

template class frontend::TokenStreamChars<
    char16_t, frontend::ParserAnyCharsAccess<frontend::GeneralParser<
                  frontend::FullParseHandler, char16_t>>>;
template class frontend::TokenStreamChars<
    char16_t, frontend::ParserAnyCharsAccess<frontend::GeneralParser<
                  frontend::SyntaxParseHandler, char16_t>>>;
