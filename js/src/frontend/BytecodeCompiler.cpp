...
class MOZ_STACK_CLASS BytecodeCompiler {
 public:
  ...
  BytecodeCompiler(JSContext* cx, LifoAlloc& alloc,
                   const ReadOnlyCompileOptions& options,
                   SourceBufferHolder& sourceBuffer,
                   HandleScope enclosingScope);

  JSScript* compileGlobalScript(ScopeKind scopeKind);
  JSScript* compileEvalScript(HandleObject environment,
                              HandleScope enclosingScope);
  ModuleObject* compileModule();
  bool compileStandaloneFunction(MutableHandleFunction fun,
 ...
 private:
  ...
  bool deoptimizeArgumentsInEnclosingScripts(JSContext* cx,
...
BytecodeCompiler::BytecodeCompiler(JSContext* cx, LifoAlloc& alloc,
                                   const ReadOnlyCompileOptions& options,
                                   SourceBufferHolder& sourceBuffer,
                                   HandleScope enclosingScope)
    : keepAtoms(cx),
...
bool BytecodeCompiler::checkLength() {
...
bool BytecodeCompiler::createScript() {
  return createScript(0, sourceBuffer.length());
}

bool BytecodeCompiler::createScript(uint32_t toStringStart,
                                    uint32_t toStringEnd) {
  script = JSScript::Create(cx, options, sourceObject, /* sourceStart = */ 0,
                            sourceBuffer.length(), toStringStart, toStringEnd);
...
bool BytecodeCompiler::handleParseFailure(const Directives& newDirectives) {
...
JSScript* BytecodeCompiler::compileScript(HandleObject environment,
                                          SharedContext* sc) {
  if (!createSourceAndParser()) return nullptr;

  if (!createScript()) return nullptr;

  Maybe<BytecodeEmitter> emitter;
  if (!emplaceEmitter(emitter, sc)) return nullptr;
  ...
  for (;;) {
    ...
    if (pn) {
      ...
      if (!emitter->emitScript(pn)) return nullptr;
      if (!NameFunctions(cx, pn)) return nullptr;
    ...
    if (!handleParseFailure(directives)) return nullptr;
  ...
  if (!scriptSource->tryCompressOffThread(cx)) return nullptr;
...
JSScript* BytecodeCompiler::compileGlobalScript(ScopeKind scopeKind) {
  GlobalSharedContext globalsc(cx, scopeKind, directives,
                               options.extraWarningsOption);
  return compileScript(nullptr, &globalsc);
}

JSScript* BytecodeCompiler::compileEvalScript(HandleObject environment,
                                              HandleScope enclosingScope) {
  EvalSharedContext evalsc(cx, environment, enclosingScope, directives,
                           options.extraWarningsOption);
...
ModuleObject* BytecodeCompiler::compileModule() {
  ...
  Maybe<BytecodeEmitter> emitter;
  if (!emplaceEmitter(emitter, &modulesc)) return nullptr;
  if (!emitter->emitScript(pn->pn_body)) return nullptr;

  if (!NameFunctions(cx, pn)) return nullptr;

  parser->handler.freeTree(pn);

  if (!builder.initModule()) return nullptr;
...
// constructor.
bool BytecodeCompiler::compileStandaloneFunction(
    MutableHandleFunction fun, GeneratorKind generatorKind,
    FunctionAsyncKind asyncKind, const Maybe<uint32_t>& parameterListEnd) {
  MOZ_ASSERT(fun);
  MOZ_ASSERT(fun->isTenured());

  if (!createSourceAndParser(parameterListEnd)) return false;
  ...
  do {
    ...
    fn = parser->standaloneFunction(fun, enclosingScope, parameterListEnd,
                                    generatorKind, asyncKind, directives,
                                    &newDirectives);
    if (!fn && !handleParseFailure(newDirectives)) return false;
  } while (!fn);
  ...
  if (fn->pn_funbox->function()->isInterpreted()) {
    ...
    Maybe<BytecodeEmitter> emitter;
    if (!emplaceEmitter(emitter, fn->pn_funbox)) return false;
    if (!emitter->emitFunctionScript(fn->pn_body)) return false;
  } else {
    fun.set(fn->pn_funbox->function());
    MOZ_ASSERT(IsAsmJSModule(fun));
  }

  if (!NameFunctions(cx, fn)) return false;

  // Enqueue an off-thread source compression task after finishing parsing.
  if (!scriptSource->tryCompressOffThread(cx)) return false;

  return true;
}

ScriptSourceObject* BytecodeCompiler::sourceObjectPtr() const {
...
ScriptSourceObject* frontend::CreateScriptSourceObject(
  ...
  if (!cx->helperThread()) {
    if (!ScriptSourceObject::initFromOptions(cx, sso, options)) return nullptr;
  }
...
class MOZ_STACK_CLASS AutoInitializeSourceObject {
 ...
 public:
  ...
  ~AutoInitializeSourceObject() {
    if (sourceObjectOut_) *sourceObjectOut_ = compiler_.sourceObjectPtr();
  }
};

JSScript* frontend::CompileGlobalScript(JSContext* cx, LifoAlloc& alloc,
                                        ScopeKind scopeKind,
                                        const ReadOnlyCompileOptions& options,
  ...
  BytecodeCompiler compiler(cx, alloc, options, srcBuf,
...
bool frontend::CompileLazyFunction(JSContext* cx, Handle<LazyScript*> lazy,
  ...
  if (!lazy->scriptSource()->parseEnded().IsNull()) {
    const mozilla::TimeDuration delta =
        mozilla::TimeStamp::Now() - lazy->scriptSource()->parseEnded();
  ...
  Rooted<JSScript*> script(
      cx,
      JSScript::Create(cx, options, sourceObject, lazy->begin(), lazy->end(),
                       lazy->toStringStart(), lazy->toStringEnd()));
  if (!script) return false;

  if (lazy->isLikelyConstructorWrapper()) script->setLikelyConstructorWrapper();
  if (lazy->hasBeenCloned()) script->setHasBeenCloned();

  BytecodeEmitter bce(/* parent = */ nullptr, &parser, pn->pn_funbox, script,
                      lazy, pn->pn_pos, BytecodeEmitter::LazyFunction);
  if (!bce.init()) return false;

  if (!bce.emitFunctionScript(pn->pn_body)) return false;

  if (!NameFunctions(cx, pn)) return false;
...
bool frontend::CompileStandaloneFunction(
  ...
  if (!scope) scope = &cx->global()->emptyGlobalScope();
