...
BinTokenReaderTester::BinTokenReaderTester(JSContext* cx, const uint8_t* start,
                                           const size_t length)
    : cx_(cx),
      start_(start),
      current_(start),
...
bool BinTokenReaderTester::raiseError(const char* description) {
...
bool BinTokenReaderTester::readBuf(uint8_t* bytes, uint32_t len) {
  MOZ_ASSERT(!cx_->isExceptionPending());
  MOZ_ASSERT(len > 0);

  if (stop_ < current_ + len) return raiseError("Buffer exceeds length");

  for (uint32_t i = 0; i < len; ++i) *bytes++ = *current_++;

  return true;
}

bool BinTokenReaderTester::readByte(uint8_t* byte) { return readBuf(byte, 1); }
...
// 1 => true
// 2 => null
bool BinTokenReaderTester::readMaybeBool(Maybe<bool>& result) {
...
bool BinTokenReaderTester::readBool(bool& out) {
  Maybe<bool> result;

  if (!readMaybeBool(result)) return false;

  if (result.isNothing()) return raiseError("Empty boolean value");
...
bool BinTokenReaderTester::readMaybeDouble(Maybe<double>& result) {
  updateLatestKnownGood();
...
bool BinTokenReaderTester::readDouble(double& out) {
  Maybe<double> result;

  if (!readMaybeDouble(result)) return false;

  if (result.isNothing()) return raiseError("Empty double value");
...
bool BinTokenReaderTester::readInternalUint32(uint32_t* result) {
  uint8_t bytes[4];
  MOZ_ASSERT(sizeof(bytes) == sizeof(uint32_t));
  if (!readBuf(bytes, 4)) return false;
...
// is reserved to `null`.
bool BinTokenReaderTester::readMaybeChars(Maybe<Chars>& out) {
  updateLatestKnownGood();

  if (!readConst("<string>")) return false;

  // 1. Read byteLength
  uint32_t byteLen;
  if (!readInternalUint32(&byteLen)) return false;
...
bool BinTokenReaderTester::readChars(Chars& out) {
  Maybe<Chars> result;

  if (!readMaybeChars(result)) return false;

  if (result.isNothing()) return raiseError("Empty string");
...
template <size_t N>
bool BinTokenReaderTester::matchConst(const char (&value)[N]) {
  MOZ_ASSERT(N > 0);
  MOZ_ASSERT(value[N - 1] == 0);
  MOZ_ASSERT(!cx_->isExceptionPending());

  if (current_ + N - 1 > stop_) return false;
...
// - "</tuple>"
bool BinTokenReaderTester::enterUntaggedTuple(AutoTuple& guard) {
  if (!readConst("<tuple>")) return false;
...
template <size_t N>
bool BinTokenReaderTester::readConst(const char (&value)[N]) {
  updateLatestKnownGood();
  if (!matchConst(value)) return raiseError("Could not find expected literal");
...
// - "</tuple>"
bool BinTokenReaderTester::enterTaggedTuple(BinKind& tag, BinFields& fields,
                                            AutoTaggedTuple& guard) {
  // Header
  if (!readConst("<tuple>")) return false;

  if (!readConst("<head>")) return false;
...
#undef FIND_MATCH
  ...
  // Now fields.
  uint32_t fieldNum;
  if (!readInternalUint32(&fieldNum)) return false;

  fields.clear();
  if (!fields.reserve(fieldNum)) return raiseError("Out of memory");
...
// length`.
bool BinTokenReaderTester::enterList(uint32_t& items, AutoList& guard) {
  if (!readConst("<list>")) return false;
  ...
  if (stop > this->stop_) return raiseError("Incorrect list length");

  guard.init(stop);

  if (!readInternalUint32(&items)) return false;

  return true;
}

void BinTokenReaderTester::updateLatestKnownGood() {
...
size_t BinTokenReaderTester::offset() const { return latestKnownGoodPos_; }

TokenPos BinTokenReaderTester::pos() { return pos(latestKnownGoodPos_); }

TokenPos BinTokenReaderTester::pos(size_t start) {
  TokenPos pos;
  pos.begin = start;
...
bool BinTokenReaderTester::AutoBase::checkPosition(const uint8_t* expectedEnd) {
...
void BinTokenReaderTester::AutoList::init(const uint8_t* expectedEnd) {
  AutoBase::init();
  this->expectedEnd_ = expectedEnd;
}

bool BinTokenReaderTester::AutoList::done() {
  ...
  if (!checkPosition(expectedEnd_)) return false;

  // Check suffix.
  if (!reader_.readConst("</list>")) return false;
...
bool BinTokenReaderTester::AutoTaggedTuple::done() {
  MOZ_ASSERT(initialized_);
  initialized_ = false;
  if (reader_.cx_->isExceptionPending()) {
    // Already errored, no need to check further.
  ...
  // Check suffix.
  if (!reader_.readConst("</tuple>")) return false;
...
bool BinTokenReaderTester::AutoTuple::done() {
  MOZ_ASSERT(initialized_);
  initialized_ = false;
  if (reader_.cx_->isExceptionPending()) {
    // Already errored, no need to check further.
