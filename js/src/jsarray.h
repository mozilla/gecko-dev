 ...
 */
extern bool WouldDefinePastNonwritableLength(HandleNativeObject obj,
                                             uint32_t index);
...
extern bool IsWrappedArrayConstructor(JSContext* cx, const Value& v,
                                      bool* result);
...
class MOZ_NON_TEMPORARY_CLASS ArraySpeciesLookup final {
 ...
 public:
  ...
  void purge() {
    if (state_ == State::Initialized) reset();
  }
};
