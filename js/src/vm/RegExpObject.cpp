...
bool VectorMatchPairs::allocOrExpandArray(size_t pairCount) {
  if (!vec_.resizeUninitialized(pairCount)) return false;
...
/* static */ bool RegExpShared::compile(
  ...
  irregexp::RegExpCode code = irregexp::CompilePattern(
      cx, re, &data, input, false /* global() */, re->ignoreCase(),
      input->hasLatin1Chars(), mode == MatchOnly, force == ForceByteCode,
      re->sticky(), re->unicode(), tables);
...
bool js::ParseRegExpFlags(JSContext* cx, JSString* flagStr,
  ...
  if (!ok) {
    ...
    JS_ReportErrorFlagsAndNumberUTF8(cx, JSREPORT_ERROR, GetErrorMessage,
                                     nullptr, JSMSG_BAD_REGEXP_FLAG,
                                     utf8.get());
    return false;
  }
...
bool js::XDRScriptRegExpObject(XDRState<mode>* xdr,
  ...
  if (mode == XDR_DECODE) {
    RegExpObject* reobj =
        RegExpObject::create(xdr->cx(), source, RegExpFlag(flagsword),
...
JSObject* js::CloneScriptRegExpObject(JSContext* cx, RegExpObject& reobj) {
  ...
  return RegExpObject::create(cx, source, reobj.getFlags(), cx->tempLifoAlloc(),
                              TenuredObject);
}
