...
size_t TraceLoggerThreadState::sizeOfExcludingThis(
  ...
  if (textIdPayloads.initialized()) {
    ...
    for (TextIdHashMap::Range r = textIdPayloads.all(); !r.empty();
         r.popFront())
...
void TraceLoggerThread::extractScriptDetails(
    uint32_t textId, const char** filename, size_t* filename_len,
...
TraceLoggerEventPayload* TraceLoggerThreadState::getOrCreateEventPayload(
  ...
  TraceLoggerEventPayload* payload =
      js_new<TraceLoggerEventPayload>(textId, str);
  if (!payload) {
    js_free(str);
    return nullptr;
  }
...
TraceLoggerEventPayload* TraceLoggerThreadState::getOrCreateEventPayload(
    const char* filename, size_t lineno, size_t colno, const void* ptr) {
    ...
    if (id == TraceLogger_Error || prev == TraceLogger_Error) {
      ...
      if (prev != id) {
        ...
        MOZ_ASSERT_IF(maybeEventText(prev),
                      strcmp(eventText(id), eventText(prev)) == 0);
      }
    } else {
      MOZ_ASSERT(id == prev);
    }
  ...
  if (options) {
    if (strstr(options, "help")) {
      ...
      printf(
          ...
          "\n"
          "  EnableActiveThread      Start logging cooperating threads "
          "immediately.\n"
          "  EnableOffThread         Start logging helper threads "
          "immediately.\n"
          "  EnableGraph             Enable spewing the tracelogging graph to "
          "a file.\n"
...
TraceLoggerEvent::TraceLoggerEvent(TraceLoggerTextId type, const char* filename,
  ...
  payload_.setEventPayload(traceLoggerState->getOrCreateEventPayload(
      filename, line, column, nullptr));
}
