...
static void DeflateStringToUTF8Buffer(const CharT* src, size_t srclen,
  ...
  while (srclen) {
    ...
    if (v < 0x0080) {
      ...
      if (dstlenp && *dstlenp + 1 > capacity) return;
      *dst++ = char(v);
      utf8Len = 1;
    } else {
      uint8_t utf8buf[4];
      utf8Len = OneUcs4ToUtf8Char(utf8buf, v);
      if (dstlenp && *dstlenp + utf8Len > capacity) return;
      for (size_t i = 0; i < utf8Len; i++) *dst++ = char(utf8buf[i]);
    }

    if (dstlenp) *dstlenp += utf8Len;
    if (numcharsp) (*numcharsp)++;
  }
}
...
static void ReportInvalidCharacter(JSContext* cx, uint32_t offset) {
  ...
  JS_ReportErrorFlagsAndNumberASCII(cx, JSREPORT_ERROR, GetErrorMessage,
                                    nullptr, JSMSG_MALFORMED_UTF8_CHAR, buffer);
}
...
static void ReportTooBigCharacter(JSContext* cx, uint32_t v) {
  ...
  JS_ReportErrorFlagsAndNumberASCII(cx, JSREPORT_ERROR, GetErrorMessage,
                                    nullptr, JSMSG_UTF8_CHAR_TOO_LARGE, buffer);
}
...
static bool InflateUTF8StringToBuffer(ContextT* cx, const UTF8Chars src,
                                      CharT* dst, size_t* dstlenp,
  ...
  auto RequireLatin1 = [&smallestEncoding] {
    *smallestEncoding =
        std::max(JS::SmallestEncoding::Latin1, *smallestEncoding);
  };
...
#define INVALID(report, arg, n2)                               \
  do {                                                         \
    if (Action == CountAndReportInvalids) {                    \
      ...
      if (v < 0x10000) {
        ...
        if (v <= 0xFFFFF) {
          ...
          if (Action == Copy) dst[j] = CharT((v >> 10) + 0xD800);
          j++;
          if (Action == Copy) dst[j] = CharT((v & 0x3FF) + 0xDC00);
    ...
    invalidMultiByteCodeUnit:
      ...
      // code unit.
      i += n - 1;
      if (Action != AssertNoInvalids) RequireUTF16();
    }
  }

  if (Action != AssertNoInvalids && Action != FindEncoding) *dstlenp = j;

  return true;
}

template <InflateUTF8Action Action, typename CharsT, class ContextT>
static CharsT InflateUTF8StringHelper(ContextT* cx, const UTF8Chars src,
                                      size_t* outlen) {
  using CharT = typename CharsT::CharT;
  *outlen = 0;

  JS::SmallestEncoding encoding;
  if (!InflateUTF8StringToBuffer<Action, CharT>(cx, src, /* dst = */ nullptr,
                                                outlen, &encoding))
    return CharsT();
  ...
  if (encoding == JS::SmallestEncoding::ASCII) {
    ...
    for (uint32_t i = 0; i < srclen; i++) dst[i] = CharT(src[i]);
  } else {
    MOZ_ALWAYS_TRUE((InflateUTF8StringToBuffer<Copy, CharT>(
        cx, src, dst, outlen, &encoding)));
  }
...
TwoByteCharsZ JS::UTF8CharsToNewTwoByteCharsZ(JSContext* cx,
                                              ...
                                              size_t* outlen) {
  return InflateUTF8StringHelper<CountAndReportInvalids, TwoByteCharsZ>(
      cx, utf8, outlen);
}
...
TwoByteCharsZ JS::LossyUTF8CharsToNewTwoByteCharsZ(JSContext* cx,
                                                   ...
                                                   size_t* outlen) {
  return InflateUTF8StringHelper<CountAndIgnoreInvalids, TwoByteCharsZ>(
      cx, utf8, outlen);
}
...
JS::SmallestEncoding JS::FindSmallestEncoding(UTF8Chars utf8) {
  ...
  MOZ_ALWAYS_TRUE((InflateUTF8StringToBuffer<FindEncoding, char16_t, JSContext>(
      /* cx = */ nullptr, utf8,
      /* dst = */ nullptr,
      /* dstlen = */ nullptr, &encoding)));
  return encoding;
}

Latin1CharsZ JS::UTF8CharsToNewLatin1CharsZ(JSContext* cx, const UTF8Chars utf8,
                                            size_t* outlen) {
  return InflateUTF8StringHelper<CountAndReportInvalids, Latin1CharsZ>(cx, utf8,
                                                                       outlen);
}
...
Latin1CharsZ JS::LossyUTF8CharsToNewLatin1CharsZ(JSContext* cx,
                                                 const UTF8Chars utf8,
                                                 size_t* outlen) {
  return InflateUTF8StringHelper<CountAndIgnoreInvalids, Latin1CharsZ>(cx, utf8,
                                                                       outlen);
}
...
bool JS::StringIsUTF8(const uint8_t* s, uint32_t length) {
  ...
  while (s < limit) {
    ...
    if (n < min || (0xD800 <= n && n < 0xE000) || n >= 0x110000) return false;
    s += len;
  }
  return true;
}
