...
class CrossCompartmentKey {
 public:
  ...
  using WrappedType = mozilla::Variant<JSObject*, JSString*, DebuggerAndScript,
                                       DebuggerAndObject>;
  ...
  JSCompartment* compartment() {
    struct GetCompartmentFunctor {
      JSCompartment* operator()(JSObject** tp) const {
        return (*tp)->compartment();
      }
      JSCompartment* operator()(JSScript** tp) const {
        return (*tp)->compartment();
      }
...
class WrapperMap {
  ...
  using OuterMap = GCHashMap<JSCompartment*, InnerMap,
...
struct JSCompartment {
 ...
 public:
  ...
  const JS::CompartmentCreationOptions& creationOptions() const {
 ...
 public:
  /* Last time at which an animation was played for a global in this
   * compartment. */
 ...
 public:
  ...
  bool hasObjectPendingMetadata() const {
    return objectMetadataState.is<js::PendingMetadata>();
  }
 ...
 public:
  void addSizeOfIncludingThis(
      ...
      size_t* compartmentObject, size_t* compartmentTables, size_t* innerViews,
      size_t* lazyArrayBuffers, size_t* objectMetadataTables,
      size_t* crossCompartmentWrappers, size_t* savedStacksSet,
      size_t* varNamesSet, size_t* nonSyntacticLexicalScopes,
 ...
 private:
  ...
  static const unsigned DebuggerObservesMask =
      ...
      DebuggerObservesAsmJS | DebuggerObservesBinarySource;
 ...
 private:
  // Used for collecting telemetry on SpiderMonkey's deprecated language
  // extensions.
  bool sawDeprecatedLanguageExtension[size_t(
      js::DeprecatedLanguageExtension::Count)];
...
inline js::Handle<js::GlobalObject*> JSContext::global() const {
  ...
  return js::Handle<js::GlobalObject*>::fromMarkedLocation(
      compartment_->global_.unsafeGet());
}
...
class MOZ_RAII AssertCompartmentUnchanged {
 public:
  explicit AssertCompartmentUnchanged(
      JSContext* cx MOZ_GUARD_OBJECT_NOTIFIER_PARAM)
...
class MOZ_RAII AutoWrapperVector : public JS::GCVector<WrapperValue, 8>,
 ...
 public:
  explicit AutoWrapperVector(JSContext* cx MOZ_GUARD_OBJECT_NOTIFIER_PARAM)
      : JS::GCVector<WrapperValue, 8>(cx), JS::AutoGCRooter(cx, WRAPVECTOR) {
    MOZ_GUARD_OBJECT_NOTIFIER_INIT;
  }
