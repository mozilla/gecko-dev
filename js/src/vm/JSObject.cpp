...
JS_PUBLIC_API const char* JS::InformalValueTypeName(const Value& v) {
  if (v.isObject()) return v.toObject().getClass()->name;
...
bool js::FromPropertyDescriptorToObject(JSContext* cx,
  ...
  // Step 4.
  if (desc.hasValue()) {
    if (!DefineDataProperty(cx, obj, names.value, desc.value())) return false;
  }
  ...
  if (desc.hasWritable()) {
    ...
    if (!DefineDataProperty(cx, obj, names.writable, v)) return false;
  }
  ...
  if (desc.hasGetterObject()) {
    if (JSObject* get = desc.getterObject())
      ...
      v.setUndefined();
    if (!DefineDataProperty(cx, obj, names.get, v)) return false;
  }
  ...
  if (desc.hasSetterObject()) {
    if (JSObject* set = desc.setterObject())
      ...
      v.setUndefined();
    if (!DefineDataProperty(cx, obj, names.set, v)) return false;
  }

  // Step 8.
  if (desc.hasEnumerable()) {
    v.setBoolean(desc.enumerable());
    if (!DefineDataProperty(cx, obj, names.enumerable, v)) return false;
  }

  // Step 9.
  if (desc.hasConfigurable()) {
    v.setBoolean(desc.configurable());
    if (!DefineDataProperty(cx, obj, names.configurable, v)) return false;
  }
...
static bool GetPropertyIfPresent(JSContext* cx, HandleObject obj, HandleId id,
  ...
  if (!HasProperty(cx, obj, id, foundp)) return false;
  if (!*foundp) {
    vp.setUndefined();
    return true;
  }
...
bool js::Throw(JSContext* cx, jsid id, unsigned errorNumber,
               const char* details) {
...
bool js::ToPropertyDescriptor(JSContext* cx, HandleValue descval,
  ...
  if (!GetPropertyIfPresent(cx, obj, id, &v, &found)) return false;
  if (found) {
    if (ToBoolean(v)) attrs |= JSPROP_ENUMERATE;
  } else {
  ...
  if (!GetPropertyIfPresent(cx, obj, id, &v, &found)) return false;
  if (found) {
    if (!ToBoolean(v)) attrs |= JSPROP_PERMANENT;
  } else {
...
void js::CompletePropertyDescriptor(MutableHandle<PropertyDescriptor> desc) {
  ...
  if (desc.isGenericDescriptor() || desc.isDataDescriptor()) {
    if (!desc.hasWritable()) desc.attributesRef() |= JSPROP_READONLY;
    desc.attributesRef() &= ~(JSPROP_IGNORE_READONLY | JSPROP_IGNORE_VALUE);
  } else {
    if (!desc.hasGetterObject()) desc.setGetterObject(nullptr);
    if (!desc.hasSetterObject()) desc.setSetterObject(nullptr);
    desc.attributesRef() |= JSPROP_GETTER | JSPROP_SETTER;
  }
  if (!desc.hasConfigurable()) desc.attributesRef() |= JSPROP_PERMANENT;
...
bool js::SetIntegrityLevel(JSContext* cx, HandleObject obj,
  ...
  if (!PreventExtensions(cx, obj, level)) return false;
  ...
  if (obj->isNative() && !obj->as<NativeObject>().inDictionaryMode() &&
    ...
    for (Shape::Range<NoGC> r(nobj->lastProperty()); !r.empty(); r.popFront()) {
      if (!shapes.append(&r.front())) return false;
    }
    ...
    if (level == IntegrityLevel::Frozen && obj->is<ArrayObject>()) {
      if (!obj->as<ArrayObject>().maybeCopyElementsForWrite(cx)) return false;
    ...
    for (size_t i = 0; i < keys.length(); i++) {
      ...
      if (level == IntegrityLevel::Sealed) {
        ...
        if (!GetOwnPropertyDescriptor(cx, obj, id, &currentDesc)) return false;

        // 9.a.iii.
        if (!currentDesc.object()) continue;
...
static bool ResolveLazyProperties(JSContext* cx, HandleNativeObject obj) {
  ...
  if (JSEnumerateOp enumerate = clasp->getEnumerate()) {
    if (!enumerate(cx, obj)) return false;
  }
  if (clasp->getNewEnumerate() && clasp->getResolve()) {
    ...
    for (size_t i = 0; i < properties.length(); i++) {
      id = properties[i];
      bool found;
      if (!HasOwnProperty(cx, obj, id, &found)) return false;
    }
  }
  return true;
}
...
bool js::TestIntegrityLevel(JSContext* cx, HandleObject obj,
  ...
  bool status;
  if (!IsExtensible(cx, obj, &status)) return false;
  if (status) {
    *result = false;
    return true;
  }
  ...
  if (obj->isNative()) {
    ...
    if (!ResolveLazyProperties(cx, nobj)) return false;
...
static inline gc::AllocKind NewObjectGCKind(const js::Class* clasp) {
  if (clasp == &ArrayObject::class_) return gc::AllocKind::OBJECT8;
  if (clasp == &JSFunction::class_) return gc::AllocKind::OBJECT2;
...
static inline JSObject* NewObject(JSContext* cx, HandleObjectGroup group,
  ...
  if (newKind == SingletonObject) {
    ...
    if (!JSObject::setSingleton(cx, nobj)) return nullptr;
    obj = nobj;
  }
...
JSObject* js::NewObjectWithClassProtoCommon(JSContext* cx, const Class* clasp,
  ...
  if (protoKey == JSProto_Null) protoKey = JSProto_Object;
...
static bool NewObjectWithGroupIsCachable(JSContext* cx, HandleObjectGroup group,
                                         NewObjectKind newKind) {
  return group->proto().isObject() && newKind == GenericObject &&
...
bool js::NewObjectScriptedCall(JSContext* cx, MutableHandleObject pobj) {
  ...
  if (script) {
    ...
    if (!ObjectGroup::setAllocationSiteObjectGroup(cx, script, pc, obj,
                                                   newKind == SingletonObject))
      return false;
  }
...
JSObject* js::CreateThis(JSContext* cx, const Class* newclasp,
  ...
  if (!GetPrototypeFromConstructor(cx, callee, &proto)) return nullptr;
...
static inline JSObject* CreateThisForFunctionWithGroup(JSContext* cx,
  ...
  if (TypeNewScript* newScript = group->newScript()) {
    ...
    // nursery.
    if (newKind == GenericObject) newKind = TenuredObject;
  ...
  if (newKind == SingletonObject) {
    ...
    return NewObjectWithGivenTaggedProto(cx, &PlainObject::class_, protoRoot,
                                         allocKind, newKind);
  }
...
JSObject* js::CreateThisForFunction(JSContext* cx, HandleObject callee,
                                    ...
                                    NewObjectKind newKind) {
  RootedObject proto(cx);
  if (!GetPrototypeFromConstructor(cx, newTarget, &proto)) return nullptr;
...
JS_FRIEND_API bool JS_CopyPropertyFrom(JSContext* cx, HandleId id,
  ...
  if (desc.getter() && !desc.hasGetterObject()) return true;
  if (desc.setter() && !desc.hasSetterObject()) return true;
...
JS_FRIEND_API bool JS_CopyPropertiesFrom(JSContext* cx, HandleObject target,
  ...
  for (size_t i = 0; i < props.length(); ++i) {
    if (!JS_CopyPropertyFrom(cx, props[i], target, obj)) return false;
  }
...
static bool CopyProxyObject(JSContext* cx, Handle<ProxyObject*> from,
  ...
  if (from->is<WrapperObject>() &&
    ...
    if (!cx->compartment()->wrap(cx, &v)) return false;
...
static bool GetScriptArrayObjectElements(
  ...
  if (!values.appendN(MagicValue(JS_ELEMENTS_HOLE), length)) return false;

  size_t initlen = arr->getDenseInitializedLength();
  for (size_t i = 0; i < initlen; i++) values[i].set(arr->getDenseElement(i));
...
static bool GetScriptPlainObjectProperties(
  ...
  if (obj->is<PlainObject>()) {
    ...
    if (!properties.appendN(IdValuePair(), nobj->slotSpan())) return false;
...
JSObject* js::DeepCloneObjectLiteral(JSContext* cx, HandleObject obj,
  ...
  if (obj->is<ArrayObject>()) {
    ...
    for (uint32_t i = 0; i < values.length(); ++i) {
      if (!DeepCloneValue(cx, values[i].address(), newKind)) return nullptr;
    }
  ...
  if (!GetScriptPlainObjectProperties(obj, &properties)) return nullptr;
  ...
  if (obj->isSingleton()) newKind = SingletonObject;
...
static bool InitializePropertiesFromCompatibleNativeObject(
  ...
  if (!dst->ensureElements(cx, src->getDenseInitializedLength())) return false;
  ...
  if (!dst->setLastProperty(cx, shape)) return false;
...
bool js::XDRObjectLiteral(XDRState<mode>* xdr, MutableHandleObject obj) {
  ...
  uint32_t isArray = 0;
  {
    ...
    if (!xdr->codeUint32(&isArray)) return false;
  }
  ...
  if (isArray) {
    ...
    if (mode == XDR_ENCODE) {
      ...
      if (!GetScriptArrayObjectElements(arr, &values)) return false;
    }

    uint32_t initialized;
    if (mode == XDR_ENCODE) initialized = values.length();
    if (!xdr->codeUint32(&initialized)) return false;
    if (mode == XDR_DECODE &&
    ...
    for (unsigned i = 0; i < initialized; i++) {
      if (!xdr->codeConstValue(values[i])) return false;
    }
    ...
    if (mode == XDR_ENCODE)
      copyOnWrite = obj->is<ArrayObject>() &&
                    obj->as<ArrayObject>().denseElementsAreCopyOnWrite();
    if (!xdr->codeUint32(&copyOnWrite)) return false;
  ...
  if (!xdr->codeUint32(&nproperties)) return false;
  ...
  for (size_t i = 0; i < nproperties; i++) {
    ...
    if (mode == XDR_DECODE) {
      if (!ValueToId<CanGC>(cx, tmpIdValue, &tmpId)) return false;
  ...
  if (mode == XDR_ENCODE) isSingleton = obj->isSingleton() ? 1 : 0;
  if (!xdr->codeUint32(&isSingleton)) return false;
...
template bool js::XDRObjectLiteral(XDRState<XDR_ENCODE>* xdr,
                                   MutableHandleObject obj);

template bool js::XDRObjectLiteral(XDRState<XDR_DECODE>* xdr,
                                   MutableHandleObject obj);
...
/* static */ bool NativeObject::fillInAfterSwap(JSContext* cx,
  ...
  if (nfixed != obj->shape()->numFixedSlots()) {
    if (!NativeObject::generateOwnShape(cx, obj)) return false;
...
static MOZ_MUST_USE bool CopyProxyValuesBeforeSwap(ProxyObject* proxy,
                                                   Vector<Value>& values) {
  ...
  if (!values.reserve(1 + proxy->numReservedSlots())) return false;
...
bool ProxyObject::initExternalValueArrayAfterSwap(JSContext* cx,
                                                  const Vector<Value>& values) {
...
bool JSObject::swap(JSContext* cx, HandleObject a, HandleObject b) {
  ...
  if (!JSObject::getGroup(cx, a)) oomUnsafe.crash("JSObject::swap");
  if (!JSObject::getGroup(cx, b)) oomUnsafe.crash("JSObject::swap");
  ...
  if (a->tenuredSizeOfThis() == b->tenuredSizeOfThis()) {
    ...
    if (aIsProxyWithInlineValues) b->as<ProxyObject>().setInlineValueArray();
    if (bIsProxyWithInlineValues) a->as<ProxyObject>().setInlineValueArray();
  } else {
    ...
    if (na) {
      ...
      for (size_t i = 0; i < na->slotSpan(); i++) {
        if (!avals.append(na->getSlot(i))) oomUnsafe.crash("JSObject::swap");
      }
    }
    ...
    if (nb) {
      ...
      for (size_t i = 0; i < nb->slotSpan(); i++) {
        if (!bvals.append(nb->getSlot(i))) oomUnsafe.crash("JSObject::swap");
      }
    }
...
static bool DefineStandardSlot(JSContext* cx, HandleObject obj, JSAtom* atom,
...
static NativeObject* DefineConstructorAndPrototype(
  ...
  if (!constructor) {
    ...
    if (!(clasp->flags & JSCLASS_IS_ANONYMOUS) || !obj->is<GlobalObject>() ||
      ...
      if (!DefineStandardSlot(cx, obj, atom, value, attrs, named)) goto bad;
    }

    ctor = proto;
  } else {
    RootedFunction fun(
        cx, NewNativeConstructor(cx, constructor, nargs, atom, ctorKind));
    if (!fun) goto bad;
    ...
    if (!DefineStandardSlot(cx, obj, atom, value, 0, named)) goto bad;
     ...
     */
    ctor = fun;
    if (!LinkConstructorAndPrototype(cx, ctor, proto)) goto bad;
  ...
  if (!cached && key != JSProto_Null) SetClassObject(obj, key, ctor, proto);
...
bad:
  ...
  if (cached) ClearClassObject(obj, key);
  return nullptr;
}
...
void JSObject::fixupAfterMovingGC() {
  ...
  if (is<NativeObject>()) {
    ...
    if (obj.denseElementsAreCopyOnWrite()) {
      ...
      if (IsForwarded(owner)) owner = Forwarded(owner);
...
static bool ReshapeForProtoMutation(JSContext* cx, HandleObject obj) {
  ...
  while (pobj && pobj->isNative()) {
    if (pobj->isSingleton()) {
      ...
      if (!JSObject::setUncacheableProto(cx, pobj)) return false;
    }

    if (!obj->isDelegate()) break;
...
static bool SetClassAndProto(JSContext* cx, HandleObject obj,
  ...
  if (!ReshapeForProtoMutation(cx, obj)) return false;

  if (proto.isObject()) {
    RootedObject protoObj(cx, proto.toObject());
    if (!JSObject::setDelegate(cx, protoObj)) return false;
  }
  ...
  if (obj->isSingleton()) {
     ...
     * consistent behavior.
     */
    if (!JSObject::splicePrototype(cx, obj, clasp, proto)) return false;
  ...
  if (oldGroup->maybeInterpretedFunction()) {
    ...
    newGroup =
        ObjectGroupCompartment::makeGroup(cx, &JSFunction::class_, proto);
...
/* static */ bool JSObject::changeToSingleton(JSContext* cx, HandleObject obj) {
  ...
  ObjectGroup* group =
      ObjectGroup::lazySingletonGroup(cx, obj->getClass(), obj->taggedProto());
...
bool js::GetObjectFromIncumbentGlobal(JSContext* cx, MutableHandleObject obj) {
  ...
  if (obj && !cx->compartment()->wrap(cx, obj)) return false;
...
JSProtoKey JS::IdentifyStandardInstance(JSObject* obj) {
  ...
  if (key != JSProto_Null && !IsStandardPrototype(obj, key)) return key;
  return JSProto_Null;
}
...
JSProtoKey JS::IdentifyStandardConstructor(JSObject* obj) {
  ...
  for (size_t k = 0; k < JSProto_LIMIT; ++k) {
    ...
    if (global.getConstructor(key) == ObjectValue(*obj)) return key;
  }
...
bool js::LookupName(JSContext* cx, HandlePropertyName name,
  ...
  for (RootedObject env(cx, envChain); env; env = env->enclosingEnvironment()) {
    if (!LookupProperty(cx, env, id, pobjp, propp)) return false;
    if (propp) {
      objp.set(env);
      return true;
    }
...
bool js::LookupNameNoGC(JSContext* cx, PropertyName* name, JSObject* envChain,
  ...
  for (JSObject* env = envChain; env; env = env->enclosingEnvironment()) {
    if (env->getOpsLookupProperty()) return false;
...
bool js::LookupNameWithGlobalDefault(JSContext* cx, HandlePropertyName name,
  ...
  for (; !env->is<GlobalObject>(); env = env->enclosingEnvironment()) {
    if (!LookupProperty(cx, env, id, &pobj, &prop)) return false;
    if (prop) break;
  }
...
bool js::HasOwnProperty(JSContext* cx, HandleObject obj, HandleId id,
                        bool* result) {
  if (obj->is<ProxyObject>()) return Proxy::hasOwn(cx, obj, id, result);

  if (GetOwnPropertyOp op = obj->getOpsGetOwnPropertyDescriptor()) {
    Rooted<PropertyDescriptor> desc(cx);
    if (!op(cx, obj, id, &desc)) return false;
    *result = !!desc.object();
    return true;
  }
...
bool js::LookupOwnPropertyPure(JSContext* cx, JSObject* obj, jsid id,
  ...
  if (isTypedArrayOutOfRange) *isTypedArrayOutOfRange = false;
    ...
    if (obj->is<TypedArrayObject>()) {
      ...
      if (IsTypedArrayIndex(id, &index)) {
        if (index < obj->as<TypedArrayObject>().length()) {
          ...
          propp->setNotFound();
          if (isTypedArrayOutOfRange) *isTypedArrayOutOfRange = true;
        }
        return true;
      }
    }
...
bool js::GetPropertyPure(JSContext* cx, JSObject* obj, jsid id, Value* vp) {
  JSObject* pobj;
  PropertyResult prop;
  if (!LookupPropertyPure(cx, obj, id, &pobj, &prop)) return false;
  ...
  return pobj->isNative() &&
         NativeGetPureInline(&pobj->as<NativeObject>(), id, prop, vp);
}
...
bool js::GetOwnPropertyPure(JSContext* cx, JSObject* obj, jsid id, Value* vp) {
  ...
  return obj->isNative() &&
         NativeGetPureInline(&obj->as<NativeObject>(), id, prop, vp);
}
...
bool js::GetOwnNativeGetterPure(JSContext* cx, JSObject* obj, jsid id,
  ...
  *native = nullptr;
  ...
  if (!getterObj->is<JSFunction>()) return true;

  JSFunction* getter = &getterObj->as<JSFunction>();
  if (!getter->isNative()) return true;
...
bool js::SetPrototype(JSContext* cx, HandleObject obj, HandleObject proto,
   ...
   * pointers.
   */
  if (proto == obj->staticPrototype()) return result.succeed();
  ...
  if (!extensible) return result.fail(JSMSG_CANT_SET_PROTO);
  ...
  while (obj2) {
    ...
    bool isOrdinary;
    if (!GetPrototypeIfOrdinary(cx, obj2, &isOrdinary, &obj2)) return false;
    if (!isOrdinary) break;
  }
  ...
  if (!MaybeConvertUnboxedObjectToNative(cx, obj)) return false;

  Rooted<TaggedProto> taggedProto(cx, TaggedProto(proto));
  if (!SetClassAndProto(cx, obj, obj->getClass(), taggedProto)) return false;
...
bool js::PreventExtensions(JSContext* cx, HandleObject obj,
                           ObjectOpResult& result, IntegrityLevel level) {
  if (obj->is<ProxyObject>())
    return js::Proxy::preventExtensions(cx, obj, result);

  if (!obj->nonProxyIsExtensible()) return result.succeed();
...
bool js::PreventExtensions(JSContext* cx, HandleObject obj,
                           IntegrityLevel level) {
  ObjectOpResult result;
  return PreventExtensions(cx, obj, result, level) &&
         result.checkStrict(cx, obj);
}
...
bool js::GetOwnPropertyDescriptor(JSContext* cx, HandleObject obj, HandleId id,
  ...
  if (GetOwnPropertyOp op = obj->getOpsGetOwnPropertyDescriptor()) {
    ...
    if (ok) desc.assertCompleteIfFound();
    return ok;
  }
...
bool js::DefineAccessorProperty(JSContext* cx, HandleObject obj,
                                PropertyName* name, JSGetterOp getter,
...
bool js::DefineAccessorElement(JSContext* cx, HandleObject obj, uint32_t index,
                               JSGetterOp getter, JSSetterOp setter,
                               unsigned attrs, ObjectOpResult& result) {
  RootedId id(cx);
  if (!IndexToId(cx, index, &id)) return false;
...
bool js::DefineDataProperty(JSContext* cx, HandleObject obj, HandleId id,
                            HandleValue value, unsigned attrs) {
  ObjectOpResult result;
  if (!DefineDataProperty(cx, obj, id, value, attrs, result)) return false;
  if (!result) {
    MOZ_ASSERT(!cx->helperThread());
    result.reportError(cx, obj, id);
    return false;
...
bool js::GetPropertyDescriptor(JSContext* cx, HandleObject obj, HandleId id,
  ...
  for (pobj = obj; pobj;) {
    ...
    if (!GetOwnPropertyDescriptor(cx, pobj, id, desc)) return false;

    if (desc.object()) return true;

    if (!GetPrototype(cx, pobj, &pobj)) return false;
  }
...
static bool DefineFunctionFromSpec(JSContext* cx, HandleObject obj,
  ...
  RootedId id(cx);
  if (!PropertySpecNameToId(cx, fs->name, &id)) return false;

  if (StandardProtoKeyOrNull(obj) == JSProto_Array &&
      id == NameToId(cx->names().values)) {
    if (!cx->options().arrayProtoValues()) return true;
  }

  JSFunction* fun = NewFunctionFromSpec(cx, fs, id);
  if (!fun) return false;

  if (intrinsic == AsIntrinsic) fun->setIsIntrinsic();
...
static bool MaybeCallMethod(JSContext* cx, HandleObject obj, HandleId id,
                            MutableHandleValue vp) {
  if (!GetProperty(cx, obj, obj, id, vp)) return false;
  if (!IsCallable(vp)) {
    vp.setObject(*obj);
    return true;
  }
...
bool JS::OrdinaryToPrimitive(JSContext* cx, HandleObject obj, JSType hint,
  ...
  if (hint == JSTYPE_STRING) {
    ...
    if (!MaybeCallMethod(cx, obj, id, vp)) return false;
    if (vp.isPrimitive()) return true;
...
bool js::ToPrimitiveSlow(JSContext* cx, JSType preferredType,
  ...
  if (!method.isNullOrUndefined()) {
    ...
    if (!js::Call(cx, method, vp, arg0, vp)) return false;
...
bool js::ToPropertyKeySlow(JSContext* cx, HandleValue argument,
  ...
  if (!ToPrimitiveSlow(cx, JSTYPE_STRING, &key)) return false;
...
bool js::IsDelegate(JSContext* cx, HandleObject obj, const js::Value& v,
                    bool* result) {
  if (v.isPrimitive()) {
    *result = false;
    return true;
...
bool js::IsDelegateOfObject(JSContext* cx, HandleObject protoObj, JSObject* obj,
                            bool* result) {
  RootedObject obj2(cx, obj);
  for (;;) {
    if (!GetPrototype(cx, obj2, &obj2)) return false;
    if (!obj2) {
      *result = false;
      return true;
    }
...
JSObject* js::PrimitiveToObject(JSContext* cx, const Value& v) {
  ...
  if (v.isNumber()) return NumberObject::create(cx, v.toNumber());
  if (v.isBoolean()) return BooleanObject::create(cx, v.toBoolean());
...
Value js::GetThisValue(JSObject* obj) {
  ...
  // exposed to script.
  if (obj->is<GlobalObject>()) return ObjectValue(*ToWindowProxyIfWindow(obj));
...
#undef TEST_SLOT_MATCHES_PROTOTYPE
        ...
        if (obj->is<EnvironmentObject>()) {
          if (slot == EnvironmentObject::enclosingEnvironmentSlot()) {
            ...
            if (slot == CallObject::calleeSlot()) slotname = "callee_slot";
...
static void DumpProperty(const NativeObject* obj, Shape& shape,
  ...
  if (shape.isDataProperty()) out.printf(" slot %d", shape.maybeSlot());
...
void JSObject::dump(js::GenericPrinter& out) const {
  ...
  if (nobj) {
    if (nobj->inDictionaryMode()) out.put(" inDictionaryMode");
    if (nobj->hasShapeTable()) out.put(" hasShapeTable");
    if (nobj->hadElementsAccess()) out.put(" had_elements_access");
    if (nobj->isIndexed()) out.put(" indexed");
    if (nobj->wasNewScriptCleared()) out.put(" new_script_cleared");
  } else {
    out.put(" not_native\n");
  }
  out.putChar('\n');
...
JS_FRIEND_API void js::DumpInterpreterFrame(JSContext* cx,
  ...
  if (!start) {
    ...
    while (!i.done() && !i.isJSJit() && i.interpFrame() != start) ++i;
  ...
  for (; !i.done(); ++i) {
    ...
    if (i.isFunctionFrame()) MaybeDumpValue("this", i.thisArgument(cx), out);
    if (!i.isJSJit()) {
      out.put("  rval: ");
    ...
    out.put("  flags:");
    if (i.isConstructing()) out.put(" constructing");
    if (!i.isJSJit() && i.interpFrame()->isDebuggerEvalFrame())
      out.put(" debugger eval");
    if (i.isEvalFrame()) out.put(" eval");
    out.putChar('\n');
...
js::gc::AllocKind JSObject::allocKindForTenure(
  ...
  if (IsProxy(this)) return as<ProxyObject>().allocKindForTenure();
  ...
  if (is<OutlineTypedObject>()) return AllocKind::OBJECT0;
...
size_t JSObject::sizeOfIncludingThisInNursery() const {
  ...
  if (is<NativeObject>()) {
    ...
    if (is<ArgumentsObject>()) size += as<ArgumentsObject>().sizeOfData();
  }
...
JS::ubi::Node::Size JS::ubi::Concrete<JSObject>::size(
  ...
  if (!obj.isTenured()) return obj.sizeOfIncludingThisInNursery();
...
void JSObject::traceChildren(JSTracer* trc) {
  ...
  if (is<ShapedObject>()) as<ShapedObject>().traceShape(trc);
  ...
  if (clasp->hasTrace()) clasp->doTrace(trc, this);
}
...
JSAtom* JSObject::maybeConstructorDisplayAtom() const {
  if (hasLazyGroup()) return nullptr;
...
MOZ_MUST_USE JSObject* js::SpeciesConstructor(
  ...
  // Step 3.
  if (ctor.isUndefined()) return defaultCtor;
  ...
  if (!GetProperty(cx, ctorObj, ctor, speciesId, &s)) return nullptr;

  // Step 6.
  if (s.isNullOrUndefined()) return defaultCtor;

  // Step 7.
  if (IsConstructor(s)) return &s.toObject();
...
MOZ_MUST_USE JSObject* js::SpeciesConstructor(
  ...
  RootedObject defaultCtor(cx,
                           &cx->global()->getConstructor(ctorKey).toObject());
