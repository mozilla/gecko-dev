...
JSCompartment::JSCompartment(Zone* zone, const JS::CompartmentOptions& options =
...
void JSCompartment::checkWrapperMapAfterMovingGC() {
  /*
...
bool JSCompartment::putWrapper(JSContext* cx,
                               const CrossCompartmentKey& wrapped,
...
static JSString* CopyStringPure(JSContext* cx, JSString* str) {
  ...
  if (str->isLinear()) {
    ...
    if (!chars.init(cx, str)) return nullptr;
  ...
  if (str->hasLatin1Chars()) {
    ...
    if (!str->asRope().copyLatin1CharsZ(cx, copiedChars)) return nullptr;

    return NewString<CanGC>(cx, copiedChars.forget(), len);
  }

  ScopedJSFreePtr<char16_t> copiedChars;
  if (!str->asRope().copyTwoByteCharsZ(cx, copiedChars)) return nullptr;

  return NewStringDontDeflate<CanGC>(cx, copiedChars.forget(), len);
}

bool JSCompartment::wrap(JSContext* cx, MutableHandleString strp) {
  MOZ_ASSERT(!cx->runtime()->isAtomsCompartment(this));
  MOZ_ASSERT(cx->compartment() == this);

  /* If the string is already in this compartment, we are done. */
  JSString* str = strp;
  if (str->zoneFromAnyThread() == zone()) return true;
...
bool JSCompartment::getNonWrapperObjectForCurrentCompartment(
  ...
  if (!CheckSystemRecursionLimit(cx)) return false;
  if (preWrap) {
...
bool JSCompartment::getOrCreateWrapper(JSContext* cx, HandleObject existing,
...
bool JSCompartment::wrap(JSContext* cx, MutableHandleObject obj) {
  MOZ_ASSERT(!cx->runtime()->isAtomsCompartment(this));
  MOZ_ASSERT(cx->compartment() == this);
  ...
  if (!getNonWrapperObjectForCurrentCompartment(cx, obj)) return false;
  ...
  if (obj->compartment() != this) {
    if (!getOrCreateWrapper(cx, nullptr, obj)) return false;
  }
...
bool JSCompartment::rewrap(JSContext* cx, MutableHandleObject obj,
  ...
  if (obj->compartment() == this) return true;
...
bool JSCompartment::wrap(JSContext* cx,
  ...
  if (!wrap(cx, desc.object())) return false;

  if (desc.hasGetterObject()) {
    if (!wrap(cx, desc.getterObject())) return false;
  }
  if (desc.hasSetterObject()) {
    if (!wrap(cx, desc.setterObject())) return false;
  }

  return wrap(cx, desc.value());
}

bool JSCompartment::wrap(JSContext* cx, MutableHandle<GCVector<Value>> vec) {
  for (size_t i = 0; i < vec.length(); ++i) {
    if (!wrap(cx, vec[i])) return false;
  }
  return true;
}
...
LexicalEnvironmentObject* JSCompartment::getNonSyntacticLexicalEnvironment(
    JSObject* enclosing) const {
  if (!nonSyntacticLexicalEnvironments_) return nullptr;
...
bool JSCompartment::addToVarNames(JSContext* cx, JS::Handle<JSAtom*> name) {
  MOZ_ASSERT(name);
  MOZ_ASSERT(!isAtomsCompartment());

  if (varNames_.put(name)) return true;

  ReportOutOfMemory(cx);
  return false;
}

void JSCompartment::traceOutgoingCrossCompartmentWrappers(JSTracer* trc) {
...
/* static */ void JSCompartment::traceIncomingCrossCompartmentEdgesForZoneGC(
    JSTracer* trc) {
...
void JSCompartment::traceGlobal(JSTracer* trc) {
  ...
  if (!JS::CurrentThreadIsHeapMinorCollecting()) varNames_.trace(trc);
}

void JSCompartment::traceRoots(
    JSTracer* trc, js::gc::GCRuntime::TraceOrMarkRuntime traceOrMark) {
  if (objectMetadataState.is<PendingMetadata>()) {
    TraceRoot(trc, &objectMetadataState.as<PendingMetadata>(),
  ...
  if (debugEnvs) debugEnvs->trace(trc);
...
void JSCompartment::finishRoots() {
  if (debugEnvs) debugEnvs->finish();

  if (lazyArrayBuffers) lazyArrayBuffers->clear();

  if (objectMetadataTable) objectMetadataTable->clear();
...
void JSCompartment::sweepGlobalObject() {
  if (global_ && IsAboutToBeFinalized(&global_)) global_.set(nullptr);
}

void JSCompartment::sweepSelfHostingScriptSource() {
...
void JSCompartment::sweepJitCompartment() {
  if (jitCompartment_) jitCompartment_->sweep(this);
}

void JSCompartment::sweepRegExps() {
  /*
...
void JSCompartment::sweepDebugEnvironments() {
  if (debugEnvs) debugEnvs->sweep();
}

void JSCompartment::sweepNativeIterators() {
  /* Sweep list of native iterators. */
  NativeIterator* ni = enumerators->next();
  while (ni != enumerators) {
    JSObject* iterObj = ni->iterObj();
    NativeIterator* next = ni->next();
    if (gc::IsAboutToBeFinalizedUnbarriered(&iterObj)) ni->unlink();
    ni = next;
  }
}
...
void JSCompartment::sweepVarNames() { varNames_.sweep(); }
...
void JSCompartment::sweepTemplateObjects() {
...
/* static */ void JSCompartment::fixupCrossCompartmentWrappersAfterMovingGC(
    JSTracer* trc) {
  MOZ_ASSERT(trc->runtime()->gc.isHeapCompacting());

  for (CompartmentsIter comp(trc->runtime(), SkipAtoms); !comp.done();
       comp.next()) {
...
void JSCompartment::fixupGlobal() {
  GlobalObject* global = *global_.unsafeGet();
  if (global) global_.set(MaybeForwarded(global));
}

void JSCompartment::fixupScriptMapsAfterMovingGC() {
...
#ifdef JSGC_HASH_TABLE_CHECKS
void JSCompartment::checkScriptMapsAfterMovingGC() {
  if (scriptCountsMap) {
...
void JSCompartment::clearTables() {
  global_.set(nullptr);
  ...
  if (savedStacks_.initialized()) savedStacks_.clear();
  if (varNames_.initialized()) varNames_.clear();
}

void JSCompartment::setAllocationMetadataBuilder(
...
void JSCompartment::clearObjectMetadata() {
  js_delete(objectMetadataTable);
  objectMetadataTable = nullptr;
}

void JSCompartment::setNewObjectMetadata(JSContext* cx, HandleObject obj) {
  assertSameCompartment(cx, this, obj);

  AutoEnterOOMUnsafeRegion oomUnsafe;
  if (JSObject* metadata =
          allocationMetadataBuilder->build(cx, obj, oomUnsafe)) {
...
static bool AddInnerLazyFunctionsFromScript(JSScript* script,
  ...
  if (!script->hasObjects()) return true;
  ObjectArray* objects = script->objects();
  for (size_t i = 0; i < objects->length; i++) {
    JSObject* obj = objects->vector[i];
    if (obj->is<JSFunction>() && obj->as<JSFunction>().isInterpretedLazy()) {
      if (!lazyFunctions.append(obj)) return false;
    }
  }
  return true;
}

static bool AddLazyFunctionsForCompartment(JSContext* cx,
                                           AutoObjectVector& lazyFunctions,
                                           AllocKind kind) {
...
static bool CreateLazyScriptsForCompartment(JSContext* cx) {
  ...
  if (!AddLazyFunctionsForCompartment(cx, lazyFunctions,
  ...
  for (size_t i = 0; i < lazyFunctions.length(); i++) {
    ...
    // a lazy script.
    if (!fun->isInterpretedLazy()) continue;
...
bool JSCompartment::ensureDelazifyScriptsForDebugger(JSContext* cx) {
...
void JSCompartment::updateDebuggerObservesFlag(unsigned flag) {
  ...
  GlobalObject* global =
      zone()->runtimeFromActiveCooperatingThread()->gc.isForegroundSweeping()
...
void JSCompartment::unsetIsDebuggee() {
  if (isDebuggee()) {
...
void JSCompartment::updateDebuggerObservesCoverage() {
  bool previousState = debuggerObservesCoverage();
  updateDebuggerObservesFlag(DebuggerObservesCoverage);
  if (previousState == debuggerObservesCoverage()) return;
...
bool JSCompartment::collectCoverage() const {
  return collectCoverageForPGO() || collectCoverageForDebug();
}

bool JSCompartment::collectCoverageForPGO() const {
  return !JitOptions.disablePgo;
}

bool JSCompartment::collectCoverageForDebug() const {
  return debuggerObservesCoverage() ||
         runtimeFromAnyThread()->profilingScripts ||
         runtimeFromAnyThread()->lcovOutput().isEnabled();
}

void JSCompartment::clearScriptCounts() {
  if (!scriptCountsMap) return;
...
void JSCompartment::clearScriptNames() {
  if (!scriptNameMap) return;
...
void JSCompartment::clearBreakpointsIn(FreeOp* fop, js::Debugger* dbg,
...
void JSCompartment::addSizeOfIncludingThis(
    mozilla::MallocSizeOf mallocSizeOf, size_t* tiAllocationSiteTables,
    size_t* tiArrayTypeTables, size_t* tiObjectTypeTables,
    size_t* compartmentObject, size_t* compartmentTables, size_t* innerViewsArg,
    size_t* lazyArrayBuffersArg, size_t* objectMetadataTablesArg,
    size_t* crossCompartmentWrappersArg, size_t* savedStacksSet,
    size_t* varNamesSet, size_t* nonSyntacticLexicalEnvironmentsArg,
    size_t* jitCompartment, size_t* privateData, size_t* scriptCountsMapArg) {
  ...
  *varNamesSet += varNames_.sizeOfExcludingThis(mallocSizeOf);
  ...
  if (callback) *privateData += callback(mallocSizeOf, this);
...
void JSCompartment::reportTelemetry() {
  // Only report telemetry for web content, not add-ons or chrome JS.
  if (creationOptions_.addonIdOrNull() || isSystem_) return;
  ...
  for (size_t i = 0; i < size_t(DeprecatedLanguageExtension::Count); i++) {
    if (sawDeprecatedLanguageExtension[i])
      runtime_->addTelemetry(
          JS_TELEMETRY_DEPRECATED_LANGUAGE_EXTENSIONS_IN_CONTENT, i);
  }
}

void JSCompartment::addTelemetry(const char* filename,
                                 DeprecatedLanguageExtension e) {
  // Only report telemetry for web content, not add-ons or chrome JS.
  if (creationOptions_.addonIdOrNull() || isSystem_) return;
  if (!filename || strncmp(filename, "http", 4) != 0) return;

  sawDeprecatedLanguageExtension[size_t(e)] = true;
}

HashNumber JSCompartment::randomHashCode() {
  ensureRandomNumberGenerator();
  return HashNumber(randomNumberGenerator.ref().next());
}

mozilla::HashCodeScrambler JSCompartment::randomHashCodeScrambler() {
...
AutoSetNewObjectMetadata::AutoSetNewObjectMetadata(
  ...
  if (cx_)
    cx_->compartment()->objectMetadataState =
...
AutoSetNewObjectMetadata::~AutoSetNewObjectMetadata() {
  ...
  if (!cx_->isExceptionPending() &&
      cx_->compartment()->hasObjectPendingMetadata()) {
    ...
    JSObject* obj =
        cx_->compartment()->objectMetadataState.as<PendingMetadata>();
