...
extern jsbytecode* UnwindEnvironmentToTryPc(JSScript* script, JSTryNote* tn);
...
class MOZ_STACK_CLASS TryNoteIter {
  ...
  void settle() {
    for (; tn_ != tnEnd_; ++tn_) {
      ...
      if (pcOffset_ - tn_->start >= tn_->length) continue;
       ...
       * filter them out.
       */
      if (tn_->stackDepth <= getStackDepth_()) break;
    }
  }
...
bool ThrowingOperation(JSContext* cx, HandleValue v);
