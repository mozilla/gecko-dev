...
static inline JSScript* GetOrCreateFunctionScript(JSContext* cx,
  ...
  if (!EnsureFunctionHasScript(cx, fun)) return nullptr;
...
class AutoRestoreCompartmentDebugMode {
 ...
 public:
  ...
  ~AutoRestoreCompartmentDebugMode() {
    if (comp_) comp_->debugModeBits = bits_;
  }

  void release() { comp_ = nullptr; }
};
 ...
 public:
  ...
  static bool isLockedInStack(JSContext* cx, Debugger& dbg) {
    for (EnterDebuggeeNoExecute* it = cx->noExecuteDebuggerTop; it;
         it = it->prev_) {
      if (&it->debugger() == &dbg) return !it->unlocked_;
    }
    return false;
  }
#endif
...
static bool ValueToStableChars(JSContext* cx, const char* fnname,
  ...
  if (!stableChars.initTwoByte(cx, linear)) return false;
  return true;
}

EvalOptions::~EvalOptions() { js_free(const_cast<char*>(filename_)); }
...
static bool ParseEvalOptions(JSContext* cx, HandleValue value,
  ...
  RootedValue v(cx);
  if (!JS_GetProperty(cx, opts, "url", &v)) return false;
  if (!v.isUndefined()) {
    RootedString url_str(cx, ToString<CanGC>(cx, v));
    if (!url_str) return false;
    JSAutoByteString url_bytes(cx, url_str);
    if (!url_bytes) return false;
    if (!options.setFilename(cx, url_bytes.ptr())) return false;
  }

  if (!JS_GetProperty(cx, opts, "lineNumber", &v)) return false;
  if (!v.isUndefined()) {
    uint32_t lineno;
    if (!ToUint32(cx, v, &lineno)) return false;
...
void BreakpointSite::inc(FreeOp* fop) {
  enabledCount++;
  if (enabledCount == 1) recompile(fop);
}

void BreakpointSite::dec(FreeOp* fop) {
  MOZ_ASSERT(enabledCount > 0);
  enabledCount--;
  if (enabledCount == 0) recompile(fop);
}
...
void Breakpoint::destroy(FreeOp* fop) {
  if (debugger->enabled) site->dec(fop);
...
void JSBreakpointSite::destroyIfEmpty(FreeOp* fop) {
  if (isEmpty()) script->destroyBreakpointSite(fop, pc);
}
...
void WasmBreakpointSite::destroyIfEmpty(FreeOp* fop) {
  if (isEmpty()) debug->destroyBreakpointSite(fop, offset);
}
...
bool Debugger::getScriptFrameWithIter(JSContext* cx, AbstractFramePtr referent,
...
bool Debugger::getScriptFrameWithIter(JSContext* cx, AbstractFramePtr referent,
  ...
  if (!p) {
    ...
    RootedDebuggerFrame frame(
        cx, DebuggerFrame::create(cx, proto, referent, maybeIter, debugger));
    if (!frame) return false;

    if (!ensureExecutionObservabilityOfFrame(cx, referent)) return false;
...
/* static */ bool Debugger::hasLiveHook(GlobalObject* global, Hook which) {
  if (GlobalObject::DebuggerVector* debuggers = global->getDebuggers()) {
    for (auto p = debuggers->begin(); p != debuggers->end(); p++) {
      Debugger* dbg = *p;
      if (dbg->enabled && dbg->getHook(which)) return true;
    }
  }
  return false;
}
...
bool Debugger::hasAnyLiveHooks(JSRuntime* rt) const {
  ...
  for (Breakpoint* bp = firstBreakpoint(); bp; bp = bp->nextInDebugger()) {
    switch (bp->site->type()) {
      case BreakpointSite::Type::JS:
        if (IsMarkedUnbarriered(rt, &bp->site->asJS()->script)) return true;
        break;
      case BreakpointSite::Type::Wasm:
        if (IsMarkedUnbarriered(rt, &bp->asWasm()->wasmInstance)) return true;
        break;
    }
  }
  ...
  for (FrameMap::Range r = frames.all(); !r.empty(); r.popFront()) {
    ...
    if (!frameObj->getReservedSlot(JSSLOT_DEBUGFRAME_ONSTEP_HANDLER)
             .isUndefined() ||
        !frameObj->getReservedSlot(JSSLOT_DEBUGFRAME_ONPOP_HANDLER)
             .isUndefined())
      return true;
  }
...
/* static */ JSTrapStatus Debugger::slowPathOnEnterFrame(
    JSContext* cx, AbstractFramePtr frame) {
  RootedValue rval(cx);
  JSTrapStatus status = dispatchHook(cx,
...
/* static */ bool Debugger::slowPathOnLeaveFrame(JSContext* cx,
  ...
  if (!getDebuggerFrames(frame, &frames)) return false;
  if (frames.empty()) return frameOk;

  /* Save the frame's completion value. */
  JSTrapStatus status;
  RootedValue value(cx);
  Debugger::resultToCompletion(cx, frameOk, frame.returnValue(), &status,
                               &value);
  ...
  if (!cx->isThrowingOverRecursed() && !cx->isThrowingOutOfMemory()) {
    ...
    for (size_t i = 0; i < frames.length(); i++) {
      ...
      if (dbg->enabled && frameobj->onPopHandler()) {
        ...
        nextStatus = dbg->processParsedHandlerResult(ac, frame, pc, success,
                                                     nextStatus, &nextValue);

        /*
         * At this point, we are back in the debuggee compartment, and any error
         * has been wrapped up as a completion value.
         */
...
/* static */ JSTrapStatus Debugger::slowPathOnDebuggerStatement(
    JSContext* cx, AbstractFramePtr frame) {
  RootedValue rval(cx);
...
/* static */ JSTrapStatus Debugger::slowPathOnExceptionUnwind(
    JSContext* cx, AbstractFramePtr frame) {
  ...
  if (frame.hasScript() && frame.script()->selfHosted()) return JSTRAP_CONTINUE;
...
bool Debugger::wrapEnvironment(JSContext* cx, Handle<Env*> env,
  ...
  if (!wrapEnvironment(cx, env, &envobj)) return false;
...
bool Debugger::wrapDebuggeeValue(JSContext* cx, MutableHandleValue vp) {
  ...
  if (vp.isObject()) {
    ...
    if (!wrapDebuggeeObject(cx, obj, &dobj)) return false;

    vp.setObject(*dobj);
  } else if (vp.isMagic()) {
    RootedPlainObject optObj(cx, NewBuiltinClassInstance<PlainObject>(cx));
    if (!optObj) return false;
    ...
    if (!DefineDataProperty(cx, optObj, name, trueVal)) return false;
...
bool Debugger::wrapDebuggeeObject(JSContext* cx, HandleObject obj,
  ...
  if (obj->is<JSFunction>()) {
    ...
    if (!EnsureFunctionHasScript(cx, fun)) return false;
  }
...
bool Debugger::unwrapDebuggeeValue(JSContext* cx, MutableHandleValue vp) {
  ...
  if (vp.isObject()) {
    ...
    if (!unwrapDebuggeeObject(cx, &dobj)) return false;
    vp.setObject(*dobj);
  }
  return true;
}
...
bool Debugger::unwrapPropertyDescriptor(
  ...
  if (desc.hasGetterObject()) {
    ...
    if (get) {
      if (!unwrapDebuggeeObject(cx, &get)) return false;
  ...
  if (desc.hasSetterObject()) {
    ...
    if (set) {
      if (!unwrapDebuggeeObject(cx, &set)) return false;
...
JSTrapStatus Debugger::reportUncaughtException(Maybe<AutoCompartment>& ac) {
...
JSTrapStatus Debugger::handleUncaughtExceptionHelper(
    Maybe<AutoCompartment>& ac, MutableHandleValue* vp,
  ...
  if (cx->isExceptionPending()) {
    if (uncaughtExceptionHook) {
      RootedValue exc(cx);
      if (!cx->getPendingException(&exc)) return JSTRAP_ERROR;
      cx->clearPendingException();
...
JSTrapStatus Debugger::handleUncaughtException(
    Maybe<AutoCompartment>& ac, MutableHandleValue vp,
    const Maybe<HandleValue>& thisVForCheck, AbstractFramePtr frame) {
  return handleUncaughtExceptionHelper(ac, &vp, thisVForCheck, frame);
}

JSTrapStatus Debugger::handleUncaughtException(Maybe<AutoCompartment>& ac) {
  return handleUncaughtExceptionHelper(ac, nullptr, mozilla::Nothing(),
                                       NullFramePtr());
}
...
/* static */ void Debugger::resultToCompletion(JSContext* cx, bool ok,
  ...
  if (ok) {
    ...
    *status = JSTRAP_THROW;
    if (!cx->getPendingException(value)) *status = JSTRAP_ERROR;
    cx->clearPendingException();
  } else {
    *status = JSTRAP_ERROR;
    value.setUndefined();
...
bool Debugger::newCompletionValue(JSContext* cx, JSTrapStatus status,
                                  const Value& value_,
...
bool Debugger::receiveCompletionValue(Maybe<AutoCompartment>& ac, bool ok,
...
static bool GetStatusProperty(JSContext* cx, HandleObject obj,
                              HandlePropertyName name, JSTrapStatus status,
                              JSTrapStatus& statusp, MutableHandleValue vp,
                              int* hits) {
  bool found;
  if (!HasProperty(cx, obj, name, &found)) return false;
  if (found) {
    ++*hits;
    statusp = status;
    if (!GetProperty(cx, obj, obj, name, vp)) return false;
  }
  return true;
}

static bool ParseResumptionValueAsObject(JSContext* cx, HandleValue rv,
                                         JSTrapStatus& statusp,
                                         MutableHandleValue vp) {
  int hits = 0;
  if (rv.isObject()) {
    RootedObject obj(cx, &rv.toObject());
    if (!GetStatusProperty(cx, obj, cx->names().return_, JSTRAP_RETURN, statusp,
                           vp, &hits))
      return false;
    if (!GetStatusProperty(cx, obj, cx->names().throw_, JSTRAP_THROW, statusp,
                           vp, &hits))
      return false;
  }
...
static bool GetThisValueForCheck(JSContext* cx, AbstractFramePtr frame,
  ...
  if (frame.debuggerNeedsCheckPrimitiveReturn()) {
    ...
    if (!cx->compartment()->wrap(cx, thisv)) return false;
...
bool Debugger::processResumptionValue(Maybe<AutoCompartment>& ac,
                                      AbstractFramePtr frame,
                                      const Maybe<HandleValue>& maybeThisv,
...
JSTrapStatus Debugger::processParsedHandlerResultHelper(
    Maybe<AutoCompartment>& ac, AbstractFramePtr frame,
    const Maybe<HandleValue>& maybeThisv, bool success, JSTrapStatus status,
    MutableHandleValue vp) {
  if (!success) return handleUncaughtException(ac, vp, maybeThisv, frame);
...
JSTrapStatus Debugger::processParsedHandlerResult(Maybe<AutoCompartment>& ac,
                                                  AbstractFramePtr frame,
  ...
  return processParsedHandlerResultHelper(ac, frame, maybeThisv, success,
                                          status, vp);
}

JSTrapStatus Debugger::processHandlerResult(Maybe<AutoCompartment>& ac,
                                            bool success, const Value& rv,
                                            AbstractFramePtr frame,
                                            jsbytecode* pc,
                                            MutableHandleValue vp) {
  JSContext* cx = ac->context();
...
static bool CallMethodIfPresent(JSContext* cx, HandleObject obj,
  ...
  if (!IsCallable(fval)) return true;

  InvokeArgs args(cx);
  if (!args.init(cx, argc)) return false;

  for (size_t i = 0; i < argc; i++) args[i].set(argv[i]);

  rval.setObject(*obj);  // overwritten by successful Call
  return js::Call(cx, fval, rval, args, rval);
}

JSTrapStatus Debugger::fireDebuggerStatement(JSContext* cx,
                                             MutableHandleValue vp) {
  ...
  return processHandlerResult(ac, ok, rv, iter.abstractFramePtr(), iter.pc(),
                              vp);
}

JSTrapStatus Debugger::fireExceptionUnwind(JSContext* cx,
                                           MutableHandleValue vp) {
  ...
  JSTrapStatus st =
      processHandlerResult(ac, ok, rv, iter.abstractFramePtr(), iter.pc(), vp);
  if (st == JSTRAP_CONTINUE) cx->setPendingException(exc);
  return st;
}

JSTrapStatus Debugger::fireEnterFrame(JSContext* cx, MutableHandleValue vp) {
...
void Debugger::fireNewScript(JSContext* cx,
  ...
  RootedValue rv(cx);
  if (!js::Call(cx, fval, object, dsval, &rv)) handleUncaughtException(ac);
}
...
void Debugger::fireOnGarbageCollectionHook(
  ...
  RootedValue rv(cx);
  if (!js::Call(cx, fval, object, dataVal, &rv)) handleUncaughtException(ac);
}

template <typename HookIsEnabledFun /* bool (Debugger*) */,
          typename FireHookFun /* JSTrapStatus (Debugger*) */>
/* static */ JSTrapStatus Debugger::dispatchHook(JSContext* cx,
  ...
  for (Value* p = triggered.begin(); p != triggered.end(); p++) {
    ...
    if (dbg->debuggees.has(global) && dbg->enabled && hookIsEnabled(dbg)) {
      ...
      if (st != JSTRAP_CONTINUE) return st;
    }
  }
  return JSTRAP_CONTINUE;
}
...
/* static */ JSTrapStatus Debugger::onTrap(JSContext* cx,
                                           MutableHandleValue vp) {
  FrameIter iter(cx);
  ...
  for (Breakpoint* bp = site->firstBreakpoint(); bp; bp = bp->nextInSite()) {
    ...
    if (!isJS && &bp->asWasm()->wasmInstance->instance() != iter.wasmInstance())
      continue;
    if (!triggered.append(bp)) return JSTRAP_ERROR;
  }
  ...
  for (Breakpoint** p = triggered.begin(); p != triggered.end(); p++) {
    ...
    if (!site || !site->hasBreakpoint(bp)) continue;
    ...
    if (hasDebuggee) {
      ...
      JSTrapStatus st = dbg->processHandlerResult(
          ac, ok, rv, iter.abstractFramePtr(), iter.pc(), vp);
      if (st != JSTRAP_CONTINUE) return st;
...
/* static */ JSTrapStatus Debugger::onSingleStep(JSContext* cx,
                                                 MutableHandleValue vp) {
  FrameIter iter(cx);
   ...
   */
  Rooted<DebuggerFrameVector> frames(cx, DebuggerFrameVector(cx));
  if (!getDebuggerFrames(iter.abstractFramePtr(), &frames)) return JSTRAP_ERROR;
    ...
    if (GlobalObject::DebuggerVector* debuggers = global->getDebuggers()) {
      for (auto p = debuggers->begin(); p != debuggers->end(); p++) {
        ...
        for (FrameMap::Range r = dbg->frames.all(); !r.empty(); r.popFront()) {
          ...
          NativeObject* frameobj = r.front().value();
          if (frame.isWasmDebugFrame()) continue;
  ...
  for (size_t i = 0; i < frames.length(); i++) {
    ...
    status = dbg->processParsedHandlerResult(ac, iter.abstractFramePtr(),
                                             iter.pc(), success, status, vp);
    if (status != JSTRAP_CONTINUE) return status;
  }
...
JSTrapStatus Debugger::fireNewGlobalObject(JSContext* cx,
  ...
  JSTrapStatus status = ok ? JSTRAP_CONTINUE : handleUncaughtException(ac, vp);
  MOZ_ASSERT(!cx->isExceptionPending());
  return status;
}
...
void Debugger::slowPathOnNewGlobalObject(JSContext* cx,
  ...
  if (global->compartment()->creationOptions().invisibleToDebugger()) return;
  ...
  for (size_t i = 0; i < watchers.length(); i++) {
    ...
    if (dbg->observesNewGlobalObject()) {
      ...
      if (status != JSTRAP_CONTINUE && status != JSTRAP_RETURN) break;
    }
  }
  MOZ_ASSERT(!cx->isExceptionPending());
}
...
/* static */ bool Debugger::slowPathOnLogAllocationSite(
  ...
  for (auto dbgp = dbgs.begin(); dbgp < dbgs.end(); dbgp++) {
    if (!activeDebuggers.append((*dbgp)->object)) return false;
  }
...
bool Debugger::isDebuggeeUnbarriered(const JSCompartment* compartment) const {
  MOZ_ASSERT(compartment);
  return compartment->isDebuggee() &&
         debuggees.has(compartment->unsafeUnbarrieredMaybeGlobal());
}
...
bool Debugger::appendAllocationSite(JSContext* cx, HandleObject obj,
  ...
  if (!cx->compartment()->wrap(cx, &wrappedFrame)) return false;

  RootedAtom ctorName(cx);
  {
    AutoCompartment ac(cx, obj);
    if (!JSObject::constructorDisplayAtom(cx, obj, &ctorName)) return false;
  }
  if (ctorName) cx->markAtom(ctorName);
...
JSTrapStatus Debugger::firePromiseHook(JSContext* cx, Hook hook,
  ...
  if (!wrapDebuggeeValue(cx, &dbgObj)) return reportUncaughtException(ac);
...
/* static */ void Debugger::slowPathPromiseHook(
  ...
  Maybe<AutoCompartment> ac;
  if (hook == OnNewPromise) ac.emplace(cx, promise);
 ...
 public:
  ...
  bool shouldRecompileOrInvalidate(JSScript* script) const override {
    return script->hasBaselineScript() &&
           compartments_.has(script->compartment());
  }
  bool shouldMarkAsDebuggee(FrameIter& iter) const override {
    ...
    return iter.hasUsableAbstractFramePtr() &&
           compartments_.has(iter.compartment());
  }
...
class MOZ_RAII ExecutionObservableFrame
 ...
 public:
  ...
  bool shouldRecompileOrInvalidate(JSScript* script) const override {
    ...
    // frame_.script().
    if (!script->hasBaselineScript()) return false;

    if (frame_.hasScript() && script == frame_.script()) return true;
...
static bool UpdateExecutionObservabilityOfScriptsInZone(
  ...
  // appending them to a vector for discarding their baseline scripts later.
  {
    ...
    if (JSScript* script = obs.singleScriptForZoneInvalidation()) {
      if (obs.shouldRecompileOrInvalidate(script)) {
        if (!AppendAndInvalidateScript(cx, zone, script, scripts)) return false;
      }
    } else {
  ...
  for (const CooperatingContext& target :
    ...
    for (JitActivationIterator actIter(cx, target); !actIter.done();
         ++actIter) {
      if (actIter->compartment()->zone() != zone) continue;
  ...
  for (JSCompartment* c : zone->compartments()) {
    for (wasm::Instance* instance : c->wasm.instances()) {
      if (!instance->debugEnabled()) continue;
...
/* static */ void Debugger::forEachDebuggerFrame(AbstractFramePtr frame,
  ...
  if (GlobalObject::DebuggerVector* debuggers = global->getDebuggers()) {
    for (auto p = debuggers->begin(); p != debuggers->end(); p++) {
      Debugger* dbg = *p;
      if (FrameMap::Ptr entry = dbg->frames.lookup(frame)) fn(entry->value());
    }
  }
}
...
/* static */ bool Debugger::getDebuggerFrames(
  ...
  forEachDebuggerFrame(frame, [&](DebuggerFrame* frameobj) {
    if (!hadOOM && !frames.append(frameobj)) hadOOM = true;
  });
  return !hadOOM;
}
...
/* static */ bool Debugger::updateExecutionObservability(
  ...
  if (!obs.singleZone() && obs.zones()->empty()) return true;
...
/* static */ bool Debugger::ensureExecutionObservabilityOfScript(
  ...
  if (script->isDebuggee()) return true;
...
/* static */ bool Debugger::ensureExecutionObservabilityOfFrame(
  ...
  if (frame.isDebuggee()) return true;
  ExecutionObservableFrame obs(frame);
  return updateExecutionObservabilityOfFrames(cx, obs, Observing);
}

/* static */ bool Debugger::ensureExecutionObservabilityOfCompartment(
    JSContext* cx, JSCompartment* comp) {
  if (comp->debuggerObservesAllExecution()) return true;
  ExecutionObservableCompartments obs(cx);
  if (!obs.init() || !obs.add(comp)) return false;
...
Debugger::IsObserving Debugger::observesAllExecution() const {
  if (enabled && !!getHook(OnEnterFrame)) return Observing;
  return NotObserving;
}

Debugger::IsObserving Debugger::observesAsmJS() const {
  if (enabled && !allowUnobservedAsmJS) return Observing;
  return NotObserving;
}

Debugger::IsObserving Debugger::observesBinarySource() const {
  if (enabled && allowWasmBinarySource) return Observing;
  return NotObserving;
}

Debugger::IsObserving Debugger::observesCoverage() const {
  if (enabled && collectCoverageInfo) return Observing;
  return NotObserving;
}
...
bool Debugger::updateObservesAllExecutionOnDebuggees(JSContext* cx,
  ...
  for (WeakGlobalObjectSet::Range r = debuggees.all(); !r.empty();
    ...
    if (comp->debuggerObservesAllExecution() == observing) continue;
    ...
    if (observing && !obs.add(comp)) return false;
  }

  if (!updateExecutionObservability(cx, obs, observing)) return false;
...
bool Debugger::updateObservesCoverageOnDebuggees(JSContext* cx,
  ...
  for (WeakGlobalObjectSet::Range r = debuggees.all(); !r.empty();
    ...
    if (comp->debuggerObservesCoverage() == observing) continue;
...
void Debugger::updateObservesAsmJSOnDebuggees(IsObserving observing) {
  for (WeakGlobalObjectSet::Range r = debuggees.all(); !r.empty();
    ...
    if (comp->debuggerObservesAsmJS() == observing) continue;

    comp->updateDebuggerObservesAsmJS();
  }
}

void Debugger::updateObservesBinarySourceDebuggees(IsObserving observing) {
  for (WeakGlobalObjectSet::Range r = debuggees.all(); !r.empty();
       r.popFront()) {
    GlobalObject* global = r.front();
    JSCompartment* comp = global->compartment();

    if (comp->debuggerObservesBinarySource() == observing) continue;
...
/* static */ bool Debugger::addAllocationsTracking(
  ...
  debuggee->compartment()->setAllocationMetadataBuilder(
...
/* static */ bool Debugger::markIteratively(GCMarker* marker) {
  ...
  for (CompartmentsIter c(rt, SkipAtoms); !c.done(); c.next()) {
    if (c->isDebuggee()) {
      ...
      if (!IsMarkedUnbarriered(rt, &global)) continue;
      ...
      for (auto p = debuggers->begin(); p != debuggers->end(); p++) {
        ...
        if (!dbgobj->zone()->isGCMarking()) continue;
...
/* static */ void Debugger::traceAllForMovingGC(JSTracer* trc) {
  JSRuntime* rt = trc->runtime();
  for (ZoneGroupsIter group(rt); !group.done(); group.next()) {
    for (Debugger* dbg : group->debuggerList()) dbg->traceForMovingGC(trc);
  }
}
...
/* static */ void Debugger::traceObject(JSTracer* trc, JSObject* obj) {
  if (Debugger* dbg = Debugger::fromJSObject(obj)) dbg->trace(trc);
}
...
/* static */ void Debugger::sweepAll(FreeOp* fop) {
  ...
  for (ZoneGroupsIter group(rt); !group.done(); group.next()) {
    ...
    while (dbg) {
      ...
      if (debuggerDying) fop->delete_(dbg);
...
/* static */ void Debugger::findZoneEdges(Zone* zone,
  ...
  for (ZoneGroupsIter group(zone->runtimeFromActiveCooperatingThread());
       !group.done(); group.next()) {
    for (Debugger* dbg : group->debuggerList()) {
      Zone* debuggerZone = dbg->object->zone();
      if (!debuggerZone->isGCMarking()) continue;
      ...
      if (debuggerZone == zone) {
        ...
        for (auto e = dbg->debuggeeZones.all(); !e.empty(); e.popFront()) {
          ...
          if (debuggeeZone->isGCMarking()) finder.addEdgeTo(debuggeeZone);
        }
      } else {
        /*
...
/* static */ bool Debugger::setEnabled(JSContext* cx, unsigned argc,
  ...
  if (!args.requireAtLeast(cx, "Debugger.set enabled", 1)) return false;
...
/* static */ bool Debugger::setHookImpl(JSContext* cx, CallArgs& args,
                                        Debugger& dbg, Hook which) {
  MOZ_ASSERT(which >= 0 && which < HookCount);
  if (!args.requireAtLeast(cx, "Debugger.setHook", 1)) return false;
  if (args[0].isObject()) {
...
/* static */ bool Debugger::setOnNewGlobalObject(JSContext* cx, unsigned argc,
  ...
  if (!setHookImpl(cx, args, *dbg, OnNewGlobalObject)) return false;
...
/* static */ bool Debugger::getAllowWasmBinarySource(JSContext* cx,
                                                     unsigned argc, Value* vp) {
...
/* static */ bool Debugger::setAllowWasmBinarySource(JSContext* cx,
                                                     unsigned argc, Value* vp) {
...
/* static */ bool Debugger::setCollectCoverageInfo(JSContext* cx, unsigned argc,
  ...
  if (!dbg->updateObservesCoverageOnDebuggees(cx, observing)) return false;
...
GlobalObject* Debugger::unwrapDebuggeeArgument(JSContext* cx, const Value& v) {
  ...
  if (obj->getClass() == &DebuggerObject::class_) {
    RootedValue rv(cx, v);
    if (!unwrapDebuggeeValue(cx, &rv)) return nullptr;
    obj = &rv.toObject();
  }
...
/* static */ bool Debugger::addDebuggee(JSContext* cx, unsigned argc,
  ...
  if (!args.requireAtLeast(cx, "Debugger.addDebuggee", 1)) return false;
  Rooted<GlobalObject*> global(cx, dbg->unwrapDebuggeeArgument(cx, args[0]));
  if (!global) return false;

  if (!dbg->addDebuggeeGlobal(cx, global)) return false;

  RootedValue v(cx, ObjectValue(*global));
  if (!dbg->wrapDebuggeeValue(cx, &v)) return false;
  args.rval().set(v);
  return true;
}
  ...
  for (ZonesIter zone(cx->runtime(), SkipAtoms); !zone.done(); zone.next()) {
    for (CompartmentsInZoneIter c(zone); !c.done(); c.next()) {
      if (c == dbg->object->compartment() ||
          c->creationOptions().invisibleToDebugger())
        continue;
      c->scheduledForDestruction = false;
      GlobalObject* global = c->maybeGlobal();
      if (global) {
        Rooted<GlobalObject*> rg(cx, global);
        if (!dbg->addDebuggeeGlobal(cx, rg)) return false;
      }
    }
  }
...
/* static */ bool Debugger::removeDebuggee(JSContext* cx, unsigned argc,
  ...
  if (!args.requireAtLeast(cx, "Debugger.removeDebuggee", 1)) return false;
  Rooted<GlobalObject*> global(cx, dbg->unwrapDebuggeeArgument(cx, args[0]));
  if (!global) return false;
  ...
  if (dbg->debuggees.has(global)) {
    ...
    if (global->getDebuggers()->empty() && !obs.add(global->compartment()))
      return false;
    if (!updateExecutionObservability(cx, obs, NotObserving)) return false;
  }
...
/* static */ bool Debugger::hasDebuggee(JSContext* cx, unsigned argc,
  ...
  if (!args.requireAtLeast(cx, "Debugger.hasDebuggee", 1)) return false;
...
/* static */ bool Debugger::getDebuggees(JSContext* cx, unsigned argc,
  ...
  if (!debuggees.resize(count)) return false;
  unsigned i = 0;
  {
...
/* static */ bool Debugger::getNewestFrame(JSContext* cx, unsigned argc,
  ...
  for (AllFramesIter i(cx); !i.done(); ++i) {
    if (dbg->observesFrame(i)) {
      ...
      if (i.isIon() && !i.ensureHasRematerializedFrame(cx)) return false;
...
/* static */ bool Debugger::construct(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (!GetProperty(cx, callee, callee, cx->names().prototype, &v)) return false;
  ...
  if (!cx->runtime()->beginSingleThreadedExecution(cx)) {
    JS_ReportErrorASCII(cx,
                        "Cannot ensure single threaded execution in Debugger");
    return false;
  }
  ...
  for (unsigned i = 0; i < args.length(); i++) {
    Rooted<GlobalObject*> debuggee(
        cx,
        &args[i].toObject().as<ProxyObject>().private_().toObject().global());
    if (!debugger->addDebuggeeGlobal(cx, debuggee)) return false;
  }
...
bool Debugger::addDebuggeeGlobal(JSContext* cx, Handle<GlobalObject*> global) {
  if (debuggees.has(global)) return true;
  ...
  if (!visited.append(object->compartment())) return false;
  ...
  if (!globalDebuggers) return false;
  ...
  auto debuggeeZonesGuard = MakeScopeExit([&] {
    if (addingZoneRelation) debuggeeZones.remove(zone);
  });
...
void Debugger::removeDebuggeeGlobal(FreeOp* fop, GlobalObject* global,
   ...
   */
  if (trackingAllocationSites) Debugger::removeAllocationsTracking(*global);
 ...
 public:
  ...
  bool parseQuery(HandleObject query) {
    ...
    if (global.isUndefined()) {
      if (!matchAllDebuggeeGlobals()) return false;
    } else {
      ...
      if (debugger->debuggees.has(globalObject)) {
        if (!matchSingleGlobal(globalObject)) return false;
      }
    }

    /* Check for a 'url' property. */
    if (!GetProperty(cx, query, query, cx->names().url, &url)) return false;
    ...
    if (displayURL.isString()) {
      ...
      if (!displayURLString) return false;
    }
  ...
  bool findScripts() {
    ...
    for (WeakGlobalObjectSet::Range r = debugger->allDebuggees(); !r.empty();
         r.popFront()) {
      for (wasm::Instance* instance :
           r.front()->compartment()->wasm.instances()) {
  ...
  Handle<ScriptVector> foundScripts() const { return vector; }
 ...
 private:
  ...
  bool delazifyScripts() {
    ...
    for (auto r = compartments.all(); !r.empty(); r.popFront()) {
      ...
      if (!comp->ensureDelazifyScriptsForDebugger(cx)) return false;
    }
    return true;
  }
  ...
  void consider(JSScript* script) {
    ...
    if (urlCString.ptr()) {
      ...
      if (!gotFilename && !gotSourceURL) return;
    }
    if (hasLine) {
    ...
    if (displayURLString) {
      ...
      if (CompareChars(s, js_strlen(s), displayURLString) != 0) return;
    }
  ...
  void consider(WasmInstanceObject* instanceObject) {
    ...
    if (hasSource && source != AsVariant(instanceObject)) return;

    if (!wasmInstanceVector.append(instanceObject)) oom = true;
  }
};
...
/* static */ bool Debugger::findScripts(JSContext* cx, unsigned argc,
  ...
  if (!query.init()) return false;

  if (args.length() >= 1) {
    RootedObject queryObject(cx, NonNullObject(cx, args[0]));
    if (!queryObject || !query.parseQuery(queryObject)) return false;
  } else {
    if (!query.omittedQuery()) return false;
  }

  if (!query.findScripts()) return false;
...
class MOZ_STACK_CLASS Debugger::ObjectQuery {
 public:
  ...
  bool parseQuery(HandleObject query) {
    ...
    RootedValue cls(cx);
    if (!GetProperty(cx, query, query, cx->names().class_, &cls)) return false;
    if (!cls.isUndefined()) {
      if (!cls.isString()) {
        JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
                                  JSMSG_UNEXPECTED_TYPE,
  ...
  bool operator()(Traversal& traversal, JS::ubi::Node origin,
    ...
    if (!className.isUndefined()) {
      ...
      if (strcmp(objClassName, classNameCString.ptr()) != 0) return true;
    }
...
bool Debugger::findObjects(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (!query.findObjects()) return false;
  ...
  for (size_t i = 0; i < length; i++) {
    ...
    if (!dbg->wrapDebuggeeValue(cx, &debuggeeVal)) return false;
...
/* static */ bool Debugger::findAllGlobals(JSContext* cx, unsigned argc,
...

  {
    ...
    for (CompartmentsIter c(cx->runtime(), SkipAtoms); !c.done(); c.next()) {
      if (c->creationOptions().invisibleToDebugger()) continue;

      c->scheduledForDestruction = false;

      GlobalObject* global = c->maybeGlobal();

      if (cx->runtime()->isSelfHostingGlobal(global)) continue;
      ...
      if (global) {
        ...
        if (!globals.append(global)) return false;
      }
    }
  }
  ...
  for (size_t i = 0; i < globals.length(); i++) {
    ...
    if (!dbg->wrapDebuggeeValue(cx, &globalValue)) return false;
    if (!NewbornArrayPush(cx, result, globalValue)) return false;
  }
...
bool Debugger::isCompilableUnit(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (!args.requireAtLeast(cx, "Debugger.isCompilableUnit", 1)) return false;
...
bool Debugger::adoptDebuggeeValue(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (!args.requireAtLeast(cx, "Debugger.adoptDebuggeeValue", 1)) return false;
...
static inline DebuggerScriptReferent GetScriptReferent(JSObject* obj) {
  ...
  if (gc::Cell* cell = GetScriptReferentCell(obj)) {
    if (cell->is<JSScript>()) return AsVariant(cell->as<JSScript>());
...
static bool DebuggerScript_getIsGeneratorFunction(JSContext* cx, unsigned argc,
                                                  Value* vp) {
  THIS_DEBUGSCRIPT_SCRIPT(cx, argc, vp, "(get isGeneratorFunction)", args, obj,
                          script);
...
static bool DebuggerScript_getIsAsyncFunction(JSContext* cx, unsigned argc,
                                              Value* vp) {
  THIS_DEBUGSCRIPT_SCRIPT(cx, argc, vp, "(get isAsyncFunction)", args, obj,
                          script);
...
static bool DebuggerScript_getDisplayName(JSContext* cx, unsigned argc,
  ...
  if (!dbg->wrapDebuggeeValue(cx, &namev)) return false;
  args.rval().set(namev);
  return true;
}
...
struct DebuggerScriptGetStartLineMatcher {
  ...
  ReturnType match(HandleScript script) { return uint32_t(script->lineno()); }
...
static bool DebuggerScript_getLineCount(JSContext* cx, unsigned argc,
  ...
  if (!referent.match(matcher)) return false;
  ...
  if (script->hasObjects()) {
    ...
    for (uint32_t i = 0; i < objects->length; i++) {
      ...
      if (obj->is<JSFunction>()) {
        ...
        if (!funScript) return false;
        s = dbg->wrapScript(cx, funScript);
        if (!s || !NewbornArrayPush(cx, result, ObjectValue(*s))) return false;
      }
    }
  }
  args.rval().setObject(*result);
...
static bool EnsureScriptOffsetIsValid(JSContext* cx, JSScript* script,
                                      size_t offset) {
  if (IsValidBytecodeOffset(cx, script, offset)) return true;
  JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
                            JSMSG_DEBUG_BAD_OFFSET);
  return false;
}
...
class BytecodeRangeWithPosition : private BytecodeRange {
 public:
  ...
  BytecodeRangeWithPosition(JSContext* cx, JSScript* script)
    ...
    updatePosition();
    while (frontPC() != script->main()) popFront();
...
class FlowGraphSummary {
 public:
  ...
  bool populate(JSContext* cx, JSScript* script) {
    if (!entries_.growBy(script->length())) return false;
...
class DebuggerScriptGetOffsetLocationMatcher {
 ...
 public:
  ...
  using ReturnType = bool;
  ReturnType match(HandleScript script) {
    if (!EnsureScriptOffsetIsValid(cx_, script, offset_)) return false;

    FlowGraphSummary flowData(cx_);
    if (!flowData.populate(cx_, script)) return false;

    result_.set(NewBuiltinClassInstance<PlainObject>(cx_));
    if (!result_) return false;

    BytecodeRangeWithPosition r(cx_, script);
    while (!r.empty() && r.frontOffset() < offset_) r.popFront();
    ...
    if (!DefineDataProperty(cx_, result_, id, value)) return false;
  ...
  ReturnType match(Handle<WasmInstanceObject*> instance) {
    ...
    size_t column;
    bool found;
    if (!instance->instance().debug().getOffsetLocation(cx_, offset_, &found,
                                                        &lineno, &column))
      return false;
...
static bool DebuggerScript_getOffsetLocation(JSContext* cx, unsigned argc,
  ...
  if (!args.requireAtLeast(cx, "Debugger.Script.getOffsetLocation", 1))
    return false;
  size_t offset;
  if (!ScriptOffset(cx, args[0], &offset)) return false;
...
static bool DebuggerScript_getAllOffsets(JSContext* cx, unsigned argc,
  ...
  if (!flowData.populate(cx, script)) return false;

  /* Second pass: build the result array. */
  RootedObject result(cx, NewDenseEmptyArray(cx));
  if (!result) return false;
  for (BytecodeRangeWithPosition r(cx, script); !r.empty(); r.popFront()) {
    if (!r.frontIsEntryPoint()) continue;
    ...
    if (!flowData[offset].hasNoEdges() && flowData[offset].lineno() != lineno) {
      ...
      bool found;
      if (!HasOwnProperty(cx, result, id, &found)) return false;
      ...
      if (offsetsv.isObject()) {
        ...
        if (!DefineDataProperty(cx, result, id, value)) return false;
      }

      /* Append the current offset to the offsets array. */
      if (!NewbornArrayPush(cx, offsets, NumberValue(offset))) return false;
    }
  }
...
class DebuggerScriptGetAllColumnOffsetsMatcher {
  ...
  bool appendColumnOffsetEntry(size_t lineno, size_t column, size_t offset) {
    ...
    RootedId id(cx_, NameToId(cx_->names().lineNumber));
    RootedValue value(cx_, NumberValue(lineno));
    if (!DefineDataProperty(cx_, entry, id, value)) return false;
  ...
  ReturnType match(HandleScript script) {
    ...
    for (BytecodeRangeWithPosition r(cx_, script); !r.empty(); r.popFront()) {
      ...
      if (r.frontIsEntryPoint() && !flowData[offset].hasNoEdges() &&
        ...
        if (!appendColumnOffsetEntry(lineno, column, offset)) return false;
      }
    }
    return true;
  }
  ...
  ReturnType match(HandleScript script) {
    ...
    for (BytecodeRangeWithPosition r(cx_, script); !r.empty(); r.popFront()) {
      ...
      if (r.frontLineNumber() == lineno_ && !flowData[offset].hasNoEdges() &&
        ...
        if (!NewbornArrayPush(cx_, result_, NumberValue(offset))) return false;
      }
    }
...
static bool DebuggerScript_getLineOffsets(JSContext* cx, unsigned argc,
  ...
  size_t lineno;
  if (!ToNumber(cx, &linenoValue)) return false;
  {
...
bool Debugger::observesFrame(AbstractFramePtr frame) const {
  if (frame.isWasmDebugFrame()) return observesWasm(frame.wasmInstance());
...
bool Debugger::observesFrame(const FrameIter& iter) const {
  ...
  if (iter.isWasm()) {
    ...
    if (!iter.wasmDebugEnabled()) return false;
...
bool Debugger::observesWasm(wasm::Instance* instance) const {
  if (!enabled || !instance->debugEnabled()) return false;
...
/* static */ bool Debugger::handleIonBailout(JSContext* cx,
  ...
  // |from|.
  ScriptFrameIter iter(cx);
  while (iter.abstractFramePtr() != to) ++iter;
  return replaceFrameGuts(cx, from, to, iter);
}
...
struct DebuggerScriptSetBreakpointMatcher {
 ...
 public:
  ...
  ReturnType match(HandleScript script) {
    ...
    if (!dbg_->ensureExecutionObservabilityOfScript(cx_, script)) return false;
    ...
    if (cx_->zone()->new_<Breakpoint>(dbg_, site, handler_)) return true;
...
class DebuggerScriptClearBreakpointMatcher {
 ...
 public:
  explicit DebuggerScriptClearBreakpointMatcher(JSContext* cx, Debugger* dbg,
                                                JSObject* handler)
  ...
  ReturnType match(Handle<WasmInstanceObject*> instance) {
    return instance->instance().debug().clearBreakpointsIn(cx_, instance, dbg_,
                                                           handler_);
  }
};
...
static bool DebuggerScript_getOffsetsCoverage(JSContext* cx, unsigned argc,
                                              Value* vp) {
  THIS_DEBUGSCRIPT_SCRIPT(cx, argc, vp, "getOffsetsCoverage", args, obj,
                          script);
...
static bool DebuggerSource_getBinary(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (!arr) return false;
  if (bytecode)
    memcpy(arr->as<TypedArrayObject>().viewDataUnshared(), bytecode->begin(),
           arrLength);
  ...
  ReturnType match(Handle<WasmInstanceObject*> wasmInstance) {
    if (wasmInstance->instance().metadata().baseURL) {
      JSString* str = NewStringCopyZ<CanGC>(
          cx_, wasmInstance->instance().metadata().baseURL.get());
...
static bool DebuggerSource_setSourceMapURL(JSContext* cx, unsigned argc,
  ...
  ScriptSource* ss = sourceObject->source();
  MOZ_ASSERT(ss);
  if (!args.requireAtLeast(cx, "set sourceMapURL", 1)) return false;

  JSString* str = ToString<CanGC>(cx, args[0]);
  if (!str) return false;

  AutoStableStringChars stableChars(cx);
  if (!stableChars.initTwoByte(cx, str)) return false;

  if (!ss->setSourceMapURL(cx, stableChars.twoByteChars())) return false;
...
bool ScriptedOnStepHandler::onStep(JSContext* cx, HandleDebuggerFrame frame,
  ...
  RootedValue rval(cx);
  if (!js::Call(cx, fval, frame, &rval)) return false;
...
bool ScriptedOnPopHandler::onPop(JSContext* cx, HandleDebuggerFrame frame,
  ...
  RootedValue completion(cx);
  if (!dbg->newCompletionValue(cx, statusp, vp, &completion)) return false;

  RootedValue fval(cx, ObjectValue(*object_));
  RootedValue rval(cx);
  if (!js::Call(cx, fval, frame, completion, &rval)) return false;

  return ParseResumptionValue(cx, rval, statusp, vp);
};

/* static */ NativeObject* DebuggerFrame::initClass(JSContext* cx,
...
/* static */ bool DebuggerFrame::getIsConstructing(JSContext* cx,
  ...
  if (!DebuggerFrame::getFrameIter(cx, frame, maybeIter)) return false;
...
static void UpdateFrameIterPc(FrameIter& iter) {
  ...
  if (iter.abstractFramePtr().isRematerializedFrame()) {
    ...
    while (activationIter.activation() != activation) ++activationIter;
    ...
    while (ionInlineIter.frameNo() != frame->frameNo()) ++ionInlineIter;
...
/* static */ bool DebuggerFrame::getOlder(JSContext* cx,
  ...
  for (++iter; !iter.done(); ++iter) {
    if (dbg->observesFrame(iter)) {
      if (iter.isIon() && !iter.ensureHasRematerializedFrame(cx)) return false;
...
/* static */ bool DebuggerFrame::getThis(JSContext* cx,
  ...
  if (!requireScriptReferent(cx, frame)) return false;
  ...
  if (referent.isWasmDebugFrame()) {
    ...
    if (handler && !prior) {
      ...
      if (!dbg->ensureExecutionObservabilityOfScript(cx, referent.script()))
        return false;
      if (!referent.script()->incrementStepModeCount(cx)) return false;
    } else if (!handler && prior) {
      // Single stepping toggled on->off.
...
static bool EvaluateInEnv(JSContext* cx, Handle<Env*> env,
  ...
  SourceBufferHolder srcBuf(chars.begin().get(), chars.length(),
  ...
  if (frame) {
    ...
    script = frontend::CompileEvalScript(cx, cx->tempLifoAlloc(), env, scope,
                                         options, srcBuf);
    if (script) script->setActiveEval();
  } else {
    ...
    // the global scope.
    script = frontend::CompileGlobalScript(cx, cx->tempLifoAlloc(), scopeKind,
                                           options, srcBuf);
  }
...
static bool DebuggerGenericEval(JSContext* cx,
                                ...
                                Debugger* dbg, HandleObject envArg,
                                FrameIter* iter) {
  ...
  if (bindings) {
    ...
    if (!envChain.append(nenv)) return false;
...
/* static */ bool DebuggerFrame::eval(JSContext* cx, HandleDebuggerFrame frame,
  ...
  return DebuggerGenericEval(cx, chars, bindings, options, status, value, dbg,
                             nullptr, &iter);
}

/* statuc */ bool DebuggerFrame::isLive() const { return !!getPrivate(); }
...
void DebuggerFrame::setOnPopHandler(OnPopHandler* handler) {
  ...
  if (prior && prior != handler) prior->drop();
...
/* static */ bool DebuggerFrame::getFrameIter(JSContext* cx,
  ...
  if (referent.isScriptFrameIterData()) {
    ...
    while (!iter.hasUsableAbstractFramePtr() ||
           iter.abstractFramePtr() != referent)
      ++iter;
...
static void DebuggerFrame_freeScriptFrameIterData(FreeOp* fop, JSObject* obj) {
  AbstractFramePtr frame =
      AbstractFramePtr::FromRaw(obj->as<NativeObject>().getPrivate());
...
static DebuggerFrame* DebuggerFrame_checkThis(JSContext* cx,
  ...
  if (checkLive) {
    if (!DebuggerFrame_requireLive(cx, frame)) return nullptr;
  }
...
#define THIS_FRAME_OWNER_ITER(cx, argc, vp, fnname, args, thisobj, maybeIter, \
                              iter, dbg)                                      \
...
/* static */ bool DebuggerFrame::environmentGetter(JSContext* cx, unsigned argc,
  ...
  if (!DebuggerFrame::getEnvironment(cx, frame, &result)) return false;
...
/* static */ bool DebuggerFrame::calleeGetter(JSContext* cx, unsigned argc,
  ...
  if (!DebuggerFrame::getCallee(cx, frame, &result)) return false;
...
/* static */ bool DebuggerFrame::constructingGetter(JSContext* cx,
  ...
  bool result;
  if (!DebuggerFrame::getIsConstructing(cx, frame, result)) return false;
...
/* static */ bool DebuggerFrame::olderGetter(JSContext* cx, unsigned argc,
  ...
  RootedDebuggerFrame result(cx);
  if (!DebuggerFrame::getOlder(cx, frame, &result)) return false;

  args.rval().setObjectOrNull(result);
  return true;
}

/* The getter used for each element of frame.arguments. See
...
static bool DebuggerArguments_getArg(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (unsigned(i) < frame.numActualArgs()) {
    ...
    {
      AutoCompartment ac(cx, script);
      if (!script->ensureHasAnalyzedArgsUsage(cx)) return false;
    }
...
/* static */ DebuggerArguments* DebuggerArguments::create(
  ...
  RootedNativeObject obj(
      cx, NewNativeObjectWithGivenProto(cx, &DebuggerArguments::class_, proto));
  if (!obj) return nullptr;
  ...
  for (unsigned i = 0; i < fargc; i++) {
    ...
    if (!getobj) return nullptr;
    id = INT_TO_JSID(i);
    if (!getobj || !NativeDefineAccessorProperty(
                       cx, obj, id, JS_DATA_TO_FUNC_PTR(GetterOp, getobj.get()),
                       nullptr, JSPROP_ENUMERATE | JSPROP_GETTER)) {
      return nullptr;
    }
...
/* static */ bool DebuggerFrame::argumentsGetter(JSContext* cx, unsigned argc,
  ...
  if (!DebuggerFrame::getArguments(cx, frame, &result)) return false;
...
/* static */ bool DebuggerFrame::offsetGetter(JSContext* cx, unsigned argc,
  ...
  size_t result;
  if (!DebuggerFrame::getOffset(cx, frame, result)) return false;
...
/* static */ bool DebuggerFrame::onStepSetter(JSContext* cx, unsigned argc,
  ...
  if (!args.requireAtLeast(cx, "Debugger.Frame.set onStep", 1)) return false;
...
/* static */ bool DebuggerFrame::onPopSetter(JSContext* cx, unsigned argc,
  ...
  if (!args.requireAtLeast(cx, "Debugger.Frame.set onPop", 1)) return false;
  if (!IsValidHook(args[0])) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
                              JSMSG_NOT_CALLABLE_OR_UNDEFINED);
    return false;
...
/* static */ bool DebuggerFrame::evalMethod(JSContext* cx, unsigned argc,
  ...
  EvalOptions options;
  if (!ParseEvalOptions(cx, args.get(1), options)) return false;
...
/* static */ bool DebuggerFrame::evalWithBindingsMethod(JSContext* cx,
  ...
  EvalOptions options;
  if (!ParseEvalOptions(cx, args.get(2), options)) return false;
...
/* static */ bool DebuggerObject::protoGetter(JSContext* cx, unsigned argc,
  ...
  RootedDebuggerObject result(cx);
  if (!DebuggerObject::getPrototypeOf(cx, object, &result)) return false;
...
/* static */ bool DebuggerObject::classGetter(JSContext* cx, unsigned argc,
  ...
  RootedString result(cx);
  if (!DebuggerObject::getClassName(cx, object, &result)) return false;
...
/* static */ bool DebuggerObject::parameterNamesGetter(JSContext* cx,
  ...
  if (!DebuggerObject::getParameterNames(cx, object, &names)) return false;
...
/* static */ bool DebuggerObject::boundArgumentsGetter(JSContext* cx,
  ...
  if (!DebuggerObject::getBoundArguments(cx, object, &result)) return false;
...
/* static */ bool DebuggerObject::globalGetter(JSContext* cx, unsigned argc,
                                               Value* vp) {
  THIS_DEBUGOBJECT(cx, argc, vp, "get global", args, object)

  RootedDebuggerObject result(cx);
  if (!DebuggerObject::getGlobal(cx, object, &result)) return false;
...
/* static */ bool DebuggerObject::allocationSiteGetter(JSContext* cx,
  ...
  RootedObject result(cx);
  if (!DebuggerObject::getAllocationSite(cx, object, &result)) return false;
...
/* static */ bool DebuggerObject::errorMessageNameGetter(JSContext* cx,
  ...
  RootedString result(cx);
  if (!DebuggerObject::getErrorMessageName(cx, object, &result)) return false;
...
/* static */ bool DebuggerObject::promiseStateGetter(JSContext* cx,
  ...
  if (!DebuggerObject::requirePromise(cx, object)) return false;
...
/* static */ bool DebuggerObject::promiseAllocationSiteGetter(JSContext* cx,
  ...
  if (!cx->compartment()->wrap(cx, &allocSite)) return false;
...
/* static */ bool DebuggerObject::promiseResolutionSiteGetter(JSContext* cx,
  ...
  if (!cx->compartment()->wrap(cx, &resolutionSite)) return false;
...
/* static */ bool DebuggerObject::promiseDependentPromisesGetter(JSContext* cx,
  ...
  Rooted<GCVector<Value>> values(cx, GCVector<Value>(cx));
  {
    ...
    if (!promise->dependentPromises(cx, &values)) return false;
  }
  for (size_t i = 0; i < values.length(); i++) {
    if (!dbg->wrapDebuggeeValue(cx, values[i])) return false;
  }
...
/* static */ bool DebuggerObject::isExtensibleMethod(JSContext* cx,
  ...
  bool result;
  if (!DebuggerObject::isExtensible(cx, object, result)) return false;
...
/* static */ bool DebuggerObject::isSealedMethod(JSContext* cx, unsigned argc,
  ...
  bool result;
  if (!DebuggerObject::isSealed(cx, object, result)) return false;
...
/* static */ bool DebuggerObject::isFrozenMethod(JSContext* cx, unsigned argc,
  ...
  bool result;
  if (!DebuggerObject::isFrozen(cx, object, result)) return false;
...
static JSObject* IdVectorToArray(JSContext* cx, Handle<IdVector> ids) {
  ...
  if (!vals.growBy(ids.length())) return nullptr;
...
/* static */ bool DebuggerObject::getOwnPropertyNamesMethod(JSContext* cx,
  ...
  if (!DebuggerObject::getOwnPropertyNames(cx, object, &ids)) return false;
...
/* static */ bool DebuggerObject::getOwnPropertySymbolsMethod(JSContext* cx,
  ...
  Rooted<IdVector> ids(cx, IdVector(cx));
  if (!DebuggerObject::getOwnPropertySymbols(cx, object, &ids)) return false;
...
/* static */ bool DebuggerObject::getOwnPropertyDescriptorMethod(JSContext* cx,
  ...
  RootedId id(cx);
  if (!ValueToId<CanGC>(cx, args.get(0), &id)) return false;
...
/* static */ bool DebuggerObject::preventExtensionsMethod(JSContext* cx,
  ...
  if (!DebuggerObject::preventExtensions(cx, object)) return false;
...
/* static */ bool DebuggerObject::sealMethod(JSContext* cx, unsigned argc,
  ...
  if (!DebuggerObject::seal(cx, object)) return false;
...
/* static */ bool DebuggerObject::freezeMethod(JSContext* cx, unsigned argc,
  ...
  if (!DebuggerObject::freeze(cx, object)) return false;
...
/* static */ bool DebuggerObject::definePropertyMethod(JSContext* cx,
  ...
  RootedId id(cx);
  if (!ValueToId<CanGC>(cx, args[0], &id)) return false;

  Rooted<PropertyDescriptor> desc(cx);
  if (!ToPropertyDescriptor(cx, args[1], false, &desc)) return false;

  if (!DebuggerObject::defineProperty(cx, object, id, desc)) return false;
...
/* static */ bool DebuggerObject::definePropertiesMethod(JSContext* cx,
  ...
  if (!ReadPropertyDescriptors(cx, props, false, &ids, &descs)) return false;
  Rooted<IdVector> ids2(cx, IdVector(cx));
  if (!ids2.append(ids.begin(), ids.end())) return false;

  if (!DebuggerObject::defineProperties(cx, object, ids2, descs)) return false;
...
/* static */ bool DebuggerObject::deletePropertyMethod(JSContext* cx,
  ...
  if (!DebuggerObject::deleteProperty(cx, object, id, result)) return false;
...
/* static */ bool DebuggerObject::callMethod(JSContext* cx, unsigned argc,
  ...
  if (callArgs.length() >= 2) {
    if (!args.growBy(callArgs.length() - 1)) return false;
    for (size_t i = 1; i < callArgs.length(); ++i) args[i - 1].set(callArgs[i]);
  }
...
/* static */ bool DebuggerObject::applyMethod(JSContext* cx, unsigned argc,
  ...
  if (callArgs.length() >= 2 && !callArgs[1].isNullOrUndefined()) {
    ...
    unsigned argc = 0;
    if (!GetLengthProperty(cx, argsobj, &argc)) return false;
...
/* static */ bool DebuggerObject::asEnvironmentMethod(JSContext* cx,
  ...
  THIS_DEBUGOBJECT_OWNER_REFERENT(cx, argc, vp, "asEnvironment", args, dbg,
                                  referent);
  if (!RequireGlobalObject(cx, args.thisv(), referent)) return false;
...
/* static */ bool DebuggerObject::forceLexicalInitializationByNameMethod(
  ...
  if (!args.requireAtLeast(
          cx, "Debugger.Object.prototype.forceLexicalInitializationByName", 1))
    return false;

  if (!DebuggerObject::requireGlobal(cx, object)) return false;

  RootedId id(cx);
  if (!ValueToIdentifier(cx, args[0], &id)) return false;
...
/* static */ bool DebuggerObject::unsafeDereferenceMethod(JSContext* cx,
  ...
  RootedObject result(cx);
  if (!DebuggerObject::unsafeDereference(cx, object, &result)) return false;
...
/* static */ bool DebuggerObject::unwrapMethod(JSContext* cx, unsigned argc,
  ...
  RootedDebuggerObject result(cx);
  if (!DebuggerObject::unwrap(cx, object, &result)) return false;
...
/* static */ NativeObject* DebuggerObject::initClass(JSContext* cx,
                                                     HandleObject obj,
...
/* static */ DebuggerObject* DebuggerObject::create(
  ...
  JSObject* obj =
      NewObjectWithGivenProto(cx, &DebuggerObject::class_, proto, newKind);
  if (!obj) return nullptr;
...
/* static */ bool DebuggerObject::getGlobal(
...
/* static */ bool DebuggerObject::getParameterNames(
  ...
  if (!result.growBy(referent->nargs())) return false;
  if (referent->isInterpreted()) {
    ...
    for (size_t i = 0; i < referent->nargs(); i++) result[i].set(nullptr);
  }
...
/* static */ bool DebuggerObject::getBoundArguments(
  ...
  if (!result.resize(length)) return false;
  for (size_t i = 0; i < length; i++) {
    result[i].set(referent->getBoundFunctionArgument(i));
    if (!dbg->wrapDebuggeeValue(cx, result[i])) return false;
  }
  return true;
}
...
/* static */ bool DebuggerObject::getErrorReport(JSContext* cx,
  ...
  if (IsCrossCompartmentWrapper(obj)) obj = CheckedUnwrap(obj);
...
/* static */ bool DebuggerObject::getErrorMessageName(
  ...
  RootedObject referent(cx, object->referent());
  JSErrorReport* report;
  if (!getErrorReport(cx, referent, report)) return false;
  ...
  if (!cx->compartment()->wrap(cx, &str)) return false;
...
/* static */ bool DebuggerObject::getErrorNotes(JSContext* cx,
  ...
  if (!errorNotesArray) return false;

  if (!cx->compartment()->wrap(cx, &errorNotesArray)) return false;
...
/* static */ bool DebuggerObject::getPrototypeOf(
  ...
  {
    AutoCompartment ac(cx, referent);
    if (!GetPrototype(cx, referent, &proto)) return false;
  }
...
/* static */ bool DebuggerObject::getOwnPropertyNames(
  ...
  for (size_t i = 0; i < ids.length(); i++) cx->markId(ids[i]);
...
/* static */ bool DebuggerObject::getOwnPropertyDescriptor(
  ...
  /* Bug: This can cause the debuggee to run! */
  {
    ...
    ErrorCopier ec(ac);
    if (!GetOwnPropertyDescriptor(cx, referent, id, desc)) return false;
  }

  if (desc.object()) {
    /* Rewrap the debuggee values in desc for the debugger. */
    if (!dbg->wrapDebuggeeValue(cx, desc.value())) return false;

    if (desc.hasGetterObject()) {
      RootedValue get(cx, ObjectOrNullValue(desc.getterObject()));
      if (!dbg->wrapDebuggeeValue(cx, &get)) return false;
      desc.setGetterObject(get.toObjectOrNull());
    }
    if (desc.hasSetterObject()) {
      RootedValue set(cx, ObjectOrNullValue(desc.setterObject()));
      if (!dbg->wrapDebuggeeValue(cx, &set)) return false;
...
/* static */ bool DebuggerObject::defineProperty(
  ...
  if (!dbg->unwrapPropertyDescriptor(cx, referent, &desc)) return false;
  JS_TRY_OR_RETURN_FALSE(cx, CheckPropertyDescriptorAccessors(cx, desc));

  Maybe<AutoCompartment> ac;
  ac.emplace(cx, referent);
  if (!cx->compartment()->wrap(cx, &desc)) return false;
  cx->markId(id);

  ErrorCopier ec(ac);
  if (!DefineProperty(cx, referent, id, desc)) return false;
...
/* static */ bool DebuggerObject::defineProperties(
  ...
  Rooted<PropertyDescriptorVector> descs(cx, PropertyDescriptorVector(cx));
  if (!descs.append(descs_.begin(), descs_.end())) return false;
  for (size_t i = 0; i < descs.length(); i++) {
    if (!dbg->unwrapPropertyDescriptor(cx, referent, descs[i])) return false;
  ...
  ac.emplace(cx, referent);
  for (size_t i = 0; i < descs.length(); i++) {
    if (!cx->compartment()->wrap(cx, descs[i])) return false;
    cx->markId(ids[i]);
  }

  ErrorCopier ec(ac);
  for (size_t i = 0; i < descs.length(); i++) {
    if (!DefineProperty(cx, referent, ids[i], descs[i])) return false;
  }
...
/* static */ bool DebuggerObject::call(JSContext* cx,
  ...
  if (!dbg->unwrapDebuggeeValue(cx, &thisv)) return false;
  Rooted<ValueVector> args2(cx, ValueVector(cx));
  if (!args2.append(args.begin(), args.end())) return false;
  for (unsigned i = 0; i < args2.length(); ++i) {
    if (!dbg->unwrapDebuggeeValue(cx, args2[i])) return false;
  }
  ...
  if (!cx->compartment()->wrap(cx, &calleev) ||
    ...
    return false;
  for (unsigned i = 0; i < args2.length(); ++i) {
    if (!cx->compartment()->wrap(cx, args2[i])) return false;
  }
  ...
  bool ok;
  {
    ...
    if (ok) {
      for (size_t i = 0; i < args2.length(); ++i) invokeArgs[i].set(args2[i]);
...
/* static */ bool DebuggerObject::forceLexicalInitializationByName(
  ...
  if (!LookupProperty(cx, globalLexical, id, &pobj, &prop)) return false;
...
/* static */ bool DebuggerObject::unsafeDereference(
  ...
  if (!cx->compartment()->wrap(cx, &referent)) return false;
...
bool DebuggerEnvironment::typeGetter(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (!environment->requireDebuggee(cx)) return false;
...
/* static */ bool DebuggerEnvironment::parentGetter(JSContext* cx,
  ...
  RootedDebuggerEnvironment result(cx);
  if (!environment->getParent(cx, &result)) return false;
...
/* static */ bool DebuggerEnvironment::objectGetter(JSContext* cx,
  ...
  RootedDebuggerObject result(cx);
  if (!environment->getObject(cx, &result)) return false;
...
/* static */ bool DebuggerEnvironment::calleeGetter(JSContext* cx,
  ...
  RootedDebuggerObject result(cx);
  if (!environment->getCallee(cx, &result)) return false;
...
/* static */ bool DebuggerEnvironment::namesMethod(JSContext* cx, unsigned argc,
  ...
  Rooted<IdVector> ids(cx, IdVector(cx));
  if (!DebuggerEnvironment::getNames(cx, environment, &ids)) return false;
...
/* static */ bool DebuggerEnvironment::findMethod(JSContext* cx, unsigned argc,
  ...
  if (!args.requireAtLeast(cx, "Debugger.Environment.find", 1)) return false;
  ...
  RootedDebuggerEnvironment result(cx);
  if (!DebuggerEnvironment::find(cx, environment, id, &result)) return false;
...
/* static */ NativeObject* DebuggerEnvironment::initClass(JSContext* cx,
                                                          HandleObject dbgCtor,
                                                          HandleObject obj) {
  Handle<GlobalObject*> global = obj.as<GlobalObject>();
  RootedObject objProto(cx,
...
/* static */ DebuggerEnvironment* DebuggerEnvironment::create(
  ...
  NewObjectKind newKind =
      IsInsideNursery(referent) ? GenericObject : TenuredObject;
  RootedObject obj(cx, NewObjectWithGivenProto(cx, &DebuggerEnvironment::class_,
                                               proto, newKind));
  if (!obj) return nullptr;
...
/* static */ DebuggerEnvironmentType DebuggerEnvironment::type() const {
  ...
  if (IsDeclarative(referent())) return DebuggerEnvironmentType::Declarative;
...
/* static */ bool DebuggerEnvironment::getNames(
  ...
  AutoIdVector ids(cx);
  {
    ...
    ErrorCopier ec(ac);
    if (!GetPropertyKeys(cx, referent, JSITER_HIDDEN, &ids)) return false;
  }
  ...
  for (size_t i = 0; i < ids.length(); ++i) {
    ...
    if (JSID_IS_ATOM(id) && IsIdentifier(JSID_TO_ATOM(id))) {
      cx->markId(id);
      if (!result.append(id)) return false;
    }
  }
...
/* static */ bool DebuggerEnvironment::find(
...

  {
    ...
    for (; env; env = env->enclosingEnvironment()) {
      bool found;
      if (!HasProperty(cx, env, id, &found)) return false;
      if (found) break;
    }
  }
...
/* static */ bool DebuggerEnvironment::getVariable(
...

  {
    ...
    bool found;
    if (!HasProperty(cx, referent, id, &found)) return false;
    if (!found) {
      result.setUndefined();
      return true;
    }
    ...
    if (referent->is<DebugEnvironmentProxy>()) {
      ...
      if (!GetProperty(cx, referent, referent, id, result)) return false;
    }
  }
...
/* static */ bool DebuggerEnvironment::setVariable(
  ...
  RootedValue value(cx, value_);
  if (!dbg->unwrapDebuggeeValue(cx, &value)) return false;

  {
    ...
    if (!SetProperty(cx, referent, id, value)) return false;
  }
...
bool Builder::Object::defineProperty(JSContext* cx, const char* name,
  ...
  if (!debugger()->wrapDebuggeeValue(cx, &propval)) return false;
...
JS_PUBLIC_API bool JS::dbg::IsDebugger(JSObject& obj) {
  ...
  return unwrapped && js::GetObjectClass(unwrapped) == &Debugger::class_ &&
...
/* static */ GarbageCollectionEvent::Ptr GarbageCollectionEvent::Create(
  ...
  for (auto& slice : stats.slices()) {
    ...
    if (!data->collections.growBy(1)) return nullptr;
...
JSObject* GarbageCollectionEvent::toJSObject(JSContext* cx) const {
  ...
  for (auto range = collections.all(); !range.empty(); range.popFront()) {
    ...
    if (!collectionObj) return nullptr;
