...
bool Proxy::getPropertyDescriptor(JSContext* cx, HandleObject proxy,
  ...
  if (!policy.allowed()) return policy.returnValue();
...
bool Proxy::defineProperty(JSContext* cx, HandleObject proxy, HandleId id,
  ...
  if (!policy.allowed()) {
    if (!policy.returnValue()) return false;
...
JS_FRIEND_API bool js::AppendUnique(JSContext* cx, AutoIdVector& base,
  ...
  if (!uniqueOthers.reserve(others.length())) return false;
  for (size_t i = 0; i < others.length(); ++i) {
    ...
    if (unique) {
      if (!uniqueOthers.append(others[i])) return false;
    }
  }
...
bool Proxy::has(JSContext* cx, HandleObject proxy, HandleId id, bool* bp) {
  ...
  *bp = false;  // default result if we refuse to perform this action
  ...
  if (handler->hasPrototype()) {
    if (!handler->hasOwn(cx, proxy, id, bp)) return false;
    if (*bp) return true;

    RootedObject proto(cx);
    if (!GetPrototype(cx, proxy, &proto)) return false;
...
bool js::ProxyHas(JSContext* cx, HandleObject proxy, HandleValue idVal,
  ...
  bool has;
  if (!Proxy::has(cx, proxy, id, &has)) return false;
...
bool js::ProxyHasOwn(JSContext* cx, HandleObject proxy, HandleValue idVal,
  ...
  bool hasOwn;
  if (!Proxy::hasOwn(cx, proxy, id, &hasOwn)) return false;
...
MOZ_ALWAYS_INLINE bool Proxy::getInternal(JSContext* cx, HandleObject proxy,
  ...
  if (handler->hasPrototype()) {
    bool own;
    if (!handler->hasOwn(cx, proxy, id, &own)) return false;
    if (!own) {
      RootedObject proto(cx);
      if (!GetPrototype(cx, proxy, &proto)) return false;
      if (!proto) return true;
...
bool js::ProxySetProperty(JSContext* cx, HandleObject proxy, HandleId id,
  ...
  ObjectOpResult result;
  RootedValue receiver(cx, ObjectValue(*proxy));
  if (!Proxy::setInternal(cx, proxy, id, val, receiver, result)) return false;
...
JSObject* Proxy::enumerate(JSContext* cx, HandleObject proxy) {
  ...
  if (handler->hasPrototype()) {
    ...
    if (!Proxy::getOwnEnumerablePropertyKeys(cx, proxy, props)) return nullptr;

    RootedObject proto(cx);
    if (!GetPrototype(cx, proxy, &proto)) return nullptr;
    if (!proto) return EnumeratedIdVectorToIterator(cx, proxy, props);
    assertSameCompartment(cx, proxy, proto);

    AutoIdVector protoProps(cx);
    if (!GetPropertyKeys(cx, proto, 0, &protoProps)) return nullptr;
    if (!AppendUnique(cx, props, protoProps)) return nullptr;
  ...
  if (!policy.allowed()) {
    if (!policy.returnValue()) return nullptr;
...
static bool proxy_LookupProperty(JSContext* cx, HandleObject obj, HandleId id,
  ...
  bool found;
  if (!Proxy::has(cx, obj, id, &found)) return false;
...
static bool proxy_DeleteProperty(JSContext* cx, HandleObject obj, HandleId id,
                                 ObjectOpResult& result) {
  if (!Proxy::delete_(cx, obj, id, result)) return false;
...
JSObject* js::proxy_WeakmapKeyDelegate(JSObject* obj) {
...
bool js::proxy_Call(JSContext* cx, unsigned argc, Value* vp) {
...
bool js::proxy_Construct(JSContext* cx, unsigned argc, Value* vp) {
  CallArgs args = CallArgsFromVp(argc, vp);
  RootedObject proxy(cx, &args.callee());
  MOZ_ASSERT(proxy->is<ProxyObject>());
...
JS_FRIEND_API JSObject* js::InitProxyClass(JSContext* cx, HandleObject obj) {
  ...
  if (!JS_DefineFunctions(cx, ctor, static_methods)) return nullptr;
