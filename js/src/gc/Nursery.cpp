...
inline void js::NurseryChunk::poisonAndInit(JSRuntime* rt, uint8_t poison) {
...
#endif
{
  ...
  if (env && *env) canAllocateStrings_ = (*env == '1');
}

bool js::Nursery::init(uint32_t maxNurseryBytes, AutoLockGCBgAlloc& lock) {
  if (!mallocedBuffers.init()) return false;
  ...
  if (chunkCountLimit_ == 0) return true;
  ...
  if (!runtime()->gc.storeBuffer().enable()) return false;
...
void js::Nursery::enable() {
  ...
  if (isEnabled() || !chunkCountLimit()) return;
  ...
  setStartPosition();
#ifdef JS_GC_ZEAL
  if (runtime()->hasZealMode(ZealMode::GenerationalGC)) enterZealMode();
#endif
...
void js::Nursery::enterZealMode() {
  if (isEnabled()) maxChunkCount_ = chunkCountLimit();
}
...
void* js::Nursery::allocate(size_t size) {
  ...
  if (runtime()->gc.hasZealMode(ZealMode::CheckNursery)) size += CanarySize;
#endif
  ...
  if (currentEnd() < position() + size) {
    ...
    if (chunkno == maxChunkCount()) return nullptr;
    if (MOZ_UNLIKELY(chunkno == allocatedChunkCount())) {
      mozilla::TimeStamp start = TimeStamp::Now();
      {
        AutoLockGCBgAlloc lock(runtime());
        if (!allocateNextChunk(chunkno, lock)) return nullptr;
      }
...
void* js::Nursery::allocateBuffer(JSObject* obj, size_t nbytes) {
  ...
  if (!IsInsideNursery(obj)) return obj->zone()->pod_malloc<uint8_t>(nbytes);
...
void* js::Nursery::reallocateBuffer(JSObject* obj, void* oldBuffer,
  ...
  if (newBytes < oldBytes) return oldBuffer;

  void* newBuffer = allocateBuffer(obj->zone(), newBytes);
  if (newBuffer) PodCopy((uint8_t*)newBuffer, (uint8_t*)oldBuffer, oldBytes);
  return newBuffer;
}
...
inline float js::Nursery::calcPromotionRate(bool* validForTenuring) const {
  ...
  if (previousGC.nurseryUsedBytes > 0) {
    ...
    if (validForTenuring) *validForTenuring = false;
    rate = 0.0f;
  }
...
#define EXTRACT_NAME(name, text) #name,
  static const char* names[] = {FOR_EACH_NURSERY_PROFILE_TIME(EXTRACT_NAME)
#undef EXTRACT_NAME
                                    ""};
...
void js::Nursery::maybeClearProfileDurations() {
  for (auto& duration : profileDurations_) duration = mozilla::TimeDuration();
}
...
void js::Nursery::collect(JS::gcreason::Reason reason) {
    ...
    for (auto& entry : tenureCounts.entries) {
      if (entry.count >= 3000) {
        ...
        if (group->canPreTenure() &&
            group->zone()->group()->canEnterWithoutYielding(cx)) {
  ...
  if (rt->gc.usage.gcBytes() >= rt->gc.tunables.gcMaxBytes()) disable();
...
void js::Nursery::freeMallocedBuffers() {
  if (mallocedBuffers.empty()) return;
...
void js::Nursery::clear() {
  ...
  if (runtime()->hasZealMode(ZealMode::GenerationalGC)) {
    ...
    if (currentChunk_ + 1 == maxChunkCount()) setCurrentChunk(0);
  } else {
    setCurrentChunk(0);
  }
...
MOZ_ALWAYS_INLINE void js::Nursery::setCurrentChunk(unsigned chunkno) {
  ...
  if (canAllocateStrings_) currentStringEnd_ = currentEnd_;
...
bool js::Nursery::allocateNextChunk(const unsigned chunkno,
  ...
  if (!chunks_.resize(newCount)) return false;
...
void js::Nursery::maybeResizeNursery(JS::gcreason::Reason reason) {
  ...
  if (runtime()->hasZealMode(ZealMode::GenerationalGC)) return;
#endif
  ...
  if (promotionRate > GrowThreshold) {
    ...
    growAllocableSpace();
             ...
             previousPromotionRate_ < ShrinkThreshold) {
...
void js::Nursery::shrinkAllocableSpace(unsigned newCount) {
  ...
  if ((newCount == 0) || (newCount == maxChunkCount())) return;

  MOZ_ASSERT(newCount < maxChunkCount());

  if (newCount < allocatedChunkCount()) freeChunksFrom(newCount);
