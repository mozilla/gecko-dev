...
void exitJsDirectory() {
  ...
  if (close(gJsDirectory) != 0) MOZ_CRASH("Could not close js directory");
  gJsDirectory = 0;
}
...
void enterJsDirectory() {
  ...
  if (result <= 0) MOZ_CRASH("Could not get current directory");
  if (result > MAX_PATH)
...
void readFull(const char* path, js::Vector<uint8_t>& buf) {
  ...
  if (!buf.growBy(info.st_size)) MOZ_CRASH("OOM");

  int result = fread(buf.begin(), 1, info.st_size, in);
  if (fclose(in) != 0) MOZ_CRASH("Could not close input file");
...
BEGIN_TEST(testBinTokenReaderTesterSimpleString) {
  ...
  CHECK(Tokenizer::equals(*found, "simple string"));  // FIXME: Find a way to
                                                      // make CHECK_EQUAL use
                                                      // `Tokenizer::equals`.
...
BEGIN_TEST(testBinTokenReaderTesterStringWithEscapes) {
  ...
  CHECK(Tokenizer::equals(
      *found, "string with escapes \0\1\0"));  // FIXME: Find a way to make
                                               // CHECK_EQUAL use
                                               // `Tokenizer::equals`.
...
BEGIN_TEST(testBinTokenReaderTesterTwoStringsInTuple) {
...

  {
    ...
    CHECK(Tokenizer::equals(*found_0, "foo"));  // FIXME: Find a way to make
                                                // CHECK_EQUAL use
                                                // `Tokenizer::equals`.

    Maybe<Chars> found_1;
    CHECK(tokenizer.readMaybeChars(found_1));
    CHECK(Tokenizer::equals(*found_1, "bar"));  // FIXME: Find a way to make
                                                // CHECK_EQUAL use
                                                // `Tokenizer::equals`.
