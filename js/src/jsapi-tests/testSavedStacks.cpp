...
BEGIN_TEST(testSavedStacks_selfHostedFrames) {
  ...
  JS::SavedFrameResult result = JS::GetSavedFrameSource(
      cx, selfHostedFrame, &str, JS::SavedFrameSelfHosted::Exclude);
  ...
  result = JS::GetSavedFrameSource(cx, selfHostedFrame, &str,
  ...
  // Line
  uint32_t line = 123;
  result = JS::GetSavedFrameLine(cx, selfHostedFrame, &line,
  ...
  // expected value.
  result = JS::GetSavedFrameSource(cx, parent, &str,
                                   JS::SavedFrameSelfHosted::Exclude);
  CHECK(result == JS::SavedFrameResult::Ok);
  lin = str->ensureLinear(cx);
  CHECK(lin);
