...
void readFull(JSContext* cx, const char* path, js::Vector<char16_t>& buf) {
  ...
  if (!buf.appendAll(intermediate)) MOZ_CRASH("Couldn't read data");
}
...
BEGIN_TEST(testBinASTReaderECMAScript2) {
  ...
  options.setIntroductionType("unit test parse").setFileAndLine("<string>", 1);
  ...
  exitJsDirectory();
  if (!dir) MOZ_CRASH("Couldn't open directory");
  ...
  for (bool found = (hFind != INVALID_HANDLE_VALUE); found;
    ...
    if (namlen < sizeof(BIN_SUFFIX)) continue;
    ...
    // Find text file.
    UniqueChars txtPath(
        static_cast<char*>(js_malloc(namlen + sizeof(PATH) + 1)));
    strncpy(txtPath.get(), PATH, sizeof(PATH));
    strncpy(txtPath.get() + sizeof(PATH) - 1, d_name, namlen);
    strncpy(txtPath.get() + sizeof(PATH) + namlen - sizeof(BIN_SUFFIX),
            TXT_SUFFIX, sizeof(TXT_SUFFIX));
    txtPath[sizeof(PATH) + namlen - sizeof(BIN_SUFFIX) + sizeof(TXT_SUFFIX) -
            1] = 0;
    ...
    js::frontend::Parser<js::frontend::FullParseHandler, char16_t> parser(
        cx, cx->tempLifoAlloc(), options, txtSource.begin(), txtSource.length(),
        /* foldConstants = */ false, txtUsedNames, nullptr, nullptr);
    if (!parser.checkOptions()) MOZ_CRASH("Bad options");

    auto txtParsed =
        parser.parse();  // Will be deallocated once `parser` goes out of scope.
    ...
    if (!binUsedNames.init()) MOZ_CRASH("Couldn't initialized binUsedNames");

    js::frontend::BinASTParser reader(cx, cx->tempLifoAlloc(), binUsedNames,
                                      options);

    auto binParsed = reader.parse(
    ...
    if (!binPrinter.init()) MOZ_CRASH("Couldn't display binParsed");
    DumpParseTree(binParsed.unwrap(), binPrinter);

    Sprinter txtPrinter(cx);
    if (!txtPrinter.init()) MOZ_CRASH("Couldn't display txtParsed");
...
#if defined(XP_WIN)
  if (!FindClose(hFind)) MOZ_CRASH("Could not close Find");
#elif defined(XP_UNIX)
  if (closedir(dir) != 0) MOZ_CRASH("Could not close dir");
