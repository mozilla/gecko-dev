...
function toFields(dtf, date, calendar, timeZone) {
  ...
  assertEq(monthPart !== undefined, true);
  assertEq(dayPart !== undefined, true);
  ...
  if (!Number.isInteger(month) && calendar === "hebrew") {
    assertEq(Object.hasOwn(hebrewMonths, monthPart.value), true, `month = ${monthPart.value}`);
  ...
  assertEq(Number.isInteger(month), true, `month = ${monthPart.value}`);
  assertEq(Number.isInteger(day), true, `day = ${dayPart.value}`);
...
for (let calendar of Intl.supportedValuesOf("calendar")) {
  ...
  for (let y = 2050; y >= 1950; --y) {
    ...
    for (let month = 1; month <= monthsInYear; ++month) {
      ...
      assertEq(fields.month, date.month, `date = ${date}`);
      assertEq(fields.day, date.day, `date = ${date}`);
    }
  }
}
