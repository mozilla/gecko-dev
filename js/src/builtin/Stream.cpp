...
template <class T>
MOZ_ALWAYS_INLINE bool Is(const HandleValue v) {
  return v.isObject() && v.toObject().is<T>();
}

#ifdef DEBUG
static bool IsReadableStreamController(const JSObject* controller) {
  return controller->is<ReadableStreamDefaultController>() ||
         controller->is<ReadableByteStreamController>();
}
#endif  // DEBUG

static inline uint32_t ControllerFlags(const NativeObject* controller) {
  MOZ_ASSERT(IsReadableStreamController(controller));
  return controller->getFixedSlot(ControllerSlot_Flags).toInt32();
}

static inline void AddControllerFlags(NativeObject* controller,
                                      uint32_t flags) {
  MOZ_ASSERT(IsReadableStreamController(controller));
...
static inline void RemoveControllerFlags(NativeObject* controller,
                                         uint32_t flags) {
  MOZ_ASSERT(IsReadableStreamController(controller));
  controller->setFixedSlot(ControllerSlot_Flags,
                           Int32Value(ControllerFlags(controller) & ~flags));
...
static inline uint32_t StreamState(const ReadableStream* stream) {
  return stream->getFixedSlot(StreamSlot_State).toInt32();
}

static inline void SetStreamState(ReadableStream* stream, uint32_t state) {
  MOZ_ASSERT_IF(stream->disturbed(), state & ReadableStream::Disturbed);
  MOZ_ASSERT_IF(stream->closed() || stream->errored(),
                !(state & ReadableStream::Readable));
  stream->setFixedSlot(StreamSlot_State, Int32Value(state));
}

bool ReadableStream::readable() const { return StreamState(this) & Readable; }

bool ReadableStream::closed() const { return StreamState(this) & Closed; }

bool ReadableStream::errored() const { return StreamState(this) & Errored; }

bool ReadableStream::disturbed() const { return StreamState(this) & Disturbed; }

inline static bool ReaderHasStream(const NativeObject* reader) {
  MOZ_ASSERT(JS::IsReadableStreamReader(reader));
  return !reader->getFixedSlot(ReaderSlot_Stream).isUndefined();
}

bool js::ReadableStreamReaderIsClosed(const JSObject* reader) {
  return !ReaderHasStream(&reader->as<NativeObject>());
}

inline static MOZ_MUST_USE ReadableStream* StreamFromController(
    const NativeObject* controller) {
  MOZ_ASSERT(IsReadableStreamController(controller));
  return &controller->getFixedSlot(ControllerSlot_Stream)
              .toObject()
              .as<ReadableStream>();
}

inline static MOZ_MUST_USE NativeObject* ControllerFromStream(
...
inline static bool HasController(const ReadableStream* stream) {
...
inline static MOZ_MUST_USE ReadableStream* StreamFromReader(
    const NativeObject* reader) {
  MOZ_ASSERT(ReaderHasStream(reader));
  return &reader->getFixedSlot(ReaderSlot_Stream)
              .toObject()
              .as<ReadableStream>();
}

inline static MOZ_MUST_USE NativeObject* ReaderFromStream(
    const NativeObject* stream) {
...
inline static bool HasReader(const ReadableStream* stream) {
...
inline static MOZ_MUST_USE JSFunction* NewHandler(JSContext* cx, Native handler,
  ...
  if (!handlerFun) return nullptr;
  handlerFun->setExtendedSlot(0, ObjectValue(*target));
  return handlerFun;
}

template <class T>
inline static MOZ_MUST_USE T* TargetFromHandler(JSObject& handler) {
  return &handler.as<JSFunction>().getExtendedSlot(0).toObject().as<T>();
}

inline static MOZ_MUST_USE bool ResetQueue(JSContext* cx,
                                           HandleNativeObject container);
...
static MOZ_MUST_USE JSObject* PromiseRejectedWithPendingError(JSContext* cx) {
  ...
  RootedValue exn(cx);
  if (!GetAndClearException(cx, &exn)) return nullptr;
  return PromiseObject::unforgeableReject(cx, exn);
}

static bool ReportArgTypeError(JSContext* cx, const char* funName,
                               const char* expectedType, HandleValue arg) {
  UniqueChars bytes =
...
static MOZ_MUST_USE bool RejectWithPendingError(
...
static MOZ_MUST_USE bool RejectNonGenericMethod(JSContext* cx,
                                                const CallArgs& args,
                                                const char* className,
                                                const char* methodName) {
  ReportValueError3(cx, JSMSG_INCOMPATIBLE_PROTO, JSDVG_SEARCH_STACK,
                    args.thisv(), nullptr, className, methodName);

  return ReturnPromiseRejectedWithPendingError(cx, args);
}

inline static MOZ_MUST_USE NativeObject* SetNewList(
    JSContext* cx, HandleNativeObject container, uint32_t slot) {
 ...
 public:
  ...
  static ByteStreamChunk* create(JSContext* cx, HandleObject buffer,
    ...
    Rooted<ByteStreamChunk*> chunk(
        cx, NewObjectWithClassProto<ByteStreamChunk>(cx));
 ...
 public:
  ...
  static PullIntoDescriptor* create(JSContext* cx,
    ...
    Rooted<PullIntoDescriptor*> descriptor(
        cx, NewObjectWithClassProto<PullIntoDescriptor>(cx));
...
ReadableStream* ReadableStream::createStream(
...
ReadableStream* ReadableStream::createDefaultStream(
    JSContext* cx, HandleValue underlyingSource, HandleValue size,
  ...
  //                         highWaterMark »).
  RootedObject controller(
      cx, CreateReadableStreamDefaultController(cx, stream, underlyingSource,
                                                size, highWaterMark));
...
ReadableStream* ReadableStream::createByteStream(
  ...
  RootedObject controller(cx, CreateReadableByteStreamController(
...
ReadableStream* ReadableStream::createExternalSourceStream(
    JSContext* cx, void* underlyingSource, uint8_t flags,
    HandleObject proto /* = nullptr */) {
  Rooted<ReadableStream*> stream(cx, createStream(cx, proto));
  if (!stream) return nullptr;

  RootedNativeObject controller(
      cx, CreateReadableByteStreamController(cx, stream, underlyingSource));
  if (!controller) return nullptr;
...
bool ReadableStream::constructor(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (!options.isUndefined()) {
    if (!GetProperty(cx, options, cx->names().size, &size)) return false;

    if (!GetProperty(cx, options, cx->names().highWaterMark, &highWaterMark))
      return false;
  }

  if (!ThrowIfNotConstructing(cx, args, "ReadableStream")) return false;
...
static MOZ_MUST_USE bool ReadableStream_locked_impl(JSContext* cx,
                                                    const CallArgs& args) {
  Rooted<ReadableStream*> stream(cx,
                                 &args.thisv().toObject().as<ReadableStream>());
...
static bool ReadableStream_locked(JSContext* cx, unsigned argc, Value* vp) {
  ...
  CallArgs args = CallArgsFromVp(argc, vp);
  return CallNonGenericMethod<Is<ReadableStream>, ReadableStream_locked_impl>(
      cx, args);
}
...
static MOZ_MUST_USE bool ReadableStream_cancel(JSContext* cx, unsigned argc,
  ...
  RootedObject cancelPromise(cx,
                             ReadableStream::cancel(cx, stream, args.get(0)));
  if (!cancelPromise) return false;
...
static MOZ_MUST_USE ReadableStreamBYOBReader* CreateReadableStreamBYOBReader(
    JSContext* cx, Handle<ReadableStream*> stream);

// Streams spec, 3.2.4.3. getReader()
static MOZ_MUST_USE bool ReadableStream_getReader_impl(JSContext* cx,
                                                       const CallArgs& args) {
  Rooted<ReadableStream*> stream(cx,
                                 &args.thisv().toObject().as<ReadableStream>());
  ...
  if (!optionsVal.isUndefined()) {
    if (!GetProperty(cx, optionsVal, cx->names().mode, &modeVal)) return false;
  }
  ...
  if (modeVal.isUndefined()) {
    ...
    int32_t notByob;
    if (!CompareStrings(cx, mode, cx->names().byob, &notByob)) return false;
    if (notByob) {
      JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
...
static bool ReadableStream_getReader(JSContext* cx, unsigned argc, Value* vp) {
  // Step 1: If ! IsReadableStream(this) is false, throw a TypeError exception.
  CallArgs args = CallArgsFromVp(argc, vp);
  return CallNonGenericMethod<Is<ReadableStream>,
                              ReadableStream_getReader_impl>(cx, args);
}

// Streams spec, 3.2.4.4. pipeThrough({ writable, readable }, options)
static MOZ_MUST_USE bool ReadableStream_pipeThrough(JSContext* cx,
...
// Streams spec, 3.2.4.5. pipeTo(dest, { preventClose, preventAbort,
// preventCancel } = {})
// TODO: Unimplemented since spec is not complete yet.
static MOZ_MUST_USE bool ReadableStream_pipeTo(JSContext* cx, unsigned argc,
                                               Value* vp) {
  JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
                            JSMSG_READABLESTREAM_METHOD_NOT_IMPLEMENTED,
                            "pipeTo");
...
static MOZ_MUST_USE bool ReadableStreamTee(
    JSContext* cx, Handle<ReadableStream*> stream, bool cloneForBranch2,
    MutableHandle<ReadableStream*> branch1,
    MutableHandle<ReadableStream*> branch2);

// Streams spec, 3.2.4.6. tee()
static MOZ_MUST_USE bool ReadableStream_tee_impl(JSContext* cx,
                                                 const CallArgs& args) {
  Rooted<ReadableStream*> stream(cx,
                                 &args.thisv().toObject().as<ReadableStream>());
  ...
  if (!ReadableStreamTee(cx, stream, false, &branch1, &branch2)) return false;
...
static bool ReadableStream_tee(JSContext* cx, unsigned argc, Value* vp) {
  // Step 1: If ! IsReadableStream(this) is false, throw a TypeError exception
  CallArgs args = CallArgsFromVp(argc, vp);
  return CallNonGenericMethod<Is<ReadableStream>, ReadableStream_tee_impl>(
      cx, args);
}
...
bool ReadableStream::locked() const {
  ...
  if (HasController(this) && (ControllerFlags(ControllerFromStream(this)) &
...
static MOZ_MUST_USE bool ReadableStreamDefaultControllerClose(
    JSContext* cx, Handle<ReadableStreamDefaultController*> controller);
...
static bool TeeReaderReadHandler(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (done && !teeState->closedOrErrored()) {
    ...
    if (!teeState->canceled1()) {
      ...
      if (!ReadableStreamDefaultControllerClose(cx, branch1)) return false;
    }
    ...
    if (!teeState->canceled2()) {
      ...
      if (!ReadableStreamDefaultControllerClose(cx, branch2)) return false;
    }
  ...
  if (teeState->closedOrErrored()) return true;
  ...
  MOZ_ASSERT(!teeState->cloneForBranch2(),
             "tee(cloneForBranch2=true) should not be exposed");
...
static MOZ_MUST_USE JSObject* ReadableStreamTee_Pull(
    JSContext* cx, Handle<TeeState*> teeState,
...
static MOZ_MUST_USE JSObject* ReadableStreamTee_Cancel(
  ...
  if (bothBranchesCanceled) {
    ...
    // Step b: Let cancelResult be ! ReadableStreamCancel(stream,
    // compositeReason).
    RootedObject cancelResult(
        cx, ReadableStream::cancel(cx, stream, compositeReasonVal));
    if (!cancelResult) {
      if (!RejectWithPendingError(cx, promise)) return nullptr;
    } else {
      // Step c: Resolve teeState.[[promise]] with cancelResult.
      RootedValue resultVal(cx, ObjectValue(*cancelResult));
      if (!PromiseObject::resolve(cx, promise, resultVal)) return nullptr;
    }
  }
...
static MOZ_MUST_USE bool ReadableStreamControllerError(
    JSContext* cx, HandleNativeObject controller, HandleValue e);
...
static bool TeeReaderClosedHandler(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (!teeState->closedOrErrored()) {
    ...
    Rooted<ReadableStreamDefaultController*> branch1(cx, teeState->branch1());
    if (!ReadableStreamControllerError(cx, branch1, reason)) return false;

    // Step a.ii: Perform ! ReadableStreamDefaultControllerErrorIfNeeded(
    //                          branch2.[[readableStreamController]], r).
    Rooted<ReadableStreamDefaultController*> branch2(cx, teeState->branch2());
    if (!ReadableStreamControllerError(cx, branch2, reason)) return false;
...
static MOZ_MUST_USE bool ReadableStreamTee(
  ...
  Rooted<ReadableStreamDefaultReader*> reader(
      cx, CreateReadableStreamDefaultReader(cx, stream));
  if (!reader) return false;
  ...
  // Step 13: Perform ! CreateDataProperty(underlyingSource1, "cancel",
  // cancel1).

  // Step 14: Let branch1Stream be ! Construct(ReadableStream,
  // underlyingSource1).
  RootedValue hwmValue(cx, NumberValue(1));
  RootedValue underlyingSource(cx, ObjectValue(*teeState));
  branch1Stream.set(ReadableStream::createDefaultStream(
      cx, underlyingSource, UndefinedHandleValue, hwmValue));
  if (!branch1Stream) return false;

  Rooted<ReadableStreamDefaultController*> branch1(cx);
  branch1 = &ControllerFromStream(branch1Stream)
                 ->as<ReadableStreamDefaultController>();
  AddControllerFlags(branch1,
                     ControllerFlag_TeeBranch | ControllerFlag_TeeBranch1);
  ...
  // Step 17: Perform ! CreateDataProperty(underlyingSource2, "cancel",
  // cancel2).

  // Step 18: Let branch2Stream be ! Construct(ReadableStream,
  // underlyingSource2).
  branch2Stream.set(ReadableStream::createDefaultStream(
      cx, underlyingSource, UndefinedHandleValue, hwmValue));
  if (!branch2Stream) return false;

  Rooted<ReadableStreamDefaultController*> branch2(cx);
  branch2 = &ControllerFromStream(branch2Stream)
                 ->as<ReadableStreamDefaultController>();
  AddControllerFlags(branch2,
                     ControllerFlag_TeeBranch | ControllerFlag_TeeBranch2);
  teeState->setBranch2(branch2);

  // Step 19: Set pull.[[branch1]] to
  // branch1Stream.[[readableStreamController]]. Step 20: Set pull.[[branch2]]
  // to branch2Stream.[[readableStreamController]].

  // Step 21: Upon rejection of reader.[[closedPromise]] with reason r,
  RootedObject closedPromise(
      cx, &reader->getFixedSlot(ReaderSlot_ClosedPromise).toObject());

  RootedObject onRejected(cx, NewHandler(cx, TeeReaderClosedHandler, teeState));
  if (!onRejected) return false;
...
static MOZ_MUST_USE PromiseObject* ReadableStreamAddReadIntoRequest(
    JSContext* cx, Handle<ReadableStream*> stream) {
  // Step 1: MOZ_ASSERT: ! IsReadableStreamBYOBReader(stream.[[reader]]) is
  // true.
  ...
  // Step 5: Append readIntoRequest as the last element of
  ...
  if (!AppendToList(cx, readIntoRequests, val)) return nullptr;
...
static MOZ_MUST_USE PromiseObject* ReadableStreamAddReadRequest(
    JSContext* cx, Handle<ReadableStream*> stream) {
  ...
  // Step 5: Append readRequest as the last element of
  ...
  // Since [[promise]] is the Record's only field, we store it directly.
  val = ObjectValue(*promise);
  if (!AppendToList(cx, readRequests, val)) return nullptr;
...
static MOZ_MUST_USE JSObject* ReadableStreamControllerCancelSteps(
    JSContext* cx, HandleNativeObject controller, HandleValue reason);
...
MOZ_MUST_USE bool ReadableStreamCloseInternal(JSContext* cx,
                                              Handle<ReadableStream*> stream);

// Streams spec, 3.4.3. ReadableStreamCancel ( stream, reason )
/* static */ MOZ_MUST_USE JSObject* ReadableStream::cancel(
    JSContext* cx, Handle<ReadableStream*> stream, HandleValue reason) {
  ...
  if (!ReadableStreamCloseInternal(cx, stream)) return nullptr;
  ...
  sourceCancelPromise =
      ReadableStreamControllerCancelSteps(cx, controller, reason);
  if (!sourceCancelPromise) return nullptr;
  ...
  if (!returnUndefined) return nullptr;
...
MOZ_MUST_USE bool ReadableStreamCloseInternal(JSContext* cx,
  ...
  SetStreamState(stream,
                 (state & ReadableStream::Disturbed) | ReadableStream::Closed);
  ...
  if (reader->is<ReadableStreamDefaultReader>()) {
    ...
    if (!val.isUndefined()) {
      ...
      for (uint32_t i = 0; i < len; i++) {
        ...
        if (!ResolvePromise(cx, readRequest, resultVal)) return false;
      }
  ...
  RootedObject closedPromise(
      cx, &reader->getFixedSlot(ReaderSlot_ClosedPromise).toObject());
  if (!ResolvePromise(cx, closedPromise, UndefinedHandleValue)) return false;
  ...
  if (stream->mode() == JS::ReadableStreamMode::ExternalSource &&
    ...
    void* source =
        controller->getFixedSlot(ControllerSlot_UnderlyingSource).toPrivate();
    cx->runtime()->readableStreamClosedCallback(cx, stream, source,
...
MOZ_MUST_USE bool ReadableStreamErrorInternal(JSContext* cx,
                                              Handle<ReadableStream*> stream,
                                              HandleValue e) {
  // Step 1: Assert: ! IsReadableStream(stream) is true (implicit).
  ...
  uint32_t state = StreamState(stream);
  SetStreamState(stream,
                 (state & ReadableStream::Disturbed) | ReadableStream::Errored);
  ...
  for (uint32_t i = 0; i < len; i++) {
    ...
    if (!PromiseObject::reject(cx, readRequest, e)) return false;
  }

  // Step b: Set reader.[[readRequests]] to a new empty List.
  if (!SetNewList(cx, reader, ReaderSlot_Requests)) return false;
  ...
  if (!PromiseObject::reject(cx, closedPromise, e)) return false;
...
static MOZ_MUST_USE bool ReadableStreamFulfillReadOrReadIntoRequest(
    JSContext* cx, Handle<ReadableStream*> stream, HandleValue chunk,
    bool done) {
...
static MOZ_MUST_USE bool ReadableStreamHasBYOBReader(ReadableStream* stream) {
  // Step 1: Let reader be stream.[[reader]].
...
static MOZ_MUST_USE bool ReadableStreamHasDefaultReader(
  ...
  return reader.isObject() &&
         reader.toObject().is<ReadableStreamDefaultReader>();
}

static MOZ_MUST_USE bool ReadableStreamReaderGenericInitialize(
    JSContext* cx, HandleNativeObject reader, Handle<ReadableStream*> stream);
...
CreateReadableStreamDefaultReader(JSContext* cx,
  ...
  if (!SetNewList(cx, reader, ReaderSlot_Requests)) return nullptr;
...
bool ReadableStreamDefaultReader::constructor(JSContext* cx, unsigned argc,
  ...
  Rooted<ReadableStream*> stream(cx,
                                 &args.get(0).toObject().as<ReadableStream>());
...
static MOZ_MUST_USE bool ReadableStreamDefaultReader_closed(JSContext* cx,
  ...
  if (!Is<ReadableStreamDefaultReader>(args.thisv()))
    return RejectNonGenericMethod(cx, args, "ReadableStreamDefaultReader",
                                  "get closed");
...
static MOZ_MUST_USE bool ReadableStreamDefaultReader_cancel(JSContext* cx,
  ...
  JSObject* cancelPromise =
      ReadableStreamReaderGenericCancel(cx, reader, args.get(0));
  if (!cancelPromise) return false;
  args.rval().setObject(*cancelPromise);
  return true;
}
...
static MOZ_MUST_USE bool ReadableStreamDefaultReader_read(JSContext* cx,
  ...
  if (!readPromise) return false;
  args.rval().setObject(*readPromise);
  return true;
}

static MOZ_MUST_USE bool ReadableStreamReaderGenericRelease(
    JSContext* cx, HandleNativeObject reader);

// Streams spec, 3.5.4.4. releaseLock ( )
static MOZ_MUST_USE bool ReadableStreamDefaultReader_releaseLock_impl(
    JSContext* cx, const CallArgs& args) {
  Rooted<ReadableStreamDefaultReader*> reader(cx);
  reader = &args.thisv().toObject().as<ReadableStreamDefaultReader>();
...
CLASS_SPEC(ReadableStreamDefaultReader, 1, ReaderSlotCount,
...
static MOZ_MUST_USE ReadableStreamBYOBReader* CreateReadableStreamBYOBReader(
    JSContext* cx, Handle<ReadableStream*> stream) {
  // Step 2: If !
  // IsReadableByteStreamController(stream.[[readableStreamController]])
...
bool ReadableStreamBYOBReader::constructor(JSContext* cx, unsigned argc,
  ...
  // exception.
  if (!Is<ReadableStream>(args.get(0))) {
    ReportArgTypeError(cx, "ReadableStreamBYOBReader", "ReadableStream",
                       args.get(0));
    return false;
  }
...
static MOZ_MUST_USE bool ReadableStreamBYOBReader_closed(JSContext* cx,
                                                         unsigned argc,
                                                         Value* vp) {
  CallArgs args = CallArgsFromVp(argc, vp);
  ...
  if (!Is<ReadableStreamBYOBReader>(args.thisv()))
    return RejectNonGenericMethod(cx, args, "ReadableStreamBYOBReader",
                                  "get closed");
...
static MOZ_MUST_USE bool ReadableStreamBYOBReader_cancel(JSContext* cx,
                                                         unsigned argc,
                                                         Value* vp) {
  CallArgs args = CallArgsFromVp(argc, vp);
...
static MOZ_MUST_USE bool ReadableStreamBYOBReader_read(JSContext* cx,
                                                       unsigned argc,
                                                       Value* vp) {
  CallArgs args = CallArgsFromVp(argc, vp);
  ...
  if (!Is<ArrayBufferViewObject>(viewVal)) {
    ReportArgTypeError(cx, "ReadableStreamBYOBReader.read", "Typed Array",
                       viewVal);
    return ReturnPromiseRejectedWithPendingError(cx, args);
  }

  Rooted<ArrayBufferViewObject*> view(
      cx, &viewVal.toObject().as<ArrayBufferViewObject>());
...
static MOZ_MUST_USE bool ReadableStreamBYOBReader_releaseLock_impl(
    JSContext* cx, const CallArgs& args) {
  Rooted<ReadableStreamBYOBReader*> reader(cx);
  reader = &args.thisv().toObject().as<ReadableStreamBYOBReader>();
...
static bool ReadableStreamBYOBReader_releaseLock(JSContext* cx, unsigned argc,
  ...
  return CallNonGenericMethod<Is<ReadableStreamBYOBReader>,
                              ReadableStreamBYOBReader_releaseLock_impl>(cx,
                                                                         args);
}
...
CLASS_SPEC(ReadableStreamBYOBReader, 1, 3, ClassSpec::DontDefineConstructor, 0,
           JS_NULL_CLASS_OPS);

inline static MOZ_MUST_USE bool ReadableStreamControllerCallPullIfNeeded(
    JSContext* cx, HandleNativeObject controller);
...
static MOZ_MUST_USE bool ReadableStreamReaderGenericRelease(
  ...
  if (stream->readable()) {
    ...
    Rooted<PromiseObject*> closedPromise(cx,
                                         &val.toObject().as<PromiseObject>());
    if (!PromiseObject::reject(cx, closedPromise, exn)) return false;
  } else {
...
static MOZ_MUST_USE JSObject* ReadableByteStreamControllerPullInto(
...
/* static */ MOZ_MUST_USE JSObject* ReadableStreamBYOBReader::read(
  ...
  // Step 5: Return !
  // ReadableByteStreamControllerPullInto(stream.[[readableStreamController]],
  // view).
  Rooted<ReadableByteStreamController*> controller(cx);
  controller =
      &ControllerFromStream(stream)->as<ReadableByteStreamController>();
...
MOZ_MUST_USE JSObject* ReadableStreamDefaultReader::read(
    JSContext* cx, Handle<ReadableStreamDefaultReader*> reader) {
  // Step 1: Let stream be reader.[[ownerReadableStream]].
  // Step 2: Assert: stream is not undefined.
  Rooted<ReadableStream*> stream(cx, StreamFromReader(reader));
  ...
  if (stream->closed()) {
    RootedObject iterResult(
        cx, CreateIterResultObject(cx, UndefinedHandleValue, true));
    if (!iterResult) return nullptr;
    RootedValue iterResultVal(cx, ObjectValue(*iterResult));
    return PromiseObject::unforgeableResolve(cx, iterResultVal);
  }

  // Step 5: If stream.[[state]] is "errored", return a new promise rejected
  //         with stream.[[storedError]].
  if (stream->errored()) {
    RootedValue storedError(cx, stream->getFixedSlot(StreamSlot_StoredError));
    return PromiseObject::unforgeableReject(cx, storedError);
...
static bool ControllerStartHandler(JSContext* cx, unsigned argc, Value* vp) {
  ...
  RootedNativeObject controller(cx,
                                TargetFromHandler<NativeObject>(args.callee()));
  ...
  if (!ReadableStreamControllerCallPullIfNeeded(cx, controller)) return false;
  args.rval().setUndefined();
  return true;
}
...
static bool ControllerStartFailedHandler(JSContext* cx, unsigned argc,
                                         Value* vp) {
  CallArgs args = CallArgsFromVp(argc, vp);
  RootedNativeObject controllerObj(
      cx, TargetFromHandler<NativeObject>(args.callee()));
  ...
  if (controllerObj->is<ReadableStreamDefaultController>()) {
    ...
    return ReadableStreamDefaultControllerErrorIfNeeded(cx, controller,
                                                        args.get(0));
  }
...
static MOZ_MUST_USE bool ValidateAndNormalizeQueuingStrategy(
    JSContext* cx, HandleValue size, HandleValue highWaterMarkVal,
...
CreateReadableStreamDefaultController(JSContext* cx,
  ...
  if (!ResetQueue(cx, controller)) return nullptr;
  ...
  double highWaterMark;
  if (!ValidateAndNormalizeQueuingStrategy(cx, size, highWaterMarkVal,
                                           &highWaterMark))
    return nullptr;
  ...
  controller->setFixedSlot(ControllerSlot_StrategyHWM,
                           NumberValue(highWaterMark));
...
bool ReadableStreamDefaultController::constructor(JSContext* cx, unsigned argc,
  ...
  Rooted<ReadableStream*> stream(cx,
                                 &streamVal.toObject().as<ReadableStream>());
  ...
  // Steps 3-11.
  RootedObject controller(
      cx, CreateReadableStreamDefaultController(cx, stream, args.get(1),
...
static MOZ_MUST_USE double ReadableStreamControllerGetDesiredSizeUnchecked(
    NativeObject* controller);

// Streams spec, 3.8.4.1. get desiredSize
// and
// Streams spec, 3.10.4.2. get desiredSize
static MOZ_MUST_USE bool ReadableStreamController_desiredSize_impl(
    JSContext* cx, const CallArgs& args) {
  RootedNativeObject controller(cx);
  ...
  args.rval().setNumber(
      ReadableStreamControllerGetDesiredSizeUnchecked(controller));
  return true;
}
...
static bool ReadableStreamDefaultController_desiredSize(JSContext* cx,
  ...
  return CallNonGenericMethod<Is<ReadableStreamDefaultController>,
                              ReadableStreamController_desiredSize_impl>(cx,
                                                                         args);
}
...
static MOZ_MUST_USE bool ReadableStreamDefaultController_close_impl(
    JSContext* cx, const CallArgs& args) {
  Rooted<ReadableStreamDefaultController*> controller(cx);
  controller = &args.thisv().toObject().as<ReadableStreamDefaultController>();

  // Steps 2-3.
  if (!VerifyControllerStateForClosing(cx, controller)) return false;

  // Step 4: Perform ! ReadableStreamDefaultControllerClose(this).
  if (!ReadableStreamDefaultControllerClose(cx, controller)) return false;
  args.rval().setUndefined();
  return true;
}
...
static bool ReadableStreamDefaultController_close(JSContext* cx, unsigned argc,
  ...
  CallArgs args = CallArgsFromVp(argc, vp);
  return CallNonGenericMethod<Is<ReadableStreamDefaultController>,
                              ReadableStreamDefaultController_close_impl>(cx,
                                                                          args);
}
...
static MOZ_MUST_USE bool ReadableStreamDefaultController_enqueue_impl(
    JSContext* cx, const CallArgs& args) {
  Rooted<ReadableStreamDefaultController*> controller(cx);
  controller = &args.thisv().toObject().as<ReadableStreamDefaultController>();
...
static MOZ_MUST_USE bool ReadableStreamDefaultController_error_impl(
    JSContext* cx, const CallArgs& args) {
  Rooted<ReadableStreamDefaultController*> controller(cx);
  controller = &args.thisv().toObject().as<ReadableStreamDefaultController>();
  ...
  if (!ReadableStreamControllerError(cx, controller, args.get(0))) return false;
  args.rval().setUndefined();
  return true;
}
...
static bool ReadableStreamDefaultController_error(JSContext* cx, unsigned argc,
  ...
  CallArgs args = CallArgsFromVp(argc, vp);
  return CallNonGenericMethod<Is<ReadableStreamDefaultController>,
                              ReadableStreamDefaultController_error_impl>(cx,
                                                                          args);
}
...
CLASS_SPEC(ReadableStreamDefaultController, 4, 7,
           ClassSpec::DontDefineConstructor, 0, JS_NULL_CLASS_OPS);
...
static MOZ_MUST_USE JSObject* ReadableStreamControllerCancelSteps(
  ...
  if (ControllerFlags(controller) & ControllerFlag_ExternalSource) {
    ...
    RootedValue rval(cx);
    rval = cx->runtime()->readableStreamCancelCallback(
        cx, stream, source, stream->embeddingFlags(), reason);
...
inline static MOZ_MUST_USE bool DequeueValue(JSContext* cx,
                                             HandleNativeObject container,
...
static JSObject* ReadableStreamDefaultControllerPullSteps(
  ...
  if (queue && queue->getDenseInitializedLength() != 0) {
    ...
    // Step d: Return a promise resolved with ! CreateIterResultObject(chunk,
    // false).
    RootedObject iterResultObj(cx, CreateIterResultObject(cx, chunk, false));
    if (!iterResultObj) return nullptr;
  ...
  Rooted<PromiseObject*> pendingPromise(
      cx, ReadableStreamAddReadRequest(cx, stream));
  if (!pendingPromise) return nullptr;

  // Step 4: Perform ! ReadableStreamDefaultControllerCallPullIfNeeded(this).
  if (!ReadableStreamControllerCallPullIfNeeded(cx, controller)) return nullptr;
...
static bool ControllerPullHandler(JSContext* cx, unsigned argc, Value* vp) {
  ...
  RemoveControllerFlags(controller,
                        ControllerFlag_Pulling | ControllerFlag_PullAgain);
...
static bool ControllerPullFailedHandler(JSContext* cx, unsigned argc,
  ...
  if (StreamFromController(controller)->readable()) {
    if (!ReadableStreamControllerError(cx, controller, e)) return false;
  }
...
static bool ReadableStreamControllerShouldCallPull(NativeObject* controller);
...
inline static MOZ_MUST_USE bool ReadableStreamControllerCallPullIfNeeded(
  ...
  if (Is<TeeState>(underlyingSource)) {
    ...
    double desiredSize =
        ReadableStreamControllerGetDesiredSizeUnchecked(controller);
    cx->runtime()->readableStreamDataRequestCallback(
        cx, stream, source, stream->embeddingFlags(), desiredSize);
  ...
  if (!pullPromise) return false;

  RootedObject onPullFulfilled(
      cx, NewHandler(cx, ControllerPullHandler, controller));
  if (!onPullFulfilled) return false;

  RootedObject onPullRejected(
      cx, NewHandler(cx, ControllerPullFailedHandler, controller));
  if (!onPullRejected) return false;
...
static bool ReadableStreamControllerShouldCallPull(NativeObject* controller) {
  ...
  if (!stream->readable()) return false;

  // Step 3: If controller.[[closeRequested]] is true, return false.
  uint32_t flags = ControllerFlags(controller);
  if (flags & ControllerFlag_CloseRequested) return false;

  // Step 4: If controller.[[started]] is false, return false.
  if (!(flags & ControllerFlag_Started)) return false;
...
static MOZ_MUST_USE bool ReadableStreamDefaultControllerClose(
  ...
  RootedNativeObject queue(cx);
  queue = &controller->getFixedSlot(QueueContainerSlot_Queue)
               .toObject()
               .as<NativeObject>();
...
static MOZ_MUST_USE bool EnqueueValueWithSize(JSContext* cx,
                                              HandleNativeObject container,
                                              HandleValue value,
                                              HandleValue sizeVal);
...
static MOZ_MUST_USE bool ReadableStreamDefaultControllerEnqueue(
  ...
  if (stream->locked() && ReadableStreamGetNumReadRequests(stream) > 0) {
    ...
    if (!success) {
      ...
      RootedValue exn(cx);
      if (!cx->getPendingException(&exn)) return false;
  ...
  // Step 6: Perform !
  // ReadableStreamDefaultControllerCallPullIfNeeded(controller). Step 6:
  // Perform
  //         ! ReadableStreamDefaultControllerCallPullIfNeeded(controller).
  if (!ReadableStreamControllerCallPullIfNeeded(cx, controller)) return false;
...
static MOZ_MUST_USE bool ReadableStreamControllerError(
  ...
  if (controller->is<ReadableByteStreamController>()) {
    ...
    if (!ReadableByteStreamControllerClearPendingPullIntos(
            cx, byteStreamController))
      return false;
  }

  // Step 3 (or 4): Perform ! ResetQueue(controller).
  if (!ResetQueue(cx, controller)) return false;
...
// Steps 1-4 done at callsites, so only assert that they have been done.
  ...
  double strategyHWM =
      controller->getFixedSlot(ControllerSlot_StrategyHWM).toNumber();
  double queueSize =
      controller->getFixedSlot(QueueContainerSlot_TotalSize).toNumber();
...
CreateReadableByteStreamController(JSContext* cx,
  ...
  controller->setFixedSlot(ControllerSlot_UnderlyingSource,
  ...
  if (!autoAllocateChunkSize.isUndefined()) {
    ...
    if (!IsInteger(autoAllocateChunkSize) ||
        autoAllocateChunkSize.toNumber() <= 0) {
  ...
  controller->setFixedSlot(ByteControllerSlot_AutoAllocateSize,
                           autoAllocateChunkSize);
  ...
  RootedValue startResult(cx);
  RootedValue controllerVal(cx, ObjectValue(*controller));
  if (!InvokeOrNoop(cx, underlyingByteSource, cx->names().start, controllerVal,
                    &startResult))
    return nullptr;
...
bool ReadableByteStreamController::hasExternalSource() {
...
bool ReadableByteStreamController::constructor(JSContext* cx, unsigned argc,
  ...
  RootedObject controller(cx, CreateReadableByteStreamController(
                                  cx, stream, args.get(1), args.get(2)));
  if (!controller) return false;
...
CreateReadableByteStreamController(JSContext* cx,
  ...
  // Step 4: Set this.[[underlyingByteSource]] to underlyingByteSource.
  controller->setFixedSlot(ControllerSlot_UnderlyingSource,
                           PrivateValue(underlyingSource));

  // Step 5: Set this.[[pullAgain]], and this.[[pulling]] to false.
  controller->setFixedSlot(ControllerSlot_Flags,
                           Int32Value(ControllerFlag_ExternalSource));
...
static MOZ_MUST_USE ReadableStreamBYOBRequest* CreateReadableStreamBYOBRequest(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
    HandleObject view);

// Streams spec, 3.10.4.1. get byobRequest
static MOZ_MUST_USE bool ReadableByteStreamController_byobRequest_impl(
    JSContext* cx, const CallArgs& args) {
  Rooted<ReadableByteStreamController*> controller(cx);
  ...
  if (byobRequest.isUndefined() &&
      pendingPullIntos->getDenseInitializedLength() != 0) {
    // Step a: Let firstDescriptor be the first element of
    // this.[[pendingPullIntos]].
    Rooted<PullIntoDescriptor*> firstDescriptor(cx);
    firstDescriptor = PeekList<PullIntoDescriptor>(pendingPullIntos);
    ...
    RootedObject view(
        cx, JS_NewUint8ArrayWithBuffer(
                cx, buffer, firstDescriptor->byteOffset() + bytesFilled,
    ...
    RootedObject request(cx,
                         CreateReadableStreamBYOBRequest(cx, controller, view));
...
static bool ReadableByteStreamController_byobRequest(JSContext* cx,
                                                     unsigned argc, Value* vp) {
...
static MOZ_MUST_USE bool ReadableByteStreamController_close_impl(
    JSContext* cx, const CallArgs& args) {
  Rooted<ReadableByteStreamController*> controller(cx);
  controller = &args.thisv().toObject().as<ReadableByteStreamController>();
  ...
  if (!ReadableByteStreamControllerClose(cx, controller)) return false;
  args.rval().setUndefined();
  return true;
}
...
static bool ReadableByteStreamController_close(JSContext* cx, unsigned argc,
  ...
  CallArgs args = CallArgsFromVp(argc, vp);
  return CallNonGenericMethod<Is<ReadableByteStreamController>,
                              ReadableByteStreamController_close_impl>(cx,
                                                                       args);
}

static MOZ_MUST_USE bool ReadableByteStreamControllerEnqueue(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
    HandleObject chunk);

// Streams spec, 3.10.4.4. enqueue ( chunk )
static MOZ_MUST_USE bool ReadableByteStreamController_enqueue_impl(
    JSContext* cx, const CallArgs& args) {
  Rooted<ReadableByteStreamController*> controller(cx);
  controller = &args.thisv().toObject().as<ReadableByteStreamController>();
  ...
  if (!chunkVal.isObject() ||
      !JS_IsArrayBufferViewObject(&chunkVal.toObject())) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
  ...
  if (!ReadableByteStreamControllerEnqueue(cx, controller, chunk)) return false;
  args.rval().setUndefined();
  return true;
}
...
static bool ReadableByteStreamController_enqueue(JSContext* cx, unsigned argc,
  ...
  CallArgs args = CallArgsFromVp(argc, vp);
  return CallNonGenericMethod<Is<ReadableByteStreamController>,
                              ReadableByteStreamController_enqueue_impl>(cx,
                                                                         args);
}

// Streams spec, 3.10.4.5. error ( e )
static MOZ_MUST_USE bool ReadableByteStreamController_error_impl(
    JSContext* cx, const CallArgs& args) {
  Rooted<ReadableByteStreamController*> controller(cx);
  controller = &args.thisv().toObject().as<ReadableByteStreamController>();
  HandleValue e = args.get(0);
...
static bool ReadableByteStreamController_error(JSContext* cx, unsigned argc,
  ...
  CallArgs args = CallArgsFromVp(argc, vp);
  return CallNonGenericMethod<Is<ReadableByteStreamController>,
                              ReadableByteStreamController_error_impl>(cx,
                                                                       args);
}
...
static void ReadableByteStreamControllerFinalize(FreeOp* fop, JSObject* obj) {
  ...
  if (controller.getFixedSlot(ControllerSlot_Flags).isUndefined()) return;

  uint32_t flags = ControllerFlags(&controller);
  if (!(flags & ControllerFlag_ExternalSource)) return;
...
static JSObject* ReadableByteStreamControllerPullSteps(
    JSContext* cx, HandleNativeObject controller) {
  // Step 1: Let stream be this.[[controlledReadableStream]].
  Rooted<ReadableStream*> stream(cx, StreamFromController(controller));
  ...
  if (queueTotalSize > 0) {
    ...
    if (stream->mode() == JS::ReadableStreamMode::ExternalSource) {
      ...
      Rooted<ByteStreamChunk*> entry(cx,
                                     ShiftFromList<ByteStreamChunk>(cx, queue));
      MOZ_ASSERT(entry);
    ...
    // Step 3.g: Return a promise resolved with ! CreateIterResultObject(view,
    // false).
    val.setObject(*view);
  ...
  if (!val.isUndefined()) {
    ...
    if (!bufferObj) return PromiseRejectedWithPendingError(cx);
    ...
    pullIntoDescriptor =
        PullIntoDescriptor::create(cx, buffer, 0, autoAllocateChunkSize, 0, 1,
                                   nullptr, ReaderType_Default);
    if (!pullIntoDescriptor) return PromiseRejectedWithPendingError(cx);
    ...
    if (!AppendToList(cx, pendingPullIntos, val)) return nullptr;
  }
...
static MOZ_MUST_USE bool ReadableStreamBYOBRequest_view_impl(
    JSContext* cx, const CallArgs& args) {
...
static MOZ_MUST_USE bool ReadableByteStreamControllerRespond(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
    HandleValue bytesWrittenVal);

// Streams spec, 3.11.4.2. respond ( bytesWritten )
static MOZ_MUST_USE bool ReadableStreamBYOBRequest_respond_impl(
    JSContext* cx, const CallArgs& args) {
  Rooted<ReadableStreamBYOBRequest*> request(cx);
  ...
  //         throw a TypeError exception.
  RootedValue controllerVal(cx,
                            request->getFixedSlot(BYOBRequestSlot_Controller));
...
static bool ReadableStreamBYOBRequest_respond(JSContext* cx, unsigned argc,
                                              Value* vp) {
  // Step 1: If ! IsReadableStreamBYOBRequest(this) is false, throw a TypeError
  //         exception.
  CallArgs args = CallArgsFromVp(argc, vp);
...
static MOZ_MUST_USE bool ReadableByteStreamControllerRespondWithNewView(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
    HandleObject view);

// Streams spec, 3.11.4.3. respondWithNewView ( view )
static MOZ_MUST_USE bool ReadableStreamBYOBRequest_respondWithNewView_impl(
    JSContext* cx, const CallArgs& args) {
  Rooted<ReadableStreamBYOBRequest*> request(cx);
  request = &args.thisv().toObject().as<ReadableStreamBYOBRequest>();
  HandleValue viewVal = args.get(0);
...
static MOZ_MUST_USE bool ReadableByteStreamControllerClose(
  ...
  RootedValue val(
      cx, controller->getFixedSlot(ByteControllerSlot_PendingPullIntos));
  RootedNativeObject pendingPullIntos(cx, &val.toObject().as<NativeObject>());
  if (pendingPullIntos->getDenseInitializedLength() != 0) {
    ...
    if (firstPendingPullInto->bytesFilled() > 0) {
      ...
      RootedValue e(cx);
      // Not much we can do about uncatchable exceptions, just bail.
      if (!cx->getPendingException(&e)) return false;
...
ReadableByteStreamControllerConvertPullIntoDescriptor(
    JSContext* cx, Handle<PullIntoDescriptor*> pullIntoDescriptor);

// Streams spec, 3.12.6.
// ReadableByteStreamControllerCommitPullIntoDescriptor ( stream,
//                                                        pullIntoDescriptor )
static MOZ_MUST_USE bool ReadableByteStreamControllerCommitPullIntoDescriptor(
    JSContext* cx, Handle<ReadableStream*> stream,
  ...
  filledView = ReadableByteStreamControllerConvertPullIntoDescriptor(
      cx, pullIntoDescriptor);
  if (!filledView) return false;
  ...
  if (readerType == ReaderType_Default) {
    // Step a: Perform ! ReadableStreamFulfillReadRequest(stream, filledView,
    // done).
    ...
    // Step b: Perform ! ReadableStreamFulfillReadIntoRequest(stream,
    // filledView, done).
    if (!ReadableStreamFulfillReadOrReadIntoRequest(cx, stream, filledViewVal,
                                                    done))
      return false;
...
static MOZ_MUST_USE bool ReadableByteStreamControllerEnqueueChunkToQueue(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
    HandleObject buffer, uint32_t byteOffset, uint32_t byteLength);

static MOZ_MUST_USE ArrayBufferObject* TransferArrayBuffer(JSContext* cx,
                                                           HandleObject buffer);
...
static MOZ_MUST_USE bool ReadableByteStreamControllerEnqueue(
  ...
  RootedArrayBufferObject transferredBuffer(cx,
                                            TransferArrayBuffer(cx, buffer));
  if (!transferredBuffer) return false;
  ...
  if (ReadableStreamHasDefaultReader(stream)) {
    ...
    if (ReadableStreamGetNumReadRequests(stream) == 0) {
      ...
      if (!ReadableByteStreamControllerEnqueueChunkToQueue(
              cx, controller, transferredBuffer, byteOffset, byteLength)) {
        return false;
      }
    } else {
    // Step b: Otherwise,
    // Step i: Assert: controller.[[queue]] is empty.
      ...
      RootedObject transferredView(
          cx, JS_NewUint8ArrayWithBuffer(cx, transferredBuffer, byteOffset,
                                         byteLength));
      if (!transferredView) return false;

      // Step iii: Perform
      //           ! ReadableStreamFulfillReadRequest(stream,
...
static MOZ_MUST_USE bool ReadableByteStreamControllerEnqueueChunkToQueue(
  ...
  if (!AppendToList(cx, queue, chunkVal)) return false;
...
// Streams spec, 3.12.11.
// ReadableByteStreamControllerFillHeadPullIntoDescriptor (
//   controler,
//   size,
//   pullIntoDescriptor
// )
static void ReadableByteStreamControllerFillHeadPullIntoDescriptor(
    ReadableByteStreamController* controller, uint32_t size,
    Handle<PullIntoDescriptor*> pullIntoDescriptor) {
// Step 1: Assert: either controller.[[pendingPullIntos]] is empty, or the
//         first element of controller.[[pendingPullIntos]] is
//         pullIntoDescriptor.
#if DEBUG
  Value val = controller->getFixedSlot(ByteControllerSlot_PendingPullIntos);
  NativeObject* pendingPullIntos = &val.toObject().as<NativeObject>();
  MOZ_ASSERT(pendingPullIntos->getDenseInitializedLength() == 0 ||
             &pendingPullIntos->getDenseElement(0).toObject() ==
                 pullIntoDescriptor);
#endif  // DEBUG
...
ReadableByteStreamControllerFillPullIntoDescriptorFromQueue(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
    Handle<PullIntoDescriptor*> pullIntoDescriptor, bool* ready) {
  *ready = false;
  ...
  // Step 4: Let maxBytesFilled be pullIntoDescriptor.[[bytesFilled]] +
  // maxBytesToCopy.
  ...
  while (totalBytesToCopyRemaining > 0) {
    ...
    // Step d:
    // Perform
    // ! CopyDataBlockBytes(pullIntoDescriptor.[[buffer]]
    //                                        .[[ArrayBufferData]],
    //                      destStart,
    ...
    RootedArrayBufferObject targetBuffer(cx, pullIntoDescriptor->buffer());
    ArrayBufferObject::copyData(targetBuffer, destStart, sourceBuffer,
    ...
    // Step i: Set totalBytesToCopyRemaining to totalBytesToCopyRemaining −
    // bytesToCopy.
  ...
  if (!*ready) {
    ...
    MOZ_ASSERT(
        controller->getFixedSlot(QueueContainerSlot_TotalSize).toNumber() == 0);
...
static MOZ_MUST_USE bool ReadableByteStreamControllerHandleQueueDrain(
  ...
  // Step 1: Assert: controller.[[controlledReadableStream]].[[state]] is
  // "readable".
  Rooted<ReadableStream*> stream(cx, StreamFromController(controller));
  MOZ_ASSERT(stream->readable());
...
// Streams spec 3.12.15. ReadableByteStreamControllerInvalidateBYOBRequest (
// controller )
static void ReadableByteStreamControllerInvalidateBYOBRequest(
    NativeObject* controller) {
  MOZ_ASSERT(controller->is<ReadableByteStreamController>());
  ...
  Value byobRequestVal =
      controller->getFixedSlot(ByteControllerSlot_BYOBRequest);
  if (byobRequestVal.isUndefined()) return;

  NativeObject* byobRequest = &byobRequestVal.toObject().as<NativeObject>();
  // Step 2: Set
  // controller.[[byobRequest]].[[associatedReadableByteStreamController]]
  //         to undefined.
...
ReadableByteStreamControllerShiftPendingPullInto(JSContext* cx,
                                                 HandleNativeObject controller);
...
ReadableByteStreamControllerProcessPullIntoDescriptorsUsingQueue(
  ...
  while (pendingPullIntos->getDenseInitializedLength() != 0) {
    ...
    double queueTotalSize =
        controller->getFixedSlot(QueueContainerSlot_TotalSize).toNumber();
    if (queueTotalSize == 0) return true;
    ...
    if (!ReadableByteStreamControllerFillPullIntoDescriptorFromQueue(
            cx, controller, pullIntoDescriptor, &ready)) {
      return false;
    }
    if (ready) {
      // Step i: Perform !
...
// Streams spec, 3.12.17. ReadableByteStreamControllerPullInto ( controller,
// view )
static MOZ_MUST_USE JSObject* ReadableByteStreamControllerPullInto(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
    Handle<ArrayBufferViewObject*> view) {
  ...
  // Step 5: Let pullIntoDescriptor be Record {[[buffer]]:
  ...
  RootedArrayBufferObject buffer(
      cx,
      &JS_GetArrayBufferViewBuffer(cx, view, &dummy)->as<ArrayBufferObject>());
  ...
  Rooted<PullIntoDescriptor*> pullIntoDescriptor(cx);
  pullIntoDescriptor =
      PullIntoDescriptor::create(cx, buffer, byteOffset, byteLength, 0,
                                 elementSize, ctor, ReaderType_BYOB);
  if (!pullIntoDescriptor) return nullptr;
  ...
  if (pendingPullIntos->getDenseInitializedLength() != 0) {
    ...
    RootedArrayBufferObject transferredBuffer(cx,
                                              TransferArrayBuffer(cx, buffer));
    if (!transferredBuffer) return nullptr;
  ...
  if (queueTotalSize > 0) {
    ...
    if (ControllerFlags(controller) & ControllerFlag_CloseRequested) {
      ...
      // Not much we can do about uncatchable exceptions, just bail.
      RootedValue e(cx);
      if (!GetAndClearException(cx, &e)) return nullptr;

      // Step ii: Perform ! ReadableByteStreamControllerError(controller, e).
      if (!ReadableStreamControllerError(cx, controller, e)) return nullptr;
  ...
  Rooted<PromiseObject*> promise(cx,
                                 ReadableStreamAddReadIntoRequest(cx, stream));
  if (!promise) return nullptr;
...
static MOZ_MUST_USE bool ReadableByteStreamControllerRespondInternal(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
    double bytesWritten);

// Streams spec 3.12.18. ReadableByteStreamControllerRespond( controller,
// bytesWritten )
static MOZ_MUST_USE bool ReadableByteStreamControllerRespond(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
    HandleValue bytesWrittenVal) {
  ...
  double bytesWritten;
  if (!ToNumber(cx, bytesWrittenVal, &bytesWritten)) return false;

  // Step 2: If ! IsFiniteNonNegativeNumber(bytesWritten) is false,
  if (bytesWritten < 0 || mozilla::IsNaN(bytesWritten) ||
      mozilla::IsInfinite(bytesWritten)) {
    JS_ReportErrorNumberASCII(cx, GetErrorMessage, nullptr,
  ...
  // Step 4: Perform ? ReadableByteStreamControllerRespondInternal(controller,
  // bytesWritten).
  return ReadableByteStreamControllerRespondInternal(cx, controller,
                                                     bytesWritten);
}

// Streams spec 3.12.19. ReadableByteStreamControllerRespondInClosedState(
// controller, firstDescriptor )
static MOZ_MUST_USE bool ReadableByteStreamControllerRespondInClosedState(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
...
// pullIntoDescriptor )
static MOZ_MUST_USE bool ReadableByteStreamControllerRespondInReadableState(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
    uint32_t bytesWritten, Handle<PullIntoDescriptor*> pullIntoDescriptor) {
  // Step 1: If pullIntoDescriptor.[[bytesFilled]] + bytesWritten >
  // pullIntoDescriptor.[[byteLength]],
  ...
  uint32_t elementSize = pullIntoDescriptor->elementSize();
  if (bytesFilled < elementSize) return true;
  ...
  if (remainderSize > 0) {
    ...
    if (!remainderObj) return false;
    RootedArrayBufferObject remainder(cx,
                                      &remainderObj->as<ArrayBufferObject>());
    ArrayBufferObject::copyData(remainder, 0, buffer, end - remainderSize,
                                remainderSize);
    ...
    if (!ReadableByteStreamControllerEnqueueChunkToQueue(
            cx, controller, remainder, 0, remainderSize)) {
      return false;
    }
  }
  ...
  // Step 8: Set pullIntoDescriptor.[[bytesFilled]] to
  // pullIntoDescriptor.[[bytesFilled]] −
  //         remainderSize.
  ...
  // Step 10: Perform !
  // ReadableByteStreamControllerProcessPullIntoDescriptorsUsingQueue(controller).
  return ReadableByteStreamControllerProcessPullIntoDescriptorsUsingQueue(
      cx, controller);
}

// Streams spec, 3.12.21. ReadableByteStreamControllerRespondInternal (
// controller, bytesWritten )
static MOZ_MUST_USE bool ReadableByteStreamControllerRespondInternal(
    JSContext* cx, Handle<ReadableByteStreamController*> controller,
    double bytesWritten) {
  // Step 1: Let firstDescriptor be the first element of
  // controller.[[pendingPullIntos]].
  RootedValue val(
      cx, controller->getFixedSlot(ByteControllerSlot_PendingPullIntos));
  RootedNativeObject pendingPullIntos(cx, &val.toObject().as<NativeObject>());
  ...
  if (stream->closed()) {
    ...
    //         firstDescriptor).
    return ReadableByteStreamControllerRespondInClosedState(cx, controller,
                                                            firstDescriptor);
  }
  ...
  //                                                      bytesWritten,
  //                                                      firstDescriptor).
  return ReadableByteStreamControllerRespondInReadableState(
      cx, controller, bytesWritten, firstDescriptor);
}

// Streams spec, 3.12.22.
...
static MOZ_MUST_USE bool ReadableByteStreamControllerRespondWithNewView(
  ...
  // Step 2: Let firstDescriptor be the first element of
  // controller.[[pendingPullIntos]].
  Rooted<PullIntoDescriptor*> firstDescriptor(cx);
  firstDescriptor = PeekList<PullIntoDescriptor>(pendingPullIntos);

  // Step 3: If firstDescriptor.[[byteOffset]] +
  ...
  bool dummy;
  RootedArrayBufferObject buffer(
      cx, &AsArrayBuffer(JS_GetArrayBufferViewBuffer(cx, view, &dummy)));
...
// Streams spec, 3.12.23. ReadableByteStreamControllerShiftPendingPullInto (
// controller )
static MOZ_MUST_USE PullIntoDescriptor*
ReadableByteStreamControllerShiftPendingPullInto(
  ...
  // Step 1: Let descriptor be the first element of
  // controller.[[pendingPullIntos]]. Step 2: Remove descriptor from
  // controller.[[pendingPullIntos]], shifting
  //         all other elements downward (so that the second becomes the first,
  //         and so on).
  RootedValue val(
      cx, controller->getFixedSlot(ByteControllerSlot_PendingPullIntos));
...
bool js::ByteLengthQueuingStrategy::constructor(JSContext* cx, unsigned argc,
  ...
  RootedObject strategy(cx,
                        NewBuiltinClassInstance<ByteLengthQueuingStrategy>(cx));
...
bool js::CountQueuingStrategy::constructor(JSContext* cx, unsigned argc,
  ...
  Rooted<CountQueuingStrategy*> strategy(
      cx, NewBuiltinClassInstance<CountQueuingStrategy>(cx));
  if (!strategy) return false;
...
inline static MOZ_MUST_USE bool DequeueValue(JSContext* cx,
  ...
  double totalSize =
      container->getFixedSlot(QueueContainerSlot_TotalSize).toNumber();
...
static MOZ_MUST_USE bool EnqueueValueWithSize(JSContext* cx,
  ...
  double size;
  if (!ToNumber(cx, sizeVal, &size)) return false;
  ...
  if (!AppendToList(cx, queue, val)) return false;

  // Step 5: Set container.[[queueTotalSize]] to
  //         container.[[queueTotalSize]] + size.
  double totalSize =
      container->getFixedSlot(QueueContainerSlot_TotalSize).toNumber();
  container->setFixedSlot(QueueContainerSlot_TotalSize,
                          NumberValue(totalSize + size));
...
inline static MOZ_MUST_USE bool ResetQueue(JSContext* cx,
  ...
  if (!SetNewList(cx, container, QueueContainerSlot_Queue)) return false;
...
inline static MOZ_MUST_USE bool InvokeOrNoop(JSContext* cx, HandleValue O,
  ...
  if (!GetProperty(cx, O, P, &method)) return false;

  // Step 4: If method is undefined, return.
  if (method.isUndefined()) return true;
 ...
 */
static MOZ_MUST_USE ArrayBufferObject* TransferArrayBuffer(
    JSContext* cx, HandleObject buffer) {
  // Step 1 (implicit).
  ...
  // Step 7.
  RootedObject transferredBuffer(
      cx, JS_NewArrayBufferWithContents(cx, size, contents));
  if (!transferredBuffer) return nullptr;
...
static MOZ_MUST_USE bool ValidateAndNormalizeHighWaterMark(
  ...
  if (!ToNumber(cx, highWaterMarkVal, highWaterMark)) return false;
...
// Streams spec, 6.4.6. ValidateAndNormalizeQueuingStrategy ( size,
//                                                            highWaterMark )
static MOZ_MUST_USE bool ValidateAndNormalizeQueuingStrategy(
    JSContext* cx, HandleValue size, HandleValue highWaterMarkVal,
    double* highWaterMark) {
...
MOZ_MUST_USE bool js::ReadableStreamReaderCancel(JSContext* cx,
...
MOZ_MUST_USE bool js::ReadableStreamReaderReleaseLock(JSContext* cx,
                                                      HandleObject readerObj) {
  MOZ_ASSERT(IsReadableStreamReader(readerObj));
  RootedNativeObject reader(cx, &readerObj->as<NativeObject>());
...
MOZ_MUST_USE bool ReadableStream::enqueue(JSContext* cx,
                                          Handle<ReadableStream*> stream,
                                          HandleValue chunk) {
  Rooted<ReadableStreamDefaultController*> controller(cx);
  controller =
      &ControllerFromStream(stream)->as<ReadableStreamDefaultController>();
...
MOZ_MUST_USE bool ReadableStream::enqueueBuffer(
    JSContext* cx, Handle<ReadableStream*> stream,
...
void ReadableStream::desiredSize(bool* hasSize, double* size) const {
  if (errored()) {
    *hasSize = false;
    return;
  }
...
/*static */ bool ReadableStream::getExternalSource(
    JSContext* cx, Handle<ReadableStream*> stream, void** source) {
...
void ReadableStream::releaseExternalSource() {
...
uint8_t ReadableStream::embeddingFlags() const {
  uint8_t flags = ControllerFlags(ControllerFromStream(this)) >>
...
MOZ_MUST_USE bool ReadableStream::updateDataAvailableFromSource(
    JSContext* cx, Handle<ReadableStream*> stream, uint32_t availableData) {
  Rooted<ReadableByteStreamController*> controller(cx);
  controller =
      &ControllerFromStream(stream)->as<ReadableByteStreamController>();
  ...
  uint32_t oldAvailableData =
      controller->getFixedSlot(QueueContainerSlot_TotalSize).toInt32();
#endif  // DEBUG
  controller->setFixedSlot(QueueContainerSlot_TotalSize,
  ...
  // of reader type.
  if (ReadableStreamGetNumReadRequests(stream) == 0) return true;
...
MOZ_MUST_USE bool ReadableStream::close(JSContext* cx,
                                        Handle<ReadableStream*> stream) {
  RootedNativeObject controllerObj(cx, ControllerFromStream(stream));
  if (!VerifyControllerStateForClosing(cx, controllerObj)) return false;
...
MOZ_MUST_USE bool ReadableStream::error(JSContext* cx,
                                        Handle<ReadableStream*> stream,
                                        HandleValue reason) {
  // Step 3: If stream.[[state]] is not "readable", throw a TypeError exception.
  if (!stream->readable()) {
...
MOZ_MUST_USE bool ReadableStream::tee(
    JSContext* cx, Handle<ReadableStream*> stream, bool cloneForBranch2,
    MutableHandle<ReadableStream*> branch1Stream,
    MutableHandle<ReadableStream*> branch2Stream) {
...
MOZ_MUST_USE NativeObject* ReadableStream::getReader(
    JSContext* cx, Handle<ReadableStream*> stream,
