...
MOZ_ALWAYS_INLINE bool IsWeakSet(HandleValue v) {
  return v.isObject() && v.toObject().is<WeakSetObject>();
}

// ES2018 draft rev 7a2d3f053ecc2336fc19f377c55d52d78b11b296
// 23.4.3.1 WeakSet.prototype.add ( value )
MOZ_ALWAYS_INLINE bool WeakSet_add_impl(JSContext* cx, const CallArgs& args) {
...
static bool WeakSet_add(JSContext* cx, unsigned argc, Value* vp) {
  // Steps 1-3.
...
MOZ_ALWAYS_INLINE bool WeakSet_delete_impl(JSContext* cx,
                                           const CallArgs& args) {
  MOZ_ASSERT(IsWeakSet(args.thisv()));
...
static bool WeakSet_delete(JSContext* cx, unsigned argc, Value* vp) {
  // Steps 1-3.
  CallArgs args = CallArgsFromVp(argc, vp);
...
MOZ_ALWAYS_INLINE bool WeakSet_has_impl(JSContext* cx, const CallArgs& args) {
  MOZ_ASSERT(IsWeakSet(args.thisv()));
...
static bool WeakSet_has(JSContext* cx, unsigned argc, Value* vp) {
  // Steps 1-3.
  CallArgs args = CallArgsFromVp(argc, vp);
...
const JSPropertySpec WeakSetObject::properties[] = {JS_PS_END};

const JSFunctionSpec WeakSetObject::methods[] = {
    JS_FN("add", WeakSet_add, 1, 0), JS_FN("delete", WeakSet_delete, 1, 0),
    JS_FN("has", WeakSet_has, 1, 0), JS_FS_END};

JSObject* WeakSetObject::initClass(JSContext* cx, HandleObject obj) {
  Handle<GlobalObject*> global = obj.as<GlobalObject>();
  RootedPlainObject proto(cx, NewBuiltinClassInstance<PlainObject>(cx));
  if (!proto) return nullptr;

  Rooted<JSFunction*> ctor(
      cx, GlobalObject::createConstructor(cx, construct,
  ...
  if (!ctor || !LinkConstructorAndPrototype(cx, ctor, proto) ||
      ...
      !GlobalObject::initBuiltinConstructor(cx, global, JSProto_WeakSet, ctor,
                                            proto)) {
    return nullptr;
  }
  return proto;
...
bool WeakSetObject::construct(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (!ThrowIfNotConstructing(cx, args, "WeakSet")) return false;
...
JSObject* js::InitWeakSetClass(JSContext* cx, HandleObject obj) {
