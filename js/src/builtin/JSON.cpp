...
static bool Quote(CharVectorT& sb, JSLinearString* str) {
  ...
  if (!sb.growByUninitialized(len * 6 + 2)) return false;
...
static bool PreprocessValue(JSContext* cx, HandleObject holder, KeyType key,
  ...
  if (scx->maybeSafely) return true;
  ...
  if (vp.isObject()) {
    ...
    if (!GetProperty(cx, obj, obj, cx->names().toJSON, &toJSON)) return false;
    ...
    if (IsCallable(toJSON)) {
      ...
      if (!js::Call(cx, toJSON, vp, arg0, vp)) return false;
    }
  }
  ...
  if (scx->replacer && scx->replacer->isCallable()) {
    ...
    if (!js::Call(cx, replacerVal, holder, arg0, vp, vp)) return false;
  }
  ...
  if (vp.get().isObject()) {
    ...
    if (cls == ESClass::Number) {
      double d;
      if (!ToNumber(cx, vp, &d)) return false;
      vp.setNumber(d);
      ...
      if (!Unbox(cx, obj, vp)) return false;
    }
  }
...
static bool JO(JSContext* cx, HandleObject obj, StringifyContext* scx) {
  ...
  if (!detect.foundCycle(cx)) return false;

  if (!scx->sb.append('{')) return false;
  ...
  if (scx->replacer && !scx->replacer->isCallable()) {
    ...
    ids.emplace(cx);
    if (!GetPropertyKeys(cx, obj, JSITER_OWNONLY, ids.ptr())) return false;
    props = ids.ptr();
  }
  ...
  for (size_t i = 0, len = propertyList.length(); i < len; i++) {
    if (!CheckForInterrupt(cx)) return false;
...
#endif  // DEBUG
    if (!GetProperty(cx, obj, obj, id, &outputValue)) return false;
    if (!PreprocessValue(cx, obj, HandleId(id), &outputValue, scx))
      return false;
    if (IsFilteredValue(outputValue)) continue;

    /* Output a comma unless this is the first member to write. */
    if (wroteMember && !scx->sb.append(',')) return false;
    wroteMember = true;

    if (!WriteIndent(scx, scx->depth)) return false;
  ...
  if (wroteMember && !WriteIndent(scx, scx->depth - 1)) return false;
...
static bool JA(JSContext* cx, HandleObject obj, StringifyContext* scx) {
  ...
  /* Step 6. */
  uint32_t length;
  if (!GetLengthProperty(cx, obj, &length)) return false;
  ...
  if (length != 0) {
...
#endif
      if (!GetElement(cx, obj, i, &outputValue)) return false;
      if (!PreprocessValue(cx, obj, i, &outputValue, scx)) return false;
      if (IsFilteredValue(outputValue)) {
        if (!scx->sb.append("null")) return false;
      } else {
        if (!Str(cx, outputValue, scx)) return false;
      }
    ...
    /* Step 10(b)(iii). */
    if (!WriteIndent(scx, scx->depth - 1)) return false;
  }
...
static bool Str(JSContext* cx, const Value& v, StringifyContext* scx) {
  ...
  /* Step 8. */
  if (v.isString()) return Quote(cx, scx->sb, v.toString());

  /* Step 5. */
  if (v.isNull()) return scx->sb.append("null");
...
bool js::Stringify(JSContext* cx, MutableHandleValue vp, JSObject* replacer_,
  ...
  if (replacer) {
    ...
    if (replacer->isCallable()) {
      ...
      /* Step 4b(iii)(2-3). */
      uint32_t len;
      if (!GetLengthProperty(cx, replacer, &len)) return false;
      ...
      if (!idSet.init(Min(len, MaxInitialSize))) return false;
      ...
      for (; k < len; k++) {
        ...
        /* Step 4b(iii)(5)(a-b). */
        if (!GetElement(cx, replacer, k, &item)) return false;
        ...
        if (item.isNumber()) {
          ...
          if (ValueFitsInInt32(item, &n) && INT_FITS_IN_JSID(n)) {
            ...
            if (!ValueToId<CanGC>(cx, item, &id)) return false;
          }
        } else {
          bool shouldAdd = item.isString();
          if (!shouldAdd) {
            ESClass cls;
            if (!GetClassOfValue(cx, item, &cls)) return false;
        ...
        if (!p) {
          /* Step 4b(iii)(5)(g)(i). */
          if (!idSet.add(p, id) || !propertyList.append(id)) return false;
        }
      }
    } else {
      replacer = nullptr;
  ...
  if (space.isObject()) {
    ...
    ESClass cls;
    if (!GetBuiltinClass(cx, spaceObj, &cls)) return false;

    if (cls == ESClass::Number) {
      double d;
      if (!ToNumber(cx, space, &d)) return false;
      space = NumberValue(d);
    } else if (cls == ESClass::String) {
  ...
  if (!PreprocessValue(cx, wrapper, HandleId(emptyId), vp, &scx)) return false;
  if (IsFilteredValue(vp)) return true;
...
static bool Walk(JSContext* cx, HandleObject holder, HandleId name,
  ...
  /* Step 1. */
  RootedValue val(cx);
  if (!GetProperty(cx, holder, holder, name, &val)) return false;
  ...
  if (val.isObject()) {
    ...
    if (isArray) {
      ...
      for (uint32_t i = 0; i < length; i++) {
        ...
        if (!IndexToId(cx, i, &id)) return false;

        /* Step 2a(iii)(1). */
        if (!Walk(cx, obj, id, reviver, &newElement)) return false;

        ObjectOpResult ignored;
        if (newElement.isUndefined()) {
          /* Step 2a(iii)(2). The spec deliberately ignores strict failure. */
          if (!DeleteProperty(cx, obj, id, ignored)) return false;
        } else {
          /* Step 2a(iii)(3). The spec deliberately ignores strict failure. */
          Rooted<PropertyDescriptor> desc(cx);
          desc.setDataDescriptor(newElement, JSPROP_ENUMERATE);
          if (!DefineProperty(cx, obj, id, desc, ignored)) return false;
        }
      }
    } else {
      /* Step 2b(i). */
      AutoIdVector keys(cx);
      if (!GetPropertyKeys(cx, obj, JSITER_OWNONLY, &keys)) return false;
...
static bool Revive(JSContext* cx, HandleValue reviver, MutableHandleValue vp) {
  ...
  if (!DefineDataProperty(cx, obj, cx->names().empty, vp)) return false;
...
bool js::ParseJSONWithReviver(JSContext* cx,
  ...
  if (!parser.parse(vp)) return false;

  /* 15.12.2 steps 4-5. */
  if (IsCallable(reviver)) return Revive(cx, reviver, vp);
  return true;
}
...
static bool json_parse(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (!linearChars.init(cx, linear)) return false;
...
JSObject* js::InitJSONClass(JSContext* cx, HandleObject obj) {
  ...
  if (!JS_DefineFunctions(cx, JSON, json_static_methods)) return nullptr;

  if (!DefineToStringTag(cx, JSON, cx->names().JSON)) return nullptr;
