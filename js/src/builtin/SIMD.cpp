...
static_assert(unsigned(SimdType::Count) == 12,
              "sync with TypedObjectConstants.h");

static bool ArgumentToLaneIndex(JSContext* cx, JS::HandleValue v,
                                unsigned limit, unsigned* lane);
...
#define FOR_EACH_SIMD(macro)                                                   \
  macro(Int8x16) macro(Int16x8) macro(Int32x4) macro(Uint8x16) macro(Uint16x8) \
      macro(Uint32x4) macro(Float32x4) macro(Float64x2) macro(Bool8x16)        \
          macro(Bool16x8) macro(Bool32x4) macro(Bool64x2)
...
bool js::IsSimdTypeName(const JSAtomState& atoms, const PropertyName* name,
                        SimdType* type) {
...
bool js::ToSimdConstant(JSContext* cx, HandleValue v, jit::SimdConstant* out) {
  ...
  Elem* mem =
      reinterpret_cast<Elem*>(v.toObject().as<TypedObject>().typedMem(nogc));
  *out = jit::SimdConstant::CreateSimd128(mem);
  return true;
}

template bool js::ToSimdConstant<Int8x16>(JSContext* cx, HandleValue v,
                                          jit::SimdConstant* out);
template bool js::ToSimdConstant<Int16x8>(JSContext* cx, HandleValue v,
                                          jit::SimdConstant* out);
template bool js::ToSimdConstant<Int32x4>(JSContext* cx, HandleValue v,
                                          jit::SimdConstant* out);
template bool js::ToSimdConstant<Float32x4>(JSContext* cx, HandleValue v,
                                            jit::SimdConstant* out);
template bool js::ToSimdConstant<Bool8x16>(JSContext* cx, HandleValue v,
                                           jit::SimdConstant* out);
template bool js::ToSimdConstant<Bool16x8>(JSContext* cx, HandleValue v,
                                           jit::SimdConstant* out);
template bool js::ToSimdConstant<Bool32x4>(JSContext* cx, HandleValue v,
                                           jit::SimdConstant* out);
...
static_assert(uint64_t(SimdOperation::Last) <= UINT16_MAX,
              "SimdOperation must fit in uint16_t");

// See also JitInfo_* in MCallOptimize.cpp. We provide a JSJitInfo for all the
// named functions here. The default JitInfo_SimdInt32x4 etc structs represent
// the SimdOperation::Constructor.
#define DEFN(TYPE, OP)                                                         \
  const JSJitInfo JitInfo_Simd##TYPE##_##OP = {                                \
      /* .getter, unused for inlinable natives. */                             \
      {nullptr}, /* .inlinableNative, but we have to init first union          \
                    member: .protoID. */                                       \
      {uint16_t(InlinableNative::Simd##TYPE)}, /* .nativeOp. Actually          \
                                                  initializing first union     \
                                                  member .depth. */            \
      {uint16_t(SimdOperation::Fn_##OP)}, /* .type_ bitfield says this in an   \
                                             inlinable native function. */     \
      JSJitInfo::InlinableNative          /* Remaining fields are not used for \
                                             inlinable natives. They are       \
                                             zero-initialized. */              \
  };
...
#define SIMD_FLOAT32X4_FUNCTION_ITEM(Name, Func, Operands)       \
  JS_INLINABLE_FN(#Name, js::simd_float32x4_##Name, Operands, 0, \
                  SimdFloat32x4_##Name),
...
#define SIMD_INT8X16_FUNCTION_ITEM(Name, Func, Operands)       \
  JS_INLINABLE_FN(#Name, js::simd_int8x16_##Name, Operands, 0, \
                  SimdInt8x16_##Name),
...
#define SIMD_INT16X8_FUNCTION_ITEM(Name, Func, Operands)       \
  JS_INLINABLE_FN(#Name, js::simd_int16x8_##Name, Operands, 0, \
                  SimdInt16x8_##Name),
...
#define SIMD_INT32X4_FUNCTION_ITEM(Name, Func, Operands)       \
  JS_INLINABLE_FN(#Name, js::simd_int32x4_##Name, Operands, 0, \
                  SimdInt32x4_##Name),
...
#define SIMD_UINT8X16_FUNCTION_ITEM(Name, Func, Operands)       \
  JS_INLINABLE_FN(#Name, js::simd_uint8x16_##Name, Operands, 0, \
                  SimdUint8x16_##Name),
...
#define SIMD_UINT16X8_FUNCTION_ITEM(Name, Func, Operands)       \
  JS_INLINABLE_FN(#Name, js::simd_uint16x8_##Name, Operands, 0, \
                  SimdUint16x8_##Name),
...
#define SIMD_UINT32X4_FUNCTION_ITEM(Name, Func, Operands)       \
  JS_INLINABLE_FN(#Name, js::simd_uint32x4_##Name, Operands, 0, \
                  SimdUint32x4_##Name),
...
#define SIMD_BOOL8X16_FUNCTION_ITEM(Name, Func, Operands)       \
  JS_INLINABLE_FN(#Name, js::simd_bool8x16_##Name, Operands, 0, \
                  SimdBool8x16_##Name),
...
#define SIMD_BOOL16X8_FUNCTION_ITEM(Name, Func, Operands)       \
  JS_INLINABLE_FN(#Name, js::simd_bool16x8_##Name, Operands, 0, \
                  SimdBool16x8_##Name),
...
#define SIMD_BOOL32X4_FUNCTION_ITEM(Name, Func, Operands)       \
  JS_INLINABLE_FN(#Name, js::simd_bool32x4_##Name, Operands, 0, \
                  SimdBool32x4_##Name),
...
template <typename T>
static bool FillLanes(JSContext* cx, Handle<TypedObject*> result,
                      const CallArgs& args) {
...
const Class SimdObject::class_ = {
    "SIMD", JSCLASS_HAS_RESERVED_SLOTS(uint32_t(SimdType::Count)),
    &SimdObjectClassOps};

/* static */ bool GlobalObject::initSimdObject(JSContext* cx,
  ...
  globalSimdObject = NewObjectWithGivenProto(cx, &SimdObject::class_, objProto,
                                             SingletonObject);
  if (!globalSimdObject) return false;

  RootedValue globalSimdValue(cx, ObjectValue(*globalSimdObject));
  if (!DefineDataProperty(cx, global, cx->names().SIMD, globalSimdValue,
                          JSPROP_RESOLVING))
    return false;
...
static bool CreateSimdType(JSContext* cx, Handle<GlobalObject*> global,
  ...
  typeDescr =
      NewObjectWithGivenProto<SimdTypeDescr>(cx, funcProto, SingletonObject);
  if (!typeDescr) return false;

  typeDescr->initReservedSlot(JS_DESCR_SLOT_KIND, Int32Value(type::Simd));
  typeDescr->initReservedSlot(JS_DESCR_SLOT_STRING_REPR,
                              StringValue(stringRepr));
  typeDescr->initReservedSlot(JS_DESCR_SLOT_ALIGNMENT,
                              Int32Value(SimdTypeDescr::alignment(simdType)));
  typeDescr->initReservedSlot(JS_DESCR_SLOT_SIZE,
                              Int32Value(SimdTypeDescr::size(simdType)));
  typeDescr->initReservedSlot(JS_DESCR_SLOT_OPAQUE, BooleanValue(false));
  typeDescr->initReservedSlot(JS_DESCR_SLOT_TYPE,
                              Int32Value(uint8_t(simdType)));
  ...
  RootedObject globalSimdObject(
      cx, GlobalObject::getOrCreateSimdGlobalObject(cx, global));
  MOZ_ASSERT(globalSimdObject);

  RootedValue typeValue(cx, ObjectValue(*typeDescr));
  if (!JS_DefineFunctions(cx, typeDescr, methods) ||
      !DefineDataProperty(
          cx, globalSimdObject, stringRepr, typeValue,
  ...
  MOZ_ASSERT(
      globalSimdObject->as<NativeObject>().getReservedSlot(slot).isUndefined());
  globalSimdObject->as<NativeObject>().setReservedSlot(slot,
                                                       ObjectValue(*typeDescr));
  return !!typeDescr;
}

/* static */ bool GlobalObject::initSimdType(JSContext* cx,
                                             Handle<GlobalObject*> global,
                                             SimdType simdType) {
#define CREATE_(Type)                                             \
  case SimdType::Type:                                            \
    return CreateSimdType(cx, global, cx->names().Type, simdType, \
                          Type##Defn::Methods);
...
/* static */ SimdTypeDescr* GlobalObject::getOrCreateSimdTypeDescr(
    JSContext* cx, Handle<GlobalObject*> global, SimdType simdType) {
  MOZ_ASSERT(unsigned(simdType) < unsigned(SimdType::Count),
             "Invalid SIMD type");
  ...
  if (globalSimdObject->as<NativeObject>()
          .getReservedSlot(typeSlotIndex)
          .isUndefined() &&
  ...
  const Value& slot =
      globalSimdObject->as<NativeObject>().getReservedSlot(typeSlotIndex);
  MOZ_ASSERT(slot.isObject());
  return &slot.toObject().as<SimdTypeDescr>();
}

bool SimdObject::resolve(JSContext* cx, JS::HandleObject obj, JS::HandleId id,
                         bool* resolved) {
  *resolved = false;
...
#define TRY_RESOLVE_(Type)                                                   \
  if (str == cx->names().Type) {                                             \
    *resolved = CreateSimdType(cx, global, cx->names().Type, SimdType::Type, \
                               Type##Defn::Methods);                         \
    return *resolved;                                                        \
  }
...
#define InstantiateCreateSimd_(Type)                      \
  template JSObject* js::CreateSimd<Type>(JSContext * cx, \
                                          const Type::Elem* data);
...
template <typename T>
struct MaybeMakeUnsigned<
    T, typename EnableIf<IsIntegral<T>::value && IsSigned<T>::value>::Type> {
...
struct Neg {
  ...
  static T apply(T x) {
    ...
    return static_cast<MaybeUnsignedT>(1U * MaybeUnsignedT(-1) *
                                       MaybeUnsignedT(x));
  }
};
template <typename T>
...
struct RecApprox {
  static_assert(IsFloatingPoint<T>::value,
                "RecApprox only supported for floating points");
  static T apply(T x) { return 1 / x; }
};
template <typename T>
struct RecSqrtApprox {
  static_assert(IsFloatingPoint<T>::value,
                "RecSqrtApprox only supported for floating points");
  static T apply(T x) { return 1 / sqrt(x); }
};
template <typename T>
struct Sqrt {
  static_assert(IsFloatingPoint<T>::value,
                "Sqrt only supported for floating points");
...
struct Div {
  static_assert(IsFloatingPoint<T>::value,
                "Div only supported for floating points");
...
struct MinNum {
  static T apply(T l, T r) {
    return IsNaN(l) ? r : (IsNaN(r) ? l : math_min_impl(l, r));
  }
};
template <typename T>
struct Maximum {
...
static T Saturate(int x) {
  static_assert(mozilla::IsIntegral<T>::value,
                "Only integer saturation supported");
  static_assert(sizeof(T) < sizeof(int),
                "Saturating int-sized arithmetic is not safe");
...
static bool StoreResult(JSContext* cx, CallArgs& args,
                        typename Out::Elem* result) {
...
template <typename In, typename Coercion, template <typename C> class Op,
          typename Out>
...
static bool CoercedBinaryFunc(JSContext* cx, unsigned argc, Value* vp) {
  ...
  CallArgs args = CallArgsFromVp(argc, vp);
  if (args.length() != 2 || !IsVectorObject<In>(args[0]) ||
      !IsVectorObject<In>(args[1]))
    return ErrorBadArgs(cx);
...
// defined behavior in C++, so we don't need to care about them here. In
// practice, this means round to nearest, tie with even (zero bit in
// significand).
template <>
...
struct ThrowIfNotInRange {
  static_assert(mozilla::IsIntegral<IntegerType>::value,
...
template <>
struct ThrowOnConvert<double, int32_t>
    : public ThrowIfNotInRange<double, int32_t> {};

template <>
struct ThrowOnConvert<double, uint32_t>
    : public ThrowIfNotInRange<double, uint32_t> {};

template <>
struct ThrowOnConvert<float, int32_t>
    : public ThrowIfNotInRange<float, int32_t> {};

template <>
struct ThrowOnConvert<float, uint32_t>
    : public ThrowIfNotInRange<float, uint32_t> {};
...
static bool FuncConvert(JSContext* cx, unsigned argc, Value* vp) {
  ...
  static_assert(!mozilla::IsSame<V, Vret>::value,
                "Can't convert SIMD type to itself");
  static_assert(V::lanes == Vret::lanes,
                "Can only convert from same number of lanes");
  static_assert(
      !mozilla::IsIntegral<Elem>::value || !mozilla::IsIntegral<RetElem>::value,
...
static bool FuncConvertBits(JSContext* cx, unsigned argc, Value* vp) {
  ...
  {
    JS::AutoCheckCannotGC nogc(cx);
    memcpy(copy, TypedObjectMemory<RetElem*>(args[0], nogc),
           Vret::lanes * sizeof(RetElem));
  }
...
// Return the array handle in typedArray and idx converted to a byte offset in
// byteStart.
static bool TypedArrayFromArgs(JSContext* cx, const CallArgs& args,
                               uint32_t accessBytes,
                               MutableHandleObject typedArray,
                               size_t* byteStart) {
...
static bool Load(JSContext* cx, unsigned argc, Value* vp) {
  ...
  if (!TypedArrayFromArgs(cx, args, sizeof(Elem) * NumElem, &typedArray,
                          &byteStart))
    return false;
  ...
  JS::AutoCheckCannotGC nogc(cx);
  SharedMem<Elem*> src = typedArray->as<TypedArrayObject>()
                             .viewDataEither()
                             .addBytes(byteStart)
                             .cast<Elem*>();
  Elem* dst = reinterpret_cast<Elem*>(result->typedMem(nogc));
  jit::AtomicOperations::podCopySafeWhenRacy(SharedMem<Elem*>::unshared(dst),
                                             src, NumElem);
...
static bool Store(JSContext* cx, unsigned argc, Value* vp) {
  ...
  SharedMem<Elem*> dst = typedArray->as<TypedArrayObject>()
                             .viewDataEither()
                             .addBytes(byteStart)
                             .cast<Elem*>();
  js::jit::AtomicOperations::podCopySafeWhenRacy(
      dst, SharedMem<Elem*>::unshared(src), NumElem);
