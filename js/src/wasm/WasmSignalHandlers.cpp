...
#define XMM_sig(p, i) \
  (((union savefpu*)(p)->uc_mcontext.mc_fpregs)->sv_xmm.sv_xmm[i])
#else
#define XMM_sig(p, i) \
  (((struct savefpu*)(p)->uc_mcontext.mc_fpstate)->sv_xmm[i])
#endif
...
static uint8_t* ContextToLR(EMULATOR_CONTEXT* context) {
...
MOZ_COLD static void SetGPRegToZero(void* gp_reg) {
  memset(gp_reg, 0, sizeof(intptr_t));
}

MOZ_COLD static void SetFPRegToLoadedValue(SharedMem<void*> addr, size_t size,
                                           void* fp_reg) {
  MOZ_RELEASE_ASSERT(size <= Simd128DataSize);
  memset(fp_reg, 0, Simd128DataSize);
...
MOZ_COLD static void SetGPRegToLoadedValue(SharedMem<void*> addr, size_t size,
                                           void* gp_reg) {
...
MOZ_COLD static void SetGPRegToLoadedValueSext32(SharedMem<void*> addr,
                                                 size_t size, void* gp_reg) {
  MOZ_RELEASE_ASSERT(size <= sizeof(int32_t));
  int8_t msb =
      AtomicOperations::loadSafeWhenRacy(addr.cast<uint8_t*>() + (size - 1));
  memset(gp_reg, 0, sizeof(void*));
  memset(gp_reg, msb >> 7, sizeof(int32_t));
  AtomicOperations::memcpySafeWhenRacy(gp_reg, addr, size);
}

MOZ_COLD static void StoreValueFromFPReg(SharedMem<void*> addr, size_t size,
                                         const void* fp_reg) {
  MOZ_RELEASE_ASSERT(size <= Simd128DataSize);
  AtomicOperations::memcpySafeWhenRacy(addr, const_cast<void*>(fp_reg), size);
}

MOZ_COLD static void StoreValueFromGPReg(SharedMem<void*> addr, size_t size,
                                         const void* gp_reg) {
  MOZ_RELEASE_ASSERT(size <= sizeof(void*));
  AtomicOperations::memcpySafeWhenRacy(addr, const_cast<void*>(gp_reg), size);
}

MOZ_COLD static void StoreValueFromGPImm(SharedMem<void*> addr, size_t size,
                                         int32_t imm) {
...
MOZ_COLD static void* AddressOfFPRegisterSlot(
    CONTEXT* context, FloatRegisters::Encoding encoding) {
  switch (encoding) {
...
MOZ_COLD static void* AddressOfGPRegisterSlot(EMULATOR_CONTEXT* context,
                                              Registers::Code code) {
  switch (code) {
...
#else
MOZ_COLD static void* AddressOfFPRegisterSlot(
    EMULATOR_CONTEXT* context, FloatRegisters::Encoding encoding) {
  switch (encoding) {
    case X86Encoding::xmm0:
...
MOZ_COLD static void SetRegisterToCoercedUndefined(
    EMULATOR_CONTEXT* context, size_t size,
...
MOZ_COLD static void SetRegisterToLoadedValue(
    EMULATOR_CONTEXT* context, SharedMem<void*> addr, size_t size,
    const Disassembler::OtherOperand& value) {
  if (value.kind() == Disassembler::OtherOperand::FPR)
    SetFPRegToLoadedValue(addr, size,
                          AddressOfFPRegisterSlot(context, value.fpr()));
  else
    SetGPRegToLoadedValue(addr, size,
                          AddressOfGPRegisterSlot(context, value.gpr()));
}

MOZ_COLD static void SetRegisterToLoadedValueSext32(
    EMULATOR_CONTEXT* context, SharedMem<void*> addr, size_t size,
    const Disassembler::OtherOperand& value) {
  SetGPRegToLoadedValueSext32(addr, size,
                              AddressOfGPRegisterSlot(context, value.gpr()));
}

MOZ_COLD static void StoreValueFromRegister(
    EMULATOR_CONTEXT* context, SharedMem<void*> addr, size_t size,
    const Disassembler::OtherOperand& value) {
  if (value.kind() == Disassembler::OtherOperand::FPR)
    StoreValueFromFPReg(addr, size,
                        AddressOfFPRegisterSlot(context, value.fpr()));
  else if (value.kind() == Disassembler::OtherOperand::GPR)
    StoreValueFromGPReg(addr, size,
                        AddressOfGPRegisterSlot(context, value.gpr()));
  else
    StoreValueFromGPImm(addr, size, value.imm());
}

MOZ_COLD static uint8_t* ComputeAccessAddress(
    EMULATOR_CONTEXT* context, const Disassembler::ComplexAddress& address) {
  MOZ_RELEASE_ASSERT(!address.isPCRelative(),
                     "PC-relative addresses not supported yet");
...
MOZ_COLD static void HandleMemoryAccess(EMULATOR_CONTEXT* context, uint8_t* pc,
                                        uint8_t* faultingAddress,
                                        const ModuleSegment* segment,
  ...
  if (address.hasBase()) {
    ...
    StoreValueFromGPReg(SharedMem<void*>::unshared(&base), sizeof(uintptr_t),
                        AddressOfGPRegisterSlot(context, address.base()));
    MOZ_RELEASE_ASSERT(reinterpret_cast<uint8_t*>(base) ==
                       instance.memoryBase());
  }
  if (address.hasIndex()) {
    uintptr_t index;
  ...
  MOZ_RELEASE_ASSERT(
      size_t(faultingAddress - accessAddress) < access.size(),
  ...
  MOZ_RELEASE_ASSERT(accessAddress + access.size() <=
                         instance.memoryBase() + instance.memoryMappedSize(),
                     "Access extends beyond the asm.js heap guard region");
  MOZ_RELEASE_ASSERT(
      accessAddress + access.size() > instance.memoryBase() + memoryLength,
  ...
  intptr_t unwrappedOffset =
      accessAddress - instance.memoryBase().unwrap(/* for value */);
  ...
  if (inBounds) {
    ...
    MOZ_RELEASE_ASSERT(wrappedAddress + size <=
                       instance.memoryBase() + memoryLength);
    switch (access.kind()) {
      ...
      case Disassembler::HeapAccess::LoadSext32:
        SetRegisterToLoadedValueSext32(context, wrappedAddress.cast<void*>(),
                                       size, access.otherOperand());
        break;
    ...
    switch (access.kind()) {
      ...
      case Disassembler::HeapAccess::LoadSext32:
        ...
        SetRegisterToCoercedUndefined(context, access.size(),
                                      access.otherOperand());
        break;
      case Disassembler::HeapAccess::Store:
        // Do nothing.
...
MOZ_COLD static bool IsHeapAccessAddress(const Instance& instance,
                                         uint8_t* faultingAddress) {
...
static bool HandleFault(PEXCEPTION_POINTERS exception) {
  ...
  if (!codeSegment || !codeSegment->isModule()) return false;
  ...
  const Instance* instance =
      LookupFaultingInstance(*moduleSegment, pc, ContextToFP(context));
  if (!instance) {
  ...
  uint8_t* faultingAddress =
      reinterpret_cast<uint8_t*>(record->ExceptionInformation[1]);
  ...
  HandleMemoryAccess(context, pc, faultingAddress, moduleSegment, *instance,
                     activation, ppc);
  return true;
}
...
static LONG WINAPI WasmFaultHandler(LPEXCEPTION_POINTERS exception) {
  ...
  if (sAlreadyInSignalHandler.get()) return EXCEPTION_CONTINUE_SEARCH;
...
static bool HandleMachException(JSContext* cx,
...
#error Unsupported architecture
  ...
  if (request.body.exception == EXC_BAD_INSTRUCTION) {
    ...
    *ppc = moduleSegment->trapCode();
    ...
    if (request.body.codeCnt != 2) return false;
...
// The choice of id here is arbitrary, the only constraint is that sQuitId !=
// sExceptionId.
...
void MachExceptionHandler::uninstall() {
  if (installed_) {
    ...
    kern_return_t kret = thread_set_exception_ports(
        thread, EXC_MASK_BAD_ACCESS | EXC_MASK_BAD_INSTRUCTION, MACH_PORT_NULL,
...
bool MachExceptionHandler::install(JSContext* cx) {
  ...
  if (kret != KERN_SUCCESS) return false;
  kret = mach_port_insert_right(mach_task_self(), port_, port_,
  ...
  kret = thread_set_exception_ports(
      thread, EXC_MASK_BAD_ACCESS | EXC_MASK_BAD_INSTRUCTION, port_,
      EXCEPTION_DEFAULT | MACH_EXCEPTION_CODES, THREAD_STATE_NONE);
  mach_port_deallocate(mach_task_self(), thread);
  if (kret != KERN_SUCCESS) return false;
...
static bool HandleFault(int signum, siginfo_t* info, void* ctx) {
  // Before anything else, prevent handler recursion.
  if (sAlreadyInSignalHandler.get()) return false;
  AutoSignalHandler ash;
...
#ifdef __mips__
    if (info->si_code != FPE_INTOVF) return false;
#endif
    Trap trap;
    BytecodeOffset bytecode;
    if (!moduleSegment->code().lookupTrap(pc, &trap, &bytecode)) return false;
  ...
  if (signum == SIGBUS) {
    // TODO: We may see a bus error for something that is an unaligned access
    // that partly overlaps the end of the heap.  In this case, it is an
    // out-of-bounds error and we should signal that properly, but to do so we
    // must inspect the operand of the failed access.
    MOZ_ALWAYS_TRUE(activation->startWasmInterrupt(ToRegisterState(context)));
...
static void WasmFaultHandler(int signum, siginfo_t* info, void* context) {
  if (HandleFault(signum, info, context)) return;
...
bool wasm::InInterruptibleCode(JSContext* cx, uint8_t* pc,
  ...
  if (!cs || !cs->isModule()) return false;
...
static bool RedirectJitCodeToInterruptCheck(JSContext* cx, CONTEXT* context) {
  ...
  if (!InInterruptibleCode(cx, pc, &moduleSegment)) return false;
  ...
  if (!activation->startWasmInterrupt(ToRegisterState(context))) return false;
...
#if defined(JS_SIMULATOR_ARM) || defined(JS_SIMULATOR_MIPS32) || \
...
static bool ProcessHasSignalHandlers() {
  // We assume that there are no races creating the first JSRuntime of the
  // process.
...
void js::InterruptRunningJitCode(JSContext* cx) {
  ...
  if (cx == TlsContext.get()) {
    ...
    // We are not on the runtime's active thread, so to do 1 and 2 above, we
    // need to halt the runtime's active thread first.
#if defined(XP_WIN)
