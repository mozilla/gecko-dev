...
static void MaybeForceDebugGC() {
  ...
  if (!sEnvVarInitialized) sDebugGCs = !!PR_GetEnv("MOZ_DEBUG_DEAD_CPOWS");
...
bool WrapperAnswer::fail(AutoJSAPI& jsapi, ReturnStatus* rs) {
  ...
  *rs = ReturnStatus(ReturnException(JSVariant(UndefinedVariant())));
  ...
  RootedValue exn(cx);
  if (!jsapi.HasException()) return true;

  if (!jsapi.StealException(&exn)) return true;
...
bool WrapperAnswer::RecvPreventExtensions(const ObjectId& objId,
  ...
  AutoJSAPI jsapi;
  if (NS_WARN_IF(!jsapi.Init(scopeForTargetObjects()))) return false;
  JSContext* cx = jsapi.cx();

  RootedObject obj(cx, findObjectById(cx, objId));
  if (!obj) return deadCPOW(jsapi, rs);

  ObjectOpResult success;
  if (!JS_PreventExtensions(cx, obj, success)) return fail(jsapi, rs);
...
bool WrapperAnswer::RecvGetPropertyDescriptor(const ObjectId& objId,
  ...
  RootedId id(cx);
  if (!fromJSIDVariant(cx, idVar, &id)) return fail(jsapi, rs);

  Rooted<PropertyDescriptor> desc(cx);
  if (!JS_GetPropertyDescriptorById(cx, obj, id, &desc)) return fail(jsapi, rs);

  if (!fromDescriptor(cx, desc, out)) return fail(jsapi, rs);
...
bool WrapperAnswer::RecvDefineProperty(const ObjectId& objId,
  ...
  Rooted<PropertyDescriptor> desc(cx);
  if (!toDescriptor(cx, descriptor, &desc)) return fail(jsapi, rs);
...
bool WrapperAnswer::RecvDelete(const ObjectId& objId, const JSIDVariant& idVar,
  ...
  ObjectOpResult success;
  if (!JS_DeletePropertyById(cx, obj, id, success)) return fail(jsapi, rs);
  return ok(rs, success);
}
...
bool WrapperAnswer::RecvHas(const ObjectId& objId, const JSIDVariant& idVar,
  ...
  *foundp = false;
  ...
  if (!JS_HasPropertyById(cx, obj, id, foundp)) return fail(jsapi, rs);
  return ok(rs);
}
...
bool WrapperAnswer::RecvHasOwn(const ObjectId& objId, const JSIDVariant& idVar,
  ...
  *foundp = false;
  ...
  if (!JS_HasOwnPropertyById(cx, obj, id, foundp)) return fail(jsapi, rs);
  return ok(rs);
}
...
bool WrapperAnswer::RecvGet(const ObjectId& objId, const JSVariant& receiverVar,
  ...
  *result = UndefinedVariant();
  ...
  RootedObject obj(cx, findObjectById(cx, objId));
  if (!obj) return deadCPOW(aes, rs);

  RootedValue receiver(cx);
  if (!fromVariant(cx, receiverVar, &receiver)) return fail(aes, rs);

  RootedId id(cx);
  if (!fromJSIDVariant(cx, idVar, &id)) return fail(aes, rs);

  JS::RootedValue val(cx);
  if (!JS_ForwardGetPropertyTo(cx, obj, id, receiver, &val))
    return fail(aes, rs);

  if (!toVariant(cx, val, result)) return fail(aes, rs);
...
bool WrapperAnswer::RecvSet(const ObjectId& objId, const JSIDVariant& idVar,
  ...
  RootedValue val(cx);
  if (!fromVariant(cx, value, &val)) return fail(aes, rs);
...
bool WrapperAnswer::RecvIsExtensible(const ObjectId& objId, ReturnStatus* rs,
  ...
  *result = false;
  ...
  bool extensible;
  if (!JS_IsExtensible(cx, obj, &extensible)) return fail(jsapi, rs);
...
bool WrapperAnswer::RecvCallOrConstruct(const ObjectId& objId,
  ...
  *result = UndefinedVariant();
  ...
  RootedValue objv(cx);
  if (!fromVariant(cx, argv[0], &objv)) return fail(aes, rs);
  ...
  *result = JSVariant(UndefinedVariant());
  ...
  for (size_t i = 0; i < argv.Length(); i++) {
    if (argv[i].type() == JSParam::Tvoid_t) {
      ...
      if (!obj) return fail(aes, rs);
      if (!outobjects.append(ObjectValue(*obj))) return fail(aes, rs);
      if (!vals.append(ObjectValue(*obj))) return fail(aes, rs);
    } else {
      RootedValue v(cx);
      if (!fromVariant(cx, argv[i].get_JSVariant(), &v)) return fail(aes, rs);
      if (!vals.append(v)) return fail(aes, rs);
    }
  }
  ...
  RootedValue rval(cx);
  {
    ...
    if (construct) {
      RootedObject obj(cx);
      if (!JS::Construct(cx, vals[0], args, &obj)) return fail(aes, rs);
      rval.setObject(*obj);
    } else {
  ...
  if (!toVariant(cx, rval, result)) return fail(aes, rs);
  ...
  for (size_t i = 0; i < outobjects.length(); i++) {
    ...
    if (JS_HasProperty(cx, obj, "value", &found)) {
      if (!JS_GetProperty(cx, obj, "value", &v)) return fail(aes, rs);
    } else {
      v = UndefinedValue();
    }
    if (!vals.append(v)) return fail(aes, rs);
  ...
  for (size_t i = 0; i < vals.length(); i++) {
    JSVariant variant;
    if (!toVariant(cx, vals[i], &variant)) return fail(aes, rs);
...
bool WrapperAnswer::RecvHasInstance(const ObjectId& objId,
  ...
  RootedValue val(cx);
  if (!fromVariant(cx, vVar, &val)) return fail(jsapi, rs);

  if (!JS_HasInstance(cx, obj, val, bp)) return fail(jsapi, rs);
...
bool WrapperAnswer::RecvGetBuiltinClass(const ObjectId& objId, ReturnStatus* rs,
  ...
  *classValue = uint32_t(js::ESClass::Other);
  ...
  js::ESClass cls;
  if (!js::GetBuiltinClass(cx, obj, &cls)) return fail(jsapi, rs);
...
bool WrapperAnswer::RecvIsArray(const ObjectId& objId, ReturnStatus* rs,
  ...
  *ans = uint32_t(IsArrayAnswer::NotArray);
  ...
  if (!JS::IsArray(cx, obj, &answer)) return fail(jsapi, rs);
...
bool WrapperAnswer::RecvGetPrototype(const ObjectId& objId, ReturnStatus* rs,
  ...
  *result = NullVariant();
  ...
  if (!JS_GetPrototype(cx, obj, &proto)) return fail(jsapi, rs);

  if (!toObjectOrNullVariant(cx, proto, result)) return fail(jsapi, rs);
...
bool WrapperAnswer::RecvRegExpToShared(const ObjectId& objId, ReturnStatus* rs,
  ...
  if (!sourceJSStr) return fail(jsapi, rs);
  nsAutoJSString sourceStr;
  if (!sourceStr.init(cx, sourceJSStr)) return fail(jsapi, rs);
...
bool WrapperAnswer::RecvGetPropertyKeys(const ObjectId& objId,
  ...
  if (!js::GetPropertyKeys(cx, obj, flags, &props)) return fail(jsapi, rs);

  for (size_t i = 0; i < props.length(); i++) {
    JSIDVariant id;
    if (!toJSIDVariant(cx, props[i], &id)) return fail(jsapi, rs);
...
bool WrapperAnswer::RecvInstanceOf(const ObjectId& objId, const JSIID& iid,
  ...
  * instanceof = false;
  ...
  if (rv != NS_OK) return fail(jsapi, rs);
