...
bool XPCConvert::IsMethodReflectable(const nsXPTMethodInfo& info) {
  if (info.IsNotXPCOM() || info.IsHidden()) return false;
  ...
  for (int i = info.GetParamCount() - 1; i >= 0; i--) {
    ...
    if (type.TagPart() == nsXPTType::T_VOID) return false;
  }
  return true;
}
...
static JSObject* UnwrapNativeCPOW(nsISupports* wrapper) {
  ...
  if (underware) {
    ...
    if (mainObj && mozilla::jsipc::IsWrappedCPOW(mainObj)) return mainObj;
  }
  return nullptr;
}
...
bool XPCConvert::NativeData2JS(MutableHandleValue d, const void* s,
  ...
  AutoJSContext cx;
  if (pErr) *pErr = NS_ERROR_XPC_BAD_CONVERT_NATIVE;
  ...
  switch (type.TagPart()) {
    ...
    case nsXPTType::T_DOMSTRING: {
      ...
      nsStringBuffer* buf;
      if (!XPCStringConvert::ReadableToJSVal(cx, *p, &buf, d)) return false;
      if (buf) buf->AddRef();
      return true;
    }
    ...
    case nsXPTType::T_CHAR_STR: {
      ...
      for (char* t = p; *t && isAscii; t++) {
        if (ILLEGAL_CHAR_RANGE(*t)) isAscii = false;
      }
    ...
    case nsXPTType::T_UTF8STRING: {
      ...
      char16_t* buffer = static_cast<char16_t*>(JS_malloc(cx, buffer_size));
      if (!buffer) return false;

      uint32_t copied;
      if (!UTF8ToUnicodeBuffer(*utf8String, buffer, &copied) || len != copied) {
...
static void CheckCharsInCharRange(const CharT* chars, size_t len) {
  for (size_t i = 0; i < len; i++) {
    if (!CheckChar16InCharRange(chars[i])) break;
  }
}
#endif
...
// static
bool XPCConvert::JSData2Native(void* d, HandleValue s, const nsXPTType& type,
                               const nsID* iid, nsresult* pErr) {
  NS_PRECONDITION(d, "bad param");

  AutoJSContext cx;
  if (pErr) *pErr = NS_ERROR_XPC_BAD_CONVERT_JS;
  ...
  switch (type.TagPart()) {
    ...
    case nsXPTType::T_CHAR: {
      ...
      if (JS_GetStringLength(str) == 0) {
        ...
        if (!JS_GetStringCharAt(cx, str, 0, &ch)) return false;
      }
#ifdef DEBUG
    ...
    case nsXPTType::T_DOMSTRING: {
      ...
      if (!str) {
        ...
        if (!AssignJSString(cx, *ws, str)) return false;
      }
      return true;
    }
    ...
    case nsXPTType::T_CHAR_STR: {
      ...
      if (JS_StringHasLatin1Chars(str)) {
        ...
        if (chars) CheckCharsInCharRange(chars, len);
      } else {
        size_t len;
        AutoCheckCannotGC nogc;
    ...
    case nsXPTType::T_WCHAR_STR: {
      ...
      if (!JS_CopyStringChars(cx, destChars, str)) return false;
      destChars[len] = 0;
...
bool XPCConvert::NativeInterface2JSObject(MutableHandleValue d,
                                          ...
                                          nsresult* pErr) {
  if (!iid) iid = &NS_GET_IID(nsISupports);

  d.setNull();
  if (!aHelper.Object()) return true;
  if (pErr) *pErr = NS_ERROR_XPC_BAD_CONVERT_NATIVE;
  ...
  if (flat) {
    if (allowNativeWrapper && !JS_WrapObject(cx, &flat)) return false;
  ...
  if (iid->Equals(NS_GET_IID(nsISupports))) {
    ...
    if (promise) {
      ...
      if (!JS_WrapObject(cx, &flat)) return false;
      d.setObjectOrNull(flat);
      return true;
    }
  }
  ...
  if (cpow) {
    if (!JS_WrapObject(cx, &cpow)) return false;
    d.setObject(*cpow);
    return true;
  }
  ...
  if (NS_FAILED(rv) && pErr) *pErr = rv;

  // If creating the wrapped native failed, then return early.
  if (NS_FAILED(rv) || !wrapper) return false;
...
// static
bool XPCConvert::JSObject2NativeInterface(void** dest, HandleObject src,
                                          const nsID* iid, nsISupports* aOuter,
                                          nsresult* pErr) {
  ...
  *dest = nullptr;
  ...
  if (!aOuter) {
    ...
    RootedObject inner(cx, js::CheckedUnwrap(src,
                                             /* stopAtWindowProxy = */ false));
    if (!inner) {
      if (pErr) *pErr = NS_ERROR_XPC_SECURITY_MANAGER_VETO;
      return false;
    }

    // Is this really a native xpcom object with a wrapper?
    XPCWrappedNative* wrappedNative = nullptr;
    if (IS_WN_REFLECTOR(inner)) wrappedNative = XPCWrappedNative::Get(inner);
    if (wrappedNative) {
  ...
  nsresult rv =
      nsXPCWrappedJS::GetNewOrUsed(src, *iid, getter_AddRefs(wrapper));
  if (pErr) *pErr = rv;
  ...
  if (aOuter) wrapper->SetAggregatedNativeObject(aOuter);
...
static nsresult JSErrorToXPCException(const char* toStringResult,
  ...
  if (data) {
    ...
    rv = XPCConvert::ConstructException(
        NS_ERROR_XPC_JAVASCRIPT_ERROR_WITH_DETAILS, formattedMsg.get(),
        ifaceName, methodName, static_cast<nsIScriptError*>(data.get()),
...
nsresult XPCConvert::JSValToXPCException(MutableHandleValue s,
  ...
  if (!s.isPrimitive()) {
    ...
    if (!unwrapped) return NS_ERROR_XPC_SECURITY_MANAGER_VETO;
    if (nsCOMPtr<nsISupports> supports =
      ...
      if (nullptr != (report = JS_ErrorFromException(cx, obj))) {
        ...
        if (str) toStringResult.encodeUtf8(cx, str);
      ...
      if (!strBytes) return NS_ERROR_FAILURE;

      return ConstructException(NS_ERROR_XPC_JS_THREW_JS_OBJECT, strBytes.ptr(),
  ...
  if (str) {
    JSAutoByteString strBytes(cx, str);
    if (!!strBytes) {
      return ConstructException(NS_ERROR_XPC_JS_THREW_STRING, strBytes.ptr(),
                                ifaceName, methodName, nullptr, exceptn, cx,
                                s.address());
    }
  }
...
// static
bool XPCConvert::NativeArray2JS(MutableHandleValue d, const void** s,
                                const nsXPTType& type, const nsID* iid,
...
static void* CheckTargetAndPopulate(const nsXPTType& type, uint8_t requiredType,
                                    size_t typeSize, uint32_t count,
                                    JSObject* tArr, nsresult* pErr) {
  ...
  if (count > max) {
    if (pErr) *pErr = NS_ERROR_OUT_OF_MEMORY;
...
// static
bool XPCConvert::JSTypedArray2Native(void** d, JSObject* jsArray,
  ...
  if (len < count) {
    if (pErr) *pErr = NS_ERROR_XPC_NOT_ENOUGH_ELEMENTS_IN_ARRAY;
  ...
  switch (JS_GetArrayBufferViewType(jsArray)) {
    case js::Scalar::Int8:
      output = CheckTargetAndPopulate(nsXPTType::T_I8, type, sizeof(int8_t),
                                      count, jsArray, pErr);
      if (!output) {
        return false;
      }
    ...
    case js::Scalar::Uint8Clamped:
      output = CheckTargetAndPopulate(nsXPTType::T_U8, type, sizeof(uint8_t),
                                      count, jsArray, pErr);
      if (!output) {
        return false;
      }
    ...
    case js::Scalar::Int16:
      output = CheckTargetAndPopulate(nsXPTType::T_I16, type, sizeof(int16_t),
                                      count, jsArray, pErr);
      if (!output) {
        return false;
      }
    ...
    case js::Scalar::Uint16:
      output = CheckTargetAndPopulate(nsXPTType::T_U16, type, sizeof(uint16_t),
                                      count, jsArray, pErr);
      if (!output) {
        return false;
      }
    ...
    case js::Scalar::Int32:
      output = CheckTargetAndPopulate(nsXPTType::T_I32, type, sizeof(int32_t),
                                      count, jsArray, pErr);
      if (!output) {
        return false;
      }
    ...
    case js::Scalar::Uint32:
      output = CheckTargetAndPopulate(nsXPTType::T_U32, type, sizeof(uint32_t),
                                      count, jsArray, pErr);
      if (!output) {
        return false;
      }
    ...
    case js::Scalar::Float32:
      output = CheckTargetAndPopulate(nsXPTType::T_FLOAT, type, sizeof(float),
                                      count, jsArray, pErr);
      if (!output) {
        return false;
      }
    ...
    case js::Scalar::Float64:
      output = CheckTargetAndPopulate(nsXPTType::T_DOUBLE, type, sizeof(double),
                                      count, jsArray, pErr);
      if (!output) {
        return false;
      }
...
// static
bool XPCConvert::JSArray2Native(void** d, HandleValue s, uint32_t count,
                                const nsXPTType& type, const nsID* iid,
                                nsresult* pErr) {
  MOZ_ASSERT(d, "bad param");
  ...
  if (!s.isObject()) {
    if (pErr) *pErr = NS_ERROR_XPC_CANT_CONVERT_PRIMITIVE_TO_ARRAY;
    return false;
  }
  ...
  if (!JS_IsArrayObject(cx, jsarray, &isArray) || !isArray) {
    if (pErr) *pErr = NS_ERROR_XPC_CANT_CONVERT_OBJECT_TO_ARRAY;
    return false;
  }
...
#define POPULATE(_mode, _t)                                                   \
  ...
  if (count > max || nullptr == (array = moz_xmalloc(count * sizeof(_t)))) {  \
    if (pErr) *pErr = NS_ERROR_OUT_OF_MEMORY;                                 \
    goto failure;                                                             \
  }                                                                           \
  for (initedCount = 0; initedCount < count; initedCount++) {                 \
    if (!JS_GetElement(cx, jsarray, initedCount, &current) ||                 \
        !JSData2Native(((_t*)array) + initedCount, current, type, iid, pErr)) \
      goto failure;                                                           \
  }                                                                           \
  PR_END_MACRO
  ...
  switch (type.TagPart()) {
    ...
    case nsXPTType::T_U8:
      ...
      break;
    case nsXPTType::T_U16:
      POPULATE(na, uint16_t);
      break;
    case nsXPTType::T_U32:
      POPULATE(na, uint32_t);
      break;
    case nsXPTType::T_U64:
      POPULATE(na, uint64_t);
      break;
    case nsXPTType::T_FLOAT:
      POPULATE(na, float);
      break;
    ...
    case nsXPTType::T_CHAR_STR:
      ...
      break;
    case nsXPTType::T_WCHAR_STR:
      POPULATE(fr, char16_t*);
      break;
    case nsXPTType::T_INTERFACE:
      POPULATE(re, nsISupports*);
      break;
    case nsXPTType::T_INTERFACE_IS:
      POPULATE(re, nsISupports*);
      break;
...
// static
bool XPCConvert::NativeStringWithSize2JS(MutableHandleValue d, const void* s,
                                         const nsXPTType& type, uint32_t count,
                                         nsresult* pErr) {
  NS_PRECONDITION(s, "bad param");
  ...
  switch (type.TagPart()) {
    case nsXPTType::T_PSTRING_SIZE_IS: {
      ...
      if (!p) break;
      JSString* str;
      if (!(str = JS_NewStringCopyN(cx, p, count))) return false;
      d.setString(str);
      break;
    }
    case nsXPTType::T_PWSTRING_SIZE_IS: {
      ...
      if (!p) break;
      JSString* str;
      if (!(str = JS_NewUCStringCopyN(cx, p, count))) return false;
      d.setString(str);
      break;
    }
    default:
...
// static
bool XPCConvert::JSStringWithSize2Native(void* d, HandleValue s, uint32_t count,
  ...
  switch (type.TagPart()) {
    case nsXPTType::T_PSTRING_SIZE_IS: {
      if (s.isUndefined() || s.isNull()) {
        if (0 != count) {
          if (pErr) *pErr = NS_ERROR_XPC_NOT_ENOUGH_CHARS_IN_STRING;
          return false;
        }
        *((char**)d) = nullptr;
        return true;
