...
NS_IMPL_CYCLE_COLLECTION_TRAVERSE_BEGIN_INHERITED(nsHTMLMediaElement, nsGenericHTMLElement)
  ...
  NS_IMPL_CYCLE_COLLECTION_TRAVERSE_NSCOMPTR_AMBIGUOUS(mSourcePointer, nsIDOMRange)
...
NS_IMPL_ENUM_ATTR_DEFAULT_VALUE(nsHTMLMediaElement, Preload, preload, nullptr)
...
void
nsHTMLMediaElement::WakeLockCreate()
{
  if (!mWakeLock) {
...
void
nsHTMLMediaElement::WakeLockRelease()
{
  if (mWakeLock) {
    mWakeLock->Unlock();
    mWakeLock = nullptr;
...
#endif
#ifdef MOZ_MEDIA_PLUGINS
  if (IsMediaPluginsEnabled() && GetMediaPluginHost()->FindDecoder(nsDependentCString(aMIMEType), nullptr))
    return true;
#endif
...
#endif
#ifdef MOZ_MEDIA_PLUGINS
  if (IsMediaPluginsEnabled() && GetMediaPluginHost()->FindDecoder(aType, nullptr)) {
