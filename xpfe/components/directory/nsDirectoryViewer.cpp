...
nsHTTPIndex::GetInterface(const nsIID &anIID, void **aResult) {
  if (anIID.Equals(NS_GET_IID(nsIFTPEventSink))) {
    ...
    if (!mRequestor) return NS_ERROR_NO_INTERFACE;
  ...
  if (anIID.Equals(NS_GET_IID(nsIPrompt))) {
    ...
    if (!aDOMWindow) return NS_ERROR_NO_INTERFACE;
  ...
  if (anIID.Equals(NS_GET_IID(nsIProgressEventSink))) {
    ...
    if (!sink) return NS_ERROR_NO_INTERFACE;
...
nsHTTPIndex::OnFTPControlLog(bool server, const char *msg) {
  ...
  dom::AutoEntryScript aes(globalObject, "nsHTTPIndex OnFTPControlLog");
  JSContext *cx = aes.cx();
  ...
  JS_CallFunctionName(cx, global, "OnFTPControlLog", params, &val);
  return NS_OK;
}
...
nsHTTPIndex::GetEncoding(char **encoding) {
  ...
  if (!encoding) return (NS_ERROR_NULL_POINTER);

  *encoding = ToNewCString(mEncoding);
  if (!*encoding) return (NS_ERROR_OUT_OF_MEMORY);
...
nsHTTPIndex::OnStartRequest(nsIRequest *request, nsISupports *aContext) {
  ...
  if (mBindToGlobalObject && mRequestor) {
    ...
    rv = xpc->WrapNative(cx, global, static_cast<nsIHTTPIndex *>(this),
                         NS_GET_IID(nsIHTTPIndex), jsobj.address());

    NS_ASSERTION(NS_SUCCEEDED(rv), "unable to xpconnect-wrap http-index");
    if (NS_FAILED(rv)) return rv;

    NS_ASSERTION(jsobj, "unable to get jsobj from xpconnect wrapper");
  ...
  rv = Assert(mDirectory, kNC_Loading, kTrueLiteral, true);
  if (NS_FAILED(rv)) return rv;
...
NS_IMETHODIMP
nsHTTPIndex::OnStopRequest(nsIRequest *request, nsISupports *aContext,
                           nsresult aStatus) {
  ...
  if (!mDirectory) return NS_BINDING_ABORTED;
  ...
  rv = mDirRDF->GetLiteral(NS_ConvertASCIItoUTF16(commentStr).get(),
...
NS_IMETHODIMP
nsHTTPIndex::OnDataAvailable(nsIRequest *request, nsISupports *aContext,
                             nsIInputStream *aStream, uint64_t aSourceOffset,
                             uint32_t aCount) {
  // If mDirectory isn't set, then we should just bail. Either an
  // error occurred and OnStartRequest() never got called, or
  // something exploded in OnStartRequest().
  ...
  return mParser->OnDataAvailable(request, mDirectory, aStream, aSourceOffset,
                                  aCount);
}

nsresult nsHTTPIndex::OnIndexAvailable(nsIRequest *aRequest,
  ...
  if (entry && NS_SUCCEEDED(rv)) {
    ...
    if (NS_SUCCEEDED(rv)) {
      ...
      if (NS_FAILED(rv)) return rv;
      if (xpstr.Last() == '/') xpstr.Truncate(xpstr.Length() - 1);
    ...
    // Since the definition of a directory depends on the protocol, we would
    // have to do string comparisons all the time. But we're told if we're a
    // container right here - so save that fact
    if (isDirType)
...
nsresult nsHTTPIndex::OnInformationAvailable(nsIRequest *aRequest,
                                             nsISupports *aCtxt,
...
nsHTTPIndex::nsHTTPIndex() : mBindToGlobalObject(true), mRequestor(nullptr) {}

nsHTTPIndex::nsHTTPIndex(nsIInterfaceRequestor *aRequestor)
    : mBindToGlobalObject(true), mRequestor(aRequestor) {}
...
nsresult nsHTTPIndex::CommonInit() {
  nsresult rv = NS_OK;
  ...
  mInner = do_CreateInstance(
      "@mozilla.org/rdf/datasource;1?name=in-memory-datasource", &rv);
...
nsresult nsHTTPIndex::Init(nsIURI *aBaseURL) {
  NS_PRECONDITION(aBaseURL != nullptr, "null ptr");
  if (!aBaseURL) return NS_ERROR_NULL_POINTER;
...
nsresult nsHTTPIndex::Create(nsIURI *aBaseURL,
...
nsHTTPIndex::GetBaseURL(char **_result) {
  *_result = ToNewCString(mBaseURL);
  if (!*_result) return NS_ERROR_OUT_OF_MEMORY;
...
void nsHTTPIndex::GetDestination(nsIRDFResource *r, nsACString &dest) {
  ...
  if (node) url = do_QueryInterface(node);
...
// which, when asked (say, by the template builder), we'll make a network
// connection to get its contents. For the moment, all we speak is ftp:// URLs,
// even though
...
bool nsHTTPIndex::isWellknownContainerURI(nsIRDFResource *r) {
  nsCOMPtr<nsIRDFNode> node;
...
NS_IMETHODIMP
nsHTTPIndex::GetSource(nsIRDFResource *aProperty, nsIRDFNode *aTarget,
                       bool aTruthValue, nsIRDFResource **_retval) {
...
NS_IMETHODIMP
nsHTTPIndex::GetSources(nsIRDFResource *aProperty, nsIRDFNode *aTarget,
                        bool aTruthValue, nsISimpleEnumerator **_retval) {
  nsresult rv = NS_ERROR_UNEXPECTED;
...
NS_IMETHODIMP
nsHTTPIndex::GetTarget(nsIRDFResource *aSource, nsIRDFResource *aProperty,
                       bool aTruthValue, nsIRDFNode **_retval) {
  nsresult rv = NS_ERROR_UNEXPECTED;

  *_retval = nullptr;

  if ((aTruthValue) && (aProperty == kNC_Child) &&
...
NS_IMETHODIMP
nsHTTPIndex::GetTargets(nsIRDFResource *aSource, nsIRDFResource *aProperty,
                        bool aTruthValue, nsISimpleEnumerator **_retval) {
  nsresult rv = NS_ERROR_UNEXPECTED;
  ...
  if ((aProperty == kNC_Child) && isWellknownContainerURI(aSource)) {
    ...
    if (NS_SUCCEEDED(rv) && (_retval)) {
      ...
      bool hasResults;
      if (NS_SUCCEEDED((*_retval)->HasMoreElements(&hasResults)) && hasResults)
      ...
      if (NS_FAILED(idx_rv)) {
        ...
        if (!mTimer) {
          rv = NS_NewTimerWithFuncCallback(
              getter_AddRefs(mTimer), nsHTTPIndex::FireTimer, this, 1,
              nsITimer::TYPE_ONE_SHOT, "nsHTTPIndex::GetTargets");
...
nsresult nsHTTPIndex::AddElement(nsIRDFResource *parent, nsIRDFResource *prop,
                                 nsIRDFNode *child) {
  if (!mNodeList) {
  ...
  if (!mTimer) {
    return NS_NewTimerWithFuncCallback(
        getter_AddRefs(mTimer), nsHTTPIndex::FireTimer, this, 1,
        nsITimer::TYPE_ONE_SHOT, "nsHTTPIndex::AddElement");
    // Note: don't addref "this" as we'll cancel the
    // timer in the httpIndex destructor
  }
...
void nsHTTPIndex::FireTimer(nsITimer *aTimer, void *aClosure) {
    ...
    if (numItems > 0) {
      ...
      for (loop = 0; loop < (int32_t)numItems; loop++) {
        nsCOMPtr<nsIRDFResource> src =
            do_QueryElementAt(httpIndex->mNodeList, 0);
  ...
  if (refireTimer) {
    NS_NewTimerWithFuncCallback(
        getter_AddRefs(httpIndex->mTimer), nsHTTPIndex::FireTimer, aClosure, 10,
        nsITimer::TYPE_ONE_SHOT, "nsHTTPIndex::FireTimer");
    // Note: don't addref "this" as we'll cancel the
    // timer in the httpIndex destructor
  }
}
...
NS_IMETHODIMP
nsHTTPIndex::Assert(nsIRDFResource *aSource, nsIRDFResource *aProperty,
...
NS_IMETHODIMP
nsHTTPIndex::Unassert(nsIRDFResource *aSource, nsIRDFResource *aProperty,
                      nsIRDFNode *aTarget) {
  nsresult rv = NS_ERROR_UNEXPECTED;
  if (mInner) {
...
nsHTTPIndex::HasAssertion(nsIRDFResource *aSource, nsIRDFResource *aProperty,
  ...
  if (mInner) {
    rv =
        mInner->HasAssertion(aSource, aProperty, aTarget, aTruthValue, _retval);
  }
  return (rv);
}
...
NS_IMETHODIMP
nsHTTPIndex::HasArcOut(nsIRDFResource *aSource, nsIRDFResource *aArc,
                       bool *result) {
...
NS_IMETHODIMP
nsHTTPIndex::BeginUpdateBatch() { return mInner->BeginUpdateBatch(); }

NS_IMETHODIMP
nsHTTPIndex::EndUpdateBatch() { return mInner->EndUpdateBatch(); }
...
NS_IMETHODIMP
nsDirectoryViewerFactory::CreateInstance(
    const char *aCommand, nsIChannel *aChannel, nsILoadGroup *aLoadGroup,
    const nsACString &aContentType, nsIDocShell *aContainer,
    nsISupports *aExtraInfo, nsIStreamListener **aDocListenerResult,
    nsIContentViewer **aDocViewerResult) {
  nsresult rv;

  bool viewSource =
      FindInReadable(NS_LITERAL_CSTRING("view-source"), aContentType);

  if (!viewSource &&
      Preferences::GetInt("network.dir.format", FORMAT_XUL) == FORMAT_XUL) {
    // ... and setup the original channel's content type
    (void)aChannel->SetContentType(
    ...
    rv = factory->CreateInstance(
        aCommand, channel, aLoadGroup,
        NS_LITERAL_CSTRING("application/vnd.mozilla.xul+xml"), aContainer,
        aExtraInfo, getter_AddRefs(listener), aDocViewerResult);
    if (NS_FAILED(rv)) return rv;

    rv = channel->AsyncOpen2(listener);
    if (NS_FAILED(rv)) return rv;

    // Create an HTTPIndex object so that we can stuff it into the script
    // context
    ...
    nsCOMPtr<nsIInterfaceRequestor> requestor =
        do_QueryInterface(aContainer, &rv);
    if (NS_FAILED(rv)) return rv;
  ...
  if (viewSource) {
    rv = factory->CreateInstance(
        ...
        NS_LITERAL_CSTRING("text/html; x-view-type=view-source"), aContainer,
        aExtraInfo, getter_AddRefs(listener), aDocViewerResult);
  } else {
    rv = factory->CreateInstance(
        "view", aChannel, aLoadGroup, NS_LITERAL_CSTRING("text/html"),
        aContainer, aExtraInfo, getter_AddRefs(listener), aDocViewerResult);
  }
  ...
  rv = scs->AsyncConvertData("application/http-index-format", "text/html",
                             listener, nullptr, aDocListenerResult);
...
nsDirectoryViewerFactory::CreateInstanceForDocument(
    nsISupports *aContainer, nsIDocument *aDocument, const char *aCommand,
    nsIContentViewer **aDocViewerResult) {
